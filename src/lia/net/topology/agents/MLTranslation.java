...
public class MLTranslation {
    ...
    private static final transient Logger logger = Logger.getLogger(MLTranslation.class.getName());

    public static OpticalSwitch fromConfigAndMsg(AFOXFullUpdateReturnMsg msg, Map<AFOXRawPort, AFOXGetInputRFIDRetMsg> rfidMap, MLAFOXConfig config) throws TopologyException {
        ...
        for (final AFOXRawPort pp : configPorts) {
            try {
                if (pp.portType == PortType.INPUT_PORT) {
                    ...
                    AFOXOSPort.Builder<Port<?, ?>> bIN = new AFOXOSPort.Builder<Port<?, ?>>(smIN.AFOXName, PortType.INPUT_PORT, os, null, null);
                    bIN.row(pp.portRow).column(pp.portColumn).customerName(smIN.CustomerName).afoxRawPort(pp).pendingExists(smIN.PendingExists);
                ...
                if (pp.portType == PortType.OUTPUT_PORT) {
                    ...
                    AFOXOSPort.Builder<Port<?, ?>> bOUT = new AFOXOSPort.Builder<Port<?, ?>>(smOUT.AFOXName, PortType.OUTPUT_PORT, os, null, null);
                    bOUT.row(pp.portRow).column(pp.portColumn).customerName(smOUT.CustomerName).afoxRawPort(pp).pendingExists(smOUT.PendingExists);
                    final AFOXGetInputRFIDRetMsg rfidMsg = rfidMap.get(pp);
                    if (rfidMsg != null && rfidMsg.SerialNumber != null) {
        ...
        for (final AFOXRawPort pp : configPorts) {
            try {
                ...
                if(pp.portType == PortType.INPUT_PORT || pp.portType == PortType.OUTPUT_PORT) {
                    AFOXOSPort.Builder<Port<?, ?>> bIN = new AFOXOSPort.Builder<Port<?, ?>>(pName, pp.portType, os, null, null);
                    ...
                    logger.log(Level.WARNING, " Afox switch accepts only INPUT_PORT or OUTPUT_PORT. The port: " + pp + " will be ignored");
                }
