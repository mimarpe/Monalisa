 ...
 * @author ramiro
 */
public class Force10Agent extends TopoAgent<MLComputerHostConfig, HostRawPort> {
    ...
    private MLLinksMsg getLinks() {
        ...
        return new MLLinksMsg(host.id(), DeviceType.F10, pMap);
    }
    ...
    private final Object expressResults() {
        if (host != null) {
            ...
            try {
                byte[] buff = Utils.writeObject(new TopoMsg(agentID, serviceID, TopoMsg.Type.FORCE10_CONFIG, host));
    ...
    public Force10Agent(String agentName, String agentGroup, String farmID) {
    ...
    private void invalidateHost() {
        ...
        for (Force10Port cp : this.host.getPorts()) {
    ...
    private void updateState() {
        try {
            ...
            host = new Force10Host(config.hostName());
            for (HostRawPort hrp : config.hostPorts()) {
                host.addPort(new Force10Port(hrp.portName, host, hrp.portType));
            }
            logger.log(Level.INFO, " Force10Agent reloaded config");
        } catch (Throwable t) {
