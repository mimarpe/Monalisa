...
public class CienaAgent extends TopoAgent<MLComputerHostConfig, HostRawPort>{
    ...
    private final Object expressResults() {
        if (host != null) {
            ...
            try {
                byte[] buff = Utils.writeObject(new TopoMsg(agentID, serviceID, TopoMsg.Type.CIENA_CONFIG, host));
    ...
    public CienaAgent(String agentName, String agentGroup, String farmID) {
    ...
    private static final MLComputerHostConfig getLocalConfig() {
        ...
        try {
            config = new MLComputerHostConfig(AppConfig.getProperty("CIENA_CONFIG_FILE"));
        } catch (Throwable t) {
            logger.log(Level.WARNING, "Unable to load ComputerHostAgent agent. Cause:", t);
            throw new InstantiationError("Unable to load ComputerHostAgent agent");
        }
