...
public class TopologyHolder implements GenericTopology {
    private static final transient Logger logger = Logger.getLogger(TopologyHolder.class.getName());
    ...
    public static final Port<?,?> getPortByOpticalLink(OutgoingLink ol) {
        switch(ol.remoteDeviceType) {
            case HOST: {
                ComputerHost host = (ComputerHost)topoHolder.getEntityByID(UUID.nameUUIDFromBytes(ol.remoteDeviceName.getBytes()));
                if(host == null) {
                    return null;
                }
            ...
            case F10: {
                Force10Host host = (Force10Host)topoHolder.getEntityByID(UUID.nameUUIDFromBytes(ol.remoteDeviceName.getBytes()));
                if(host == null) {
                    return null;
                }
                return host.getPortByName(ol.remotePortName);
            ...
            case CIENA: {
                CienaHost host = (CienaHost)topoHolder.getEntityByID(UUID.nameUUIDFromBytes(ol.remoteDeviceName.getBytes()));
                if(host == null) {
                    return null;
                }
                return host.getPortByName(ol.remotePortName);
            ...
            case AFOX: {
                OpticalSwitch host = (OpticalSwitch)topoHolder.getEntityByID(UUID.nameUUIDFromBytes(ol.remoteDeviceName.getBytes()));
                if(host == null) {
                    return null;
                }
                ...
                for(OSPort port: osPorts) {
                    ...
                    if(afx.portType == port.type() && afx.portRow == afxPort.getRow() && afx.portColumn == afxPort.getColumn()) {
                        return port;
                    }
                }
    ...
    private void checkPorts(UUID deviceID, DeviceType deviceType, Map<UUID, OutgoingLink> currentLinks) {
        ...
        switch(deviceType) {
            ...
            default: {
                logger.log(Level.WARNING, "[ TopologyHolder ] Unable to handle (yet) device type: " + deviceType + " Update needed?");
                break;
            }
        }
    ...
    private void checkForPorts(OpticalSwitch newHost) {
        ...
        if(nLinks.equals(oLinks)) return;
    ...
    public void notifyRemoteMsg(byte[] msg, TopologyNotifier notif) throws IOException, ClassNotFoundException {
        ...
        if (msg != null) {
            ...
            try {
                ...
                if(topoMsg == null) {
                    logger.log(Level.WARNING, "[ TopologyHolder ] [ notifyRemoteMsg ] Null TopoMsg after deserialization");
                    return;
                }
                ...
                switch(topoMsgType) {
                        ...
                        try {
                            for(Map.Entry<UUID, OutgoingLink> entry: map.entrySet()) {
                                ...
                                switch(linksMsg.entityType) {
                                    ...
                                    default: {
                                        logger.log(Level.WARNING, "[ TopologyHolder ] Unable to handle (yet) TopoMsg type: '" + linksMsg.entityType + "' ! Update needed?");
                                        break;
                                    }
                                }
                                if(sp == null) {
                                ...
                                try {
                                    ...
                                    logger.log(Level.WARNING, " [ TopologyHolder ] Unable to set link for: " + sp + " dp: " + dp, t);
                                }
                            }
                        } finally {
                    ...
                    default: {
                        logger.log(Level.WARNING, "[ TopologyHolder ] Unable to handle (yet) TopoMsg type: '" + topoMsgType + "' ! Update needed?");
                        break;
                    }
                }
            } finally {
