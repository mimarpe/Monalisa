...
public class AuthZTrustManager implements X509TrustManager {
	...
	public AuthZTrustManager(KeyStore ks) throws NoSuchAlgorithmException, KeyStoreException {
		final String tmf_algo = AppConfig.getProperty("lia.util.security.AuthZMonitorTrustManagerAlgo", TrustManagerFactory.getDefaultAlgorithm());
	...
	public void checkClientTrusted(X509Certificate[] chain, String authType) throws CertificateException {
		...
		final String clientDN = chain[0].getSubjectDN().toString();
			...
			if (!authorizationManager.checkClient(clientDN)) {
				logger.log(Level.INFO," Client [" + clientDN + "] it's not authorized in " + authorizationManager.getAuthzServer());
				/* not an authorized client, close this connection */
				throw new CertificateException(" Client [" + clientDN + "] it's not authorized in " + authorizationManager.getAuthzServer());				
			}
			else {
				logger.log(Level.INFO," Client [" + clientDN + "] IS authorized in " + authorizationManager.getAuthzServer());				
			}
