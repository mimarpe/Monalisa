...
public class LogWatcher {
	...
	private static final transient Logger logger = 
		Logger.getLogger("lia.util.LogWatcher");
	...
	public BufferedReader getNewChunk() {
		...
		logger.log(Level.FINE, "[LogWatcher] ### getNewChunck(): file size " +
				newFileSize);
		if (haveNewLog(file)) {			
			logger.log(Level.INFO, "[LogWatcher] New log file: " + filename);
			actualNBytesMD5 = (int) ((newFileSize > NBYTES_MD5) ? 
					NBYTES_MD5 : newFileSize);
			try {
				...
				logger.log(Level.WARNING, "[LogWatcher] Error computing MD5 digest for " +
						path, e);
				lastMD5Digest = null;
			}		
		}
		...
		try {
			...
			logger.log(Level.WARNING, "[LogWatcher] Error reading log file ",
					e);
		}
		return br;
	}
	 ...
	 * parameter.
	 */
	protected byte[] computeDigest(File file, int nBytes) 
