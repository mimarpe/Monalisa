...
public class ShutdownManager extends Thread {
	 ...
	 * Logger Name 
	 */
    private static final String ComponentName = "lia.util.ShutdownManager";
    
    /**
	 * Logging component
	 */
	private static final transient Logger logger = Logger.getLogger(ComponentName);
	...
	private Vector registeredModules;
	...
	private ShutdownManager() {
		...
		try   {
			...
			logger.log(Level.SEVERE, "IllegalArgumentException caught. Unable to register ShutdownManager" +
					e.getMessage());
		} catch (IllegalStateException e) {
			logger.log(Level.SEVERE, "IllegalStateException caught. Unable to register ShutdownManager." +
					" Maybe the JVM is already shutting down. " +
					e.getMessage());
		} catch (SecurityException e) {
			logger.log(Level.SEVERE, "SecurityException caught. Unable to register ShutdownManager." +
					e.getMessage());
		}		
	}
	...
	public void addModule(ShutdownReceiver module) {
		if (module == null) {
			logger.log(Level.INFO, "ShutdownManager: the module is NULL and will not" +
					"be registered.");
			return ;
		}
