...
public class ConfigurationManager implements XMLWritable {
	 ...
	 * Logging component
	 */
	private static final transient Logger logger = Logger.getLogger("lia.util.Pathload.server.ConfigurationManager");		
				...
				if (token != null) {
					...
					if (tokenResult != null) {
						...
						log(Level.INFO, "Token " +
								...
								" aquired by " + tokenResult.getOwner());
					}
				}			
			}
		}		
	...
	private Token createNewToken(Token oldToken) {
		...
		if (oldToken != null) {			
			if (peerCache.contains(oldToken.getOwner())) {
				log(Level.FINER, "Peer " + oldToken.getOwner().toString() 
						+ " lost the token.");
			}			
		}
