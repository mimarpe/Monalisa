...
public class PathloadConnector extends ThreadedPage {
	 ...
	 * Logging component
	 */
	private static final transient Logger logger = Logger.getLogger("lia.util.Pathload.server.PathloadConnector");	
	...
	public void execGet() {
		...
		try {
			if (!haveParametersToProcess()) throw new IllegalArgumentException("HTML Request contains no parameters.");
			...
			if ((testIpAddress == null) ||
					(!testIpAddress.equalsIgnoreCase(ipAddress))) {
				throw new IllegalArgumentException("Could not parse your ip (" + ipAddress + ") correctly.");
			}
			if (!isPublicAddress(testIpAddress)) {
				throw new IllegalArgumentException("I will not accept your IP Address " +
						"(" + ipAddress + ") because it is private.");
			}
			...
			if ((ver == null) ||
					...
					(farmName == null) ||
					(farmGroups == null)) {
				throw new IllegalArgumentException("Could not parse all POST Data parameters correctly: " +
						"[ver: " + ver + "]" +
						"[hostname: " + hostname + "]" +
						"[ipAddress: " + ipAddress + "]" +
						"[farmName: " + farmName + "]");
			...
			switch (iAction) {
				...
				case PathloadClient.ACTION_SHUTDOWN:
					...
					break;
				default: throw new IllegalArgumentException("Unknown action id: " + iAction);
			}
	...
	private int getActionFromString(String action) {
		...
		if (action == null) return PathloadClient.ACTION_UNKNOWN;
