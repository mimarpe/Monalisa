...
public class PathloadVersionFilter implements Filter {
	 ...
	 * Logging component
	 */
	private static final transient Logger logger = Logger.getLogger("lia.util.Pathload.server.servlet.PathloadVersionFilter");
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if (ver == null) {
			if (logger.isLoggable(Level.FINEST)) {
				logger.log(Level.FINE, "Misformed message from farm " + farmName + 
						" dropped. Version info is missing.");
			}
			return ;
		}
		...
		try {
			...
			if (version < PathloadVersionFilter.MINIMUM_VERSION) {
				if (logger.isLoggable(Level.FINEST)) {
					logger.log(Level.FINE, "Message from farm " + farmName + 
							" dropped. Minimum allowed version is " + 
			...
			logger.log(Level.FINE, "Message from farm " + farmName + 
					" dropped because of a NumberFormatException while " +
					"parsing the version number: " + ver);
		}
