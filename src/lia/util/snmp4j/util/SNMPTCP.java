...
public class SNMPTCP {
	...
	public SNMPTCP(String defaultOptions, SNMPFactory snmp, String snmpAddress, String farmName, String clusterName, String moduleName){
		this.snmp = snmp;
	...
	public void setFlags(boolean tcpRtoAlgorithmFlag, 	boolean tcpRtoMinFlag, boolean tcpRtoMaxFlag, 	boolean tcpMaxConnFlag, 
			boolean tcpActiveOpensFlag, boolean tcpPassiveOpensFlag, boolean tcpAttemptFailsFlag, boolean tcpEstabResetsFlag, 
			boolean tcpCurrEstabFlag, boolean tcpInSegsFlag, boolean tcpOutSegsFlag, 	boolean tcpRetransSegsFlag, boolean tcpInErrsFlag, 
			boolean tcpOutRstsFlag, boolean tcpHCInSegsFlag, boolean tcpHCOutSegsFlag, boolean tcpConnFlag, boolean tcpConnectionFlag, 
	...
	public void getResults(Vector res) {
		...
		if (tcpConnectionTable.size() != 0) {
			for (Enumeration en = tcpConnectionTable.keys(); en.hasMoreElements(); ) {
				...
				if (localAddress == null || localPort == null || remoteAddress == null || remotePort == null) continue;
				eResult result1 = new eResult();
				result1.FarmName  = farmName;
				result1.ClusterName = clusterName;
				result1.Module = moduleName;
				...
				if (localAddressType != null) result1.addSet("TcpConnectionLocalAddressType", localAddressType);
				String remoteAddressType = (String)h.get("TcpConnectionRemAddressType");
				if (remoteAddressType != null) result1.addSet("TcpConnectionRemAddressType", remoteAddressType);
				String connectionState = (String)h.get("TcpConnectionState");
				if (connectionState != null) result1.addSet("TcpConnectionState", connectionState);
				String connectionProcess = (String)h.get("TcpConnectionProcess");
				if (connectionProcess != null) result1.addSet("TcpConnectionProcess", connectionProcess);
				res.add(result1);
			}
		}
		if (tcpListenerTable.size() != 0) {
			for (Enumeration en = tcpListenerTable.keys(); en.hasMoreElements(); ) {
				...
				if (localAddress == null || localPort == null) continue;
				eResult result1 = new eResult();
				result1.FarmName  = farmName;
				result1.ClusterName = clusterName;
				result1.Module = moduleName;
				...
				if (localAddressType != null) result1.addSet("TcpListenerLocalAddressType", localAddressType);
				String connectionProcess = (String)h.get("TcpListenerProcess");
				if (connectionProcess != null) result1.addSet("TcpListenerProcess", connectionProcess);
				res.add(result1);
			}
		}
	}
	...
	static final String RToAlgorithms[][] = new String[][] {
		{ "1", "Other" },
		{ "2", "Constant" },
		{ "3", "Rsre" },
		{ "4", "Vanj" },
	...
	private String getTCPRToAlgorithm(String type) {
		...
		for (int i=0; i<RToAlgorithms.length; i++)
			if (RToAlgorithms[i][0].equals(type)) return RToAlgorithms[i][1];
		return "Unknown";
	}
	
	static final String connStates[][] = new String[][] {
		{ "1", "Closed" },
		{ "2", "Listen" },
		{ "3", "SynSent" },
		{ "4", "SynReceived" },
	...
	private String getTCPConnState(String state) {
		...
		for (int i=0; i<connStates.length; i++)
			if (connStates[i][0].equals(state)) return connStates[i][1];
		return "Unknown";
	}
	
	static final String addressTypes[][] = new String[][] { 
		{ "0", "Unknown" },
		{ "1", "IPv4" },
		{ "2", "IPv6" },
		{ "3", "IPv4z" },
	...
	private String getTCPAddressType(String type) {
		...
		for (int i=0; i<addressTypes.length; i++)
			if (addressTypes[i][0].equals(type)) return addressTypes[i][1];
		return "Unknown";
	}
