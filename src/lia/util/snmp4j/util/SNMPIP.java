...
public class SNMPIP {
	...
	public SNMPIP(String defaultOptions, SNMPFactory snmp, String snmpAddress, String farmName, String clusterName, String moduleName){
		this.snmp = snmp;
	...
	public void setFlags(	boolean ipForwardingFlag, boolean ipDefaultTTLFlag, boolean ipInReceivesFlag, boolean ipInHdrErrorsFlag, 
			boolean ipInAddrErrorsFlag, 	boolean ipForwDatagramsFlag, 	boolean ipInUnknownProtosFlag, boolean ipInDiscardsFlag, 
			boolean ipInDeliversFlag, boolean ipOutRequestsFlag, boolean ipOutDiscardsFlag, boolean ipOutNoRoutesFlag, boolean ipReasmTimeoutFlag, 
			boolean ipReasmReqdsFlag, boolean ipReasmOKsFlag, boolean ipReasmFailsFlag, boolean ipFragOKsFlag, boolean ipFragFailsFlag, 
			boolean ipFragCreatesFlag, boolean ipRoutingDiscardsFlag) {
			...
			for (int k=0; k<pdu[i].size(); k++) {
				try {
					...
					if (oid.startsWith(ipOutRequestsOID) && ipOutRequestsFlag) {
						...
						String cap = "Unknown";
						if (param.equals("1")) cap = "Forwarding";
						else if (param.equals("2")) cap = "Not-forwarding";
	...
	public boolean setIpDefaultTTL(int ttl) {
		...
		PDU pdu[] = snmp.run(defaultOptions+" -p SET "+snmpAddress+" "+ipDefaultTTLOID+"0={i}"+ttl, null, null);
