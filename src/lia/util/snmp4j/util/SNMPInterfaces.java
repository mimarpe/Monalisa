...
public class SNMPInterfaces {
	...
	private Hashtable ifSupport = new Hashtable();
	...
	public SNMPInterfaces(String defaultOptions, SNMPFactory snmp, String snmpAddress, String farmName, String clusterName, String moduleName){
		this.snmp = snmp;
	...
	public void setFlags(boolean ifTypeFlag, boolean ifMtuFlag, boolean ifSpeedFlag, boolean ifPhysAddressFlag, 
			boolean ifAdminStatusFlag, boolean ifOperStatusFlag, boolean ifLastChangeFlag, boolean ifInOctetsFlag, boolean ifInUcastPktsFlag, 
			boolean ifInDiscardsFlag, boolean ifInErrorsFlag, boolean ifInUnknownProtosFlag, boolean ifOutOctetsFlag, boolean ifOutUcastPktsFlag, 
			boolean ifOutDiscardsFlag, boolean ifOutErrorsFlag, boolean ipAdEntAddrFlag, boolean ipAdEntIfIndexFlag, boolean ipAdEntNetMaskFlag, 
	...
	public void getResults(Vector res) {
			...
			for (int k=0; k<pdu[i].size(); k++) {
				try {
					...
					if (oid.startsWith(ifDescrOID)) {
						...
						if (param.equals("1")) status = "Up";
						else if (param.equals("2")) status = "Down";
						else if (param.equals("3")) status = "Testing";
		...
		for (Enumeration en = ifSupport.keys(); en.hasMoreElements(); ) {
			...
			if (result.NodeName != null) res.add(result);
		}
	}
	...
	public boolean setIfAdminStatus(int ifIndex, int status) {
		...
		PDU pdu[] = snmp.run(defaultOptions+" -p SET "+snmpAddress+" "+ifAdminStatusOID+"."+ifIndex+"={i}"+status, null, null);
	...
	static final String types[][] = new String[][] { { " 1", "other" }, 
		{ "2", "regular1822" },
		{ "3", "hdh1822" },
		{ "4", "ddnX25" },
		{ "5", "rfc877x25" }, 
	...
	private String getIfType(String type) {
		...
		for (int i=0; i<types.length; i++) {
			if (types[i][0].equals(type)) return types[i][1];
		}
		return type;
	}
	
	static final String operStatus[][] = new String[][] { { "1", "Up" }, { "2", "Down" }, { "3", "Testing" }, { "4", "Unknown" }, 
	...
	private String getIfOperStatus(String id) {
		...
		for (int i=0; i<operStatus.length; i++)
			if (operStatus[i][0].equals(id)) return operStatus[i][1];
		return "Unknown";
	}
...
//		logger.setLevel(Level.FINEST);
		...
		SNMPInterfaces route = new SNMPInterfaces("-c private -v 2c", factory, "udp:141.85.99.136/161", "RB", "SNMPInterfaces", "SNMPModule");
		Vector v = new Vector();
		route.getResults(v);
