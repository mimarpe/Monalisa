...
public class SNMPMPLS {
	...
	public SNMPMPLS(String defaultOptions, SNMPFactory snmp, String snmpAddress, String farmName, String clusterName, String moduleName){
		this.snmp = snmp;
		...
		for (int i=0; i<pdu.length; i++) {
			...
			for (int k=0; k<pdu[i].size(); k++) {
				try {
					...
					if (mplsInterfaceFlag && checkMPLSInterfaces(mplsIfs, oid, vb)) continue;
					if (mplsInterfacePerfFlag && checkMPLSInterfacesPerf(mplsIfsPerf, oid, vb)) continue;
					...
					if (mplsInSegmentFlag && checkMPLSInSegment(mplsInSegments, oid, vb)) continue;
					if (mplsInSegmentPerfFlag && checkMPLSInSegmentPerf(mplsInSegmentsPerf, oid, vb)) continue;
					...
					if (mplsOutSegmentFlag && checkMPLSOutSegment(mplsOutSegments, oid, vb)) continue;
					if (mplsOutSegmentPerfFlag && checkMPLSOutSegmentPerf(mplsOutSegmentsPerf, oid, vb)) continue;
					...
					if (mplsXCTableFlag && checkMPLSXCTable(mplsXCTable, oid, vb)) continue;
					if (mplsMaxLabelStackDepthFlag && oid.startsWith(mplsMaxLabelStackDepthOID)) {
						...
						result.addSet("MplsMaxLabelStackDepth", Long.valueOf(((UnsignedInteger32)vb.getVariable()).getValue()));
						continue;
					}
					...
					if (mplsLabelStackFlag && checkMPLSStackLabel(mplsStackLabel, oid, vb)) continue;
					...
					if (mplsGenericLabelFlag && checkMPLSGenericLabel(mplsGenericLabel, oid, vb)) continue;
				}catch (Throwable t) {
	...
	private boolean checkMPLSInterfaces(Hashtable mplsIfs, String oid, VariableBinding vb) {
		...
		if (oid.startsWith(mplsInterfaceLabelMinInOID)) {
			...
			if (!ifsInitialized) initializeInterfaces();
	...
	private boolean checkMPLSInterfacesPerf(Hashtable mplsIfsPerf, String oid, VariableBinding vb) {
		...
		if (oid.startsWith(mplsInterfacePerfInLabelLookupFailuresOID)) {
			...
			if (!mplsIfsPerf.containsKey(ifName)) {
				...
				result.Module = moduleName;
				result.NodeName = ifName;
				result.addSet("MplsInterfacePerfInLabelLookupFailures", Long.valueOf(((Counter32)vb.getVariable()).getValue()));
			} else {
				eResult result = (eResult)mplsIfsPerf.get(ifName);
				result.addSet("MplsInterfacePerfInLabelLookupFailures", Long.valueOf(((Counter32)vb.getVariable()).getValue()));
			}
		...
		if (oid.startsWith(mplsInterfacePerfOutFragmentedPktsOID)) {
			...
			if (!mplsIfsPerf.containsKey(ifName)) {
				...
				result.Module = moduleName;
				result.NodeName = ifName;
				result.addSet("MplsInterfacePerfOutFragmentedPkts", Long.valueOf(((Counter32)vb.getVariable()).getValue()));
			} else {
				eResult result = (eResult)mplsIfsPerf.get(ifName);
				result.addSet("MplsInterfacePerfOutFragmentedPkts", Long.valueOf(((Counter32)vb.getVariable()).getValue()));
			}
	...
	private boolean checkMPLSInSegment(Hashtable mplsInSegments, String oid, VariableBinding vb) {
		...
		if (oid.startsWith(mplsInSegmentInterfaceOID)) {
			...
			String ifName = null;
			if (param.equals("0")) ifName = "PerPlatform";
			else {
				if (!ifsInitialized) initializeInterfaces();
		...
		if (oid.startsWith(mplsInSegmentLabelOID)) {
			if  (param.equals("0")) return true; // must be ignored as specified
	...
	private boolean checkMPLSXCTable(Hashtable mplsXCTable, String oid, VariableBinding vb) {
		...
		if (oid.startsWith(mplsXCLspIdOID)) {
			...
			if (!mplsXCTable.containsKey(id)) {
				...
				if (!p[1].equals("0")) buf.append("_SEGIN_").append(p[1]);
				if (!p[2].equals("0")) buf.append("_SEGOUT_").append(p[2]);
	...
	private boolean checkMPLSGenericLabel(Hashtable genericLabel, String oid, VariableBinding vb) {
		...
		if (oid.startsWith(mplsLdpEntityGenericIfIndexOrZeroOID)) {
			...
			if (!genericLabel.containsKey(id)) {
				String ifName = null;
				if (param.equals("0")) return true; // don't added
				if (!ifsInitialized) initializeInterfaces();
				ifName = (String)ifSupport.get(param);
				if (ifName == null) {
					logger.warning("Got no correspondence as interface for "+param);
	...
	static final String[][] adrFamilyNumbers = new String[][] {
		{ "0", "Other" },
		{ "1", "IpV4" },
		{ "2", "IpV6" },
		{ "3", "Nsap" },
	...
	private String getMPLSAdrFamily(String fam) {
		...
		for (int i=0; i<adrFamilyNumbers.length; i++)
			if (adrFamilyNumbers[i][0].equals(fam)) return adrFamilyNumbers[i][1];
		return "Unknown";
	}
	
	static final String[][] mplsOwner = new String[][] {
		{ "1", "Unknown" },
		{ "2", "Other" },
		{ "3", "Snmp" },
		{ "4", "Ldp" },
	...
	private String getMPLSOwner(String owner) {
		...
		for (int i=0; i<mplsOwner.length; i++)
			if (mplsOwner[i][0].equals(owner)) return mplsOwner[i][1];
		return "Unknown";
	}
	...
	static final String[][] storageTypes = new String[][] {
		{ "1", "Other" },
		{ "2", "Volatile" },
		{ "3", "NonVolatile" },
		{ "4", "Permanent" },
	...
	private String getMPLSStorageType(String type) {
		...
		for (int i=0; i<storageTypes.length; i++)
			if(storageTypes[i][0].equals(type)) return storageTypes[i][1];
		return "Unknown";
	}
	
	static final String[][] adminStatus = new String[][] {
		{ "1", "Up" },
		{ "2", "Down" },
		{ "3", "Testing" }
	};
	...
	private String getMPLSAdminStatus(String status) {
		...
		if (status == null) return "Null";
		for (int i=0; i<adminStatus.length; i++)
			if (adminStatus[i][0].equals(status)) return adminStatus[i][1];
		return "Unknown";
	}
	
	static final String[][] operStatus = new String[][] {
		{ "1", "Up" },
		{ "2", "Down" },
		{ "3", "Testing" },
		{ "4", "Unknown" },
	...
	private String getMPLSOperStatus(String status) {
		...
		for (int i=0; i<operStatus.length; i++)
			if (operStatus[i][0].equals(status)) return operStatus[i][1];
		return "Unknown";
	}
	...
	public boolean setMPLSXCNotificationsEnable(boolean set) {
		...
		int tr = (set ? 1 : 2);
		PDU pdu[] = snmp.run(defaultOptions+" -p SET "+snmpAddress+" "+mplsXCNotificationsEnableOID+".0"+"={s}"+tr, null, null);
