...
public class SNMPOSPF {
	...
	static Logger logger = Logger.getLogger("lia.util.snmp.SNMPFactory");
	
	private SNMPFactory snmp;
	...
	public SNMPOSPF(String defaultOptions, SNMPFactory snmp, String snmpAddress, String farmName, String clusterName, String moduleName){
		this.snmp = snmp;
	...
	public boolean supportOSPF() {
		...
		if (pdu == null || pdu.length == 0) return false;
		boolean support = false;
		for (int i=0; i<pdu.length; i++) {
			if (pdu[i].getType() == PDU.REPORT) {
				if (logger.isLoggable(Level.FINEST))
					logger.log(Level.FINE, SNMPFactory.getReport(pdu[i]));
				continue;
			}
	...
	public void getResults(Vector res) {
		...
		if (pdu == null || pdu.length == 0) return; // no result
		eResult result = null;
		...
		for (int i=0; i<pdu.length; i++) {
			...
			for (int k=0; k<pdu[i].size(); k++) {
				try {
					...
					if (param == null || param.length() == 0 || param.equals("Null")) continue;
					if (result == null) {
						result = new eResult();
					...
					if (oid.startsWith(ospfRouterIdOID) && ospfRouterIdFlag) {
						...
						result.addSet("OspfExternLSACksumSum", Integer.valueOf(((Integer32)vb.getVariable()).getValue()));
						...
						result.addSet("OspfMulticastExtensions", Integer.valueOf(((Integer32)vb.getVariable()).getValue()));
					} else if (oid.startsWith(ospfExitOverflowIntervalOID) && ospfExitOverflowIntervalFlag) {
						result.addSet("OspfExitOverflowInterval", Integer.valueOf(((Integer32)vb.getVariable()).getValue()));
						...
						if (!ospfAreaTable.containsKey(id)) {
							...
							result1.Module = moduleName;
							result1.NodeName = "OSPF_Area_"+id;
							result1.addSet("OspfAreaLSACksumSum", Integer.valueOf(((Integer32)vb.getVariable()).getValue()));
						} else {
							eResult result1 = (eResult)ospfAreaTable.get(id);
							result1.addSet("OspfAreaLSACksumSum", Integer.valueOf(((Integer32)vb.getVariable()).getValue()));
						}
						...
						if (!ospfLSDBAreaTable.containsKey(id)) {
							...
							result1.ClusterName = clusterName;
							result1.Module = moduleName;
							result1.addSet("OspfLsdbSequence", Integer.valueOf(((Integer32)vb.getVariable()).getValue()));
						} else {
							eResult result1 = (eResult)ospfLSDBAreaTable.get(id);
							result1.addSet("OspfLsdbSequence", Integer.valueOf(((Integer32)vb.getVariable()).getValue()));
						}
						...
						if (!ospfLSDBAreaTable.containsKey(id)) {
							...
							result1.ClusterName = clusterName;
							result1.Module = moduleName;
							result1.addSet("OspfLsdbChecksum", Integer.valueOf(((Integer32)vb.getVariable()).getValue()));
						} else {
							eResult result1 = (eResult)ospfLSDBAreaTable.get(id);
							result1.addSet("OspfLsdbChecksum", Integer.valueOf(((Integer32)vb.getVariable()).getValue()));
						}
						...
						if (!ospfIFTable.containsKey(id)) {
							...
							result1.ClusterName = clusterName;
							result1.Module = moduleName;
							result1.addSet("OspfIfRtrDeadInterval", Integer.valueOf(((Integer32)vb.getVariable()).getValue()));
						} else {
							eResult result1 = (eResult)ospfIFTable.get(id);
							result1.addSet("OspfIfRtrDeadInterval", Integer.valueOf(((Integer32)vb.getVariable()).getValue()));
						}
						...
						if (!ospfIFTable.containsKey(id)) {
							...
							result1.ClusterName = clusterName;
							result1.Module = moduleName;
							result1.addSet("OspfIfPollInterval", Integer.valueOf(((Integer32)vb.getVariable()).getValue()));
						} else {
							eResult result1 = (eResult)ospfIFTable.get(id);
							result1.addSet("OspfIfPollInterval", Integer.valueOf(((Integer32)vb.getVariable()).getValue()));
						}
		...
		if (result != null && result.param_name != null && result.param_name.length != 0) res.add(result);
		if (ospfAreaTable.size() != 0) {
			for (Enumeration en = ospfAreaTable.keys(); en.hasMoreElements(); ) {
				String id = (String)en.nextElement();
				if (id == null) continue;
				eResult result1 = (eResult)ospfAreaTable.get(id);
				if (result1 != null && result1.param_name != null && result1.param_name.length != 0) res.add(result1);
			}	
		}
		if (ospfStubAreaTable.size() != 0) {
			for (Enumeration en = ospfStubAreaTable.keys(); en.hasMoreElements(); ) {
				...
				if (result1 != null && result1.NodeName != null && result1.param_name != null && result1.param_name.length != 0) res.add(result1);
			}
		}
	...
	public boolean setOSPFRouterID(String routerID) {
		...
		PDU pdu[] = snmp.run(defaultOptions+" -p SET "+snmpAddress+" "+ospfRouterIdOID+"0={s}"+routerID, null, null);
		if (pdu == null || pdu.length == 0) return false;
		for (int i=0; i<pdu.length; i++) {
			if (pdu[i].getErrorStatus() != 0) {
				logger.info(pdu[i].toString());
				return false;
			}
			for (int k=0; k<pdu[i].size(); k++) {
				try {
	...
	public boolean setOSPFAdminStat(boolean enabled) {
		...
		PDU pdu[] = snmp.run(defaultOptions+" -p SET "+snmpAddress+" "+ospfAdminStatOID+"0={s}"+status, null, null);
		if (pdu == null || pdu.length == 0) return false;
		for (int i=0; i<pdu.length; i++) {
			if (pdu[i].getErrorStatus() != 0) {
				logger.info(pdu[i].toString());
	...
	public boolean setOSPFASBdrRtrStatus(boolean flag) {
		...
		PDU pdu[] = snmp.run(defaultOptions+" -p SET "+snmpAddress+" "+ospfASBdrRtrStatusOID+"0={s}"+status, null, null);
		if (pdu == null || pdu.length == 0) return false;
		for (int i=0; i<pdu.length; i++) {
			if (pdu[i].getErrorStatus() != 0) {
				logger.info(pdu[i].toString());
	...
	public boolean setOSPFExtLsdbLimit(int value) {
		...
		PDU pdu[] = snmp.run(defaultOptions+" -p SET "+snmpAddress+" "+ospfExtLsdbLimitOID+"0={s}"+value, null, null);
		if (pdu == null || pdu.length == 0) return false;
		for (int i=0; i<pdu.length; i++) {
			if (pdu[i].getErrorStatus() != 0) {
				logger.info(pdu[i].toString());
	...
	public boolean setOSPFMulticastExtensions(int value) {
		...
		PDU pdu[] = snmp.run(defaultOptions+" -p SET "+snmpAddress+" "+ospfMulticastExtensionsOID+"0={s}"+value, null, null);
		if (pdu == null || pdu.length == 0) return false;
		for (int i=0; i<pdu.length; i++) {
			if (pdu[i].getErrorStatus() != 0) {
				logger.info(pdu[i].toString());
	...
	public boolean setOSPFExitOverflowInterval(int value) {
		...
		PDU pdu[] = snmp.run(defaultOptions+" -p SET "+snmpAddress+" "+ospfExitOverflowIntervalOID+"0={s}"+value, null, null);
		if (pdu == null || pdu.length == 0) return false;
		for (int i=0; i<pdu.length; i++) {
			if (pdu[i].getErrorStatus() != 0) {
				logger.info(pdu[i].toString());
	...
	public boolean setOSPFDemandExtensions(boolean flag) {
		...
		String status = (flag?"1":"2");
		PDU pdu[] = snmp.run(defaultOptions+" -p SET "+snmpAddress+" "+ospfDemandExtensionsOID+"0={s}"+status, null, null);
		if (pdu == null || pdu.length == 0) return false;
		for (int i=0; i<pdu.length; i++) {
			if (pdu[i].getErrorStatus() != 0) {
				logger.info(pdu[i].toString());
	...
	public boolean setOSPFAuthType(int areaID, int ospfAuthType) {
		...
		PDU pdu[] = snmp.run(defaultOptions+" -p SET "+snmpAddress+" "+ospfAuthTypeOID+areaID+"={i}"+ospfAuthType, null, null);
		if (pdu == null || pdu.length == 0) return false;
		for (int i=0; i<pdu.length; i++) {
			if (pdu[i].getErrorStatus() != 0) {
				logger.info(pdu[i].toString());
	...
	public boolean setOSPFImportASExtern(int areaID, boolean status) {
		...
		int s = (status?1:2);
		PDU pdu[] = snmp.run(defaultOptions+" -p SET "+snmpAddress+" "+ospfImportASExternOID+areaID+"={i}"+s, null, null);
		if (pdu == null || pdu.length == 0) return false;
		for (int i=0; i<pdu.length; i++) {
			if (pdu[i].getErrorStatus() != 0) {
				logger.info(pdu[i].toString());
	...
	static final String[][] rowStatus = new String[][] {
		{ "1", "Active" },
		{ "2", "NotInService" },
		{ "3", "NotReady" },
		{ "4", "CreateAndGo" },
	...
	private String getOSPFAreaStatus(String status) {
		...
		for (int i=0; i<rowStatus.length; i++)
			if (rowStatus[i][0].equals(status)) return rowStatus[i][1];
		return "Unknown";
	}
	
	static final String[][] metricTypes = new String[][] {
		{ "1", "OspfMetric" },
		{ "2", "ComparableCost" },
		{ "3", "NonComparable" }
	};
	...
	private String getOSPFMetricType(String type) {
		...
		for (int i=0; i<metricTypes.length; i++)
			if (metricTypes[i][0].equals(type)) return metricTypes[i][1];
		return "Unknown";
	}
	
	static final String[][] lsdbTypes = new String[][] {
		{ "1", "RouterLink" },
		{ "2", "NetworkLink" },
		{ "3", "SummaryLink" },
		{ "4", "AsSummaryLink" },
	...
	private String getOSPFLSDBType(String type) {
		...
		if (type == null) return "Null";
		for (int i=0; i<lsdbTypes.length; i++)
			if (lsdbTypes[i][0].equals(type)) return lsdbTypes[i][1];
		return "Unknown";
	}
	...
	private String getOSPFIfType(String type) {
		...
		for (int i=0; i<ifTypes.length; i++)
			if(ifTypes[i][0].equals(type)) return ifTypes[i][1];
		return "Unknown";
	}
	
	static final String[][] ifStates = new String[][] {
		{ "1", "Down" },
		{ "2", "Loopback" },
		{ "3", "Waiting" },
		{ "4", "PointToPoint" },
	...
	private String getOSPFIfState(String state) {
		...
		for (int i=0; i<ifStates.length; i++)
			if (ifStates[i][0].equals(state)) return ifStates[i][1];
		return "Unknown";
	}
	
	static final String[][] ifStatus = new String[][] {
		{ "1", "Active" },
		{ "2", "NotInService" },
		{ "3", "NotReady" },
		{ "4", "CreateAndGo" },
	...
	private String getOSPFIfStatus(String status) {
		...
		for (int i=0; i<ifStatus.length; i++)
			if (ifStatus[i][0].equals(status)) return ifStatus[i][1];
		return "Unknown";
	}
	...
	public static void main(String args[]) {
		...
		SNMPOSPF route = new SNMPOSPF("-c private -v 2c", factory, "udp:141.85.99.136/161", "RB", "SNMPOSPF", "SNMPModule");
