...
public class SNMPSystem {
	...
	public SNMPSystem(String defaultOptions, SNMPFactory snmp, String snmpAddress, String farmName, String clusterName, String moduleName){
		this.snmp = snmp;
	...
	public void setFlags(	boolean sysDescrFlag, boolean sysUpTimeFlag, boolean sysContactFlag, boolean sysNameFlag, boolean sysLocationFlag) {
	...
	public boolean setSysLocation(String location) {
		...
		PDU pdu[] = snmp.run(defaultOptions+" -p SET "+snmpAddress+" "+sysLocationOID+"={s}"+location, null, null);
	...
	public boolean setSysContact(String contact) {
		...
		PDU pdu[] = snmp.run(defaultOptions+" -p SET "+snmpAddress+" "+sysContactOID+"={s}"+contact, null, null);
		if (pdu == null || pdu.length == 0) return false;
		for (int i=0; i<pdu.length; i++) {
			if (pdu[i].getErrorStatus() != 0) {
				logger.info(pdu[i].toString());
