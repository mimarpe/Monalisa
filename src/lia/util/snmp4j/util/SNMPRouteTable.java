...
public class SNMPRouteTable {
	...
	public SNMPRouteTable(String defaultOptions, SNMPFactory snmp, String snmpAddress, String farmName, String clusterName, String moduleName){
		this.snmp = snmp;
	...
	public void setFlags(boolean ifDescrFlag, boolean ipRouteIfIndexFlag, boolean ipRouteMetric1Flag, 
			boolean ipRouteMetric2Flag, boolean ipRouteMetric3Flag, boolean ipRouteMetric4Flag, boolean ipRouteNextHopFlag, 
			boolean ipRouteTypeFlag, boolean ipRouteProtoFlag, boolean ipRouteAgeFlag, boolean ipRouteMaskFlag, 
			boolean ipRouteMetric5Flag) {
	...
	public void getResults(Vector res) {
			...
			for (int k=0; k<pdu[i].size(); k++) {
				try {
					...
					if (oid.startsWith(ipRouteDestOID)) {
						...
						String type = "Unknown";
						if (param.equals("1")) type = "Other";
						else if (param.equals("2")) type = "Invalid";
						else if (param.equals("3")) type = "Direct";
						else if (param.equals("4")) type = "Indirect";
						if (!ifSupport.containsKey(id)) {
							eResult result = new eResult();
							ifSupport.put(id, result);
							result.FarmName  = farmName;
		...
		for (Enumeration en = ifSupport.keys(); en.hasMoreElements(); ) {
			...
			if (result.NodeName != null && result.param_name != null && result.param_name.length != 0) res.add(result);
		}
	}
	...
	public boolean addRoute(String routeDest, String ifName, int routeMetric, String nextHop, String routeMask) {
		...
		Hashtable index2if = new Hashtable();
		PDU[] pdu = snmp.run(defaultOptions+" -p GETBULK "+snmpAddress+" 1.3.6.1.2.1.2", null, null);
		if (pdu == null || pdu.length == 0) return false; // no result
		for (int i=0; i<pdu.length; i++) {
			if (pdu[i].getType() == PDU.REPORT) {
				if (logger.isLoggable(Level.FINEST))
					logger.log(Level.FINE, SNMPFactory.getReport(pdu[i]));
...
//		command.append(".").append(ipRouteDestOID).append(routeDest).append("={s}").append(routeDest).append(" ");
	...
	final static String[][] routeProtos = new String[][] {
		{ "1", "Other" },
		{ "2", "Local" },
		{ "3", "Netmgmt" },
		{ "4", "Icmp" },
	...
	private String getIpRouteProto(String type) {
		...
		for (int i=0; i<routeProtos.length; i++)
			if (routeProtos[i][0].equals(type)) return routeProtos[i][1];
		return type;
	}
	...
	public static void main(String args[]) {
		...
		SNMPRouteTable route = new SNMPRouteTable("-c private -v 2c", factory, "udp:141.85.99.136/161", "RB", "SNMPRouteTable", "SNMPModule");
		Vector v = new Vector();
		route.getResults(v);
