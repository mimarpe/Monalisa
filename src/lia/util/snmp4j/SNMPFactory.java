...
public class SNMPFactory implements PDUFactory {
	...
	public PDU[] run(String command, SNMPListener listener, TableListener tableListener) {
		...
		if (command == null || command.length() == 0) return null; // nothing to process here
		String args[] = command.split(" ");
		if (args == null || args.length == 0) return null; // again nothing to process
		...
		try {
			if (operation == LISTEN) {
				...
				else logger.warning("No SNMPListener specified");
			}
	...
	public static String getReport(PDU response) {
		...
		if (SnmpConstants.usmStatsUnsupportedSecLevels.equals(oid)) {
			...
			return "REPORT contains unknown OID ("
					+ oid.toString() + ").";
		}
		return " Current counter value is " +
				vb.getVariable().toString() + ".";
	}
	...
	private PDU[] walk(Snmp snmp, PDU request, Target target) throws IOException {
		...
		for (int i=0; i<ret.length; i++) ret[i] = (PDU)responses.get(i);
		return ret;
	}
	...
	private boolean processWalk(PDU response, PDU request, OID rootOID) {
		...
		for (int i=0; (!finished) && (i<response.size()); i++) {
			...
			if ((vb.getOid() == null) ||
				...
				if (logger.isLoggable(Level.FINEST))
					logger.warning("Variable received is not lexicographic successor of requested one: "+vb.toString() + " <= "+lastOID);
				finished = true;
			}
			else {
				if (logger.isLoggable(Level.FINEST))
	...
	private void table(TableListener listener) throws IOException {
		...
		if (useDenseTableOperation) {
			tableUtils.getDenseTable(target, columns, listener, listener,
					lowerBoundIndex, upperBoundIndex);
		}
		else {
			tableUtils.getTable(target, columns, listener, listener,
					lowerBoundIndex, upperBoundIndex);
		}
		snmp.close();
	}
	...
	private void checkOptions() {
		if ((operation == WALK) &&
			...
			throw new IllegalArgumentException(
			"There must be exactly one OID supplied for walk operations "+vbs.size());
		}
