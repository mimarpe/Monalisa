...
public class FDTListener {
	private static final transient String COMPONENT = "lia.util.fdt.FDTListener";
	...
	public void instanceStarted(String alias, LisaXDRModule instance) {
		if (logger.isLoggable(Level.INFO))
			logger.log(Level.INFO, "[XDRDaemon] [{0}] Active XDR Module added to XDR-dispatcher : ", new Object[] { alias });
	...
	public XDRMessage execModuleCommand(String module, String command, List args) {
		...
		if (logger.isLoggable(Level.FINEST)) {
			logger.log(Level.FINEST, "New XDR command received. Module: "+module + "Cmd: "+command + " Args:" + args);
		}
	...
	private class LisaXDRTcpServer implements Runnable {
		private static final transient String COMPONENT = "lia.util.fdt.LisaXDRTcpServer";
		final transient Logger logger = Logger.getLogger(COMPONENT);
		...
		public LisaXDRTcpServer(String name, ServerSocket ss) throws Exception {
			setMyName((name == null) ? "( LISA ) XDRTcpServer :- Listening on port [ " + ss.getLocalPort() + " ] " : name);
			this.ss = ss;
	...
	private class LisaXDRTcpSocket extends XDRTcpSocket {
		private static final transient String COMPONENT = "lia.util.fdt.LisaXDRSSLTcpSocket";
		final transient Logger logger = Logger.getLogger(COMPONENT);
		private String name;
		...
		protected void initSession() throws Exception {
			logger.log(Level.INFO, "SESSION-INIT for " + getLocalAddress() + " <->" + getInetAddress() + " Peer DN: " + getPeerDN());
		...
		protected void xdrSession() throws Exception {
			...
			while (true) {
				...
				if ("create_pipes".equals(sCommand)) {
					...
					XDRMessage xdrOutput = FDTListener.this.execModuleCommand(sModule, sModuleCommand, args.subList(2, args.size()));
					write(xdrOutput);
				}
				// unknown command
				else {
