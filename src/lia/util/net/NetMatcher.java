...
public class NetMatcher
{
    ...
    private static final transient Logger logger = Logger.getLogger("lia.util.net.NetMatcher");
    ...
    public static final void main(String[] args) {
        ...
        for(int i = 0; i < addresses.length; i++) {
            if ( nm.matchInetNetwork(addresses[i]) ) {
                System.out.println(" Matched IP  -> " + addresses[i]);
            } else {
                System.out.println(" Missed IP  -> " + addresses[i]);
                
                if (nm.containsName(addresses[i])){
                	System.out.println("But matched the host hostname -> "+addresses[i]);
                }
            }
        }
    }
...
class InetNetwork
{
    ...
    {
        return fqdn == null ? 
        			network.equals(maskIP(ip, netmask)) 
        		: 
        			ip.getCanonicalHostName().toLowerCase().endsWith(fqdn);
    }
    ...
    {
        return fqdn == null ? 
        			network.getHostAddress() + "/" + netmask.getHostAddress() 
        		: 
        			fqdn;
    }
    ...
    {
        return fqdn == null ? 
        			maskIP(network, netmask).hashCode() 
        		: 
        			fqdn.hashCode();
    }
