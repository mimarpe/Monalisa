...
public class TimeoutClientSocketFactory implements RMIClientSocketFactory, Serializable {
    ...
    private static final transient Logger logger = Logger.getLogger(TimeoutClientSocketFactory.class.getName());

    /** maximum time to connect with the other endPoint */ 
    private static final int CONNECT_TIMEOUT = Integer.valueOf(AppConfig.getProperty("lia.util.net.TimeoutClientSocketFactory.CONNECT_TIMEOUT", "10")).intValue() * 1000; //10s
    /** maximum time to wait for data */
    private static final int SO_TIMEOUT = Integer.valueOf(AppConfig.getProperty("lia.util.net.TimeoutClientSocketFactory.SO_TIMEOUT", "20")).intValue()  * 1000; //20s
