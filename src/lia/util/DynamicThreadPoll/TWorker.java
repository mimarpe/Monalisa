/*
 * $Id: TWorker.java 6865 2010-10-10 10:03:16Z ramiro $
 */
...
class TWorker extends Thread implements Comparable<TWorker> {   
    ...
    private static final transient Logger logger = Logger.getLogger(TWorker.class.getName());
            ...
            synchronized(waitingJobLock) {
                while(myJob == null && alive.get()) {
                    try {
                        ...
                        logger.log(Level.WARNING, " [ TWorker ] " + myID + " [ HANDLED ]  got interrupted exception waiting for job", ie);
                        Thread.interrupted();
                    } catch(Throwable t) {
                        logger.log(Level.WARNING, " [ TWorker " + myID + " ] [ HANDLED ] got general exception waiting for job", t);
                    }
                }
            }//end sync
            ...
            if ( myJob != null ) {
                ...
                try {
                    ...
                    if ( logger.isLoggable(Level.FINEST)){
                        logger.log(Level.FINEST, " [ TWorker " + myID + " ] Interrupted Exception in procesing job: " + myJob, ie);
                    }
                    ey = ie;
                }  catch ( Throwable t ) {
                    if ( logger.isLoggable(Level.FINEST)){
                        logger.log(Level.FINEST, " [ TWorker " + myID + " ] Interrupted Exception in procesing job: " + myJob, t);
                    }
                    ey = t;
                }
                ...
                synchronized ( syncNotifyResult ) {
                    try {
                        ...
                        logger.log(Level.WARNING, "[ TWorker ] [ HANDLED ] TWorker " + myID + " got exc in notification hook", t);
                    }
                }//end sync
            }
        }
    ...
    public String toString() {
        ...
        if ( myJob == null  ) sb.append(" status = waiting ");
        else sb.append(" status = running ");
        return sb.toString();
    }
