...
public class SecureContextExecutor {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.util.SecureContextExecutor");
		...
		try {
			...
			if (credentialSource.equalsIgnoreCase("files")) {
				...
				if (privateKeyPath == null || privateKeyPath.length() == 0)
					throw new Exception("Cannot find [lia.Monitor.login.privateKeyFile] entry in ml.properties oe entry is null");

				String certChainPath = AppConfig.getProperty("lia.Monitor.login.certChainFile", "/etc/grid-security/hostcert.pem");

				if (certChainPath == null || certChainPath.length() == 0)
					throw new Exception("[SEC_REGISTRATION] Cannot find [lia.Monitor.login.certChainFile] entry in ml.properties");

				// use relative path?
				boolean useRelativePath = Boolean.valueOf(AppConfig.getProperty("lia.Monitor.login.relativePath", "true")).booleanValue();
				...
				String privateKeyScript = AppConfig.getProperty("lia.Monitor.login.privateKeyScript", "getPrivateKey.sh");
				if (privateKeyScript == null || privateKeyScript.length() == 0)
					throw new Exception("[SEC_REGISTRATION] Cannot find [lia.Monitor.login.privateKeyScript] entry in ml.properties");
				...
				if (certChainScript == null || certChainScript.length() == 0)
					throw new Exception("[SEC_REGISTRATION] Cannot find [lia.Monitor.login.certChainScript] entry in ml.properties");

				boolean useSudo = Boolean.valueOf(AppConfig.getProperty("lia.Monitor.login.useSudoScripts", "true")).booleanValue();
