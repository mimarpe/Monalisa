...
class PublisherThread extends Thread implements Observer {
    ...
    private static final transient Logger logger = Logger.getLogger(PublisherThread.class.getName());
            ...
            synchronized(reloadSyncLock) {
                ...
                if(manifests != null && manifests.length > 0) {
                    for(int i=0; i<manifests.length; i++) {
                        ...
                        setName("( ML ) PublisherThread [ " + publisherName + " ] for Class " + className + " :- Instantiated");
                ...
                if(publisher == null) throw new Exception("No MLLoggerPublisher-Class defined for " + publisherName);
            }//end sync
        } finally {
    ...
    public void setClassPath(String[] newClassPath) {
        if(Arrays.equals(newClassPath, this.classPath)) {
            if(logger.isLoggable(Level.FINE)) {
                logger.log(Level.FINE, " [ PublisherThread ] " + publisherName + " :- Old class path and new class path are the same: " + Arrays.toString(newClassPath));
            }
            return;
        }
        ...
        synchronized(reloadSyncLock) {
            ...
            try {
                ...
                logger.log(Level.WARNING, " [ PublisherThread ] [ setClassPath ] Got exception trying to reload the thread context", t);
            }
    ...
    private void notifyPublisher(MLLogMsg mlm) {
        try {
            ...
            logger.log(Level.WARNING, "[ PublisherThread ] :- " + getName() + " Got exception! trying to publish " + mlm, t);
        }
    }
    ...
    boolean publish(MLLogMsg mlm) {
        ...
        logger.log(Level.INFO, "[ PublisherThread ] :- " + publisherName + " publishe() called but hasToRun = " + hasToRun);
        return false;
    }
                ...
                synchronized(reloadSyncLock) {
                    while(hasToRun.get() && publisher == null) {
                        if(logger.isLoggable(Level.FINER)) {
                            logger.log(Level.FINER, " [ PublisherThread ] :- " + publisherName + " has no publisher defined ... will wait");
                        }
                        reloadSyncLock.wait();
                    }
                    if(!hasToRun.get()) break;
                }
    ...
    public String toString() {
        return " PublisherThread [ " + publisherName + " ] MLLoggerPublisher-Class: {" + publisherClassName + "} ClassPath: " + Arrays.toString(classPath);
    }
        ...
        if(o != null) {
            if(o instanceof MLContainer) {
                ...
                try {
                    ...
                    logger.log(Level.WARNING, " [ PublisherThread ] [ update ] Got exception trying to reload the publisher", t);
                }
            }
        }
    }
