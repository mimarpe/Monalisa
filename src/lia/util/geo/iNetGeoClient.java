...
//		debug =	Boolean.valueOf(AppConfig.getProperty("lia.Monitor.debug", "false")).booleanValue();
	...
	private ILink resolveLink(String linkName){
		...
		synchronized(configData){
			for(int k=0; k<configData.size(); k++){
				...
				if(name.equals(linkName)){
					...
					// read source IP
					do{ name = st.nextToken(); } while( name.equals(""));
					link.fromIP = name;
					// read destination IP
					do{ name = st.nextToken(); } while( name.equals(""));
					link.toIP = name;
		...
		if(link == null)
			return null;
        try {
            link.fromLAT = Double.parseDouble((String)link.from.get("LAT"));
            link.fromLONG = Double.parseDouble((String)link.from.get("LONG"));
            link.toLAT = Double.parseDouble((String)link.to.get("LAT"));
            link.toLONG = Double.parseDouble((String)link.to.get("LONG"));
