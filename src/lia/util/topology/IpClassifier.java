...
public final class IpClassifier extends Thread {
	...
	private static final transient Logger logger = Logger.getLogger("lia.util.topology.IpClassifier");
	...
	private static IpClassifier ipClassifier = new IpClassifier();

	/** new and unclassified IPs */
	private Vector newIPs; 
	
	/** classes of IPs, by TTL */
	private Hashtable ipClasses;
	
	/** dictionary with all sets of IPs on the same machine */
	private Vector ipIDic;
	...
	private String getIPids(String ip){
		Vector ips = (Vector) searchOldIpIDic(ip);
		if(ips == null)
			return ip+"\n";
		else{
