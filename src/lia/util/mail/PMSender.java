/*
 * $Id: PMSender.java 7275 2012-06-27 16:10:10Z ramiro $
 */
...
public class PMSender extends MailSender implements Runnable {
    ...
    private static final Logger logger = Logger.getLogger("lia.util.mail.PMSender");
    ...
    private static ProxyWorker pw = null;
    ...
    private static class EMsgEntry implements Delayed {
        ...
        public int compareTo(Delayed o) {
            ...
            if(eme.nextRetry > nextRetry) return -1;
            if(eme.nextRetry < nextRetry) return 1;
    ...
    public static synchronized PMSender getInstance() {
        if (me == null) {
            me = new PMSender();
            MonALISAExecutors.getMLHelperExecutor().scheduleWithFixedDelay(me, 10 + Math.round(Math.random() * 10), 10 + Math.round(Math.random() * 10), TimeUnit.SECONDS);
        }
        return me;
    }
    ...
    public static String getStatus() {
        ...
        if(mailq.size() > 0 && logger.isLoggable(Level.FINEST)) {
            for ( Iterator<EMsgEntry> it = mailq.iterator(); it.hasNext(); ) {
                EMsgEntry em = it.next();
                sb.append("\n ---> [ ").append(new Date(em.entryTime)).append(" <> ").append(new Date(em.nextRetry)).append( "] <---- \n");
        ...
        if(waitingNotif.size() > 0 && logger.isLoggable(Level.FINEST)) {
            for ( Iterator<Map.Entry<Integer, EMsgEntry>> it = waitingNotif.entrySet().iterator(); it.hasNext(); ) {
                Map.Entry<Integer, EMsgEntry> entry = it.next();
                EMsgEntry emsge = entry.getValue();
                
                sb.append("\n ---> [ ").append(new Date(emsge.entryTime)).append(" <> ").append(new Date(emsge.nextRetry)).append( "] <---- \n");
    ...
    private static void reloadCfgParams() {
        ...
        if(logger.isLoggable(Level.FINEST)) {
            logger.log(Level.FINEST, " [ PMS ] [ reloadCfgParams ]  " +
                    " DELIVER_INTERNAL_NOTIF = " + DELIVER_INTERNAL_NOTIF + 
                    " MAX_PMS_MSGS = " + MAX_PMS_MSGS +
                    " RETRY_AFTER = " + RETRY_AFTER +
                    " RETRY_AFTER_FACTOR = " + RETRY_AFTER_FACTOR +
