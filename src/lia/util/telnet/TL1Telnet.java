/*
 * $Id: TL1Telnet.java 7265 2012-06-25 22:50:55Z ramiro $
 */
...
public abstract class TL1Telnet {
	...
	private TL1TelnetThread pingDaemon;
	...
	private class TL1TelnetThread extends Thread {
		...
		public void run() {
				...
				try {
					...
					if (!connected.get()) {
						try {
							...
							cleanup();
							logger.log(Level.WARNING, " [ TL1Telnet ] Got Exception while connect()-ing to the switch", t);
						}
					}
						...
						synchronized (lock) {
							if (nanoNow > lastCmdSentNanos + MAX_CONN_PING_DELAY_DEFAULT_NANOS) {
								try {
									...
									logger.log(Level.WARNING, " [ TL1Telnet ] Got Exception while ping()-ing the connection... ", t);
									cleanup();
								}
							}
						}
			...
			// Exit main Thread
			logger.log(Level.WARNING, " [ TL1TelnetDaemon ] for [" + localConnName + "] will exit now ... active == false");
			cleanup();
		}
	}

	TL1Telnet(String username, String passwd, String hostName, int port) throws Exception {
		fDate = new Date();
		...
		if (username == null || passwd == null || hostName == null) {
			throw new OSTelnetException("[ TL1Telnet ] Username, Passwd and Hostname MUST BE != null", OSTelnetException.NULL_AUTH_PARAMS);
		}
		...
		try {
			...
			logger.log(Level.WARNING, "[ TL1Telnet ] Exception trying to connect ", t);
		}
	...
	private void connect() throws Exception {
		...
		synchronized (lock) {
			...
			if (nextConnect > nanoNow) {
				...
				throw new OSTelnetException(" [ TL1Telnet ] Cannot connect sooner then " + fDate, OSTelnetException.CANNOT_CONNECT_SOONER);
			}
	...
	private final void doAuth(String userName, String passwd) throws OSTelnetException, IOException {
		internalExec(true, "act-user::" + userName + ":" + AUTH_CONN_CTAG + "::" + passwd + TL1_FINISH_CMD, AUTH_CONN_CTAG, TL1_COMPLD_TAG);
	}
	...
	private void cleanup() {
		...
		try {
			...
			logger.log(Level.WARNING, " [ TL1Telnet ] Got exception closing the connection [ " + localConnName + " ] to the switch", t);
		}
	}
	 ...
	 */
	private boolean readTL1Respose(final String ctag, final String expectedTL1Code) throws OSTelnetException, IOException {
		...
		try {
			while (true) {
				...
				if (line == null) {
					...
					cleanup();
					throw new OSTelnetException("Expected TL1 end tag ';' ... null response ( maybe the stream was closed )",
			...
			if (logger.isLoggable(Level.FINEST)) {
				logger.log(Level.FINEST,
				        " readTL1Response for ctag: " + ctag + " expTL1Code: " + expectedTL1Code + " got from NE:\n" + sbLogFinest.toString());
			}
		}
	 ...
	 */
	public final void execCmd(final String cmd, final String ctag, final String expectedTL1Code) throws OSTelnetException, IOException {
	 ...
	 * @throws IOException
	 */
	private void internalExec(boolean skipConnCheck, final String cmd, final String ctag, final String expectedTL1Code) throws OSTelnetException,
	        IOException {
		...
		synchronized (lock) {
			...
			try {
				...
				if (logger.isLoggable(Level.FINER)) {
					logger.log(Level.WARNING, "[ TL1Telnet ] Got OSTelnetException in doCmd() for cmd [" + cmd + "]", ote);
				}
				throw ote;
	 ...
	 * @see #execCmd(String, String, String)
	 */
	public final String[] execCmdAndGet(final String cmd, final String ctag, final String expectedTL1Code) throws OSTelnetException, IOException {
		synchronized (lock) {
