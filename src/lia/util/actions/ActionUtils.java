...
public class ActionUtils {
	...
	private static final Logger	logger	= Logger.getLogger("lia.util.actions.ActionUtils");
	...
	static{
		...
		try{
			es = lia.util.threads.MLExecutorsFactory.getScheduledExecutorService("lia.util.Actions.ActionExecutor");
		}
		catch (Exception e){
	...
	public static String apply(final SeriesState ss, final String sMessage, final String sFormatParam, final MLProperties mlp){
		...
		if (sFormat.indexOf("#INTERVAL_ERR")>=0){
			final DB db = new DB("SELECT as_last_err FROM action_states WHERE as_file='"+Format.escSQL(ss.getFile())+"' AND as_key='"+Format.escSQL(ss.getKey())+"';");
		...
		if (sFormat.indexOf("#INTERVAL_OK")>=0){
			final DB db = new DB("SELECT as_last_ok FROM action_states WHERE as_file='"+Format.escSQL(ss.getFile())+"' AND as_key='"+Format.escSQL(ss.getKey())+"';");
