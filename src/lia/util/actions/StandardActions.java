...
public class StandardActions implements ActionTaker {
	...
	private static final class SendMail implements Runnable {
		 ...
		 * @param _subject
		 */
		SendMail(final String[] _vsTo, final String _body, final MailSender _msSender, final String _from, final String _subject) {
			this.vsTo = _vsTo;
			this.body = _body;
				...
				synchronized (hmJabberSessions) {
					...
					if (conn == null) {
						...
						if (this.host.toLowerCase().equals("google")) {
							...
							conn = StandardActions.this.bSSL ? new org.jivesoftware.smack.SSLXMPPConnection(this.host, StandardActions.this.iPort, this.service) : new org.jivesoftware.smack.XMPPConnection(this.host, StandardActions.this.iPort, this.service);
					...
					try {
						for (int i = 0; i < this.vs.length; i++) {
							conn.createChat(this.vs[i]).sendMessage(this.line);
						}
					} catch (Exception e) {
		...
		public void run() {
			try {
				synchronized (hmYahooSessions) {
					...
					try {
						for (int i = 0; i < this.vs.length; i++) {
							_session.sendMessage(this.vs[i], this.line);
						}
					} catch (Throwable e) {
						// an exception in sending is most probably because the previously
						// established
	 ...
	 * event logger
	 */
	static final Logger logger	= Logger.getLogger("lia.util.actions.StandardActions");
	...
	public void initAction(final int iEntry, final MLProperties prop) throws Exception {
		...
		if (sType.equals("command"))
			this.iType=0;
		else
		if (sType.equals("log"))
			this.iType=2;
		else
		if (sType.equals("yahoo"))
			this.iType=3;
		else
		if (sType.equals("jabber"))
			this.iType=4;
		else
		if (sType.equals("annotation"))
			this.iType=5;
		else
			throw new Exception("action."+iEntry+".type should have one of the [email,command,log,yahoo,jabber,annotation] values");
		...
		if (this.iType==1){
			...
			if (this.sFrom.length()==0 || this.sTo.length()==0 || this.sSubject.length()==0 || this.sBody.length()==0)
		...
		if (this.iType==3 || this.iType==4){
			...
			if (this.sLine.length()==0 || this.sUser.length()==0 || this.sPassword.length()==0 || this.sAccounts.length()==0)
		...
		if (this.iType == 4){
			...
			if (this.sHost.length()==0)
				throw new Exception("you should also define action."+iEntry+".[ssl=boolean (default: true), host=string ('google' is special), port=number (default: 5222 or 5223 if SSL), resource=string (default: MLRepository), service=string (default: value of 'host')] for Jabber accounts");
		}
		...
		if (this.sMessageOK.length()==0 && this.sMessageERR.length()==0 && this.sMessageFlipFlop.length()==0)
			throw new Exception("at least one of the action."+iEntry+".[report_ok,report_err,report_flip_flop] should have a value explaining the event");
	}
	...
	public void takeAction(final SeriesState ss){
			...
			ActionUtils.getExecutor().submit(new Runnable() {
				...
				public void run() {
					try {
						...
						final Process p = MLProcess.exec(new String[] { "/bin/sh", "-c", sExec }, StandardActions.this.lTimeout);
		...
		if (this.iType == 4) {
			...
			ActionUtils.getExecutor().submit(new JabberSender(_sHost, vs, _sLine, _sPassword, _sResource, _sService, _sUser));
		}
		
		if (this.iType==5){
			// annotations
			final Set<Integer> sGroups = Annotation.decodeGroups(ActionUtils.apply(ss, sMessage, this.sAnnotationGroups, this.mlp));
			final Set<String> sSeries = Annotation.decode(ActionUtils.apply(ss, sMessage, this.sAnnotationSeries, this.mlp));
		
			final String sText = ActionUtils.apply(ss, sMessage, this.sAnnotationText, this.mlp);

			if (logger.isLoggable(Level.FINE))
				logger.log(Level.FINE, "Taking annotation action for "+ss.iState +" @ "+ss.getKey()+" : "+sGroups+" / "+sSeries +" / "+sText);
			...
			if (ss.iState == ActionUtils.STATE_OK){
	...
	private static final void closeOpenAnnotations(final Annotation similarTo, final long lDeleteAnnotationIfBelow){
		String q = "SELECT * FROM annotations WHERE a_to>extract(epoch from now())::int AND a_text='"+Formatare.mySQLEscape(similarTo.text)+"'";
	 ...
	 */
	public static boolean sendMail(final SeriesState ss, final String sMessage, final MLProperties mlp, final String sFrom, final String sTo, final String sSubject, final String sBody, final boolean bProxyMail){
