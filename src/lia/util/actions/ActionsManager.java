...
public class ActionsManager implements DataReceiver, Runnable, AppConfigChangeListener {
	...
	private static final Logger	logger = Logger.getLogger("lia.util.actions.ActionsManager");
	...
	public static synchronized ActionsManager getInstance(){		
		if (amInstance==null){
			if (!TransparentStoreFactory.isMemoryStoreOnly()){
				...
				if (db.query("CREATE TABLE action_states (as_file varchar(255), as_key varchar(255) not null, as_value int not null default 0, as_last_ok int default 0, as_last_err int default 0, as_last_flipflop int default 0, as_lastchange int default 0);", true))		
					db.query("CREATE UNIQUE INDEX action_states_pkey ON action_states(as_file, as_key);");
			}
	...
	private void reload() {
		synchronized (this.oLock) {
			...
			if (!f.exists() || !f.isDirectory() || !f.canRead()){
				logger.log(Level.INFO, "Disabling because the property 'lia.util.actions.base_folder' doesn't point to an accessible folder");
			...
			if (this.alAllActions.size()==0){
				...
				logger.log(Level.INFO, "Observing: "+this.alAllActions.size()+" total files, "+this.alPeriodicActions.size()+" periodic actions and "+this.alEventActions.size()+" event-based actions");
			}
		}
	}
					...
					for (int i = this.alAllActions.size() - 1; i >= 0; i--) {
						...
						if (a.getAndResetActionTaken()) {
							...
							for (int j = alSeries.size() - 1; j >= 0; j--) {
								...
								if (ss.iState==ActionUtils.STATE_FLIPFLOP)
									sTimeField = "as_last_flipflop";
								
								db.query("UPDATE action_states SET "+
										"as_value="+ss.iState+
										",as_lastchange="+lNow+
										","+sTimeField+"="+lNow+
										" WHERE as_file='"+Format.escSQL(a.sFile)+"' AND as_key='"+Format.escSQL(ss.getKey())+"';");
								
								if (db.getUpdateCount()<1)
									db.query("INSERT INTO action_states "+
											"(as_file, as_key, as_value, as_lastchange,"+sTimeField+") VALUES "+
											"('" + Format.escSQL(a.sFile) + "', '" + Format.escSQL(ss.getKey()) + "', " + ss.iState + ", "+lNow+", "+lNow+");");
