...
public final class Action implements Observer {
	...
	private static final Logger	logger	= Logger.getLogger("lia.util.actions.Action");
	...
	public final class SeriesState implements Comparable<SeriesState> {
		...
		public String toString(){
			return "FILE: "+Action.this.sFile+", NAMES: "+this.alSeriesNames+", STATE: "+this.iState+", HISTORY: "+this.llHistory;
		}
		...
		void event(final boolean bOk, final String sNewValue){
			...
			while (it.hasNext()){
				...
				if ((bOk && iCount>=Action.this.iSuccessThreshold) || (!bOk && iCount>=Action.this.iErrorThreshold)){
	...
	private ArrayList<SeriesState> getSeries(){
		...
		for (int i=0; i<iSeriesCount; i++){
			...
			final ArrayList<SeriesState> alSeriesTemp = new ArrayList<SeriesState>(alRet.size() * (v!=null && v.size()>0 ? v.size() : 1));
	...
	private void reload(){
		...
		try{	
			...
			logger.log(Level.INFO, "Disabing "+this.mlp.getConfigFileName()+" because a rule could not be instantiated (class="+sDecisionClass+")", t);
		}
		...
		for (int i=0; i<iActionsCount; i++){
			...
			try{
				...
				logger.log(Level.WARNING, "Could not parse action "+i+" ("+this.sFile+") : "+e+" ("+e.getMessage()+")");
			}
		}
		
		if ((this.alSeries.size()==1 && this.alSeries.get(0).alSeriesNames.size()==0) || this.alActions.size()==0){
			logger.log(Level.INFO, "Disabing "+this.mlp.getConfigFileName()+" because series count="+this.alSeries.size()+", actions count="+this.alActions.size());
		...
		if (logger.isLoggable(Level.FINER))
			logger.log(Level.FINER, "Thresholds: success="+this.iSuccessThreshold+", error="+this.iErrorThreshold+", flipflop="+this.iFlipFlopThreshold);
