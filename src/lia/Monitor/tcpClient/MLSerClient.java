...
public abstract class MLSerClient implements LocalDataFarmProvider {
    ...
    /** The Logger */
    protected static final Logger logger = Logger.getLogger("lia.Monitor.tcpClient.MLSerClient");
    ...
    private Object syncLocalClients_Modif = new Object();
    ...
    static {
        try {
            executor = MLExecutorsFactory.getScheduledExecutorService("lia.Monitor.tcpClient.MLSerClient", 5, 512, 5);
        } catch (Throwable e) {
    ...
    public void process(Object obj) {
        ...
        if (msg.tag.startsWith(monMessage.ML_TIME_TAG)) {
            ...
            msg = null;
            return;
        } else if (msg.tag.startsWith("uptime")) {

            msg = null;
            return;
        } else if (msg.tag.startsWith(monMessage.ML_VERSION_TAG)) {
            setMLVersion((String) msg.result);
            msg = null;
            return;
        } else if (msg.tag.startsWith(monMessage.ML_CONFIG_TAG)) {
            MFarm mfarm = (MFarm) msg.result;
            newConfig(mfarm);
            setTotalParams();
            msg = null;
            return;
        } else if (msg.tag.equals(monMessage.ML_RESULT_TAG)) {
            final MonMessageClientsProxy newMsg = decompressResultIfNeeded(msg);
            processPredicateOrFilterResult(newMsg);
        } else if (msg.tag.startsWith(monMessage.ML_APP_CTRL_TAG)) {
            if (logger.isLoggable(Level.FINER)) {
    ...
    protected void processPredicateOrFilterResult(MonMessageClientsProxy msg) {
        ...
        } else if (msg.ident instanceof String) {
    ...
    void notifyAppCtrlPendingClients(int secStatus) {
        ...
        synchronized (syncAppCtrl) {
            ...
            for (Iterator<AppControlClient> vit = vAppControlClients.iterator(); vit.hasNext();) {
                vit.next().appControlStatus(this, (appCtrlStatus == APP_CONTROL_STARTED));
            }
        }
        // send the unsent messsages
        synchronized (vUnsentAppMsgs) {
            for (Iterator<AppControlMessage> mit = vUnsentAppMsgs.iterator(); mit.hasNext();) {
                directSendAppControlMsg(mit.next());
            }
    ...
    private void processAppControlResult(MonMessageClientsProxy msg) {
        ...
        if (msg.tag.equals(AppControlMessage.APP_CONTROL_MSG_AUTH)) {
        ...
        } else if (msg.tag.equals(AppControlMessage.APP_CONTROL_MSG_AUTH_FINISHED)) {
            try {
        ...
        } else if (msg.tag.equals(AppControlMessage.APP_CONTROL_MSG_ERR)) {
        ...
        } else if (msg.tag.equals(AppControlMessage.APP_CONTROL_MSG_AUTH_RETRY)) {
            ...
            restartAppControlAuth();
        } else if (msg.tag.equals(AppControlMessage.APP_CONTROL_MSG_PROXY_ERR)) {
            ...
            restartAppControlAuth();
        } else if (msg.tag.equals(AppControlMessage.APP_CONTROL_MSG_CMD)) {
            if (initSecCtx != null) {
                try {
