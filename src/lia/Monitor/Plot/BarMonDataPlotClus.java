...
public class BarMonDataPlotClus implements LocalDataFarmClient, DataPlotter, ComponentListener, WindowListener, ActionListener {
    ...
    public BarMonDataPlotClus(DataPlotterParent parent, LocalDataFarmProvider dataprovider, 
            Vector predicates, Vector clusters, HashMap yAxisUnit) {
        ...
        bco = new BarChartOpt("RealTime profile", title, false, false, "", (baseUnit != null)?baseUnit.toString() : null);
        bco.setVisible(true);
...
//			System.out.println(" Got eResult " + ro);
			...
			if (r.param != null && r.param_name != null) {
				for (int k=0; k<r.param.length && k<r.param_name.length; k++) {
					if (r.param[k] instanceof String) {
						...
						stringResultPanel.addStringResult(timeUtil.getTime(r.time), r.FarmName, r.ClusterName, r.NodeName, r.Module, r.param_name[k], (String)r.param[k], true);
	...
	public void actionPerformed(ActionEvent e) {
	   ...
	   if (e.getSource() == mPeriod) {
		    PlotIntervalSelector is = new PlotIntervalSelector(bco, 
		    		timeOfFirstResult, 
