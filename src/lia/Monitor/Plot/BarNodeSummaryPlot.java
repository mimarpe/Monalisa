...
/**
 * uses GlobalClusterBarChart from plot.jar to plot summary info about
 * a cluster 
 */
public class BarNodeSummaryPlot implements LocalDataFarmClient, 
											  DataPlotter, 
											  ComponentListener, 
											  WindowListener,
											  ActionListener {
	...
	public BarNodeSummaryPlot(DataPlotterParent parent, LocalDataFarmProvider dataprovider, 
			Vector predicates, Vector clusters, int summaryType, boolean pieOrBar, HashMap yAxisUnit){
        ...
        gcbc = new ClusterBarChart(title, false, false, summaryType, pieOrBar, (baseUnit != null) ? baseUnit.toString() : null);
        ...
        ttask = new TimerTask() {
            public void run() {
                Thread.currentThread().setName(" ( ML ) - Plot - BarNodeSummaryPlot Timer Thread");
				update();
            }
        };
    ...
    boolean plotResult(Result r) {
    	...
    	try {
    		...
    		String nn = (addClusterName?r.ClusterName+"/":"")+r.NodeName;
    		for (int i = 0; i < parameters.length; i++) {
    			for (int j = 0; j < r.param.length; j++) {
    				if (parameters[i].equals(r.param_name[j])) {
    ...
    synchronized public void newFarmResult(MLSerClient client, Object ro) {
    	...
    	if(ro == null && (resultsCount <= 0)){
    	    if(!queryHasResults){
				...
				JOptionPane.showMessageDialog(gcbc, 
	...
	/**
	 * this is not the local time! Instead, it is the interval for which
	 * the data displayed is summarized
	 */
    public void setLocalTime(String dd) {
    	if (dd != null) {
    		try{ 
    			...
    			gcbc.setTimeZone(timeZone = MonDataPlot.adjustTimezone(dd1));
    		}catch(Exception ex){
...
//		}
	...
	public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == mPeriod) {
            PlotIntervalSelector is = new PlotIntervalSelector(gcbc, 
...
//                	System.out.println("start ="+new Date(start)+" end="+new Date(end));
                }
                queryHasResults = false;
                // register as a listener for these predicates
                for(Iterator pit = predicates.iterator(); pit.hasNext(); ) {
