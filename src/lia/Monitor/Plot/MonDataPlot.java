...
public class MonDataPlot implements LocalDataFarmClient, DataPlotter, ActionListener, ComponentListener, WindowListener, TimeSliderAdjustmentListener {

    /** Logger name */
    private static final transient String COMPONENT = "lia.Monitor.Plot";
    ...
    public MonDataPlot(DataPlotterParent parent, LocalDataFarmProvider dataprovider, Vector predicates) {
        this.dataprovider = dataprovider;
        this.parent = parent;

        // since we're in a History Node Plot, we should have a single predicate
        ...
        ttask = new TimerTask() {
            public void run() {
                Thread.currentThread().setName(" ( ML ) - Plot - MonDataPlot Timer Thread");
				update();
            }
        };
...
//        }
	...
	private void formStringResultFrame() {
		...
		JOptionPane.showMessageDialog(p.getChartFrame(), "There is string data available also... please see View->Show string results menu item.");
	}
			...
			if (r.param != null && r.param_name != null) {
				for (int k=0; k<r.param.length && k<r.param_name.length; k++) {
					if (r.param[k] instanceof String) {
						...
						stringResultPanel.addStringResult(timeUtil.getTime(r.time), r.FarmName, r.ClusterName, r.NodeName, r.Module, r.param_name[k], (String)r.param[k], false);
        ...
        if (dd != null) {
            try{
				...
				if (!localTimeZone) {
					...
					if (timeSelector != null) timeSelector.setTimeZone(timeZone);
				}
            }catch(Exception e){
    ...
    static String adjustTimezone(String tz) {
    	if(tz.equals("CDT") || tz.equals("CST") 
        		|| tz.equals("EDT") || tz.equals("EST"))
        	return tz+"N";
        if (tz.equals("PKT")) tz = "GMT+5";
        else if (tz.equals("KST")) tz = "GMT+9"; 
        return tz;
    }
	...
	boolean plotResult(Result r) {
		...
		if (p == null || p.chartFrame == null) return false;
		synchronized (p.chartFrame.getTreeLock()) {
			if (r == null || r.param_name == null || r.param == null) return false;
			boolean ret = false;
			if (r.time < predicate.tmin || (predicate.tmax > 0 && r.time > predicate.tmax)) return false;
			try {
				timeOfLastResult = Math.max(timeOfLastResult, r.time);
				timeOfFirstResult = Math.min(timeOfFirstResult, r.time);
				if (timeSelector != null) timeSelector.setRange(timeOfFirstResult, timeOfLastResult, NTPDate.currentTimeMillis());
				for (int i = 0; i < predicate.parameters.length; i++) {
					for (int j = 0; j < r.param.length; j++) {
						if (predicate.parameters[i].equals(r.param_name[j])) {
							if (!Double.isNaN(r.param[j]) && !Double.isInfinite(r.param[j])) {
								if (currentUnit != null && baseUnit != null && currentUnit.containsKey(predicate.parameters[i])) {
									Unit u = (Unit)currentUnit.get(predicate.parameters[i]);
									p.add(predicate.parameters[i], r.time, convert(u, r.param[j]));
								} else
									p.add(predicate.parameters[i], r.time, r.param[j]);
								receivedNewData = true;
								ret = true;
							} else
    ...
    public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource().equals(mTime)) {
			if (localTimeZone) {
				...
				p.setTimeZone(TimeZone.getDefault().getID());
				if (timeSelector != null) timeSelector.setTimeZone(TimeZone.getDefault().getID());
		...
		if (e.getSource().equals(mnPeriod)) {
			...
			else timeSelector.setRange(timeOfFirstResult, timeOfLastResult, NTPDate.currentTimeMillis());
        ...
        if (e.getSource().equals(mPeriod)) {
            PlotIntervalSelector is = new PlotIntervalSelector(p.getChartFrame(), 
            		timeOfFirstResult, (continuous ? -1 : timeOfLastResult),
					localTime, timeZone); 
            is.setVisible(true);
            if(! is.closedOK()){
                is.dispose();
            ...
            continuous = (end == -1);
       		try{
                ...
                predicate.tmin = start; predicate.tmax = end;
       			dataprovider.addLocalClient(this, predicate);
       			resultsCount++;
                queryHasResults = false;
       		}catch(Exception ex){
	...
	public void adjustmentValueChanged() {
		...
		try {
			...
			if (continuous) {
				...
				logger.log(
						Level.INFO,
						"Registering for data on last "
						+ start / 1000 / 60
						+ " minutes");
			} else {
				logger.log(
						Level.INFO,
						"Registering for data from ="
						+ new Date(start)
						+ " to="
						+ new Date(end));
			}
