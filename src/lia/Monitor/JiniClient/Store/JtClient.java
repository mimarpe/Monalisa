...
public class JtClient extends MLSerClient {
	...
	private Hashtable<Integer, ClientHolder> htClientHolders;
	 ...
	 * @throws Exception
	 */
	public JtClient(ServiceID sid, String name, InetAddress _address, 
			ConnMessageMux _msgMux) throws Exception {
	...
	private static final boolean configStoreEnabled = AppConfig.getb("lia.Monitor.JiniClient.Store.JtClient.configStoreEnabled", false);
	...
	public void postSetMLVersion(final String version) {
		...
		if (sOldVersion == null || !sOldVersion.equals(version)) {
			...
			if (!TransparentStoreFactory.isMemoryStoreOnly() && version.indexOf("-") > 0) {
				...
				final DB db = new DB();
				db.query("UPDATE abping_aliases SET version='" + Formatare.mySQLEscape(sVersion) + "' WHERE name='" + Formatare.mySQLEscape(FarmName) + "';");
			}
		}
	}
	...
	public static String getMLVersion(final String sFarmName){
		...
		if (sRet==null){
			...
			final DB db = new DB("SELECT version FROM abping_aliases WHERE name='"+Formatare.mySQLEscape(sFarmName)+"';");
