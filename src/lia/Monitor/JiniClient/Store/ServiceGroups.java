...
public final class ServiceGroups extends Observable implements AppConfigChangeListener {
	...
	private static final Logger	logger = Logger.getLogger("lia.Monitor.JiniClient.Store.ServiceGroups");
	...
	public void reload(){
		...
		synchronized (groups){
			...
			final DB db = new DB(AppConfig.getProperty("lia.Monitor.JiniClient.Store.ServiceGroups.query", "SELECT site,groupname FROM pledged_dynamic WHERE length(groupname)>0;"));
