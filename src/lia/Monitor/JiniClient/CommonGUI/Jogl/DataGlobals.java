...
public class DataGlobals 
{
	...
	private static final transient Logger logger = Logger.getLogger(DataGlobals.class.getName());
...
//	    dest.put(data, 0, data.length);
    ...
    public static int getBubbleID( GL gl, String imgPath, float[] outherColor)
    {
        int bubbleID = 0;
        ...
        logger.log( Level.FINEST, "Loading texture for "+sFile+" has texture id: "+bubble_texture_id+ " -> done in "+(endTime-startTime)+" ms.");	
...
//        float fX, fY;
        ...
        gl.glNewList( bubbleID, GL.GL_COMPILE);
        {
            //draw first up face
            ...
            gl.glTexCoord2f( 0.5f, 0.5f);gl.glVertex3f( 0, 0, 0.5f); 
            for ( int slice=0; slice<nrPoints; slice++) {
                gl.glTexCoord2f( pts[slice][0]*.5f+.5f, pts[slice][1]*.5f+.5f);gl.glVertex3f( pts[slice][0], pts[slice][1], 0.5f);
            }
            gl.glTexCoord2f( pts[0][0]*.5f+.5f, pts[0][1]*.5f+.5f);gl.glVertex3f( pts[0][0], pts[0][1], 0.5f);
            gl.glEnd();
    ...
    public static int getCubeID( GL gl, String imgPath, String sObject)
    {
        int ID = 0;
        int texture_id = 0;
        ...
        long endTime = NTPDate.currentTimeMillis();
        logger.log( Level.FINEST, "Loading "+sObject+" texture has id: "+texture_id+ " -> done in "+(endTime-startTime)+" ms.");	
        ...
        gl.glNewList( ID, GL.GL_COMPILE);
        {
            ...
            gl.glBegin(GL.GL_QUADS);
	            // Front Face
	            gl.glTexCoord2f(0.0f, 0.0f); gl.glVertex3f(-dim, -dim,  dim);	// Bottom Left Of The Texture and Quad
	            gl.glTexCoord2f(1.0f, 0.0f); gl.glVertex3f( dim, -dim,  dim);	// Bottom Right Of The Texture and Quad
	            gl.glTexCoord2f(1.0f, 1.0f); gl.glVertex3f( dim,  dim,  dim);	// Top Right Of The Texture and Quad
	            gl.glTexCoord2f(0.0f, 1.0f); gl.glVertex3f(-dim,  dim,  dim);	// Top Left Of The Texture and Quad
	            // Back Face
	            gl.glTexCoord2f(1.0f, 0.0f); gl.glVertex3f(-dim, -dim, -dim);	// Bottom Right Of The Texture and Quad
	            gl.glTexCoord2f(1.0f, 1.0f); gl.glVertex3f(-dim,  dim, -dim);	// Top Right Of The Texture and Quad
	            gl.glTexCoord2f(0.0f, 1.0f); gl.glVertex3f( dim,  dim, -dim);	// Top Left Of The Texture and Quad
    ...
    public static int loadTexture( GL gl, String imgPath)
    {
        ...
        long endTime = NTPDate.currentTimeMillis();
        logger.log( Level.FINEST, "Loading texture "+imgPath+" with gl texture id "+texture_id+ " -> done in "+(endTime-startTime)+" ms.");
    ...
    public static int loadTexture( GL gl, BufferedImage bi, String textureName)
    {
        ...
        long endTime = NTPDate.currentTimeMillis();
        logger.log( Level.FINEST, "Loading texture "+textureName+" with gl texture id "+texture_id+ " -> done in "+(endTime-startTime)+" ms.");
    ...
    public static int loadTextureAlpha( GL gl, BufferedImage bi, String textureName)
    {
        ...
        long endTime = NTPDate.currentTimeMillis();
        logger.log( Level.FINEST, "Loading alpha texture "+textureName+" with gl texture id "+texture_id+ " -> done in "+(endTime-startTime)+" ms.");
