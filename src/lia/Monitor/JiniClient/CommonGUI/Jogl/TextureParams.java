...
public class TextureParams 
{
	...
	public static String sMapExt = ".oct";/** map file extension */
...
//    protected VectorO vTopLeft, vTopRight, vBottomLeft, vBottomRight;
    ...
    public void setSpecialFlags ( int flags) { special_flags|=flags; }
    public int getSpecialFlags () { return special_flags; }
    public boolean checkSpecialFlag( int flag) { return ( (special_flags&flag)>0 ); }
    public static boolean checkFlagIsSet( int flags, int one_flag) { return ( (flags&one_flag)>0 ); }
...
//    public static int[] texturesY = {2,2,2,2,2,2,2,2,2,2};
    ...
    static {
        texturesX0=Integer.parseInt(System.getProperty("nrXtiles0","4"));
        texturesY0=Integer.parseInt(System.getProperty("nrYtiles0","2"));
...
//    public static float[] depthZ = {4f, 1f, .5f, .25f, .125f, .0625f, .03125f, .015625f, .0078125f/*, .00390625f*/};
	...
	protected void setParent(Texture parent) {
		this.parent = parent;
        if (parent!=null) this.special_flags=parent.special_flags;
	}
...
//    }
	 ...
	 * @param tT
	 */
	public static void drawMapSlice( GL gl, float[][][] grid_pointer, int startX, int startY, int nx, int ny, float lT, float bT, float rT, float tT)
	{
            ...
            synchronized( JoglPanel.globals.syncGrid) {
				if ( JoglPanel.globals.globeRadius==-1 ) {
					...
					for( int y=0; y<ny; y++)
						for( int x=0; x<nx; x++) {
							gl.glBegin(GL.GL_TRIANGLE_STRIP);
							gl.glTexCoord2f( lT+x*stepxt, tT-y*stepyt);gl.glVertex3f( grid_pointer[0][startY+y][startX+x], grid_pointer[1][startY+y][startX+x], grid_pointer[2][startY+y][startX+x]);
							...
							//gl.glTexCoord2f( lT+x*stepxt, tT-(y+1)*stepyt);gl.glVertex3f( grid_pointer[0][startY+y+1][startX+x], grid_pointer[1][startY+y+1][startX+x], grid_pointer[2][startY+y+1][startX+x]);
							gl.glTexCoord2f( lT+(x+1)*stepxt, tT-(y+1)*stepyt);gl.glVertex3f( grid_pointer[0][startY+y+1][startX+x+1], grid_pointer[1][startY+y+1][startX+x+1], grid_pointer[2][startY+y+1][startX+x+1]);
							//gl.glTexCoord2f( lT+(x+1)*stepxt, tT-y*stepyt);gl.glVertex3f( grid_pointer[0][startY+y][startX+x+1], grid_pointer[1][startY+y][startX+x+1], grid_pointer[2][startY+y][startX+x+1]);
							gl.glEnd();
						}
				}
