 ...
 */
public class VrvsJoglPanel extends JoglPanel
...
//		optPan.cbNodeOpts.setLightWeightPopupEnabled(false);
		...
		BackgroundWorker.schedule( new TimerTask() {
			public void run() {
				Thread.currentThread().setName(" ( ML ) - VrvsJoglPanel - update colors and values and list of available meetings Thread");
...
//					VrvsNodesRenderer vnr = (VrvsNodesRenderer)renderer.getActiveNodesRenderer();
					optPan.meetingsUpdate.updateMeetingsMenu( dglobals.snodes, optPan.communityMenuList, optPan.meetingsMenuList);
