...
public class JoGLArc3D 
{
	 ...
	 */
	public JoGLArc3D( double fromLAT, double fromLONG, double toLAT, double toLONG, double radius, float degreeIncrement, double fractionRadius, float zTranslation) 
	{
	    if ( radius == -1 )
	    ...
	    if ( bPlaneProjection ) {
	        ...
	        this.zTranslation = 0;
		    vStart = new VectorO( fromLONG/360f*(float)Globals.MAP_WIDTH, fromLAT/180f*Globals.MAP_HEIGHT, 0);
		    vEnd = new VectorO( toLONG/360f*(float)Globals.MAP_WIDTH, toLAT/180f*Globals.MAP_HEIGHT, 0);
			...
			angleVectors = 2*Math.PI*vStart.distanceTo(vEnd)/Math.sqrt(Globals.MAP_WIDTH*Globals.MAP_WIDTH+Globals.MAP_HEIGHT*Globals.MAP_HEIGHT);
		...
		nrPoints = ( angleVectors < degreeIncrement*Math.PI/180 ) ? 5 : (int)( 5 + angleVectors/degreeIncrement/Math.PI*180);
...
*/	
	public void drawArc( GL gl)
	{
		...
		if ( bPlaneProjection ) {
			...
			for( int i=0; i<=nrPoints; i++) {
				...
				if ( points!=null ) points.add(new VectorO(vPoint));//add point to list of points that make the arc
	...
	public void drawArcAxis( GL gl)
	{
		if ( vCrossProduct == null )
			return;
		gl.glBegin(GL.GL_LINES);
			gl.glVertex3f( 0.0f, 0.0f, 0.0f);
			gl.glVertex3f( (float)vCrossProduct.getXprojection(), (float)vCrossProduct.getYprojection(), (float)vCrossProduct.getZprojection()+(float)zTranslation);
		gl.glEnd();
	}
	
	public void setZtranslation( float z) { zTranslation = z; }
	public float getZtranslation() { return zTranslation; }
