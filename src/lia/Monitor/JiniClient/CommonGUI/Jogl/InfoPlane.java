...
public class InfoPlane 
{
    ...
    private int hSpace = 4;//horizontal space between two components
    private int vSpace = 4; //vertical space between two components
     ...
     *   blue colors */
    private final float [][]first_color_set = new float[][]{ new float[]{0.2f, 0.2f, 0.6f}, new float[]{0.8f, 0.8f, 1f}};
    /** yellow colors */
    private final float [][]second_color_set = new float[][]{ new float[]{0x55/255f, 0x3c/255f, 0x35/255f}, new float[]{0x97/255f, 0xbc/255f, 0x8d/255f}};
...
//            logLevel = Level.FINEST;
    ...
    public InfoPlane( GL gl, boolean bl2r, boolean bt2b, int nBorder, int nFont)
    {
	    bLeft2Right = bl2r;
	    bTop2Bottom = bt2b;
        Init(gl);
    ...
    public InfoPlane( GL gl, int nBorder, int nFont)
    {
        this(gl);
        nBorderSize = nBorder;
        nFontSize = nFont;
    ...
    public InfoPlane( GL gl, float alphaValue)
    {
        this(gl);
        drawObject.alpha = alphaValue;
    }
    
    public InfoPlane( GL gl)
    {
        Init(gl);
    }
    public InfoPlane( GL gl, boolean bl2r, boolean bt2b)
    {
	    bLeft2Right = bl2r;
	    bTop2Bottom = bt2b;
        Init(gl);
    ...
    public InfoPlane( GL gl, boolean bl2r, boolean bt2b, float alphaValue)
    {
        this(gl, bl2r, bt2b);
        drawObject.alpha = alphaValue;
    }
    ...
    private int nLeftMargin=10, nRightMargin=10, nTopMargin=10, nBottomMargin=10;
    private int nCurrentXPosition=0, nCurrentYPosition =0;//current positions
    private int nCurrentMaxHeight=0;
    private int nHorizontalSpacing=10, nVerticalSpacing=10;//spacing between info boxes
    /**
    ...
    private Dimension getInfoBoxDims( int width, int height, boolean bShrinkWidth, boolean bShrinkHeight, String sTitle, String sDesc, int nImageID, int nImageW, int nImageH, float[] coords) {
	            ...
	            if ( yD > yImg+2+nImageH ) {//1.2
	                ...
	                if ( yD+hD <= nImageH+2 ) {//1.1.1
	                    ...
	                    if ( maxH<nImageH+2+nFontSize ) {
	                        ...
	                        String sRest = drawObject.getRemainingText( x+xD, y+yD, wT, yImg+2+nImageH-yD, sDesc, false);//allow image height to be surpass with maximum one line
                            ...
                            if ( hD2 == 0 )//there is no room to draw rest
...
//                    }
                    if ( bShrinkHeight ) {
                        ...
                        if ( hD == 0 ) //no description drawn, so reduce height to yT+hT+vSpace (only one!)
...
//            }
     ...
     */
    public void doInfoBox( int width, int height, boolean bShrinkWidth, boolean bShrinkHeight, String sTitle, String sDesc, int nImageID, int nImageW, int nImageH, float[] coords)
    {
        //get color set
        ...
        if ( bShrinkHeight && bShrinkWidth )
            dIB = getInfoBoxDims( width, height, bShrinkWidth, bShrinkHeight, sTitle, sDesc, nImageID, nImageW, nImageH, coords);
...
//            bImageHasMaxHeight = true;
        ...
        if ( bIsImage ) {//if we have an image in box, draw around it
	        ...
	        if ( hT <= nImageH+2 ) {//1.
	            ...
	            //title has room inside the image's height
	            drawObject.setLineWidth(1);drawObject.setColor(1f, 1f, 1f);
	            ...
	            drawObject.fillRectangle( x+xT-hSpace/2, y+yT-vSpace/2, wT+hSpace, hT+vSpace+vSpace/2, FOURTH_DEPTH);//draw in front of the other fill
	            //draw line below title
                if ( bAlphaOnlyBg )
                    drawObject.setOpaque(true);
	            drawObject.setLineWidth(vSpace/2);drawObject.setColor(1f, 1f, 1f);
	            int yLine = yT+hT+vSpace/2+vSpace/4;//at the middle of second half of vSpace from just below text
	            ...
	            if ( yD > yImg+2+nImageH ) {//1.2
	                //draw description using left height and maxW
	                int leftH = maxY-yD;
	                drawObject.setLineWidth(1);drawObject.setColor(0f, 0f, 0f);
	                ...
	                if ( yD+hD <= nImageH+2 ) {//1.1.1
	                    ...
	                    if ( maxH<nImageH+2+nFontSize ) {
	                        ...
	                        //and then draw in full info box width
	                        drawObject.setLineWidth(1);drawObject.setColor(0f, 0f, 0f);
	                        String sRest = drawObject.drawText( x+xD, y+yD, wT, yImg+2+nImageH-yD, sDesc, false);//allow image height to be surpass with maximum one line
	                        //compute new starting y position for remaining text
                            //2 variants: if text can be drawn in left space, use finalverticalspacing
                            //else, do not use
                            //try to see if space left 
	            ...
	            if ( maxH<nImageH+2+nFontSize ) {//2.1
	                //if there is no vertical space left for text to be put from image end to info box end  
	                //draw title without last line, and that's it
                    if ( bAlphaOnlyBg ) drawObject.setOpaque(true);
	                drawObject.setLineWidth(1);drawObject.setColor(1f, 1f, 1f);
	                drawObject.drawText( x+xT, y+yT, wT, nImageH+2, sTitle, true);//limit title height to image height
	                //compute text's limited height
	                hT = drawObject.getTextHeight( wT, sTitle, nImageH+2, true, false);
	                //draw background for title
                    if ( bAlphaOnlyBg ) drawObject.setOpaque(false);
	                drawObject.setColorv(color_set[0]);
	                drawObject.fillRectangle( x+xT-hSpace/2, y+yT-vSpace/2, wT+hSpace, hT+vSpace, FOURTH_DEPTH);//draw in front of the other fill
                    //draw line below title
                    if ( bAlphaOnlyBg ) drawObject.setOpaque(true);
                    drawObject.setLineWidth(vSpace/2);drawObject.setColor(1f, 1f, 1f);
                    int yLine = yT+hT+vSpace/2+vSpace/4;//at the middle of second half of vSpace from just below text
                    ...
                    if ( bAlphaOnlyBg ) drawObject.setOpaque(false);
                    drawObject.setColorv(color_set[0]);
                    drawObject.fillRectangle( x+xT-hSpace/2, y+yT-vSpace/2, lineWidth+hSpace, hT+vSpace+vSpace/2, FOURTH_DEPTH);//draw in front of the other fill
                    //draw line below title
                    if ( bAlphaOnlyBg ) drawObject.setOpaque(true);
                    drawObject.setLineWidth(vSpace/2);drawObject.setColor(1f, 1f, 1f);
                    int yLine = yT+hT+vSpace/2+vSpace/4;//at the middle of second half of vSpace from just below text
                    drawObject.drawLine( x+xT-hSpace/2, y+yLine, x+xT+lineWidth+hSpace/2, y+yLine);
                    //get description drawing
                    //if max height allows it
                    int yD = yT+hT+2*vSpace;//starting vertical position for drawing description
                    if ( yD < maxY ) {
        ...
        if ( bAlphaOnlyBg ) drawObject.setOpaque(true);
        drawObject.setLineWidth(nBorderSize);drawObject.setColorv(color_set[0]);
    ...
    public void doInfoBox( String sTitle, String sDesc, int nImageID, int nImageW, int nImageH, VectorO vPoint)
    {
        ...
        if ( bDebug ) System.out.println("Debug in InfoPlane.doInfoBox");
        Dimension d1 = drawObject.getTextDimensions(sTitle);
        if ( bDebug ) System.out.println( "title="+sTitle+" has dimensions: ("+d1.width+" px, "+d1.height+" px)");
        ...
        if ( bIsImage && h<nImageH+2/*picture border up and down*/ )
            h=nImageH+2;
        Dimension d2 = drawObject.getTextDimensions(sDesc);
        if ( bDebug ) System.out.println( "description="+sDesc+" has dimensions: ("+d2.width+" px, "+d2.height+" px)");
        if ( w < d2.width )
            w = d2.width;
        //final width
        w = 2*nBorderSize+hSpace+w+hSpace;
        if ( bDebug ) System.out.println( "info box width="+w);
        int y_desc = nBorderSize+vSpace+h+vSpace;//starting y position for description
        if ( bDebug ) System.out.println( "Desc text Y start position: "+y_desc);
        h = y_desc+d2.height+vSpace+nBorderSize;//final height
        if ( bDebug ) System.out.println( "info box height="+h);
        if ( bDebug ) System.out.println( "info box starting position: ("+x+" px, "+y+" px)");
        //draw outher border of the info box
        if ( bAlphaOnlyBg ) drawObject.setOpaque(true);
        drawObject.setLineWidth(nBorderSize);drawObject.setColorv(color_set[0]);
        ...
        if ( bAlphaOnlyBg ) drawObject.setOpaque(false);
        drawObject.setColorv(color_set[0]);
        drawObject.fillRectangle( x+nBorderSize+hSpace/2, y+nBorderSize+vSpace/2, w-2*nBorderSize-(bIsImage?2*hSpace+2+nImageW:0)-hSpace, vSpace/2+d1.height+vSpace+1/*line width*/, FOURTH_DEPTH);//draw in front of the other fill
        //draw line below title
        if ( bAlphaOnlyBg ) drawObject.setOpaque(true);
        drawObject.setLineWidth(1);drawObject.setColor(1f, 1f, 1f);
        drawObject.drawLine(x+nBorderSize+hSpace/2, y+nBorderSize+vSpace+d1.height+vSpace,
                x+w-nBorderSize-(bIsImage?2*hSpace+2+nImageW:0)-hSpace/2, y+nBorderSize+vSpace+d1.height+vSpace);
        if ( bIsImage ) {
	        //draw image border
	        drawObject.drawRectangle(x+w-nBorderSize-hSpace-nImageW-2, y+nBorderSize+vSpace, nImageW+2, nImageH+2);
	        //draw image
	        drawObject.drawImage( x+w-nBorderSize-hSpace-nImageW-1, y+nBorderSize+vSpace+1, nImageW, nImageH, nImageID);
        };
...
//        nWidthRB = Globals.ROTATION_BAR_WIDTH;
        ...
        if ( mouse_x>=nTotalWidth-nHeightRB && mouse_x<=nTotalWidth && mouse_y>=0 && mouse_y<=nTotalHeight )
            nSelectedBars |= POSITION_RIGHT;
        if ( mouse_y>=nTotalHeight-nHeightRB && mouse_y<=nTotalHeight && mouse_x>=0 && mouse_x<=nTotalWidth )
    ...
    public static int checkPointOnSpot( int mouse_x, int mouse_y, int nSpotPosition)
    {
        ...
        if ( mouse_x>= startSX-nSpotDim && mouse_x<=startSX+nSpotDim 
                && mouse_y>=startSY-nSpotDim && mouse_y<=startSY+nSpotDim )
...
//        startY = nHeight-nHeightRB;
        ...
        drawObject.setColor( cBorderRB);
        if ( bVerticalBar )
            drawObject.fillRectangle( startSX-1, startY+nBorderRB+nSpotDim, 2, nHeightRB-(nBorderRB+nSpotDim)*2, SECOND_DEPTH);
        else
            drawObject.fillRectangle( startX+nBorderRB+nSpotDim, startSY-1, nWidthRB-(nBorderRB+nSpotDim)*2, 2, SECOND_DEPTH);
        //start drawing the spot
        drawObject.setColor( cSpot);
...
//            drawObject.setFontSpaceY(oldFontSpaceY);
    ...
    private class Drawing2DTo3D {
        ...
        private GL gl;//graphical context used for drawing in 3D
        private MyGLUT ipglut = new MyGLUT();
        
        private VectorO vTopLeftCorner;//left-top corner for 2d plane in 3d space
        private VectorO vXAxis;//2d x axis mapped in 3d space
        private VectorO vYAxis;//2d y axis mapped in 3d space
        private VectorO vZAxis;//eye direction, perpendicular to (x,y) plane
        
        private VectorO vRotAxis1, vRotAxis2;//rotation axis for 3d objects to get into info plane
        ...
        private float fontScaleY, fontScaleX;//scale factor for x and y axis, should be the same
         ...
         */
        public int getTextHeight( int width, String sText, int desiredHeight, boolean bOnlyLess, boolean bFinalVertSpacing)
        {
                ...
                while( lineText.length()>0 ) {
                    ...
                    if ( bFirst ) {
                        ...
                        if ( currentHeight > fHeight ) {//if this line's height is over
                            ...
                            if ( bOverOnce )//if allowed to have more than desired height
                                return (int)Math.ceil(currentHeight*fontScaleY/pixelHeight);
                            else//else return anterior height
        ...
        public Dimension getTextDimensions( String sText)
        {
            ...
            for (int i=0; i<result.length; i++) {
                ...
                if ( bDebug ) System.out.println("Debug in Drawing2DTo3D.getTextDimensions ipglut.glutStrokeLength("+result[i]+")="+faux);
                if ( faux>dimx )
                    dimx = faux;
            }
            //compute text height based on number of lines
            dimy = (result.length*(1+fontSpaceY)-fontSpaceY)*(fontHeightTop-fontHeightBottom);
            if ( bDebug ) System.out.println("Debug in Drawing2DTo3D.getTextDimensions float ("+dimx+","+dimy+")");
            if ( bDebug ) System.out.println("Debug in Drawing2DTo3D.getTextDimensions dx="+dx+" nWidth="+nWidth+" fontSize="+fontSize+" pixelWidth="+pixelWidth+" fontHeightTop="+fontHeightTop+" fontScaleX="+fontScaleX);
            if ( bDebug ) System.out.println("Debug in Drawing2DTo3D.getTextDimensions int ("+(int)Math.ceil(dimx*fontScaleX/pixelWidth) +","+ (int)Math.ceil(dimy*fontScaleY/pixelHeight)+")");
            //convert 3d dimensions in 2d integer dimensions
            return new Dimension((int)Math.ceil(dimx*fontScaleX/pixelWidth), (int)Math.ceil(dimy*fontScaleY/pixelHeight));
        }
        ...
        public void drawImage( int x, int y, int width, int height, int imgID)
        {
            ...
            gl.glBegin( GL.GL_TRIANGLE_STRIP);
                gl.glTexCoord2f( lT, tT);gl.glVertex3f( 0f, 0f, 0f);
                gl.glTexCoord2f( lT, bT);gl.glVertex3f( 0f, -fHeight, 0f);
                gl.glTexCoord2f( rT, tT);gl.glVertex3f( fWidth, 0f, 0f);
                gl.glTexCoord2f( rT, bT);gl.glVertex3f( fWidth, -fHeight, 0f);
            gl.glEnd();
...
//            int y2 = y+height-1;
        ...
        public void setColor( int r, int g, int b) { 
            red = r/255f; green = g/255f; blue = b/255f;
            if ( bUseAlpha )
                gl.glColor4f( red, green, blue, alpha);
            else
                gl.glColor4f( red, green, blue, 1.0f);
         ...
         * @param string
         */
        private void renderString(GL gl, int font, String string) 
        {
            if ( bFontTypeBitmap ) {
