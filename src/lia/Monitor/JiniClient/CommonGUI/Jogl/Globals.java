        ...
        synchronized (objSync_US) {
            ...
            ttUpdateShadow = new TimerTask() {
                public void run() {
                     ...
                     */
                    int nTries = 20;
                    while ( JoglPanel.globals.mainPanel.isVisible() && Texture.texturesToLoad.size()>0 && nTries>0 ) {
                        nTries --;
                        try {
                    ...
                    if ( nTries>0 && JoglPanel.globals.mainPanel.isVisible() ) {
                    	ChangeRootTexture.init(Texture.nInitialLevel, ChangeRootTexture.CRT_KEY_MODE_UPDATE, JoglPanel.globals.mainPanel.monitor.main.jpbTextLoadProgress, JoglPanel.globals.mainPanel.monitor.main.jTextLoadBar);
...
//		float aux;
	...
	public static void printMemory()
	{
		...
		System.out.println("Total memory: "+tmem+" "+tc+" Free memory: "+fmem+" "+fc+" Maximal memory: "+mmem+" "+mc);
	}
...
//	    vPct.AddVector(vStart);
	    ...
	    positionStart.setZ( positionStart.getZprojection()+JoglPanel.globals.globeRadius-JoglPanel.globals.globeVirtualRadius);
	    positionEnd = new VectorO(vEnd);
	    positionEnd.setZ( positionEnd.getZprojection()+JoglPanel.globals.globeRadius-JoglPanel.globals.globeVirtualRadius);
	    positionAxis = positionStart.CrossProduct(positionEnd);
	    double alphaPos;
	    alphaPos = Math.acos( Globals.limitValue(positionStart.DotProduct( positionEnd)/positionStart.getRadius()/positionEnd.getRadius(), -1, 1) )*180/Math.PI;//in degree
	    ...
	    if ( interRadius < JoglPanel.globals.globeRadius )
	        interRadius = (positionEnd.getRadius()-JoglPanel.globals.globeRadius)*fraction+JoglPanel.globals.globeRadius;
...
//		Rv = JoglPanel.globals.globeVirtualRadius;
     ...
     */
    public static void drawCircle(GL gl, int slices, float fRadius)
    {
    	if ( slices == 0 )
    		return;
     ...
     * @param fRadius - radius of circle
     */
    public static void drawHashedHexagon(GL gl)
    {
        //draw hexagon
        int i;
