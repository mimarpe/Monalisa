 ...
 * JoglPanel
 */
public abstract class JoglPanel extends JPanel 
...
//          toolbar.add(comboNodesRenderer);
    ...
    public void setSerMonitor(SerMonitorBase ms) {
        ...
        try {
            ...
            Texture.nInitialLevel = Integer.parseInt(AppConfig.getProperty("jogl.Texture.nForceInitialLevel", prefs.get( "Texture.StartUpLevel", AppConfig.getProperty("jogl.Texture.nInitialLevel","0"))));
...
//            System.out.println("maximum allowed level of detail is: "+Texture.nMaximumLevel);
            Texture.textureDataSpace.setMaxTextureCacheSize(Float.parseFloat(prefs.get( "TextureDataSpace.maxTextureCacheSize", "10")));
        } catch ( Exception ex) {
            //????
            System.out.println("[JoglPanel] Could not save texture start up level preference, error: "+ex.getMessage());
        }
    }
...
//        panelTools.setBorder(BorderFactory.createLineBorder(Color.RED));
        ...
        proj_button = new JButton(null, globals.bMapTransition2Sphere?dglobals.iconSphereProj:dglobals.iconPlaneProj);
