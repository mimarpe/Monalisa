...
//    private static final transient Logger logger = Logger.getLogger(COMPONENT);
     ...
     */
    private HashMap hmPathTextures;
    private DoubleLinkedList dllTextures;
    /** the maximum number of textures in this cache */
    private int maxTextureCacheSize=30;
    private LinkedList invalidTextures;
...
//        this.maxTextureCacheSize = maxTextureCacheSize;
    ...
    public JoGLCachedTexture findTexture(String resultID) {
    	...
    	synchronized (dllTextures) {
    		if((result = (JoGLCachedTexture) hmPathTextures.remove(resultID)) != null){
        		...
        		if ( bDebug )
        			System.out.println("----["+dllTextures.getSize()+"]----------- reusing texture "+result.resultID+" -> "+result.joglID);
    		}
    	}
    	return result;
    }
    ...
    public void cacheTexture(JoGLCachedTexture jct) {
    	synchronized (dllTextures) {
    		...
    		if ( bDebug )
    			System.out.println("####["+dllTextures.getSize()+"]############# caching texture "+jct.resultID+" -> "+jct.joglID);
		}
    }
