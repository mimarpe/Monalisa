 ...
 * @author mluc
 */
public class FarmsJoglPanel extends JoglPanel
										implements ActionListener 
{
    private static final long serialVersionUID = -3246699653643938275L;
	/** Logger used by this class */
	private static final transient Logger logger = Logger.getLogger(FarmsJoglPanel.class.getName());

    private Object syncGparamCombobox = new Object();
...
//		  optPan.setMaximumSize(new Dimension(1000, 80));
		  ...
		  BackgroundWorker.schedule( new TimerTask() {
		      public void run() {
		          Thread.currentThread().setName(" ( ML ) - JOGL - FarmsJoglPanel Timer Thread: update colors and values for options panel");
                  ...
                  if ( anr.OptionsPanelValues!=null ) {
    		          try {
                          ...
                          if ( hWANOptions!=null ) {
         		              optPan.csWAN.setValues( ((Double)hWANOptions.get( "MinValue")).doubleValue(), ((Double)hWANOptions.get( "MaxValue")).doubleValue());				
        		              optPan.csWAN.setLimitValues( ((Double)hWANOptions.get( "LimitMinValue")).doubleValue(), ((Double)hWANOptions.get( "LimitMaxValue")).doubleValue());
        		              optPan.csWAN.setColors(((Color)hWANOptions.get("MinColor")), ((Color)hWANOptions.get("MaxColor")));  // red(min) --> yellow(max)
                          };
    		          } catch (Exception ex) {
                      ...
                      try {
                          ...
                          if ( hNFOptions!=null ) {
                              optPan.csNetFlow.setValues( ((Double)hNFOptions.get( "MinValue")).doubleValue(), ((Double)hNFOptions.get( "MaxValue")).doubleValue());              
                              optPan.csNetFlow.setLimitValues( ((Double)hNFOptions.get( "LimitMinValue")).doubleValue(), ((Double)hNFOptions.get( "LimitMaxValue")).doubleValue());
                              optPan.csNetFlow.setColors(((Color)hNFOptions.get("MinColor")), ((Color)hNFOptions.get("MaxColor")));  // red(min) --> yellow(max)
                          };
                      } catch (Exception ex) {
    		          ...
    		          try {
                          ...
                          if ( hPingOptions!=null ) {
        		              optPan.csPing.setValues( ((Double)hPingOptions.get( "MinValue")).doubleValue(), ((Double)hPingOptions.get( "MaxValue")).doubleValue());				
        		              optPan.csPing.setColors(((Color)hPingOptions.get("MinColor")), ((Color)hPingOptions.get("MaxColor")));  // red(min) --> yellow(max)
                          };
    		          } catch (Exception ex) {
...
//	            togglePingLinksGroup();
	        ...
	        ((FarmsNodesRenderer)renderer.getActiveNodesRenderer()).changeLinkAnimationStatus(optPan.kbAnimateWAN.isSelected());
