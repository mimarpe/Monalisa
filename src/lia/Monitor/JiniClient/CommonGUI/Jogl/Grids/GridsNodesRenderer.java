...
//					}
					...
					float nFactor=1;
					switch ( node.myID ) {
					case 0: nFactor=2; break;
					case 1: nFactor=1.5f; break;
					case 2: nFactor=1; break;
					default: 
					}
...
//		packageID = DataGlobals.getCubeID( gl, "lia/images/ml_package.jpg", "Package Cube");
		...
		gl.glNewList( packageID, GL.GL_COMPILE);
		{
	...
	private float[] topCenter = new float[] {0,0,.5f};
	 ...
	 * @param radius
	 */
	private void constructNode( GL gl, int components, float[] fractions, Color[] colorsUp, Color[] colorsDown)
	{
...
//		int max_points = 32;//total number of points to be drawn
		...
		for( int i =0; i<components; i++) {
			...
			for( int j=0; j<points; j++) {
				...
				pa1[0]=p1[0];pa1[1]=p1[1];pa1[2]=p1[2];
				pa2[0]=p2[0];pa2[1]=p2[1];pa2[2]=p2[2];
				...
				gl.glVertex3fv( aux, 0);
				gl.glColor3f(colorsDown[i].getRed()/255f, colorsDown[i].getGreen()/255f, colorsDown[i].getBlue()/255f);
				//---end code insertion---//
	...
	public void computeLinks( GL gl, Object[] graphicalAttrs) {
		...
		try {
			for( Iterator it = ((HashMap)graphicalAttrs[2]).keySet().iterator(); it.hasNext(); ) {
				...
				+ (Max>0?(int)((MAX_SPEED_Link_SEGMENT_STEPS-MIN_SPEED_Link_SEGMENT_STEPS)*val/Max):0);
				...
				if ( obj == null ) {
					...
					//					arc3D.drawArc(gl);
					JoGLArc3D Darc3D;
					Darc3D = new JoGLArc3D( link.fromLAT, link.fromLONG, link.toLAT, link.toLONG, globeRad, 10, 0.05, -globeRad+globeVirtRad/*, 0.2*/);
...
//						gl.glRotatef( aPackage.fRotAngle2, 0, 0, 1);
						...
						System.out.println( "Exception drawing package for link: "+aPackage.sDescription+". Package may be during initialisation. Ignore it.");
						ex.printStackTrace();
					}
					gl.glPopMatrix();
				};
	...
	public boolean isDeadLink( Object link, HashMap hLinkAttrs, HashMap localNodes)
	{
		...
		if ( nodeFrom==null || nodeTo==null
				|| !localNodes.containsKey(nodeFrom) || !localNodes.containsKey(nodeTo)
				|| nodeFrom.bHiddenOnMap || nodeTo.bHiddenOnMap 
				|| !getLinks(nodeFrom).containsKey(nodeTo)
				|| !isValidLink(link)
		)
			return true;
		return false;
	...
	protected class LinkPackage
	{
		...
		public LinkPackage() {}//empty constructor to have something...
	}    
...
//									} catch(Exception ex) {Max=0;}
									if ( link.data!=null && link.data instanceof Double && (data=((Double) (link.data)).doubleValue())!=0 && ( data<Max && data<link.speed) && Max>0 )
