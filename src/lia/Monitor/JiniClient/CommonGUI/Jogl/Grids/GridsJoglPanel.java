...
//	private long lMinUpdateNodeTime = 30000;
	 ...
	 */
	private HashMap hNodesPreds = new HashMap();
	...
	private String gridmap_suffix = "_GridMap";
...
//		Border raisedetched = BorderFactory.createEtchedBorder(EtchedBorder.RAISED);
		...
		jtbLCG = new JToggleButton("<html><center>LCG<br>Tier-0 to Tier-1", MainBase.loadIcon("lia/images/grids/lcg_logo.png"), true);//<html><center>LCG<br>(Tier-0/Tier-1)
...
//		jtbLCG.setBorder(raisedetched);
		...
		jtbEGEE = new JToggleButton("<html>Enabling Grids<br>for E-sciencE", MainBase.loadIcon("lia/images/grids/egee_logo.png"), true);
...
//					System.out.println("<mluc> Node "+node+" has sid "+node.sid+" and key "+ikey);
					if ( ikey==null || !node.client.containsPredicate(ikey) ) {
						 ...
						 predGrids = new monPredicate ("*","%"+gridmap_suffix, "*", -120000, -1 , new String[] {"Info"} , null);
...
//							 System.out.println("<mluc> Registration of first node.");
						 ...
						 logger.info("MonALISA service that provides GridMap info is "+node.shortName+" ["+node.IPaddress+"]");
...
//		}
	...
	public void newFarmResult(MLSerClient client, Object res) {
		...
		if ( bNewNode )
			DataRenderer.sendGlobeEvent( DataRenderer.GLOBE_OPTION_PANEL_CHANGE, Integer.valueOf(DataRenderer.GLOBE_EVENT_PARAM_NODES_CHANGED));
	}
...
//			System.out.println("node "+siteAddress+"\nconnected to: "+((LcgLdapInfo)info).connectedTo);
		...
		for ( int i=0; i<dglobals.vnodes.size(); i++) {
			rcNode aNode = (rcNode)dglobals.vnodes.get(i);
...
//			}
			...
			if ( lcgSite.siteSupportContact!=null )
				buf.append("\nContact: ").append((lcgSite.siteSupportContact.startsWith("mailto: ")?lcgSite.siteSupportContact.substring(8):lcgSite.siteSupportContact));
			buf.append("\nSite type: ").append((lcgSite.tierType>=0?"Tier "+lcgSite.tierType:"N/A"));
//			node.shortName += "\nAvg RAM size: "+lcgSite.mainMemoryRAMSize;
			if ( lcgSite.operatingSystemName != null && lcgSite.operatingSystemRelease!=null & lcgSite.operatingSystemVersion!=null )
				buf.append("\nOperating system: ").append(lcgSite.operatingSystemName).append(" ").append(lcgSite.operatingSystemVersion).append(" ").append(lcgSite.operatingSystemRelease);
			...
			//and add a new one
			if ( node.IPaddress!=null && ( lcgSite.connectedTo==null || !node.IPaddress.equals(lcgSite.connectedTo) ) ) {
...
//				System.out.println("node "+lcgSite);
				...
				node.errorCount = -1;//first draw the nodes as red dots
			else*/
...
//			}
			...
			if ( gridSite.siteSupportContact!=null )
				buf.append("\nContact: ").append((gridSite.siteSupportContact.startsWith("mailto: ")?gridSite.siteSupportContact.substring(8):gridSite.siteSupportContact));
//			buf.append("\nSite type: ").append((gridSite.tierType>=0?"Tier "+gridSite.tierType:"N/A"));
			node.shortName = buf.toString();
			//if the link to the other end has changed, then remove the current link
			//and add a new one
			if ( node.IPaddress!=null && ( gridSite.connectedTo==null || !node.IPaddress.equals(gridSite.connectedTo) ) ) {
...
//		int tier1Count = 0;
		...
		for ( Enumeration en = dglobals.vnodes.elements(); en.hasMoreElements(); ) {
			rcNode aNode = (rcNode)en.nextElement();
			try {
		...
		int colorID = 1;
		for ( Enumeration en = dglobals.vnodes.elements(); en.hasMoreElements(); ) {
			rcNode node1 = (rcNode)en.nextElement();
			for ( Enumeration en2 = dglobals.vnodes.elements(); en2.hasMoreElements(); ) {
				rcNode node2 = (rcNode)en2.nextElement();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if ( e.getSource() instanceof JToggleButton ) {
			...
			if ( bAtLeastOneNode || bLinksChanged )
		        DataRenderer.sendGlobeEvent( DataRenderer.GLOBE_OPTION_PANEL_CHANGE, Integer.valueOf(DataRenderer.GLOBE_EVENT_PARAM_LINKS_CHANGED));
			if ( bAtLeastOneNode )
				DataRenderer.sendGlobeEvent( DataRenderer.GLOBE_OPTION_PANEL_CHANGE, Integer.valueOf(DataRenderer.GLOBE_EVENT_PARAM_NODES_CHANGED));
		}
	}
