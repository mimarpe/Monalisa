...
public class ZoomMapRenderer implements GLEventListener 
{
	...
	private MyGLU myglu = new MyGLU();
	private MyGLUT myglut = new MyGLUT();
...
//		gl.glColorMaterial(GL.GL_FRONT, GL.GL_AMBIENT_AND_DIFFUSE);
		...
		try {
			...
			logger.log( Level.FINEST, "Loading sky texture -> "+(bi==null?"error! -> ":"")+" -> done in "+(endTime-startTime)+" ms.");	
		} catch (Exception ex) {
		...
		try {
			...
			endTime = NTPDate.currentTimeMillis();
			logger.log( Level.FINEST, "Loading no texture"+(biNOT==null?" error!":"")+" -> done in "+(endTime-startTime)+" ms.");	
		} catch (Exception ex) {
        ...
        if ( Texture.nInitialLevel>0 ) {
            BackgroundWorker.schedule(new TimerTask() {
                public void run() {
                	...
                	ChangeRootTexture.init(Texture.nInitialLevel, ChangeRootTexture.CRT_KEY_MODE_LOAD, JoglPanel.globals.mainPanel.monitor.main.jpbTextLoadProgress, JoglPanel.globals.mainPanel.monitor.main.jTextLoadBar);
...
//        gl.glEndList();
        ...
        gl.glNewList( hexagonID, GL.GL_COMPILE);
        {
            //gl.glDisable( GL.GL_TEXTURE_2D);
...
//        float supl_radius = 0.00001f;
        ...
        if ( !AppConfig.getProperty("jogl.rederer.map.noBorder", "false").equals("true") ) {
            ...
            JoglPanel.globals.countriesBordersID0 = doCoutryMap( gl, true, "lia/images/joglpanel/shapefile/cntry98.shp", new int[] {232}, new int[][] { {135}}, 218f/255f, 218f/255f, 170f/255f);/*144f/255f, 151f/255f, 141f/255f*///US big border, because states have their own borders
            JoglPanel.globals.countriesBordersID90 = doCoutryMap( gl, false, "lia/images/joglpanel/shapefile/cntry98.shp", new int[] {232}, new int[][] { {135}}, 218f/255f, 218f/255f, 170f/255f);//US big border, because states have their own borders
            JoglPanel.globals.usStatesBordersID0 = doCoutryMap( gl, true, "lia/images/joglpanel/shapefile/usa_st.shp", new int[] {0,49}, new int[][] { null, null}, 223.0f/255.0f, 224.0f/255.0f, 134.0f/255.0f);//alaska and small isles on far east of world map
            JoglPanel.globals.usStatesBordersID90 = doCoutryMap( gl, false, "lia/images/joglpanel/shapefile/usa_st.shp", new int[] {0,49}, new int[][] { null, null}, 223.0f/255.0f, 224.0f/255.0f, 134.0f/255.0f);//alaska and small isles on far east of world map
        }
		//gl.glEnable(GL.GL_TEXTURE_2D);
    ...
    private int doCitiesList( GL gl, CapitalsGISInfo cities, int hexagonID, boolean bPlaneProj)
    {
        float []coords=null;
        VectorO VzAxis = new VectorO(0,0,1);
        ...
        gl.glNewList( cityGisID, GL.GL_COMPILE);
        {
            ...
            for ( int i=0; i<cities.getTotalNumber(); i++) {
                ...
                if ( !bPlaneProj ) {
                    VectorO vDir;
                    vDir = new VectorO( coords[0], coords[1], coords[2]-JoglPanel.globals.globeVirtualRadius+JoglPanel.globals.globeRadius+supl_radius);
                    vDir.Normalize();
                    VectorO VRotAxis = VzAxis.CrossProduct(vDir);
                    // rotate z to Vdir around vectorial product with dot product 
                    gl.glRotatef( (float)(Math.acos(VzAxis.DotProduct(vDir))* 180 / Math.PI), VRotAxis.getX(), VRotAxis.getY(), VRotAxis.getZ());
                };
     ...
     * @return
     */
    private int doCoutryMap(GL gl, boolean bPlaneProj, String sShapeFileName, int[] ignoreShapeList, int [][]ignorePartList, float fRed, float fGreen, float fBlue) 
    {
...
//            float []AntCoords=null;
    ...
    //        nf.setMinimumFractionDigits(2);
            ...
            gl.glNewList( countryID, GL.GL_COMPILE);
            {
                gl.glPushMatrix();
...
//                                                          System.out.println("ignoring part "+i+"."+iPart);
                                ...
                                if ( !bIgnore ) {
                                    ...
                                    for ( int k = psShape.panPartStart[iPart-1]; k<j; k++) {
                                        if ( bPlaneProj )
                                            CurCoords = Globals.point2Dto3D0( (float)psShape.padfY[k], (float)psShape.padfX[k], CurCoords, supl_radius);
                                        else
                                            CurCoords = Globals.point2Dto3D90( (float)psShape.padfY[k], (float)psShape.padfX[k], CurCoords, supl_radius);
...
//                                                  System.out.println("ignoring part "+i+"."+iPart);
        ...
        logger.fine("doCoutryMap operation for "+sShapeFileName + (bPlaneProj?" plane projection":" sphere projection")+" took "+(lEndTime-lStartTime)+" miliseconds.");
        return countryID;
    }
    ...
    public void displayTest(GLAutoDrawable gLDrawable)
    {
      ...
      gl.glBegin(GL.GL_TRIANGLE_STRIP); 
	      ...
	      gl.glTexCoord2f( 0f, 1f);gl.glVertex3f( -5f, 5f, 0f);
	      ...
	      gl.glTexCoord2f( 0f, 0f);gl.glVertex3f( -5f, -5f, 0f);
	      ...
	      gl.glTexCoord2f( 1f, 1f);gl.glVertex3f( 5f, 5f, 0f);
	      ...
	      gl.glTexCoord2f( 1f, 0f);gl.glVertex3f( 5f, -5f, 0f);
      gl.glEnd();
      DrawFrustum(gl);
      gl.glFlush();
    }
...
//			return;
		...
		gl.glGetIntegerv(GL.GL_POLYGON_MODE, params, 0);
		if ( JoglPanel.globals.charPressed=='F' ) {
			JoglPanel.globals.charPressed='f';
			if ( params[0] == GL.GL_FILL )
				gl.glPolygonMode( GL.GL_FRONT, GL.GL_LINE);
			else if ( params[0] == GL.GL_LINE )
				gl.glPolygonMode( GL.GL_FRONT, GL.GL_FILL);
		};				
		if ( JoglPanel.globals.charPressed=='B' ) {
			JoglPanel.globals.charPressed='b';
			if ( params[1] == GL.GL_FILL )
				gl.glPolygonMode( GL.GL_BACK, GL.GL_LINE);
			else if ( params[1] == GL.GL_LINE )
				gl.glPolygonMode( GL.GL_BACK, GL.GL_FILL);
		}
		...
		myglu.gluLookAt( gl, 
			(float)JoglPanel.globals.EyePosition.getXprojection(), (float)JoglPanel.globals.EyePosition.getYprojection(), (float)JoglPanel.globals.EyePosition.getZprojection(),
			(float)JoglPanel.globals.EyePosition.getXprojection()+(float)JoglPanel.globals.EyeDirection.getXprojection(), (float)JoglPanel.globals.EyePosition.getYprojection()+(float)JoglPanel.globals.EyeDirection.getYprojection(), (float)JoglPanel.globals.EyePosition.getZprojection()+(float)JoglPanel.globals.EyeDirection.getZprojection(),
			(float)JoglPanel.globals.EyeNormal.getXprojection(), (float)JoglPanel.globals.EyeNormal.getYprojection(), (float)JoglPanel.globals.EyeNormal.getZprojection() );
...
//        gl.glDisable(GL.GL_FOG);
		...
		gl.glBegin( GL.GL_TRIANGLE_STRIP);
			gl.glTexCoord2f( lT, tT);gl.glVertex3f( JoglPanel.globals.VskyLT.getX(),JoglPanel.globals.VskyLT.getY(),JoglPanel.globals.VskyLT.getZ());
			gl.glTexCoord2f( lT, bT);gl.glVertex3f( JoglPanel.globals.VskyLB.getX(),JoglPanel.globals.VskyLB.getY(),JoglPanel.globals.VskyLB.getZ());
			gl.glTexCoord2f( rT, tT);gl.glVertex3f( JoglPanel.globals.VskyRT.getX(),JoglPanel.globals.VskyRT.getY(),JoglPanel.globals.VskyRT.getZ());
			gl.glTexCoord2f( rT, bT);gl.glVertex3f( JoglPanel.globals.VskyRB.getX(),JoglPanel.globals.VskyRB.getY(),JoglPanel.globals.VskyRB.getZ());
		gl.glEnd();
...
//        gl.glEnable(GL.GL_FOG);
	...
	public void drawBordersAndCities( GL gl) {
    ...
    public void displayAfter(GLAutoDrawable gLDrawable) 
    {
        ...
        if ( /*!JoglPanel.globals.mainPanel.renderer.sr.IsInRotation() && */Globals.bShowRotationBar && ( !Globals.bAutoHideRotationBar || Globals.bAutoHideRotationBar && Globals.nShowRBs!=0 ) ) {
            gl.glEnable(GL.GL_BLEND);
            ...
            if ( (Globals.nShowRBs&InfoPlane.POSITION_LEFT)>0 )
                ipScroll.doRotationBar( InfoPlane.POSITION_LEFT, JoglPanel.globals.nRBHotSpotPos, Globals.bShowRotationBarTooltip);
            if ( (Globals.nShowRBs&InfoPlane.POSITION_TOP)>0 )
                ipScroll.doRotationBar( InfoPlane.POSITION_TOP, JoglPanel.globals.nRBHotSpotPos, Globals.bShowRotationBarTooltip);
            if ( (Globals.nShowRBs&InfoPlane.POSITION_RIGHT)>0 )
                ipScroll.doRotationBar( InfoPlane.POSITION_RIGHT, JoglPanel.globals.nRBHotSpotPos, Globals.bShowRotationBarTooltip);
            if ( (Globals.nShowRBs&InfoPlane.POSITION_BOTTOM)>0 )
                ipScroll.doRotationBar( InfoPlane.POSITION_BOTTOM, JoglPanel.globals.nRBHotSpotPos, Globals.bShowRotationBarTooltip);
            gl.glDisable(GL.GL_BLEND);
        };
...
//		JoglPanel.globals.lLastRefreshTime = NTPDate.currentTimeMillis();
	...
	public void reshape(GLAutoDrawable gLDrawable, int x, int y, int width, int height) 
	{
		...
		float D;
		D = width/2f/(float)Math.tan(Globals.FOV_ANGLE/2.0 * Math.PI/180.0f)*Globals.MAP_WIDTH/Xpt_max;
...
//		JoglPanel.globals.bIsIdle = true;
	...
	public void displayChanged(GLAutoDrawable arg0, boolean arg1, boolean arg2) {
		//nothing
        //System.out.println("OpenGL Canvas display changed");
	}
	...
	private void DrawFrustum( GL gl)
	{
		...
		gl.glTranslatef( (float)JoglPanel.globals.EyePosition.getXprojection(), (float)JoglPanel.globals.EyePosition.getYprojection(), (float)JoglPanel.globals.EyePosition.getZprojection());
...
//			gl.glVertex3f( right_near, top_near, -Globals.NEAR_CLIP);
	...
	public void drawCone( GL gl, int nMaxPoints)
	{
	...
	public void setAxesID( GL gl)
	{
	    if ( axesID == 0 ) {
			...
			gl.glNewList( axesID, GL.GL_COMPILE);
			{
...
*/	
	 ...
	 * @param vStartFrom
	 */
	public void drawTextFrom( GL gl, String sText, float[] startFrom)
	{
	    ...
	    if ( bIncXPositive ) {//texts are put from left to right
		    if ( start_text_x + dx_text < 0.475f*dx ) //another text can be put on same line
		        start_text_dx = start_text_x;
		    else {//go to next line
		        start_text_dx = -0.475f*dx;
		        if ( !bIncYPositive )//which line?
		    ...
		    if ( start_text_x - dx_text > -0.475f*dx )//another text can be put on same line
		        ...
		        if ( !bIncYPositive ) //which line? up or down?
			        start_text_y -= (start_text_max_height + 0.025f*dy);
		        else
			        start_text_y += start_text_max_height + 0.025f*dy;
		        start_text_max_height = 0f;
...
//			gl.glVertex3f( vPoint2.getX(), vPoint2.getY(), vPoint2.getZ());
    ...
    public void buildClouds( GL gl)
    {
        ...
        if ( JoglPanel.globals.clouds_tid!=-1 ) {
	        ...
	        gl.glNewList( JoglPanel.globals.clouds_opengl_id, GL.GL_COMPILE);
	        {
...
//                    System.out.println(new Date(NTPDate.currentTimeMillis())+" here2");
	 ...
	 * @param gl
	 */
	public static void buildMoon(GL gl)
	{
        ...
        //System.out.println("routerID="+ routerID);
        gl.glNewList( JoglPanel.globals.moon_opengl_id, GL.GL_COMPILE);
        {
	...
	public static void drawMoon(GL gl)
	{
	    if ( JoglPanel.globals.mapAngle==90 && JoglPanel.globals.moon_opengl_id!=0 ) {
	        gl.glPushMatrix();
		    gl.glTranslatef(JoglPanel.globals.vMoonPosition.getX(), 
		            JoglPanel.globals.vMoonPosition.getY(),
	...
	public static void drawTexturedSphere( GL gl, int tid, int slices_x, int slices_y)
	{
	...
	public static void drawSphere( GL gl, int slices_x, int slices_y)
	{
		float dThetaLONG, dThetaLAT;	//minimal angle to get from one point to the next
		dThetaLONG = (float)(2*Math.PI/slices_x);
		dThetaLAT = (float)(Math.PI/slices_y);
