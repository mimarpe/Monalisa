...
//	public int nodesShow = 1;         // what do the bubbles are = one of NODE_xxx constants
	...
	private int nLevels;//levels of detail for drawing a cap as several spherical segments drawn one on top of another
    ...
    private HashMap mstAttrs = new HashMap();
	/**
...
//		Color color;
        ...
        int components;
        int nodeID;
		for ( Iterator it = ((HashMap)((HashMap)graphicalAttrs[1])).keySet().iterator(); it.hasNext(); ) {
		    node = (rcNode)it.next();
		    //save the matrix
		    gl.glPushMatrix();
			try {
				obj = ((HashMap)((HashMap)graphicalAttrs[1])).get(node);
...
//					color = (Color)hAttrs.get("NodeColor");
                    ...
                    if ( nodeID>0 && vectors!=null ) {
                        if ( bOnTopView )
                            vNewPos = recomputeVectors( ((HashMap)((HashMap)graphicalAttrs[1])), htComputedNodes, node, vectors, radius);
                        else 
                            vNewPos = vectors[1];
                        if ( components==1 ) {
...
*/
		...
		if ( capID==0 ) {
			...
			gl.glNewList( capID, GL.GL_COMPILE);
			{
...
//        p1[2] = Vp.getZ(); p2[2] = Vp.getZ()+.5f;
			...
			for( Iterator it = ((HashMap)graphicalAttrs[2]).keySet().iterator(); it.hasNext(); ) {
				...
				if ( conID > 0 ) {//if there is an object to be drawn
					...
					try {
					    ...
					    //2. operation: rotate from (0,0,1) to Vdir
					    gl.glRotatef( fLinkArrowRotationAngle, vLinkArrowRotationAxis.getX(), vLinkArrowRotationAxis.getY(), vLinkArrowRotationAxis.getZ());
					    //1. operation: scale to radius dimmensions:
    ...
    private void computeMST( GL gl, Object[] graphicalAttrs, boolean bInvalidateLinks)
    {
        //get mst information
        Vector mstData;
		...
		try {
	        for ( int i=0; i<mstData.size(); i++) {
			    ...
			    if ( obj == null ) {
					nFrom = (rcNode)JoglPanel.dglobals.snodes.get(dlink.fromSid);
					nTo = (rcNode)JoglPanel.dglobals.snodes.get(dlink.toSid);
		...
		try {
			...
			for( Iterator it = mstAttrs.keySet().iterator(); it.hasNext(); ) {
				...
				nFrom = (rcNode)JoglPanel.dglobals.snodes.get(sidFrom);
				nTo = (rcNode)JoglPanel.dglobals.snodes.get(sidTo);
		        if ( nFrom==null || nTo==null || nFrom.bHiddenOnMap || nTo.bHiddenOnMap || !mstData.contains(dlink) )
		            it.remove();
			}
		} catch (Exception ex) {
		...
		try {
			...
			for( Iterator it = mstAttrs.keySet().iterator(); it.hasNext(); ) {
		        ...
		        if ( obj == null ) {
			        ...
			        gl.glLineWidth(2f);
					JoGLArc3D arc3D = new JoGLArc3D( ((Float)hLinkAttrs.get("fromLAT")).floatValue(), ((Float)hLinkAttrs.get("fromLONG")).floatValue(), ((Float)hLinkAttrs.get("toLAT")).floatValue(), ((Float)hLinkAttrs.get("toLONG")).floatValue(), globeRad, 10, 0.02, -globeRad+globeVirtRad);
					arc3D.drawArc(gl);
			        gl.glLineWidth(1f);
					gl.glEndList();
...
//					arc3D.drawArc(gl);
				    ...
				    if ( link.peersQuality != null )  
				        Darc3D = new JoGLDirectedArc3D(link.fromLAT, link.fromLONG, link.toLAT, link.toLONG, globeRad, 10, 0.05, -globeRad+globeVirtRad, 0.2);
				    else // only ( link.inetQuality.length >= 3 ) 
				        Darc3D = new JoGLDirectedArc3D(link.fromLAT, link.fromLONG, link.toLAT, link.toLONG, globeRad, 10, 0.05, -globeRad+globeVirtRad, 0.4);
					Darc3D.drawArc(gl);
					gl.glEndList();
			        hLinkAttrs.put("LinkID", Integer.valueOf(linkID));
...
*/
	...
	private Color getNodeColor(rcNode n) {
	   ...
	   if(val == -1)
		 return null;
	   double delta = ((VrvsJoglPanel)JoglPanel.globals.mainPanel).optPan.csNodes.maxValue - ((VrvsJoglPanel)JoglPanel.globals.mainPanel).optPan.csNodes.minValue;
	   if (Math.abs(delta) < 1E-5)		// innocent hack
		 return NodeMinColor;
	   int R = (int) (val * (NodeMaxColor.getRed() - NodeMinColor.getRed()) / delta)
				 + NodeMinColor.getRed();
	   int G = (int) (val * (NodeMaxColor.getGreen() - NodeMinColor.getGreen()) / delta)
				 + NodeMinColor.getGreen();
	   int B = (int) (val * (NodeMaxColor.getBlue() - NodeMinColor.getBlue()) / delta)
				 + NodeMinColor.getBlue();
...
//	private float[] maxQualityColor = {0.0f, 0.0f, 1.0f};
    ...
    void computeTrafficColors( HashMap links){
        ...
        if ( min_i == Double.MAX_VALUE || max_i == Double.MIN_VALUE ) min_i = max_i = 0;
        if ( min_p == Double.MAX_VALUE || max_p == Double.MIN_VALUE ) min_p = max_p = 0;
    ...
    private float[] getLinkQualityColor(ILink lnk, float[] color) {
		...
		try {
			Hashtable hPingOptions = (Hashtable)OptionsPanelValues.get("ping");
	        if(((VrvsJoglPanel)JoglPanel.globals.mainPanel).optPan.kbShowPing.isSelected() && lnk.inetQuality != null) {
	        ...
	        if(((VrvsJoglPanel)JoglPanel.globals.mainPanel).optPan.kbShowPeers.isSelected() && lnk.peersQuality != null) {
        ...
        if ( maxValue > minValue ) {
            R = (int) (val * ( maxColor.getRed() - minColor.getRed()) / delta)
            + minColor.getRed();
            G = (int) (val * ( maxColor.getGreen() -  minColor.getGreen()) / delta)
            + minColor.getGreen();
            B = (int) (val * ( maxColor.getBlue() -  minColor.getBlue()) / delta)
            + minColor.getBlue();
        } else {
            R = (int) (val * ( minColor.getRed() - maxColor.getRed()) / delta)
            + maxColor.getRed();
            G = (int) (val * ( minColor.getGreen() - maxColor.getGreen()) / delta)
            + maxColor.getGreen();
            B = (int) (val * ( minColor.getBlue() - maxColor.getBlue()) / delta)
            + maxColor.getBlue();
        }
    ...
    public boolean isDeadLink( Object link, HashMap hLinkAttrs, HashMap localNodes)
    {
        ...
        if ( nodeFrom==null || nodeTo==null 
            || !localNodes.containsKey(nodeTo) || !localNodes.containsKey(nodeFrom)
            || nodeFrom.bHiddenOnMap || nodeTo.bHiddenOnMap 
            || !contains
			|| !isValidLink(link)
			)
            return true;
            ...
            if ( clusterUsers !=null && clusterUsers.getNodes().size()>0 ) {
                ...
                for ( int i=0; i<clUList.size(); i++) {
                    ...
                    if ( nLastSep != -1 ) {
                        ...
                        if ( nFirstSep < nLastSep-1 )//there is an user id in there
    ...
    { 
        ...
        if( link.peersQuality != null && ((VrvsJoglPanel)JoglPanel.globals.mainPanel).optPan.kbShowPeers.isSelected() ) {
            sName = "Peer link ";
            sDescription += "\nQuality: " + ((VrvsJoglPanel)JoglPanel.globals.mainPanel).optPan.csPeers.formatter.format(getLinkQuality(link)) + " %";
        }
        if( link.inetQuality != null && ((VrvsJoglPanel)JoglPanel.globals.mainPanel).optPan.kbShowPing.isSelected() ) {
            sName += "ABPing";
            sDescription += "\nRTime: " + ((VrvsJoglPanel)JoglPanel.globals.mainPanel).optPan.csPing.formatter.format(link.inetQuality[0]) + " ms";
            sDescription += "\nRTT = "+((VrvsJoglPanel)JoglPanel.globals.mainPanel).optPan.csPing.formatter.format(link.inetQuality[1])+" ms";
            sDescription += "\nLost packages: " + ((VrvsJoglPanel)JoglPanel.globals.mainPanel).optPan.csPing.formatter.format(link.inetQuality[3]*100.0) + " %";
        }
