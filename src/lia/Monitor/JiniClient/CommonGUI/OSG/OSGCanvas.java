	...
	public void setNodes(Map<ServiceID, rcNode> nodes) {
		...
		for (int i=0; i<currentNodes.size(); i++) {
			...
			if (!nodes.containsKey(n.sid)) {
				...
				rcNode nn1 = (rcNode)nodes.get(n.sid);
				if (!nn1.equals(n)) {
					currentNodes.remove(i);
					owner.ftpHelper.deleteNode(n);
					owner.jobHelper.deleteNode(n);
...
//				} else {
					cc = getColor(val);
//				}
				if(val < minPerformance) minPerformance = val;
				if(val > maxPerformance) maxPerformance = val;
				if (!nbstr.containsKey(node)) {
					String str = (String)df.format(val);
	...
	public boolean haveOsgModules(rcNode n){
		return n.client.farm.getModuleList().contains("monOsgVoJobs") || n.client.farm.getModuleList().contains("monOsgVO_IO"); 
	}
	...
	private void drawString(Graphics gg, String str, int x, int y) {
		...
		if (owner.ftpTransferType == OSGFTPHelper.FTP_OUTPUT || owner.ftpTransferType == OSGFTPHelper.FTP_OUTPUT_RATE) {
	...
	public void drawConn(Graphics gg, rcNode n, rcNode n1, String lbl, Color col, int nOrder) {
		...
		if (n.equals(n1)) {
			if (n.osgLimits == null) return;
			final int arcx = n.osgLimits.width/2 + 30;
			final int arcy = 30;
			gg.drawOval(n.osgLimits.x+n.osgLimits.width/2, n.osgLimits.y+n.osgLimits.height/2-arcy/2, arcx, arcy);
			...
			Link link = new Link(n.osgLimits.x+n.osgLimits.width/2, n.osgLimits.y+n.osgLimits.height/2-arcy/2, n, arcx, arcy);
			links.add(link);
			return;
		}
...
//				((Graphics2D)g).setPaint(defaultPaint);
		...
		if(owner.onlyToolTip==false){
			if (io != null && isNotAllZero(io)) dim += fillSize;
			if (cpu != null && isNotAllZero(cpu)) dim += fillSize;
			if (jobs != null && isNotAllZero(jobs)) dim += fillSize;
			if (fjobs != null && fjobs[0]!=0.0 && fjobs[0]==fjobs[1]+fjobs[2]) dim += fillSize;
			if (voCpu != null && totalCpuTime!=0.0) dim += fillSize;
		}
		...
		if (cpu != null && isNotAllZero(cpu) && owner.onlyToolTip==false) {
			...
			double idle = cpu[2];
			double err = cpu[3];
			if (usr == 0.0 && sys == 0.0 && idle == 0.0 && err == 0.0) usr = sys = idle = err = 1.0;
			...
			g.fillArc(x-dim, y-dim, 2*dim, 2*dim, angle, angle1);
			g.setColor(Color.black);
			if (n.equals(pickX) && io == null)// && disk == null)
				g.setColor(Color.red);
			g.drawOval(x-dim, y-dim, 2*dim, 2*dim);
			if (n.equals(pickX) && io == null)// && disk == null)
				g.drawOval(x-dim-1, y-dim-1, 2*dim, 2*dim);
		...
		if (jobs != null && isNotAllZero(jobs) && owner.onlyToolTip==false) {
			...
			g.setColor(Color.black);
			if (n.equals(pickX) && io == null && cpu == null)// && disk == null)
				g.setColor(Color.red);
			g.drawOval(x-dim, y-dim, 2*dim, 2*dim);
			if (n.equals(pickX) && io == null && cpu == null)// && disk == null)
				g.drawOval(x-dim-1, y-dim-1, 2*dim, 2*dim);
		...
		if (fjobs != null && fjobs[0]!=0.0 && fjobs[0]==fjobs[1]+fjobs[2] && owner.onlyToolTip==false) {
			double fjs = fjobs[1];
			double fje = fjobs[2];
			if (fjs == 0.0 && fje == 0.0) fjs = fje = 1.0;
		...
		if (voCpu != null && totalCpuTime!=0.0 && owner.onlyToolTip==false) {
			...
			for(Iterator it = voCpu.keySet().iterator(); it.hasNext(); ){
				...
				if(voName.equals("Total")) continue;
		...
		double busy = nod[1];
		double free = nod[0];
		if (busy == 0 && free == 0) free = busy = 1.0;
...
//			((Graphics2D)g).setPaint(defaultPaint);
	...
	private String getToolTipText(rcNode n) {
		...
		if (voCpu!= null) {
			...
			if(totalCpuTime != 0.0){
				for(Iterator it = voCpu.keySet().iterator(); it.hasNext(); ){
					...
					if(voCpuTime!=0.00)
						osgtt.addEntry("CPU TIME for " + voName + " ", OSGColor.getVoColor(voName), df, voCpuTime, voCpuTime/totalCpuTime*100);
				}
			}
			osgtt.changeBg();
		}
		...
		if (jobs != null) {
			osgtt.addEntry("Running Jobs", OSGConstants.RUNNING_JOBS.color, zf, jobs[0], jobs[0]/(jobs[1]+jobs[0])*100);
			osgtt.addEntry("Idle Jobs", OSGConstants.IDLE_JOBS.color, zf, jobs[1], jobs[1]/(jobs[1]+jobs[0])*100);
			osgtt.changeBg();
		}
		if (fjobs != null) {
	...
	public String getToolTipText(MouseEvent e) {
		...
		for (int i=0; i<currentNodes.size(); i++) {
			...
			if (owner.showOnlyConnectedNodes && vGraph != null) {
				...
				if (nn.neighbors == null || nn.neighbors.size() == 0) continue;
			}
		...
		for (int i=0; i<currentNodes.size(); i++) {
			...
			if (owner.showOnlyConnectedNodes && vGraph != null) {
				...
				for (int k=0; k<currentNodes.size(); k++) {
					rcNode n1 = (rcNode)currentNodes.get(k);
					GraphNode nn1 = vGraph.getGN(n1);
					if (nn1.neighbors == null || nn1.neighbors.size() == 0) continue;
...
//				System.out.println("pick = "+pick);
		...
		if (but == MouseEvent.BUTTON3) {
			...
			if(owner.currentLayout.equals("Elastic") || 
			        owner.cmdPane.cbLayout.getSelectedItem().equals("Layered") || 
