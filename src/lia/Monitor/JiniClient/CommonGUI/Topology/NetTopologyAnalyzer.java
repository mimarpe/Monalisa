...
public class NetTopologyAnalyzer implements LocalDataFarmClient {
    ...
    private static final transient Logger logger = Logger.getLogger(NetTopologyAnalyzer.class.getName());
...
//	private Hashtable replacedLinks;	//list of pairs oldLink -> newLink (see replaceNodeWith)
	...
	private Vector ignoredFarmsIP;	// list with IPs of the farms to be ignored - extracted from confIgnoreFarms
...
//		replacedLinks = new Hashtable();
	...
	void registerAsListenerForNewNodes(){
		...
		for(final rcNode n : nodes.values()){
			if(!knownNodes.contains(n)){
				...
				monPredicate pre =
					new monPredicate("*", "Tracepath", "*", -1 * 60 * 1000, -1, null, null);
	...
	private boolean removeNode(rcNode n) {
		...
		if(logger.isLoggable(Level.FINER))
			logger.log(Level.FINER, "Decreasing USAGE COUNT for "+n.UnitName+" @ "+n.IPaddress+" to "+(uc-1));
		return false;
	}
...
//			long t1 = NTPDate.currentTimeMillis();
			...
			if(logger.isLoggable(Level.FINE))
				logger.log(Level.FINE, "Processing Result FROM "+ipns+" TO "+ipnw+" << "+er.Module+" param.len="+er.param.length);
...
//		System.out.println("Creating node "+name+" for:\n"+hostInfo);
	...
	private boolean searchAndReplaceRegion(Vector someTrace, Vector trace, int start, int end){
		...
		for(int i=0; i<someTrace.size(); i++){
			...
			if(someLink != link &&
					someLink.n1.IPaddress.equals(link.n1.IPaddress) 
...
//				System.out.println("Matched!");
				...
				for(int j=0; j<len; j++){
					...
					useNode(link.n1); useNode(link.n2);
					link.usageCount++;
...
//						+" DST="+ns2.UnitName+"->"+nf2.UnitName);
	...
	private Vector<EntityLink> updateRouterTrace(Vector<EntityLink> oldTrace, Vector<Map<String, String>> hashTrace){
		...
		for(int i=0; i<hashTrace.size(); i++){
			...
			if(monitorNode != null && farmNode == null){
				...
				farmNode.setShortName(); farmNode.LAT = monitorNode.LAT; farmNode.LONG = monitorNode.LONG;
				farms.add(farmNode);
			}
			...
			if(crtNode == null) {
				crtNode = createRCNode(hostInfo, (String) hostInfo.get("host"));
			...
			if(link != null && 
					(! link.n1.IPaddress.equals(lastNode.IPaddress) || 
					 ! link.n2.IPaddress.equals(crtNode.IPaddress))){
				removeLink(link);
				link = null;
			}
	 ...
	 */
	private Vector<EntityLink> updateEntityTrace(Vector<EntityLink> oldTrace, Vector<Map<String, String>> hashTrace, String what, Vector whatVect){
		Vector<EntityLink> rez = oldTrace != null ? oldTrace : new Vector<EntityLink>();
		rcNode crtNode = null;
		rcNode lastNode = null;
		rcNode farmNode = null;
		...
		if(res instanceof eResult){
		    ...
		    if ( res instanceof Result ){
		    	...
		    	if(r.ClusterName.equals("Tracepath")){
		        	...
		        	logger.log(Level.FINE, "Ignored Result is of Result type and is in cluster "+((Result)res).ClusterName);
		    	}
		    }else{
		    	if(res != null)
