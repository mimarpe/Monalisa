...
abstract public class SerMonitorBase extends JiniClient implements LocalDataFarmClient, ResultProcesserInterface {
	...
	public void addGraph(graphical g) {
		...
		synchronized (global_param) {
			if (global_param.size() > 0) {
				for (Iterator<String> it = global_param.iterator(); it.hasNext();) {
					g.new_global_param(it.next());
				}
			}
		}
	} // addGraph
	...
	public synchronized void AddProxyService(ServiceItem si) throws Exception {
		...
		if (proxyEntry != null) {
			if (proxyEntry.length > 0) {
				...
				logger.log(Level.INFO, " [ SerMonitorBase ] [ AddProxyService ] CONNECT PROXY at " + hostName + ":" + portNumber);
	...
	public void inializeMonitorUnit(ServiceItem si) {
		try {
			...
			tClient tcl = new tClient(un, ipad, host, mlPort, registryPort, MLCP, mle, sie, esie, proxytmClient, si.serviceID);
		...
		synchronized (SItoInit) {
			while (workingSTreads.size() > 0 || SItoInit.size() > 0) {
				if (logger.isLoggable(Level.FINE))
					logger.log(Level.FINE, "Waiting for STH (working:" + workingSTreads.size() + ", SI to init:" + SItoInit.size() + ") to finish "
					        + message);
				try {
					SItoInit.wait();
			...
			if (node != null) {
				try {
					...
					for (Iterator it2 = node.wconn.values().iterator(); it2.hasNext();) {
						...
						if (srcLat != osLink.fromLAT || srcLong != osLink.fromLONG || dstLat != osLink.toLAT || dstLong != osLink.toLONG) {
		...
		try {
			...
			if (n != null) {
				...
				if (n.client != null && n.client.trcframe != null && n.client.trcframe.address != null) {
					MonitorControl m = controlModules.remove(n.client.trcframe.address.getHostAddress() + ":" + n.client.trcframe.remoteRegistryPort);
