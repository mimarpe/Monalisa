...
public class FarmTreePanel extends JPanel implements ActionListener, MouseMotionListener {
	...
	/** Logger Name */
	private static final transient String COMPONENT = "lia.Monitor.tcpClient";
	/** The Logger */ 
...
//	protected Hashtable nameNodes = null;
	...
	public FarmTreePanel(String name, GroupsPanel basicPanel) {
			...
			public String getToolTipText(MouseEvent evt) {
				try{
					...
					if (obj instanceof rcNode) {
						...
						if (nn.client == null || nn.client.farm == null) return "";
						...
						if (groupFarms.containsKey(obj)) {
							...
							for (Enumeration e = vv.elements(); e.hasMoreElements(); ) {
								...
								if (nn == null || nn.client == null || nn.client.farm == null) continue;
							...
							nrParams = base.size();
							return  "<html>"+nrFarms+" service"+(nrFarms != 1 ? "s" : "")+"<br>"+nrClusters+" cluster"+(nrClusters != 1 ? "s" : "")+"<br>"
							+nrNodes+" node"+(nrNodes != 1 ? "s" : "")+"<br>"
							+nrParams+" unique param"+(nrParams != 1 ? "s" : "")+"<br>"
							+tParams+" total param"+(tParams != 1 ? "s" : "")+"</html>";
						}
				...
				synchronized (getTreeLock()) {
					...
					if (!((TreeNode)path.getLastPathComponent()).isLeaf()) {
						if (Math.abs(mouseevent.getX() - rect.getX()) < 20) {
							((TreeRenderer)treeRenderer).mouseEvent((DefaultMutableTreeNode)path.getLastPathComponent());
		...
		treeView.setBorder(BorderFactory.createBevelBorder(
				BevelBorder.LOWERED, Color.white, Color.white, new Color(0, 98,
						137), new Color(0, 59, 95)));
			...
			public void keyPressed(KeyEvent e) { // for key up there is a problem when crossing from one group up to the next one
				synchronized (getTreeLock()) {
					...
					for(int j=0; j<selections.length; j++){
						Object sel = ((DefaultMutableTreeNode)selections[j].getLastPathComponent()).getUserObject();
						if (sel instanceof String) { // group selected
							tree.setSelectionPath(selections[j]);
						}
					}
				}
			}
	...
	void updateTreeSelection(){
		...
		if (selections != null) for (int i=0; i<selections.length; i++) currentSelectedPaths.add(selections[i]);
		...
		if (selections.length == 1 && (((DefaultMutableTreeNode)selections[0].getLastPathComponent()).getUserObject() instanceof rcNode)) {
	...
	void reunionVectors(final TreeSet set, final Vector base, final Vector probe) {
		...
		if (set == null || base == null || probe == null) return;
		for (int i=0; i<probe.size(); i++) {
	...
	private void auxiliarAdding(final TreeSet set, final Vector base) {
		...
		if (set == null || base == null) return;
		for (int i=0; i<base.size(); i++) {
			Object el = base.get(i);
			if (el == null || el.toString() == null) continue;
			set.add(el.toString());
		}
	}
	...
	private Vector getClusterList(Object userObject, Hashtable nodes, Hashtable nodesTreeSets) {
		...
		if (userObject instanceof rcNode) {
			...
			if (nn.client == null || nn.client.farm == null) return new Vector();
			MFarm farm = nn.client.farm;
...
//			logger.info(buf.toString());
			...
			for (Iterator it = farm.getClusters().iterator(); it.hasNext(); ) {
				MCluster cluster = (MCluster)it.next();
			...
			if (farms == null) return new Vector();
			Vector v = null;
			TreeSet ss = null;
	...
	private Vector getParameters(MFarm farm) {
		...
		if (farm.getClusters() == null || farm.getClusters().size() == 0) return new Vector();
		for (int i=0; i<farm.getClusters().size(); i++) {
			MCluster cluster = (MCluster)farm.getClusters().get(i);
			if (cluster.getNodes() == null || cluster.getNodes().size() == 0){ 
				if (intersectSelected) return new Vector(); // intersect vs empty set means an empty set also
				continue;
			}
			for (int j=0; j<cluster.getNodes().size(); j++) {
				MNode node = (MNode)cluster.getNodes().get(j);
	...
	private Vector getModuleList(Object userObject){
		...
		if (userObject instanceof rcNode) {
			rcNode nn = (rcNode)userObject;
			if (nn == null || nn.client == null || nn.client.farm == null) return new Vector();
...
//				e.printStackTrace();
	...
	protected boolean checkGroupSelected(DefaultMutableTreeNode node) {
		...
		if (!(node.getUserObject() instanceof rcNode)) return false;
	...
	protected boolean groupSelected(String group) {
		...
		if (!smb.SGroups.containsKey(group)) return false;
		return ((Integer) smb.SGroups.get(group)).intValue() != 0;
	}
	...
	public void add(Map<ServiceID, rcNode> v) {
		...
		synchronized (getTreeLock()) {
			...
			currentSelectedPaths.clear();
			if (selPaths != null) for (int i=0; i<selPaths.length; i++) currentSelectedPaths.add(selPaths[i]);
						...
						if (!groupSelected(str)) {
							if (currentSelectedPaths != null && currentSelectedPaths.size() != 0)
								for (int i=0; i<currentSelectedPaths.size(); i++) {
									...
									if (obj instanceof rcNode) {
										rcNode nn = (rcNode)obj;
										String groupSelected = (String)((DefaultMutableTreeNode)(((DefaultMutableTreeNode)p.getLastPathComponent()).getParent())).getUserObject();
										if (nn != null && groupSelected != null && nn.equals(node) && groupSelected.equals(str)) {
			...
			for (int k=0; k<nodes.size(); k++) {
				...
				boolean test = false;
				if (node.sid != null && !v.containsKey(node.sid)) test = true;
				else  {
					rcNode nn1 = (rcNode)v.get(node.sid);
					if (!nn1.equals(node)) test = true;
				}
				if (test) {
					dirtyFlag = true;
					Vector vv = extrageGroups(node.mlentry.Group);
			...
			if (dirtyFlag) {
				...
				if (currentSelectedPaths.size() != 0) {
					...
					for (int i=0; i<selPaths.length; i++) selPaths[i] = (TreePath)currentSelectedPaths.get(i);
	...
	protected void updateFarmInfo(rcNode node) {
		...
		synchronized (getTreeLock()) {
			...
			text.append(node.client.getFarmName()).append("@").append(node.client.getHostName()).append(":").append(node.client.getPort()).append("<br>");
			text.append("Local Time: ").append(node.client.localTime).append(" MonALISA Version: ").append(node.client.mlVersion).append("<br>");
			text.append("IP Address: ").append(node.client.getIPAddress()).append("<br>");
			text.append("Group: ").append(node.client.mle.Group).append("<br>");
			text.append("Location: ").append(node.client.mle.Location).append(", Country: ").append(node.client.mle.Country).append(", LAT: ").append(node.client.mle.LAT);
			text.append(", LONG: ").append(node.client.mle.LONG).append("<br>");
			ExtendedSiteInfoEntry esie = node.client.getExtendedSiteInfoEntry();
			text.append("Contact: ").append(esie.localContactName).append( " email: ").append(esie.localContactEMail).append("<br>");
	...
	protected boolean canShowFarmStatistics() {
		...
		synchronized (getTreeLock()) {
			if (tree.getSelectionPaths().length != 1) return false;
			rcNode farm = (rcNode)(((DefaultMutableTreeNode)tree.getSelectionPath().getLastPathComponent()).getUserObject());
	...
	public void showFarmStatics() {
		...
		synchronized (getTreeLock()) {
			...
			if (!(((DefaultMutableTreeNode)paths[0].getLastPathComponent()).getUserObject() instanceof rcNode)) return;
	...
	protected boolean canShowLinkStatistics() {
		...
		synchronized (getTreeLock()) {
			if (tree == null || nodes == null) return false;
			TreePath paths[] = tree.getSelectionPaths();
			if (paths == null || paths.length != 1) return false;
			if (!(((DefaultMutableTreeNode)paths[0].getLastPathComponent()).getUserObject() instanceof rcNode)) return false;
			rcNode farm = (rcNode)(((DefaultMutableTreeNode)paths[0].getLastPathComponent()).getUserObject());
			for (Enumeration e = nodes.elements(); e.hasMoreElements(); ) {
				rcNode node = (rcNode)e.nextElement();
				if (node.equals(farm)) {
	...
	public void showLinkStatistics() {
		...
		synchronized (getTreeLock()) {
			if (tree.getSelectionPaths().length != 1) return;
			rcNode farm = (rcNode)(((DefaultMutableTreeNode)tree.getSelectionPath().getLastPathComponent()).getUserObject());
			for (Enumeration e = nodes.elements(); e.hasMoreElements(); ) {
				rcNode node = (rcNode)e.nextElement();
				if (node.equals(farm)) {
	...
	public String[] getSelectedFarms() {
		...
		Vector v = new Vector();
		TreePath[] selPaths = tree.getSelectionPaths();
		if (selPaths == null || selPaths.length == 0) return null;
		for (int i = 0; i < selPaths.length; i++) {
			TreePath path = selPaths[i];
			Object obj = ((DefaultMutableTreeNode)path.getLastPathComponent()).getUserObject();
			if (obj instanceof rcNode) {
				rcNode nn = (rcNode)obj;
				String str = null;
				if (nn.client != null && nn.client.farm != null)
					...
					str = nn.toString();
				if (str != null && !v.contains(str)) v.add(str);
			}
		}
		...
		for (int i=0; i<tree.getRowCount(); i++) {
			...
			if (obj instanceof rcNode) {
				...
				if (node != null && !farmsChecked.contains(node)) {
					...
					try{
						if (node.client != null && node.client.trcframe != null && node.client.trcframe.address!=null) {
							control = MonitorControl.connectTo(node.client.trcframe.address.getHostAddress(), node.client.trcframe.remoteRegistryPort);
						} 
					}
					catch (Exception e){
						e.printStackTrace();
					...
					if (control != null) {
						...
						try {
							SerMonitorBase.controlModules.put(node.client.trcframe.address.getHostAddress()+":"+node.client.trcframe.remoteRegistryPort, control);
						} catch (Exception e) { 
//							e.printStackTrace();
						}
					}
...
//					e.printStackTrace();
		...
		public void mouseEvent(DefaultMutableTreeNode node) {
			...
			if (currentSelectedPaths != null && currentSelectedPaths.size() != 0) {
				...
				for (int i=0; i<paths.length; i++) paths[i] = (TreePath)currentSelectedPaths.get(i);
...
//			TreePath path = new TreePath(node.getPath());
	...
	Icon getFarmIcon() {
		...
		if (farmIcon != null) return farmIcon;
	...
	Icon getLockedIcon() {
		...
		if (lockedIcon != null) return lockedIcon;
	...
	Icon getUnlockedIcon() {
		...
		if (unlockedIcon != null) return unlockedIcon;
	...
	Icon getBlankIcon() {
		...
		if (blankIcon != null) return blankIcon;
	...
	private Icon getLocksIcon() {
		...
		if (locksIcon != null) return locksIcon;
...
//	}
	...
	Icon getPlusIcon() {
		...
		if (plusIcon != null) return plusIcon;
	...
	Icon getMinusIcon() {
		...
		if (minusIcon != null) return minusIcon;
	...
	private Icon getGaugeIcon() {
		...
		if (gaugeIcon != null) return gaugeIcon;
...
//		}
	...
	private Cursor createPlusCursor() {
		...
		if (dim == null) return Cursor.getDefaultCursor();
		int w1 = (int)dim.getWidth();
		int h1 = (int)dim.getHeight();
		if (w1 == 0 || h1 == 0) return Cursor.getDefaultCursor();
		if (w < w1 || h < h1) {
	...
	private Cursor createMinusCursor() {
		...
		return Toolkit.getDefaultToolkit().createCustomCursor(image, new Point(image.getWidth(null)/2, image.getHeight(null)/2), "Minus");
	}
