	...
	class PlotListener implements ActionListener {
			...
			for (int i=0; i<farms.length; i++) {
				if (pclusters.length != 0) {
					...
					for (int j=0; j<nodes.length; j++) {
						String cluster = nodes[j].getClusterName();
		...
		private long getHistoryTime(Object source) {
			...
			if (source.equals(valPanel.mn2D) || source.equals(valPanel.mn3D)) {
				...
				source == valPanel.mnBarAverage
					|| source == valPanel.mnPieAverage
					|| source == valPanel.mnBarSum
					|| source == valPanel.mnPieSum
					|| source == valPanel.mnBarIntegral
					|| source == valPanel.mnPieIntegral
					|| source == valPanel.mnBarMinMax
					|| source == valPanel.mnPieMinMax) {
				type = TYPE_SUMMARY;
			...
			for (int i = 0; i < lm.getSize(); i++) {
				if (lm.getElementAt(i) != null
					&& lm.getElementAt(i).toString().indexOf("McastGanglia")
						!= -1) {
			...
			for (Iterator cit = clusters.iterator(); cit.hasNext();) {
				...
				if (cluster.indexOf("IEPM-BW") != -1
					|| cluster.indexOf("_WS") != -1
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (selectedNodes == null || predicates == null || selectedNodes.length == 0 || predicates.size() == 0) {
				helpSelection();
				return;
			}
			if (source == valPanel.mn2D) {
				...
				if (clusterSelected) {
					if (mod) {
						crtPlot =
							new MultipleMonDataPlotClusGlobal(
								thisReference,
								selectedNodes,
								predicates,
								clusters);
				...
				if (clusterSelected) {
					if (mod) {
						crtPlot =
							new MultipleMonData3DPlotClusGlobal(
								thisReference,
								selectedNodes,
								predicates,
								clusters);
				...
				crtPlot =
					new MultipleBarMonDataPlotClus(
						thisReference,
						selectedNodes,
						predicates,
						clusters, currentUnits);
			} else if (
				source == valPanel.mnBarAverage
					|| source == valPanel.mnPieAverage
					|| source == valPanel.mnBarSum
				...
				if (clusterSelected) {
					boolean pieOrBar =
						!(source == valPanel.mnPieAverage
							...
							|| source == valPanel.mnPieMinMax);
					int summaryType = -1;
					if (source == valPanel.mnBarAverage
						|| source == valPanel.mnPieAverage)
					...
					if (valPanel.lastSummaryBtn == valPanel.clusterSummary)
						crtPlot =
							new MultipleBarClusterSummaryPlot(
								thisReference,
								selectedNodes,
								predicates,
								clusters,
								summaryType,
								pieOrBar, currentUnits);
					else if (valPanel.lastSummaryBtn == valPanel.nodeSummary)
						crtPlot =
							new MultipleBarNodeSummaryPlot(
								thisReference,
								selectedNodes,
								predicates,
								clusters,
						...
						logger.log(
							Level.WARNING,
							"event from unknown source " + source);
					}
				} else
			...
			if (crtPlot != null) {
				...
				if (uniqueFarm()) {
					...
					for (Iterator it = serMonitorBase.getVNodes().iterator(); it.hasNext(); ) {
...
//				crtPlot.setLocalTime(localTime);
		...
		private String formFarmName() {
			...
			if (ret.length() == 0) return "unavailable";
		...
		private boolean uniqueFarm() {
			...
			for (Iterator it = predicates.iterator(); it.hasNext(); ) {
				monPredicate pred = (monPredicate)it.next();
				if (!pred.Farm.equals(farmName)) return false;
			}
			return true;
		}
	}
