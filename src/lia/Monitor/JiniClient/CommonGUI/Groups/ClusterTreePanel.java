...
//						int nrNodes = ((MCluster) obj).getNodes().size();
						...
						for (int i=0; i<defaultmutabletreenode.getChildCount(); i++) {
							MNode node = (MNode)(((DefaultMutableTreeNode)defaultmutabletreenode.getChildAt(i)).getUserObject());
							reunionVectors(set, base, node.getParameterList());
							nrParams += node.getParameterList().size();
						}
						int tparams=base.size();
						return  "<html>"+nrNodes+" node"+(nrNodes != 1 ? "s" : "")+"<br>"
						+tparams+" unique param"+(tparams != 1 ? "s" : "")+"<br>"
						+nrParams+" total param"+(nrParams != 1 ? "s" : "")+"</html>";
	...
	private Vector getParameters(MCluster cluster) {
		...
		for (int i=0; i<nodes.length; i++) {
			Vector cl = nodes[i].client.farm.getClusters();
			for (int k=0; k<cl.size(); k++) {
				MCluster c = (MCluster)cl.get(k);
				if (c.getName().equals(cluster.getName())) clusters.add(c);
			}
		}
		for (int k=0; k<clusters.size(); k++) {
			MCluster cl = (MCluster)clusters.get(k);
			for (int i=0; i<cl.getNodes().size(); i++) {
				MNode node = (MNode)cl.getNodes().get(i);
				if (basicPanel.groupPanel.intersectSelected) { // is node common node ?
					...
					for (int j=0; j<clusters.size(); j++) {
						...
						if (mnodes == null || mnodes.size() == 0) break; 
						for (int l=0; l<mnodes.size(); l++) {
							MNode n = (MNode)mnodes.get(l);
							if (n.getName().equals(node.getName())) { is = true; break; }
						}
						if(!is) {
							break;
						}
				...
				if (node.getParameterList() == null || node.getParameterList().size() == 0) {
					if (basicPanel.groupPanel.intersectSelected) return new Vector(); // intersect vs empty set means an empty set also
					continue;
				}
				if (base == null) {
	...
	private Vector getParameters(MNode node) {
		...
		Vector clusters = new Vector();
		for (int i=0; i<rcnodes.length; i++) {
			Vector cl = rcnodes[i].client.farm.getClusters();
			for (int k=0; k<cl.size(); k++) {
				MCluster c = (MCluster)cl.get(k);
				if (c.getName().equals(cluster.getName())) clusters.add(c);
			}
		...
		for (int k=0; k<clusters.size(); k++) {
			...
			for (int i=0; i<mnodes.size(); i++) {
				...
				if (mnode.getName().equals(node.getName())) nodes.add(mnode);
			}
		}
		...
		for (int i=0; i<nodes.size(); i++) {
			...
			if (modules != null) 
				for (int ii=0; ii<modules.size(); ii++) { 
					...
					if (oo == null) { modules.remove(ii); ii--; }
				}
	...
	protected Vector order(Vector base) {
		...
		for (Iterator it = base.iterator(); it.hasNext(); ) {
			Object obj = it.next();
			if (obj == null || obj.toString() == null) continue;
	...
	public boolean areNodesSelected() {
		...
		if (selPaths == null || selPaths.length == 0) return false;
		return true;
	}
		...
		synchronized (getTreeLock()) {
			...
			if (clusters.size() > 0) {
				...
				for (Iterator it = clusters.iterator(); it.hasNext(); ) {
					...
					Vector v = null;
					if (map.containsKey(cluster.toString())) v = (Vector)map.get(cluster.toString());
					else { v = new Vector(); map.put(cluster.toString(), v); }
					v.add(cluster);
				}
				clusters.clear();
				for (Iterator it = map.keySet().iterator(); it.hasNext(); ) {
				...
				for (Iterator it = clusters.iterator(); it.hasNext(); ) {
					...
					for (Iterator e1 = vv.iterator(); e1.hasNext(); ) {
						...
						Vector v = null;
						if (map.containsKey(nod.toString())) v = (Vector)map.get(nod.toString());
						else { v = new Vector(); map.put(nod.toString(), v); }
						v.add(nod);
					}
					vv.clear();
			...
			if (selPaths != null) {
				for (int i=0; i<selPaths.length; i++) {
					Object obj = ((DefaultMutableTreeNode)currentSelectedPaths[i].getLastPathComponent()).getUserObject();
					if (obj instanceof MCluster) {
						...
						MCluster cluster = (MCluster)((DefaultMutableTreeNode)(((DefaultMutableTreeNode)currentSelectedPaths[i].getLastPathComponent())).getParent()).getUserObject();
						...
						for (int j=0; j<root.getChildCount(); j++) {
							...
							if (cluster.name.equals(c.name)) {
								...
								for (int k=0; k<root.getChildAt(j).getChildCount(); k++) {
									MNode n = (MNode)((DefaultMutableTreeNode)(root.getChildAt(j).getChildAt(k))).getUserObject();
	...
	protected TreePath[] checkPathChanges(TreePath[] selPaths) {
		...
		for (int i=0; i<selPaths.length; i++) {
			Object obj = ((DefaultMutableTreeNode)selPaths[i].getLastPathComponent()).getUserObject();
			if (obj instanceof MCluster) {
	...
	private Icon getClusterIcon() {
		...
		if (clusterIcon != null) return clusterIcon;
