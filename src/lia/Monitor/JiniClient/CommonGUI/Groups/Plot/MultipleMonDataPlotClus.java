...
public class MultipleMonDataPlotClus
	implements
		LocalDataFarmClient,
		MultipleDataPlotter,
		ActionListener,
	...
	public MultipleMonDataPlotClus(
		DataPlotterParent parent,
		rcNode[] selectedNodes,
		Vector predicates,
		Vector clusters) {
		...
		if (params.length > 1) {
			logger.log(
					Level.WARNING,
			"More that one parameter in MonDataPlotClus!");
		}
		...
		for (Iterator pit = predicates.iterator(); pit.hasNext();) {
			...
			if (params != null && params.length > 0) {
				...
				for (int i=0; i<params.length; i++)
					if (params[i].equals(title)) {
						found = true;
						break;
					}
				if (found) {
					minimumPredicateTime = Math.min(minimumPredicateTime, pred.tmin);
					if (pred.tmax < 0) maximumPredicateTime = pred.tmax;
					else if (maximumPredicateTime >= 0) maximumPredicateTime = Math.max(maximumPredicateTime, pred.tmax);
					if (params.length != 1) {
						pred = new monPredicate(pred.Farm, pred.Cluster, pred.Node, pred.tmin, pred.tmax, new String[] {title}, null);
						long d = -1 * pred.tmin;
						if (d > inter) inter = d;
					}
					sendPredicate(pred);
	...
	boolean plotResult(Result r) {
		...
		synchronized (p.chartFrame.getTreeLock()) {
			if (r == null || r.param == null || r.param_name == null) return false;
			boolean ret = false;
			if (r.time < minimumPredicateTime || (maximumPredicateTime > 0 && r.time > maximumPredicateTime)) return false;
			try {
				double tdis = r.time;
				...
				String key;
				if (multipleFarms)
					key = (addClusterName ? r.FarmName.trim()+"/"+r.ClusterName + "/" : r.FarmName.trim()+"/")+r.NodeName.trim();
				else
					...
					if (r.param_name[i].equals(title)) {
						if (!Double.isNaN(r.param[i]) && !Double.isInfinite(r.param[i])) {
							if (currentUnit != null && baseUnit != null && currentUnit.containsKey(r.param_name[i])) {
								...
								p.add(key, tdis, convert(u, r.param[i]));
							} else
								p.add(key, tdis, r.param[i]);
							ret = true;
							receivedNewData = true;
						} else 
	...
	public void setLocalTime(String dd) {
		...
		if (multipleFarms) return; // don't want to set the local time parameter
	...
	public boolean stopIt(rcNode node) {
		...
		try {
			p.removeSet(node.client.farm.getName().trim()+"/");
		} catch (Exception e) { 
