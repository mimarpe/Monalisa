...
public class MultipleMonDataPlot
	implements
		LocalDataFarmClient,
		MultipleDataPlotter,
		ActionListener,
...
//	Object rdSync = new Object();
	...
	public MultipleMonDataPlot(
		...
		if (predicate.parameters == null || predicate.parameters.length == 0) return;
		title = predicate.Node;
		if (predicates.size() > 1) {
			logger.log(
				Level.WARNING,
				"More that one predicate in MonDataPlot!");
		}
		...
		for (int i=0; i<selectedNodes.length; i++)
			if (selectedNodes[i].client.farm.toString().equals(predicate.Farm)) {
				dataprovider = selectedNodes[i].client;
				break;
			}
