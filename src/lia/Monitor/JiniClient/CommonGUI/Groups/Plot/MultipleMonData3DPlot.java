...
public class MultipleMonData3DPlot 
	implements
		LocalDataFarmClient,
		MultipleDataPlotter,
		ActionListener,
						...
						if (predicate.parameters[i].equals(r.param_name[j])) {
							if (!Double.isNaN(r.param[j]) && !Double.isInfinite(r.param[j])) {
								if (currentUnit != null && baseUnit != null && currentUnit.containsKey(predicate.parameters[i])) {
									...
									p.addPoint(predicate.parameters[i], r.time, convert(u, r.param[j]));
								} else
									p.addPoint(predicate.parameters[i], r.time, r.param[j]);
								receivedNewData = true;
								ret = true;
							} else 
