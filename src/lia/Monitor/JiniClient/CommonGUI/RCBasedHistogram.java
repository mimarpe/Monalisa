...
public class RCBasedHistogram extends JPanel implements graphical, ComponentListener {
	 ...
	 */
	public RCBasedHistogram(String title, String xAxisName, String yAxisName,
							String [] labels, boolean isVertical){
		
		this.logger = Logger.getLogger(RCBasedHistogram.class.getName());
		...
		for(int i=0; i<labels.length; i++){
			min[i] = 0; max[i] = 1; count[i] = 1;
		}
		
		barChart = new StackedBarChartPanel(title,
				xAxisName, yAxisName, labels, min, max, count, isVertical);
	...
	protected void setNodeSerValue(rcNode n, double value1, double value2, double value3){
		...
		if(vv == null)
			...
			if(vv[0] != value1 || vv[1] != value2 || vv[2] != value3){
				...
				vv[0] = value1; vv[1] = value2; vv[2] = value3;
				shouldRedraw = true;
			}
		}
	}
