...
public final class GlobeTextureLoader {
	...
	// logger
	private static Logger logger = Logger.getLogger("lia.Monitor.JiniClient.CommonGUI.GlobeTextureLoader");
	...
	private int [] buffNightImgLine;	// see setShadow
...
*/		
	...
	private BufferedImage loadSomeImage(String fileName, boolean tryDefault){
		...
		try{
			...
			if(bi == null)
				logger.log(Level.WARNING, "Failed loading any globe texture."+fileName.substring(fileName.lastIndexOf("/")));
		}
	    return bi;
	}
...
//			System.out.println("Found image w="+width+" h="+height);
			...
			for(int i=0; i<height; i++){
				...
				for(int j=0; j<width; j++){
					...
					pxIntColors[j] = (0xff << 24) | ((0xff & px3BytesColors[base]) << 16) 
						| ((0xff & px3BytesColors[base+1]) << 8) 
						| (0xff & px3BytesColors[base+2]);
				}
	...
	private void setShadow(double declination, double rightAscension, double GST) {
		...
		for (int y = 0; y < imgHeight; y++) {
			double sinLat = Math.sin( - ((PI / (double) (imgHeight - 1)) * (double) y - PId2));
			double cosLat =	Math.cos( - ((PI / (double) (imgHeight - 1)) * (double) y - PId2));
			...
			for (int x = 0; x < imgWidth; x++) {
				double temp = diff + (PI2 / (double) (imgWidth - 1)) * (double) x - PI;
				if (temp < 0.0D)
					temp = -temp;
				if (temp > PI)
					temp = PI2 - temp;
