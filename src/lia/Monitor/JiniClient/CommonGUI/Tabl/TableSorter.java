...
public class TableSorter extends AbstractTableModel//TableMap
    ...
    /** Logger name */
    private static final transient String COMPONENT = "lia.Monitor.JiniClient.CommonGUI.Tabl";
    ...
    public class SortingColumnsClass
    {
		...
		public void xchange( int position1, int position2)
		{
			...
			int max = nCols;
			if ( position1 > max ) max = position1;
			if ( position2 > max ) max = position2;
...
//			System.out.println("real_pos.length="+real_pos.length);
    ...
    public static double getNumber(String text) throws NumberFormatException
	{
    	...
    	int i=0;
    	while ( i<text.length() && text.charAt(i)!='-' && text.charAt(i)!='+' && ( text.charAt(i)<'0'||text.charAt(i)>'9' ) && text.charAt(i)!='.' )
    		i++;
    	while ( i<text.length() && ( text.charAt(i)=='-' || text.charAt(i)=='+' || ( text.charAt(i)>='0'&&text.charAt(i)<='9' ) || text.charAt(i)=='.' ) ) {
    		if ( text.charAt(i) == '.' ) {
    			if ( bFrac)
    ...
    public int compareRowsByColumn(int row1, int row2, int column) {
        ...
        String sType = (String)model.getColumnType(column);//getValueAt( -1, column);//only the underlying model can return a valid value for -1 row
        // Check for nulls.
...
//        Object o2 = data.getValueAt(row2, column); 
        ...
        if ( sType.compareTo("number") == 0 ) {
    	    ...
    	    for ( i=0; i<nums1.length && i<nums2.length; i++) {
    	        n1 = 0;
    	        try { n1 = (int)getNumber(nums1[i]);//Integer.parseInt(nums1[i]); 
    	        } catch ( NumberFormatException nfex ) { n1 = 0; };
    	        n2 = 0;
    	        try { n2 = (int)getNumber(nums2[i]);//Integer.parseInt(nums2[i]);
    	        } catch ( NumberFormatException nfex ) { n2 = 0; };
...
//        			System.out.println("model: column moved from "+tableView.convertColumnIndexToModel(e.getFromIndex())+" to "+tableView.convertColumnIndexToModel(e.getToIndex()));
        	   		sortingColumns.xchange( tableView.convertColumnIndexToModel(e.getFromIndex()), tableView.convertColumnIndexToModel(e.getToIndex()));
