...
public class TabPanBase extends JPanel implements graphical, LocalDataFarmClient, ItemListener  {
	...
	/** The Logger */
	protected static final Logger logger = Logger.getLogger("lia.Monitor.JiniClient.CommonGUI.Tabl.TabPanBase");
...
//		fixedScroll.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
	...
	class RefreshThread extends Thread {
		...
		private Object objTableChange = new Object();
		public void run() {
			boolean run=true;
			while ( run ) {
        ...
        for(int i=0; i<vnodes.size(); i++){
            ...
            if( ! crtNodes.contains(n) ) {
                ...
                for ( int j=0; j<colcount; j++) {
                    colP = (ColumnProperties)myModel.getColumnInfo(j);
	...
	public void customizeColumn(ColumnProperties colP) {
        ...
        if ( colP.getPredicate()!=null && vnodes!=null ) {
            ...
            for ( int j=0; j<vnodes.size(); j++)
                ((rcNode)vnodes.get(j)).client.addLocalClient(this, colP.getPredicate());
        }
...
//        	prefs = Preferences.userNodeForPackage(monitor.mainClientClass);
		...
		int colcount = myModel.getColumnCount();
		for ( int i=0; i<colcount; i++) {
			colP = (ColumnProperties)myModel.getColumnInfo(i);
    ...
    public void itemStateChanged(ItemEvent e) {
    	...
    	if ( obj instanceof JCheckBoxMenuItem ) {
    		...
    		for ( ; index<myModel.getColumnCount(); index++) {
    			...
    			if (colP.getMenuItem().equals((JCheckBoxMenuItem)obj))
    				break;
    			if ( !colP.isVisible() )
    				hiddencols++;
    		}
...
//            colP.setVisible(bCheckCol);
        	...
        	JCheckBoxMenuItem mi = new JCheckBoxMenuItem( colP.getName().replaceAll("<br>", "")/*getCodeName()*/, bCheckCol);
        	colP.setMenuItem(mi);
            ...
            if ( isSelected && row>=0 && row < table.getModel().getRowCount() ) {//((TableSorter)table.getModel()).indexes.length ) {
	            ...
	            try {
					if ( table.getModel() instanceof TableSorter )
						node = (rcNode) ((TableSorter)table.getModel()).getNode(row);//vnodes.get(((TableSorter)table.getModel()).indexes[row]);
	            } catch (Exception ex) {
	                node = null;
	            }
				if ( node !=null) {
...
//            		ex.printStackTrace();
            ...
            if ( nValue <= minVal )
            	...
            	int red, green, blue;
            	red = minColor.getRed()+(maxColor.getRed()-minColor.getRed())*(nValue-minVal)/(maxVal-minVal);
            	green = minColor.getGreen()+(maxColor.getGreen()-minColor.getGreen())*(nValue-minVal)/(maxVal-minVal);
            	blue = minColor.getBlue()+(maxColor.getBlue()-minColor.getBlue())*(nValue-minVal)/(maxVal-minVal);
    ...
    protected class rcRendererBg2Label extends JLabel implements TableCellRenderer {
    	private Color infminColor;
    	private Color infmaxColor;
    	private Color medColor;
    	private Color supminColor;
    	private Color supmaxColor;
    	double infminVal;
    	double infmaxVal;
    	double supminVal;
    	double supmaxVal;
    	...
    	public rcRendererBg2Label( double infminVal, Color infminColor, double infmaxVal, Color infmaxColor,
    			double supminVal, Color supminColor, double supmaxVal, Color supmaxColor,
    			Color medColor) {
    		super();
			if ( commonFont!=null )
				this.setFont(commonFont);
...
//            		ex.printStackTrace();
    ...
    protected class rcRendererProgressBar extends JProgressBar implements TableCellRenderer {
        ...
        private Color verylightColor;
        private Color lightColor;
        private Color darkColor;
        private Color verydarkColor;
        ...
        public rcRendererProgressBar() {
			...
			int r1, g1, b1;
			r1 = r+surplus; if ( r1>255 ) r1 =255; if ( r1<0 ) r1=0;
			g1 = g+surplus; if ( g1>255 ) g1 =255; if ( g1<0 ) g1=0;
			b1 = b+surplus; if ( b1>255 ) b1 =255; if ( b1<0 ) b1=0;
			lightColor = new Color( r1, g1, b1);
			r1 = r+2*surplus; if ( r1>255 ) r1 =255; if ( r1<0 ) r1=0;
			g1 = g+2*surplus; if ( g1>255 ) g1 =255; if ( g1<0 ) g1=0;
			b1 = b+2*surplus; if ( b1>255 ) b1 =255; if ( b1<0 ) b1=0;
...
//				rcNode node = (rcNode) nodes.get (text ); 				
				...
				if ( xtable.getModel() instanceof TableSorter )
					node = (rcNode) ((TableSorter)xtable.getModel()).getNode(row);//vnodes.get(((TableSorter)table.getModel()).indexes[row]);
				if ( node !=null) {
					node.client.setVisible(true);
...
//			setHorizontalTextPosition(JLabel.CENTER);
		...
		public Component getTableCellRendererComponent(JTable xtable, Object value,
				boolean isSelected, boolean hasFocus, int row, int column) {
			if (xtable != null) {
...
//			setText((value == null) ? "" : value.toString());
			...
			if ( sortMode==1 ) {
				...
				setIcon(transparentImg);
	        try { setValue(value); } catch (Throwable t) { } 
			return this;
		}
	}
...
//		grapPan.setMaxFlowData(n, v) ; i
    ...
    void setResult(MLSerClient client, Result r){
        if (r == null || r.param_name == null || r.param == null) return;
        rcNode node = (rcNode)nodes.get(client.tClientID);
