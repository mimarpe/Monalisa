...
public class EarthGroup2 extends BranchGroup {
	private BufferedImage buffBigImg;
	private BufferedImage buffSliceImg;
	private int [] buffImgLine;
	
	private int divisions = 64;   // how many points are /x and /y and /z, 2^k
	private Point3d points[][];	  // divisions x divisions
	private Vector3f normals[][];	  // divisions x divisions
	private int shapesXcount = Integer.valueOf(AppConfig.getProperty("lia.Monitor.globeTexture.slices","2")).intValue(); // should be a divisor of divisions !!!, 2^q
	private int shapesYcount = Integer.valueOf(AppConfig.getProperty("lia.Monitor.globeTexture.slices","2")).intValue(); // should be a divisor of divisions !!!, 2^r
	private int linesPerShape = divisions / shapesYcount;
	private int colsPerShape = divisions / shapesXcount;
	...
	private double radius = 1.0;  // shpere's radius
	
	private String textureScaleFilter;
	...
	private Shape3D create3DShape(int lin, int col){
		...
		for(int j=colsPerShape*col; j<colsPerShape*(1+col); j++){
			...
			for(int i=linesPerShape*lin; i<=linesPerShape*(1+lin); i++){
				...
				float jTexB = jTex + 1.0f/(float)linesPerShape;
				...
				triPoints.add(b); texCoords.add(tcb); normVects.add(nb);
				triPoints.add(a); texCoords.add(tca); normVects.add(na);
			}
