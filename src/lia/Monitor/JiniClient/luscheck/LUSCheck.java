...
public class LUSCheck extends JiniClient {
	...
	private static final Logger logger = Logger.getLogger("lia.Monitor.JiniClient.util.LUSCheck");
	Timer checkTimer;
	...
	public LUSCheck(){
		...
		if(triggerScript != null && triggerScript.trim().length() > 0){
			...
			logger.log(Level.WARNING, "Trigger script not defined. Will NOT take any actions in case LUS doesn't respond!");
			triggerScript = null;
		}
		...
		String mailDest = AppConfig.getProperty("lia.Monitor.JiniClient.util.LUSCheck.mailTo", "Catalin.Cirstoiu@cern.ch");
		...
		logger.log(Level.INFO, "Triggering alert if ServiceItems are not updated after "+(siUpdateTimeout / 1000)+" seconds.");
	...
	void checkLUS(){
		...
		if(psi == null || ssi == null || psi.length == 0 || ssi.length == 0 || siUpdateDelay > siUpdateTimeout){
		...
		if(logger.isLoggable(Level.FINER))
			logger.log(Level.FINER, "Periodical check finished with "
						+(psi == null ? "NULL" : ""+psi.length)+" proxies, "
						+(ssi == null ? "NULL" : ""+ssi.length)+" services and "
						+(siUpdateDelay/1000)+" seconds update delay.");
	}
