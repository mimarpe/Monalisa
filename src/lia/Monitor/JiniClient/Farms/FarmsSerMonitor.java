...
public class FarmsSerMonitor extends SerMonitorBase {
    ...
    /** Logger name */
    private static final transient String COMPONENT = "lia.Monitor.JiniClient.Farms";
    ...
    static long NETFLOW_LINK_EXPIRE_TIME = 2 * 60 * 1000; // after some time, consider link lost
    ...
    public FarmsSerMonitor(MainBase main, Class<?> mainClientClass) {
        ...
        logger.info("Monitor constructor initialisation time is: " + (main.nStopCreateMonitorTime - main.nStartCreateMonitorTime) + " miliseconds");
                ...
                try {
                    ...
                    if (pick != null) {
                        ...
                        for (int i = 0; i < graphs.size(); i++) {
                            ((graphical) graphs.elementAt(i)).setMaxFlowData(pick, djv);
                        }
                    }
                } catch (Throwable t) {
    ...
    public void setNodePosition(rcNode node) {
        ...
        if (node.LAT == null || node.LONG == null || node.CITY == null /* || node.mlentry.Group.compareTo("OSwitch")==0 */) {
            if (node.mlentry != null && node.mlentry.LAT != null && !node.mlentry.LAT.trim().equals("") && !node.mlentry.LAT.equals("N/A")
                    && node.mlentry.LONG != null && !node.mlentry.LONG.trim().equals("") && !node.mlentry.LONG.equals("N/A")) {
    ...
    public void addNode(final ServiceItem si, final DataStore dataStore, final tClient client, final String unitName, final String ipad) {
        ...
        final MonMessageClientsProxy configuration = (client.msgMux != null ? client.msgMux.knownConfigurations.get(n.sid) : null);
        ...
        if (!main.bGridsClient) {
            ...
            if (hasGlobeLinks) {
                switch (globeLinksType) {
                    ...
                    case OPENFLOW: {
                        if (unitName.equals("FASTMON_GLIF2012")) {
                            logger.log(Level.INFO, "Register for FASTMON_GLIF2012 - OpenFlow results");
                            monPredicate pre4 = new monPredicate("*", "Flows_gva%", "*", t3, -1, new String[] {
                                "traffic_out"
                            }, null);
                            client.addLocalClient(this, pre4);
                        }
    ...
    public void processResult(MLSerClient client, eResult er) {
        ...
        rcNode ns = (rcNode) snodes.get(client.tClientID);
        if (ns == null) {
            return;
        }
        ...
        if (topoConfigCluster != null && er.ClusterName.startsWith(topoConfigCluster) && er.param != null && er.param_name != null) {
                    ...
                    if (map_col != null) {
                        ...
                        try {
                            for (Iterator it = map_col.iterator(); it.hasNext();) {
                                ...
                                String key = OSLink.ToString(ns.szOpticalSwitch_Name, opLink.port, opLink.destination, opLink.destinationPortName);
                                ...
                                if (osLink == null) {
                                    ...
                                    if (opLink.type.shortValue() == OpticalLink.TYPE_HOST) {
                                        // create new link
                                        osLink = new OSLink(ns,
                                                            null,
                                                            opLink.port,
                                                            opLink.destinationPortName,
                                                            ns.szOpticalSwitch_Name,
                                                            opLink.destination,
                                                            opLink.opticalLinkID);
                                        osLink.setFlags(OSLink.OSLINK_FLAG_FAKE_NODE);
                                        if (AppConfig.getProperty("lia.Monitor.OSGmap.debug", "false").equals("true")) {
                                            logger.warning(key + " Got fake link for " + ns + " - " + opLink.destination + " ->: " + opLink.port.name
                                        ...
                                        try {
                                            rcNode node = null;
                                            for (Iterator it2 = snodes.values().iterator(); it2.hasNext();) {
                                                node = (rcNode) it2.next();
                                                if (node.szOpticalSwitch_Name != null && node.szOpticalSwitch_Name.equals(opLink.destination)) {
                                                    osLink = new OSLink(ns,
                                                                        node,
                                                                        opLink.port,
                                                                        opLink.destinationPortName,
                                                    ...
                                                    if (AppConfig.getProperty("lia.Monitor.OSGmap.debug", "false").equals("true"))
                                                        logger.warning(key + " Got link for " + ns + " - " + opLink.destination + " ->: "
                                                                + opLink.port.name + "(" + opLink.port.type + ")" + osLink.toString() + " - "
                        ...
                        for (Iterator it = newOsi.localHosts.keySet().iterator(); it.hasNext();) {
                            ...
                            if (newOsi.osPorts != null) {
                                for (int k = 0; k < link.length; k++)
                                    for (int i = 0; i < newOsi.osPorts.length; i++) {
                                        final OSwPort port = newOsi.osPorts[i];
                                        if (port.type == OSwPort.INPUT_PORT && port.name.equals(link[k].destinationPortName)) {
                                            String linkId = null;
                                            if (newOsi.crossConnects != null) {
                                                for (int j = 0; j < newOsi.crossConnects.length; j++) {
                                                    final OSwCrossConn cc = newOsi.crossConnects[j];
                                            ...
                                            String key = OSLink.ToString(ns.szOpticalSwitch_Name,
                                                                         port,
                                                                         link[k].destination,
                                                                         link[k].destinationPortName);
                                            if (ns.wconn.containsKey(key)) {
                                                OSLink osLink = (OSLink) ns.wconn.get(key);
                                                osLink.updateStateV2(port.fiberState, port.powerState, linkId);
                                                osLink.setDirty(true);
                                            } else {
                                                OSLink osLink = new OSLink(ns, null, port, null, ns.szOpticalSwitch_Name, hostName, linkId);
                                                osLink.setFlags(OSLink.OSLINK_FLAG_FAKE_NODE);
                                                osLink.updateStateV2(port.fiberState, port.powerState, linkId);
                                                osLink.setDirty(true);
                                                ns.wconn.put(key, osLink);
                    ...
                    if (newOsi.osPorts != null && newOsi.osPorts.length != 0) {
                        for (int i = 0; i < newOsi.osPorts.length; i++) {
                            ...
                            if (link.type == OSwLink.TYPE_HOST) {
                                ...
                                // create new link
                                String key = OSLink.ToString(ns.szOpticalSwitch_Name, port, link.destination, link.destinationPortName);
                                if (ns.wconn.containsKey(key)) {
                                    osLink = (OSLink) ns.wconn.get(key);
                                    osLink.updateStateV2(port.fiberState, port.powerState, linkId);
                                    osLink.setDirty(true);
                                } else {
                                    osLink = new OSLink(ns, null, port, link.destinationPortName, ns.szOpticalSwitch_Name, link.destination, linkId);
                                    osLink.setFlags(OSLink.OSLINK_FLAG_FAKE_NODE);
                                    osLink.updateStateV2(port.fiberState, port.powerState, linkId);
                                    osLink.setDirty(true);
                                    ns.wconn.put(key, osLink);
                                ...
                                try {
                                    ...
                                    for (Iterator it2 = snodes.values().iterator(); it2.hasNext();) {
                                        node = (rcNode) it2.next();
                                        if (node.szOpticalSwitch_Name != null && node.szOpticalSwitch_Name.equals(link.destination)) {
                                            String linkId = null;
                                            if (newOsi.crossConnects != null) {
                                                for (int j = 0; j < newOsi.crossConnects.length; j++) {
                                                    final OSwCrossConn cc = newOsi.crossConnects[j];
    ...
    public void processResult(MLSerClient client, Result r) {
        ...
        if (hasGlobeLinks) {
            ...
            final boolean hNF = (globeLinksType == GlobeLinksType.NETFLOW) && cName != null && cName.equals("NetFlow");
            final boolean hFDT = (globeLinksType == GlobeLinksType.FDT) && cName != null && cName.equals("Links_Summary") && fName != null
                    && fName.equals("FDT_SC10");
            final boolean hOF = (globeLinksType == GlobeLinksType.OPENFLOW) && fName != null && fName.equals("FASTMON_GLIF2012");
            if (hNF || hFDT || hOF) {
                addNetFlow(ns, r);
                gupdate();
                return;
    ...
    protected void addNetFlow(rcNode nw, Result r) {
        ...
        switch (globeLinksType) {
            ...
            case OPENFLOW: {
                ...
                final int idx = r.getIndex("traffic_out");
                if (idx < 0) {
                    return;
                }
                final String flowName = r.NodeName;
                if (flowName == null) {
                    return;
                }

                int fIDX = 1;
                final String linkName = "OpenFlow_" + flowName;
                objLink = nw.wconn.get(linkName + "___" + fIDX);
                if (objLink != null && !(objLink instanceof NFLink)) {
                    return; // invalid object stored??
                }
                ...
                if (objLink == null) {
                    final ArrayList ilinksList = iNetGeo.getLinks(linkName);
                    final ArrayList<NFLink> nfLinks = NFLink.convert(ilinksList);
                    if (nfLinks != null) {
                        for (final NFLink nf : nfLinks) {
                            ...
                            nf.time = r.time;
                            if (DEBUG) {
                                System.out.println("New OpenFlow link added: " + linkName + "___" + fIDX + " ---> " + nf);
                            }
                            nw.wconn.put(linkName + "___" + fIDX, nf);
                            fIDX++;
                        }
                    }
                } else {
                    NFLink link = null;
                    for (;;) {
                        link = (NFLink) nw.wconn.get(linkName + "___" + fIDX);
                        if (link != null) {
                            if (DEBUG) {
                                System.out.println("UPDATE OpenFlow link added: " + linkName + "___" + fIDX + " ---> " + link);
                            }
                ...
                if (objLink instanceof ILink) {
                    ...
                    if (now - link.time > NETFLOW_LINK_EXPIRE_TIME) {
                        switch (globeLinksType) {
                            ...
                            default: {
                                logger.log(Level.WARNING, " *UNDEFINED _ DEFAULT _ LINK* : " + link + " inactive -> removing");
                                break;
                            }
                        }
                        n.wconn.remove(key);
