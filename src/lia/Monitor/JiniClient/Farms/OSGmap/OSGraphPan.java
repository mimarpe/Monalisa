...
public class OSGraphPan extends JPanel implements MouseListener, MouseMotionListener, LayoutChangedListener, ActionListener {
	
	/** Logger Name */
	private static final transient String COMPONENT = "lia.Monitor.JiniClient.Farms.OSGmap";
	/** The Logger */ 
		...
		if (link.szSourcePort != null) { // type one sw
			...
			if (link.szSourceNewPort.type == OSwPort.OUTPUT_PORT) {
				...
				if (dest.getOSwConfig() != null && dest.getOSwConfig().osPorts != null) {
					for (int i = 0; i<dest.getOSwConfig().osPorts.length; i++) {
						OSwPort p = dest.getOSwConfig().osPorts[i];
				...
				try {
					if (source.getOSwConfig() != null && source.getOSwConfig().osPorts != null) {
						for (int i = 0; i<source.getOSwConfig().osPorts.length; i++) {
							OSwPort p = source.getOSwConfig().osPorts[i];
...
//		System.out.println("mouseDragged "+e);
	...
	public OSMonitorControl getAddFirstMonitor() {
		...
		for (int i=0; i<selectedNodes.size(); i++) {
			...
			if ((obj = LTFNcontains(0, n)) != null) {
				rcNode n1 = (rcNode)obj[3];
				if (n1 != null && n1.client.osControl != null) return n1.client.osControl;
				continue;
			}
	...
	public OSMonitorControl getDelFirstMonitor() {
		...
		for (int i=0; i<participatingRCNodes.size(); i++) {
			...
			if (n.client.osControl != null) return n.client.osControl;
		}
		return null;
 	}
	...
	protected Vector pathBetweenEndPoints(rcNode n1, rcNode n2, Vector path) {
		...
		for (Enumeration en = n1.wconn.keys(); en.hasMoreElements(); ) {
			...
			if (obj instanceof OSLink) {
				OSLink link = (OSLink)obj;
				if (link.szSourcePort != null && (link.szSourcePort.type.shortValue() == OSPort.INPUT_PORT || link.linkID != null)) continue;
				if (link.szSourceNewPort != null && (link.szSourceNewPort.type == OSwPort.INPUT_PORT || link.linkID != null)) continue;
		...
		for (int i=0; i<v.size(); i++) {
			...
			if (link.szSourcePort != null) {
				...
				if (link.szSourceNewPort.type == OSwPort.INPUT_PORT) {
					String key = link.szDestName+":"+link.szDestPort;
					p.put(key, new Object[] { link.szSourceName, link.szSourceNewPort.name, link.rcDest, link.rcSource, link.szDestName, link.szDestPort });
				} else {
					String key = link.szSourceName+":"+link.szSourceNewPort.name;
					p.put(key, new Object[] { link.szDestName, link.szDestPort, link.rcSource, link.rcDest, link.szSourceName, link.szSourceNewPort.name }); 
				}
			}
		}
				...
				if (LTFNcontains(0, next) != null) {
					...
					if (next.getOpticalSwitchInfo() != null) {
						if (next.getOpticalSwitchInfo().crossConnects == null)
							break;
						for (Iterator it = next.getOpticalSwitchInfo().crossConnects.keySet().iterator(); it.hasNext(); ) {
							OpticalCrossConnectLink ccl = (OpticalCrossConnectLink)next.getOpticalSwitchInfo().crossConnects.get(it.next());
						...
						if (next.getOSwConfig().crossConnects == null)
							break;
						for (int i = 0; i<next.getOSwConfig().crossConnects.length; i++) {
							OSwCrossConn ccl = next.getOSwConfig().crossConnects[i];
							if (ccl.sPort.name.equals(destPort)) {
								nextPort = ccl.dPort.name;
								break;
							}
				...
				if (LTFNcontains(0, sourceNode) != null) {
					...
					if (sourceNode.getOpticalSwitchInfo() != null) {
						if (sourceNode.getOpticalSwitchInfo().crossConnects == null)
							break;
						for (Iterator it = sourceNode.getOpticalSwitchInfo().crossConnects.keySet().iterator(); it.hasNext(); ) {
							OpticalCrossConnectLink ccl = (OpticalCrossConnectLink)sourceNode.getOpticalSwitchInfo().crossConnects.get(it.next());
						...
						if (sourceNode.getOSwConfig().crossConnects == null) 
							break;
						for (int i = 0; i<sourceNode.getOSwConfig().crossConnects.length; i++) {
							OSwCrossConn ccl = sourceNode.getOSwConfig().crossConnects[i];
							if (ccl.dPort.name.equals(sourcePort)) {
