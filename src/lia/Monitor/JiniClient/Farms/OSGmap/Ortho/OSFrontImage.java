...
public class OSFrontImage {
	...
	private static Paint colorSupply[] = new Paint[] {
												   new Color(36,255,0),
												   		new Color(16,16,214),
												   		new Color(255, 144, 62),
												   new Color(16, 16, 214),
														new Color(164, 176, 255),
												   new Color(234, 159, 255),
														new Color(255, 154, 154),
												   VERY_LIGHT_RED,
														VERY_LIGHT_BLUE,
												   VERY_LIGHT_GREEN,
														VERY_LIGHT_YELLOW,
	...
	private static Paint antiColorSupply[]= new Paint[] {
		  new Color(14,118,22),
		  		new Color(82, 158, 244),
		  		new Color(168, 115, 13),
		  new Color(82, 158, 244),
				new Color(255-164, 255-176, 255),
		  new Color(255-234, 255-159, 255),
				new Color(255, 255-154, 255-154),
		  VERY_DARK_RED,
				VERY_DARK_BLUE,
		  VERY_DARK_GREEN,
				VERY_DARK_YELLOW,
	...
	private static void drawOSLedsForOSI(rcNode node, Graphics2D g2, int startX, int startY, int width, int height ,OpticalSwitchInfo osi) {
		...
		synchronized (lock) {
			for (Iterator it = osi.crossConnects.keySet().iterator(); it.hasNext(); ) {
				OpticalCrossConnectLink link = (OpticalCrossConnectLink)osi.crossConnects.get(it.next());
				...
				if (!portNames.containsKey(link.dPort)) {
					...
					if (!blinkMapping.containsKey(key)) blinkMapping.put(key, new Boolean(Math.random() <= 0.5));
					if (!thread.getRunning()) thread.start();
					x++;
					if (x == portsW) { x = 0; y++; }
					nr++;
				}
	...
	private static void drawOSLedsForOSI(rcNode node, Graphics2D g2, int startX, int startY, int width, int height, OSwConfig osi) {
		if (osi.crossConnects == null) {
			if (logger.isLoggable(Level.FINE))
				logger.warning("Node "+node.shortName+" doesn't received yet any crossconnects");
			return;
		...
		synchronized (lock) {
			for (int i=0; i<osi.crossConnects.length; i++) {
				final OSwCrossConn link = osi.crossConnects[i];
	...
	public static void drawOSLeds(rcNode node, Graphics2D g2, int startX, int startY, int width, int height) {
		...
		if (node == null || g2 == null) // error drawing
			return;
	...
	protected static void drawLeds(rcNode node, int nrPorts, Graphics2D g2, int startX, int startY, int width, int height) {
				...
				if (port2link.containsKey(p)) {
					...
					if (o instanceof OpticalCrossConnectLink) {
						...
						if (colors.containsKey(link)) {
							...
							if (blink) c = (Color)antiColors.get(link);
						}
	...
	public static OSwConfig getNewOpticalSwitchInfo(rcNode node) {
		...
		if (gmle == null)
			return null;
		byte array[] = (byte[])gmle.hash.get("OSIConfig");
		if (array == null) // no field to process
			return null;
		try {
			ByteArrayInputStream bais = new ByteArrayInputStream(array);
			ObjectInputStream ois = new ObjectInputStream(bais);
