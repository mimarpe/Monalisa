 ...
 */
public class ConfigFrame extends JFrame implements ActionListener, ItemListener, WindowListener, MouseListener, ComponentListener {

	/** Logger used by this class */
	private static final transient Logger logger = Logger.getLogger("lia.Monitor.JiniClient.Farms.OSGmap.Config");
	
	private PortsPanel portsPanel = null;
	private LegendDialog legendDialog;
	private ReducedPortTable portTable;
	private ConnectionTable connTable;
	
	private JCheckBox fdxCheck;
	
	private JButton legendButton;
	private JButton connectButton;
	private JButton disconnectButton;
	
	private  JButton gmplsButton;
	public GMPLSAdmin gmplsAdmin;
	
	private JButton destroyLinkSimulateButton;
	...
	private Hashtable inputPortStates;
	private Hashtable inputPortPower;
	private Hashtable inputSignalTypes;
	private Hashtable outputPortStates;
	private Hashtable outputPortPower;
	
	private Hashtable conns;
	private Hashtable connState;
	private Hashtable connLoss;
	private Hashtable connSwTime;
	private Hashtable connDuration;
	
	private Vector portAdminListeners;
	...
	public void setConnState(String port1, String port2, String state, String insertionLoss, String swTime, String duration) {
...
//		ConfigFrame f = new ConfigFrame("Test", ctrl);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source.equals(legendButton)) {
			...
			legendDialog.setLocation(this.getX() + (this.getWidth() - legendDialog.getWidth()) / 2, this.getY() + (this.getHeight() - legendDialog.getHeight()) / 2);
		...
		if (source.equals(connectButton)) {
			...
			logger.info("Sending connect for device "+getTitle()+" between ports "+inputPort+" and "+outputPort);
		...
		if (source.equals(disconnectButton)) {
			...
			String inputPort = portsPanel.portInClicked;
			String outputPort = portsPanel.portOutClicked;
			logger.info("Sending disconnect for device "+getTitle()+" between ports "+inputPort+" and "+outputPort);
		...
		if (source.equals(destroyLinkSimulateButton)) {
			if (currentClickedPort == null ||  control == null) return;
			...
			if (state) {
				try {
					...
					logger.info("Simulate fiber failure for "+currentClickedPort+" failed with "+ex.getLocalizedMessage());
				}
			} else {
				try {
					...
					logger.info("End simulation of fiber failure for "+currentClickedPort+" failed with "+ex.getLocalizedMessage());
				}
			}
		}
	}
	...
	public void windowClosing(WindowEvent e) {
		...
		if (control == null || control.provider == null || ((tClient)control.provider).trcframe == null ||
				((tClient)control.provider).trcframe.serMonitorBase == null || ((tClient)control.provider).trcframe.serMonitorBase.main == null)
			return;
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (eyeRect != null && eyeRect.contains(p1)) {
			if (e.isControlDown()) setSimulateState();
			e.consume();
		} 
	}
