...
public class OSMonitorControl implements PortAdminListener, LocalDataFarmClient {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.Monitor.JiniClient.Farms.OSGmap.Config.OSMonitorControl");
	...
	private Vector<String> alreadyAddedPorts;
...
//		System.out.println("rmi port = "+remoteRegistryPort);
		...
		if (keyStore == null || keyStore.length() == 0) return null;
		try {
    ...
    public void changePortState(String portName, String newSignalType) {
		try {
			if (remoteCtrl != null) {
				...
				logger.log(Level.INFO, "Remote cmd CHANGE_PORT_STATE["+portName+","+newSignalType+"] returned code "+remoteCode);
			}
		} catch (Throwable t)  {
    ...
    public void disconnectPorts(String inputPort, String outputPort, boolean fullDuplex) {
		...
		try {
            if(remoteCtrl != null) {
                ...
                logger.log(Level.INFO, " Remote cmd DISCONNECT_PORTS["+inputPort+","+outputPort+"] returned: " + remoteCode);
               System.out.println(" Remote cmd DISCONNECT_PORTS["+inputPort+","+outputPort+"] returned: " + remoteCode);
			}
        }catch(Throwable t){
    ...
    public void connectPorts(String inputPort, String outputPort, boolean fullDuplex) {
        ...
        try {
            if(remoteCtrl != null) {
                ...
                //TODO - Change the log level
                logger.log(Level.INFO, " Remote cmd CONNECT_PORTS["+inputPort+","+outputPort+"] returned: " + remoteCode);
               System.out.println(" Remote cmd CONNECT_PORTS["+inputPort+","+outputPort+"] returned: " + remoteCode);
            }
        }catch(Throwable t){
...
//					System.out.println(p);
				...
				if (info.crossConnects != null && info.crossConnects.size() != 0) {
					for (Iterator<OSPort> it = info.crossConnects.keySet().iterator(); it.hasNext(); ) {
						OpticalCrossConnectLink link = info.crossConnects.get(it.next());
...
//							} 
		...
		synchronized (opticalSwInfos) {
			...
			if (info != null && frame != null) {
				...
				for (int i = 0; i < info.osPorts.length; i++) {
					OSwPort port = info.osPorts[i];
					if (port.type == OSwPort.MULTICAST_PORT) continue;
	...
	public void setPortList(ArrayList<String> portList) {
		...
		synchronized (opticalSwInfos) {
			...
			for (Iterator<String> it = portList.iterator(); it.hasNext(); ) {
	...
	private void processResult(Result res) {
		...
		if (frame == null || info == null || res == null || res.param_name == null || res.param == null) return;
	...
	public boolean deletePath(String id) {
		...
		try {
			...
			if (ret != null && ret.equals("OK")) return true;
