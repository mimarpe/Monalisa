...
public class VoJobs extends JPanel implements graphical, ActionListener, 
						ComponentListener, LocalDataFarmClient {
	/** Logger name */
	...
	Color [] colorsTotalJobs = new Color [] {new Color(128, 255, 128), new Color(255, 255, 128), new Color(255, 128, 128) };
	...
	public VoJobs(){
		...
		preds[0] = new monPredicate("*", "VO_JOBS", "*", -2 * 60 * 1000, -1, new String[] {"Running Jobs", "Idle Jobs", "Held Jobs"}, null);
		preds[1] = new monPredicate("*", "PN%", "*", -2 * 60 * 1000, -1, new String[] {"NoCPUs", "Load5"}, null);
		preds[2] = new monPredicate("*", "osgVO_JOBS", "*", -2 * 60 * 1000, -1, new String[] { "RunningJobs", "IdleJobs", "HeldJobs" }, null);
		preds[3] = new monPredicate("*", "LcgVO_JOBS", "*", -2 * 60 * 1000, -1, new String[] { "RunningJobs", "IdleJobs", "HeldJobs" }, null);
...
//								System.out.println("new vo: "+vo);
								...
								graphs.put(vo, cbJobs); graphsPan.add(cbJobs);
 ...
 */
	...
	public void updateNodes() {
		...
		for(int i=0; i<vnodes.size(); i++){
			...
			if((! crtNodes.contains(n)) && (n.mlentry.Group.indexOf("grid3")>=0 || n.mlentry.Group.indexOf("osg")>=0 || n.mlentry.Group.indexOf("OSG")>=0)){
				...
				for(int j=0; j<preds.length; j++)
					n.client.addLocalClient(this, preds[j]);
...
//		combinedChartPanel.redraw();
	...
	void setResult(MLSerClient client, Result r){
		...
		// VO_JOBS 
		rcNode n = (rcNode) nodes.get(client.tClientID);
		if ( n==null )
			return;
		
		if(r.ClusterName.equals("VO_JOBS")){
			if (newModuleDetected.contains(n)) return;
			Hashtable rlist = (Hashtable) results.get(n);
			if(rlist == null){
				rlist = new Hashtable();
				results.put(n, rlist);
...
//				else 
				if(param.equals("Idle Jobs")) add = true;
				else if(param.equals("Running Jobs")) add = true;
				else if(param.equals("Held Jobs")) add = true;
				if(add){
...
//				if(param.equals("Total Jobs")) add = true;
//				else 
				if(param.equals("IdleJobs")) { add = true;  }
				else if(param.equals("RunningJobs")) { add = true;  }
				else if(param.equals("HeldJobs")) { add = true;  }
				if(add){
					if (!newModuleDetected.contains(n)) {
						...
						for (Enumeration en = rlist.keys(); en.hasMoreElements(); ) {
							String key = (String)en.nextElement();
							if (key.indexOf("Idle Jobs") != -1 || key.indexOf("Running Jobs") != -1 || key.indexOf("Held Jobs") != -1) v.add(key);
						}
						for (int ii=0; ii<v.size(); ii++) rlist.remove(v.get(ii));
					}
					String key = /*r.ClusterName+"/"+*/r.NodeName+"/"+param;
					Value v = (Value) rlist.get(key);
					if(v == null){
