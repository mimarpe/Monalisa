...
public class Mmap extends MapCanvasBase {  
    ...
    static Logger logger = Logger.getLogger(Mmap.class.getName());
    JoptPan optPan;
        ...
        for ( final rcNode ns : nodes.values() ) {
            if ( ns.wconn.size() > 0 )  {
                ...
                for ( Enumeration e1 = ns.wconn.elements(); e1.hasMoreElements(); ) {
                    ...
                    if(v < minPerformance) minPerformance = v;
                    if(v > maxPerformance) maxPerformance = v;
                    v = link.speed;
                    if(v < minLimit) minLimit = v;
                    if(v > maxLimit) maxLimit = v;
                }
            }
        }
        ...
        for ( final rcNode n : nodes.values() ) {
            if ( n.wconn.size() > 0 && !n.bHiddenOnMap )  {
                for ( Enumeration e1 = n.wconn.elements(); e1.hasMoreElements(); ) {
                    ...
                    if ( deltaLimitVal > 0 ) {
                        ((Graphics2D)g).setStroke( new BasicStroke((float)(1+2.5*(link.speed-optPan.csWAN.minLimitValue)/(deltaLimitVal+0.1))));
                    };
...
//      int klat2 = (int) img2panY(rad2imgY(xlat2));
        ...
        int klong1 = long2panX((String) link.from.get("LONG"));
        int klat1 = lat2panY((String) link.from.get("LAT"));
        int klong2 = long2panX((String) link.to.get("LONG"));
        int klat2 = lat2panY((String) link.to.get("LAT"));
        ...
        for(final rcNode nod : nodes.values() ){
            ...
            if(nod.wconn.size() > 0) {
                for(Enumeration el = nod.wconn.elements(); el.hasMoreElements(); ){
    			    ...
    			    if ( !(objLink instanceof ILink) )
    			        continue;
    				ILink link = (ILink) objLink;
                    int klong = long2panX((String) link.from.get("LONG"));
                    int klat = lat2panY((String) link.from.get("LAT"));
                    if(Math.abs(klong - x) < 6 && Math.abs(klat - y) < 6){
                        return "Router @ "+link.from.get("CITY")+" / "+link.from.get("COUNTRY");
                    }
                    klong = long2panX((String) link.to.get("LONG"));
                    klat = lat2panY((String) link.to.get("LAT"));
                    if(Math.abs(klong - x) < 6 && Math.abs(klat - y) < 6){
                ...
                if(optPan.kbShowWAN.isSelected())
                    for ( Enumeration e1 = nod.wconn.elements(); e1.hasMoreElements(); ) {
                        ...
                        if  ( useILink(link , null, LINK_TYPE_DIRECTED, null, 1, CheckPos) ) {
                            ...
                            + " Mbps -> " + optPan.csWAN.formatter.format(((Double) (link.data)).doubleValue() * 100 / link.speed)
                            + " % ";
            ...
            if ( optPan.kbShowPing.isSelected() && nod.conn.size() > 0 ) {
                ...
                for ( Enumeration e1 = nod.conn.keys(); e1.hasMoreElements(); ) {
                    ...
                    if  ( useILink(link , null, LINK_TYPE_DIRECTED, null, 1, CheckPos) ) {
                        String text, shortText;
                        text = "ABPing: " + link.fromIP +"->" + link.toIP 
                        + ": RTTime= " +	optPan.csPing.formatter.format(link.inetQuality[0]) 
                        + ", RTT= " + optPan.csPing.formatter.format(link.inetQuality[1]) 
                        + " ms, Lost Packages= " + optPan.csPing.formatter.format(link.inetQuality[2] * 100) + " % ";
                        shortText = "ABPing: " +  link.fromIP +"->" + link.toIP
                        + ": RTT=" + optPan.csPing.formatter.format(link.inetQuality[1])
                        + " (Lost=" + optPan.csPing.formatter.format(link.inetQuality[2] * 100) + " %)";
