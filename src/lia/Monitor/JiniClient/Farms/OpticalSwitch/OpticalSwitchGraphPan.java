package lia.Monitor.JiniClient.Farms.OpticalSwitch;
...
import lia.Monitor.JiniClient.Farms.OpticalSwitch.Ortho.OSFrontImage;
import lia.Monitor.JiniClient.Farms.OpticalSwitch.Ortho.OpticalSwitchConnectivityToolTip;
import lia.Monitor.JiniClient.Farms.OpticalSwitch.topology.GraphTopology;
import lia.Monitor.JiniClient.Farms.OpticalSwitch.topology.GridLayoutAlgorithm;
import lia.Monitor.JiniClient.Farms.OpticalSwitch.topology.LayeredTreeLayoutAlgorithm;
import lia.Monitor.JiniClient.Farms.OpticalSwitch.topology.NoLayoutLayoutAlgorithm;
import lia.Monitor.JiniClient.Farms.OpticalSwitch.topology.RadialLayoutAlgorithm;
import lia.Monitor.JiniClient.Farms.OpticalSwitch.topology.RandomLayoutAlgorithm;
import lia.Monitor.JiniClient.Farms.OpticalSwitch.topology.SpringLayoutAlgorithm;
...
public class OpticalSwitchGraphPan extends JPanel implements MouseListener, MouseMotionListener, LayoutChangedListener, ActionListener {
	
	/** Logger Name */
	private static final transient String COMPONENT = "lia.Monitor.JiniClient.Farms.OpticalSwitch";
	/** The Logger */ 
	static final transient Logger logger = Logger.getLogger(COMPONENT);
	
	Map<ServiceID, rcNode> GlobalNodes= new ConcurrentHashMap<ServiceID, rcNode>();//snodes
	...
	static BufferedImage telescentShadow;
	static BufferedImage hostShadow;
	static BufferedImage cienaShadow;
	static BufferedImage force10Shadow;
	...
	public static OpticalSwitchConnectivityToolTip OSConToolTip = new OpticalSwitchConnectivityToolTip();
	
	private BufferedImage osImage;//optical switch image
	private BufferedImage glowOSImage;
	private BufferedImage grayosImage; 
	private int osImgWidth = 106;//set at loading iamge moment
	private int osImgHeight = 32;//set at loading iamge moment
	
	private BufferedImage telescentImage;
	private BufferedImage glowTelescentImage;
	private BufferedImage grayosTelescentImage;
	private int telescentosImgWidth = 63;//set at loading iamge moment
	private int telescentosImgHeight = 100;//set at loading iamge moment

	private BufferedImage hostImage;
	private BufferedImage glowHostImage;
	private BufferedImage grayHostImage;
	private int hostImgWidth = 100;//set at loading iamge moment
	private int hostImgHeight = 40;//set at loading iamge moment

	private BufferedImage cienaImage;
	private BufferedImage glowCienaImage;
	private BufferedImage grayCienaImage;
	private int cienaImgWidth = 42;//set at loading iamge moment
	private int cienaImgHeight = 56;//set at loading iamge moment

	private BufferedImage force10Image;
	private BufferedImage glowForce10Image;
	private BufferedImage grayForce10Image;
	private int force10ImgWidth = 38;//set at loading iamge moment
	private int force10ImgHeight = 32;//set at loading iamge moment

	SerMonitorBase monitor;
	OpticalSwitchPan gmapPan;
...
//	private DecimalFormat lblFormat = new DecimalFormat("###,###.#");
	...
	private Vector oslinks = new Vector();
	private String linkToolTip = null;
	Vector participatingRCNodes = new Vector();
	
	private static OpticalSwitchGraphPan _instance = null;
	
	public static synchronized final OpticalSwitchGraphPan getInstance(OpticalSwitchPan gmapPan) {
		if (_instance == null)
			_instance = new OpticalSwitchGraphPan(gmapPan);
		return _instance;
	}
	
	public static synchronized final OpticalSwitchGraphPan getInstance() { 
		return _instance;
	}
	
	private OpticalSwitchGraphPan(OpticalSwitchPan gmapPan) {
		...
		telescentImage = loadBuffImage("lia/images/TelescentAFOX.gif");
		grayosTelescentImage = loadBuffImage("lia/images/TelescentAFOX_gray.gif");
		glowTelescentImage = loadGlowBuffImage("lia/images/TelescentAFOX.gif");
		telescentosImgWidth = telescentImage.getWidth();
		telescentosImgHeight = telescentImage.getHeight();
		if (telescentImage != null) {
			telescentShadow = new BufferedImage(telescentosImgWidth + 14, telescentosImgHeight + 14, BufferedImage.TYPE_INT_ARGB); 
			Graphics2D g = telescentShadow.createGraphics();
			g.drawImage(telescentImage, null, 6, 6);
			g.dispose();
			blurOp = getBlurOp(7);
			blurOp.filter(createShadowMask(telescentImage), telescentShadow);
		}
		
		hostImage = loadBuffImage("lia/images/host.gif");
		grayHostImage = loadBuffImage("lia/images/host_gray.gif");
		glowHostImage = loadGlowBuffImage("lia/images/host.gif");
		hostImgWidth = hostImage.getWidth();
		hostImgHeight = hostImage.getHeight();
		if (hostImage != null) {
			hostShadow = new BufferedImage(hostImgWidth + 14, hostImgHeight + 14, BufferedImage.TYPE_INT_ARGB); 
			Graphics2D g = hostShadow.createGraphics();
			g.drawImage(hostImage, null, 6, 6);
			g.dispose();
			blurOp = getBlurOp(7);
			blurOp.filter(createShadowMask(hostImage), hostShadow);
		}
		
		cienaImage = loadBuffImage("lia/images/ciena.gif");
		grayCienaImage = loadBuffImage("lia/images/ciena_gray.gif");
		glowCienaImage = loadGlowBuffImage("lia/images/ciena.gif");
		cienaImgWidth = cienaImage.getWidth();
		cienaImgHeight = cienaImage.getHeight();
		if (cienaImage != null) {
			cienaShadow = new BufferedImage(cienaImgWidth + 14, cienaImgHeight + 14, BufferedImage.TYPE_INT_ARGB); 
			Graphics2D g = cienaShadow.createGraphics();
			g.drawImage(cienaImage, null, 6, 6);
			g.dispose();
			blurOp = getBlurOp(7);
			blurOp.filter(createShadowMask(cienaImage), cienaShadow);
		}
		
		force10Image = loadBuffImage("lia/images/force10.gif");
		grayForce10Image = loadBuffImage("lia/images/force10_gray.gif");
		glowForce10Image = loadGlowBuffImage("lia/images/force10.gif");
		force10ImgWidth = force10Image.getWidth();
		force10ImgHeight = force10Image.getHeight();
		if (force10Image != null) {
			force10Shadow = new BufferedImage(force10ImgWidth + 14, force10ImgHeight + 14, BufferedImage.TYPE_INT_ARGB); 
			Graphics2D g = force10Shadow.createGraphics();
			g.drawImage(force10Image, null, 6, 6);
			g.dispose();
			blurOp = getBlurOp(7);
			blurOp.filter(createShadowMask(force10Image), force10Shadow);
		}
		...
		uim.put("ToolTip.background",Color.white);
	}
	...
	class LayoutUpdateTimerTask extends TimerTask {
		...
		public void run() {
		    Thread.currentThread().setName(" ( ML ) - OSGMap - OSGraphPan Layout Update Timer Thread: checks stop condition and sets layout.");
			if ( shouldStop() ) {
				this.cancel();
				return;
			};
	...
	public static BufferedImage loadBuffImage(String imageFileName){
		...
		try{
			if ( imageFileName == null )
				return null;
			ClassLoader myClassLoader = OpticalSwitchGraphPan.class.getClassLoader();
			URL imageURL;
	...
	private void addFakes( rcNode node) {
		...
		synchronized (getTreeLock()) {
			...
			for (Enumeration it2 = node.wconn.keys(); it2.hasMoreElements(); ) {
				...
				if (objLink == null) continue;
				if( objLink instanceof OSLink ) {
					hlink = (OSLink)objLink;
					if (!hlink.checkFlag(OSLink.OSLINK_FLAG_FAKE_NODE)) continue; // not interested if  not fake
					boolean added = false;
					for ( int i=0; i<lLinkToFakeNode.size(); i++) {
						Object[] con = (Object[])lLinkToFakeNode.get(i);
						if (con[3].equals(node)) {
							rcNode fake = (rcNode)con[0];
							if (fake != null && fake.UnitName != null && hlink != null && hlink.szDestName != null && fake.UnitName.equals(hlink.szDestName)) {
								added = true;
								lLinkToFakeNode.remove(i);
								hlink.rcSource = node;
								hlink.rcDest = fake;
								if (hlink.szSourceOSPort != null) {
									if (hlink.szSourceOSPort.type() == PortType.OUTPUT_PORT) {
			...
			for (int i=0; i<lLinkToFakeNode.size(); i++) {
				...
				if (n.equals(rcnode)) {
					...
					for (Enumeration en = nodes.keys(); en.hasMoreElements(); ) {
						...
						if (o instanceof ServiceID) continue;
			...
			for (int k = 0; k < lLinkToFakeNode.size(); k++) {
				...
				if (ilink != null) {
					...
					for (Enumeration en = rnode.wconn.keys(); en.hasMoreElements(); ) {
						...
						if (ilink.equals(val)) { contains = true; break; }
					}
					if (!contains) {
						for (int i=0; i<lLinkToFakeNode.size(); i++) {
							Object[] obj= (Object[])lLinkToFakeNode.get(i);
							if ( obj[3] != null && obj[3].equals(rnode) && obj[0] != null && ((rcNode)obj[0]).UnitName != null && ((rcNode)obj[0]).UnitName.equals(node.UnitName)) {
								lLinkToFakeNode.remove(i);
				...
				if (olink != null) {
					...
					for (Enumeration en = rnode.wconn.keys(); en.hasMoreElements(); ) {
						...
						if (olink.equals(obj)) { contains = true; break; }
					}
					if (!contains) {
						for (int i=0; i<lLinkToFakeNode.size(); i++) {
							Object[] obj= (Object[])lLinkToFakeNode.get(i);
	...
	public void syncNodes() {
		...
		synchronized (getTreeLock()) {
			...
			for (Enumeration en = nodes.keys(); en.hasMoreElements(); ) {
				Object key = en.nextElement();
				if (!(key instanceof ServiceID)) continue; // only real nodes
				...
				if (!GlobalNodes.containsKey(key)) {
					...
					rcNode n1 = (rcNode)GlobalNodes.get(key);
					if (!n1.equals(n)) {
						v.add(key);
						if (OSConToolTip != null)
							OSConToolTip.removeCheck(n);
...
//				if (n.getOpticalSwitch() != null && !(n.getOpticalSwitch() instanceof OpticalSwitch)) {
//					if (!v.contains(key))
//						v.add(key);
//					if (OSConToolTip != null)
//						OSConToolTip.removeCheck(n);
//				}
			...
			for (final rcNode n : GlobalNodes.values() ) {
				if (n != null && n.mlentry != null && n.mlentry.Group != null &&  tClient.isOSgroup(n.mlentry.Group)) {
//					if (n.client != null) {
//						GenericMLEntry gmle = n.client.getGMLEntry();
//						if (gmle.hash != null && !gmle.hash.containsKey("TOPO_CONFIG_CLUSTER"))
//							continue;
//					}
...
//		System.out.println("Sync: "+nodes.size()+": "+GlobalNodes.size());
	}
	...
	public void paintFakeNode( Graphics g, rcNode n, FontMetrics fm) {
		...
		if (selectedNodes.contains(n)) {
			...
			BasicStroke stroke = new BasicStroke(2.0f,
					...
					b.getLineJoin(),
					b.getMiterLimit(),
					new float[] {16.0f,4.0f}, // Dash pattern
					b.getDashPhase());
			g2.setStroke(stroke);
		...
		if (n.equals(pickX))
			g.setColor( Color.red);
		g.drawString( n.shortName, n.limits.x + 7 + wunit_fake - w2, n.limits.y + 5 + hunit_fake - h2 + fm.getAscent());
		...
		if (currentSelectedLinkPoz.containsKey(n)) {
			...
			paintPort((Graphics2D)g, ((Integer)o[0]).intValue(), ((Integer)o[1]).intValue(), (String)o[2], fm, (Color)o[3], (Color)o[4]);
		}
	}
	...
	public void paintNode(Graphics g, rcNode n, FontMetrics fm) {
		...
		g.drawString(nodeName, n.limits.x + 7 + wunit - w2 -1, 
				n.limits.y + fm.getAscent() -1/*+ hunit - h2 + fm.getAscent()*/);
		g.drawString(nodeName, n.limits.x + 7 + wunit - w2 +1, 
				n.limits.y + fm.getAscent() +1/*+ hunit - h2 + fm.getAscent()*/);
		g.setColor( new Color(0,0,77));
		if (n.equals(pickX))
			g.setColor(Color.red);
		g.drawString(nodeName, n.limits.x + 7 + wunit - w2, 
				n.limits.y + fm.getAscent() /*+ hunit - h2 + fm.getAscent()*/);
	...
	public void paintHostNode(Graphics g, rcNode n, FontMetrics fm) {
		...
		if ( h2 > hunit ) 
			hunit = h2;
		if(n.limits == null)
			n.limits = new Rectangle(x-wunit_host, y-hunit_host, wunit_host*2, hunit_host*2 );
		else
			n.limits.setBounds(x-wunit_host, y-hunit_host, wunit_host*2, hunit_host*2);
		//draw new OS
		if (gmapPan.kbShowShadow.isSelected())
			((Graphics2D)g).drawImage(hostShadow, blurOp, x-hostImgWidth/2, y-hostImgHeight/2);
		if (participatingRCNodes.contains(n)) 
			g.drawImage(glowHostImage, x-hostImgWidth/2, y-hostImgHeight/2, null);
		else
			g.drawImage(hostImage, x-hostImgWidth/2, y-hostImgHeight/2, null);
		if (selectedNodes.contains(n)) {
			g.setColor(Color.red);
			Graphics2D g2 = (Graphics2D)g;
			BasicStroke b = (BasicStroke)g2.getStroke();
			BasicStroke stroke = new BasicStroke(2.0f,
					...
					b.getDashPhase());
			g2.setStroke(stroke);
			g.drawRect(x-hostImgWidth/2, y-hostImgHeight/2, hostImgWidth, hostImgHeight);
			g2.setStroke(b);
		}
//		OSFrontImage.drawOSLeds( n, (Graphics2D)g, n.limits.x+12, n.limits.y+19, 70, 10);
		
		g.setColor(Color.white);
		g.drawString(nodeName, n.limits.x + 7 + wunit_host - w2 -1, 
				n.limits.y - fm.getAscent()/2 -1/*+ hunit - h2 + fm.getAscent()*/);
		g.drawString(nodeName, n.limits.x + 7 + wunit_host - w2 +1, 
				n.limits.y - fm.getAscent()/2 +1/*+ hunit - h2 + fm.getAscent()*/);
		g.setColor( new Color(0,0,77));
		if (n.equals(pickX))
			g.setColor(Color.red);
		g.drawString(nodeName, n.limits.x + 7 + wunit_host - w2, 
				n.limits.y - fm.getAscent()/2);
	...
	public void paintCienaNode(Graphics g, rcNode n, FontMetrics fm) {
		...
		if ( h2 > hunit ) 
			hunit = h2;
		if(n.limits == null)
			n.limits = new Rectangle(x-wunit_ciena, y-hunit_ciena, wunit_ciena*2, hunit_ciena*2 );
		else
			n.limits.setBounds(x-wunit_ciena, y-hunit_ciena, wunit_ciena*2, hunit_ciena*2);
		//draw new OS
		if (gmapPan.kbShowShadow.isSelected())
			((Graphics2D)g).drawImage(cienaShadow, blurOp, x-cienaImgWidth/2, y-cienaImgHeight/2);
		if (participatingRCNodes.contains(n)) 
			g.drawImage(glowCienaImage, x-cienaImgWidth/2, y-cienaImgHeight/2, null);
		else
			g.drawImage(cienaImage, x-cienaImgWidth/2, y-cienaImgHeight/2, null);
		if (selectedNodes.contains(n)) {
			g.setColor(Color.red);
			Graphics2D g2 = (Graphics2D)g;
			BasicStroke b = (BasicStroke)g2.getStroke();
			BasicStroke stroke = new BasicStroke(2.0f,
					...
					b.getDashPhase());
			g2.setStroke(stroke);
			g.drawRect(x-cienaImgWidth/2, y-cienaImgHeight/2, cienaImgWidth, cienaImgHeight);
			g2.setStroke(b);
		}
		
		g.setColor(Color.white);
		g.drawString(nodeName, n.limits.x + 7 + wunit_ciena - w2 -1, 
				n.limits.y - fm.getAscent()/2 -1/*+ hunit - h2 + fm.getAscent()*/);
		g.drawString(nodeName, n.limits.x + 7 + wunit_ciena - w2 +1, 
				n.limits.y - fm.getAscent()/2 +1/*+ hunit - h2 + fm.getAscent()*/);
		g.setColor( new Color(0,0,77));
		if (n.equals(pickX))
			g.setColor(Color.red);
		g.drawString(nodeName, n.limits.x + 7 + wunit_ciena - w2, 
				n.limits.y - fm.getAscent()/2 /*+ hunit - h2 + fm.getAscent()*/);
	...
	public void paintForce10Node(Graphics g, rcNode n, FontMetrics fm) {
		...
		int w2, h2;
		w2= force10ImgWidth/2;
		if ( w2 > wunit_force10 ) 
			wunit_force10 = w2;
		h2=force10ImgHeight/2;
		if ( h2 > hunit_force10 ) 
			hunit_force10 = h2;
		w2 = (fm.stringWidth(nodeName) + 15) / 2;
		h2 = (fm.getHeight() + 10) / 2;
		...
		if ( h2 > hunit ) 
			hunit = h2;
		if(n.limits == null)
			n.limits = new Rectangle(x-wunit_force10, y-hunit_force10, wunit_force10*2, hunit_force10*2 );
		else
			n.limits.setBounds(x-wunit_force10, y-hunit_force10, wunit_force10*2, hunit_force10*2);
		//draw new OS
		if (gmapPan.kbShowShadow.isSelected())
			((Graphics2D)g).drawImage(force10Shadow, blurOp, x-force10ImgWidth/2, y-force10ImgHeight/2);
		if (participatingRCNodes.contains(n)) 
			g.drawImage(glowForce10Image, x-force10ImgWidth/2, y-force10ImgHeight/2, null);
		else
			g.drawImage(force10Image, x-force10ImgWidth/2, y-force10ImgHeight/2, null);
		if (selectedNodes.contains(n)) {
			g.setColor(Color.red);
			Graphics2D g2 = (Graphics2D)g;
			BasicStroke b = (BasicStroke)g2.getStroke();
			BasicStroke stroke = new BasicStroke(2.0f,
					...
					b.getDashPhase());
			g2.setStroke(stroke);
			g.drawRect(x-force10ImgWidth/2, y-force10ImgHeight/2, force10ImgWidth, force10ImgHeight);
			g2.setStroke(b);
		}
		
		g.setColor(Color.white);
		g.drawString(nodeName, n.limits.x + 7 + wunit_force10 - w2 -1, 
				n.limits.y  -1/*+ hunit - h2 + fm.getAscent()*/);
		g.drawString(nodeName, n.limits.x + 7 + wunit_force10 - w2 +1, 
				n.limits.y  +1/*+ hunit - h2 + fm.getAscent()*/);
		g.setColor( new Color(0,0,77));
		if (n.equals(pickX))
			g.setColor(Color.red);
		g.drawString(nodeName, n.limits.x + 7 + wunit_force10 - w2, 
				n.limits.y  /*+ hunit - h2 + fm.getAscent()*/);
	...
	public void paintOSNode(Graphics g, rcNode n, FontMetrics fm) {
		...
		int w2, h2;
		w2= telescentosImgWidth/2;
		if ( w2 > wunit_os ) 
			wunit_os = w2;
		h2=telescentosImgHeight/2;
		if ( h2 > hunit_os ) 
			hunit_os = h2;
		w2 = (fm.stringWidth(nodeName) + 15) / 2;
		h2 = (fm.getHeight() + 10) / 2;
		...
		if ( h2 > hunit ) 
			hunit = h2;
		if(n.limits == null)
			n.limits = new Rectangle(x-wunit_os, y-hunit_os, wunit_os*2, hunit_os*2 );
		else
			n.limits.setBounds(x-wunit_os, y-hunit_os, wunit_os*2, hunit_os*2);
		//draw new OS
		if (gmapPan.kbShowShadow.isSelected())
			((Graphics2D)g).drawImage(telescentShadow, blurOp, x-telescentosImgWidth/2, y-telescentosImgHeight/2);
		if (participatingRCNodes.contains(n)) 
			g.drawImage(glowTelescentImage, x-telescentosImgWidth/2, y-telescentosImgHeight/2, null);
		else
			g.drawImage(telescentImage, x-telescentosImgWidth/2, y-telescentosImgHeight/2, null);
		if (selectedNodes.contains(n)) {
			g.setColor(Color.red);
			Graphics2D g2 = (Graphics2D)g;
			BasicStroke b = (BasicStroke)g2.getStroke();
			BasicStroke stroke = new BasicStroke(2.0f,
					...
					b.getDashPhase());
			g2.setStroke(stroke);
			g.drawRect(x-telescentosImgWidth/2, y-telescentosImgHeight/2, telescentosImgWidth, telescentosImgHeight);
			g2.setStroke(b);
		}
		Graphics2D g2 = (Graphics2D)g; 
		g2.setColor(Color.white);
		g2.fillRect((n.limits.x+n.limits.width/2)-35, (n.limits.y+n.limits.height/2)-5, 70, 10);
		
//		g2.setColor(Color.red);
//		g2.drawRect(n.limits.x, n.limits.y, n.limits.width, n.limits.height);
		
		OSFrontImage.drawOSLeds( n, g2, (n.limits.x+n.limits.width/2)-35, (n.limits.y+n.limits.height/2)-5, 70, 10);
		
		g.setColor(Color.white);
		g.drawString(nodeName, n.limits.x + 7 + wunit_os - w2 -1, 
				n.limits.y - fm.getAscent()/2 -1/*+ hunit - h2 + fm.getAscent()*/);
		g.drawString(nodeName, n.limits.x + 7 + wunit_os - w2 +1, 
				n.limits.y - fm.getAscent()/2 +1/*+ hunit - h2 + fm.getAscent()*/);
		g.setColor( new Color(0,0,77));
		if (n.equals(pickX))
			g.setColor(Color.red);
		g.drawString(nodeName, n.limits.x + 7 + wunit_os - w2, 
				n.limits.y - fm.getAscent()/2 /*+ hunit - h2 + fm.getAscent()*/);
	...
	private void paintPort(Graphics2D g2, int x, int y, String portName, FontMetrics fm, Color fiberColor, Color stateColor) {
		...
		g2.drawString(portName, x+(w+10)/2-w/2+1, y + h/2+g2.getFontMetrics().getHeight()/2 - 1);
	}
...
//		if (link.szSourceOSPort != null) { // type one sw
//			if (link.szSourceOSPort.type() == PortType.OUTPUT_PORT) {
				source = link.rcSource;
				dest = link.rcDest;
				portSource = getOSPortLabel(link.szSourceOSPort);
//				portDest = link.szDestPort;
				portDest = getOSPortLabel(link.szDestinationOSPort);
//			} else {
//				source = link.rcDest;
//				dest = link.rcSource;
////				portSource = link.szDestPort;
//				portSource = getOSPortLabel(link.szDestinationOSPort);
//				portDest = getOSPortLabel(link.szSourceOSPort);
//			}
//		}
		...
		if (showPorts) {
			...
			if (difx * dify < 0) angle *= -1.0;
			if (dify < 0) angle += Math.PI;
			if (difx * dify < 0) angle -= Math.PI;
			angle += Math.PI;
			...
			currentSelectedLinkPoz.put(source, new Object[] {Integer.valueOf((int)x1o), Integer.valueOf((int)y1o), portSource, (link.checkState(OSLink.STATE_DISCONNECTED) ? Color.BLACK : Color.GRAY), sourceCol });
			currentSelectedLinkPoz.put(dest, new Object[] {Integer.valueOf((int)x2o), Integer.valueOf((int)y2o), portDest, (link.checkState(OSLink.STATE_DISCONNECTED) ? Color.BLACK : Color.GRAY), destCol });
		}
		
		Link ll = new Link(x1p, y1p, x2p, y2p, link.linkID, link);
		links.add(ll);
		if (link.linkID != null) haveLinkWithID = true;
		
		if (linkSelected != null && link.linkID != null && linkSelected.equals(link.linkID)) {
			col = linkSelectionColor; 
		}
		
		if (col.equals(OSLink.OSLINK_COLOR_DISCONNECTED) || col.equals(OSLink.OSLINK_COLOR_FREE_2) || col.equals(OSLink.OSLINK_COLOR_FAIL)) {
			...
			Graphics2D g2 = (Graphics2D)gg;
			BasicStroke b = (BasicStroke)g2.getStroke();
			BasicStroke stroke = new BasicStroke( (showPorts ? 3.0f : 2.0f),
					b.getEndCap(),
					b.getLineJoin(),
					b.getMiterLimit(),
					new float[] {16.0f,4.0f}, // Dash pattern
	...
	public void paintComponent(Graphics g) {
		...
		((Graphics2D)g).setRenderingHint(RenderingHints.KEY_ANTIALIASING, gmapPan.kbMakeNice.isSelected() ? RenderingHints.VALUE_ANTIALIAS_ON :
					...
					try {
						for (Enumeration it2 = n.wconn.keys(); it2.hasMoreElements(); ) {
							...
							if ( objLink instanceof OSLink ) {
								...
								if (linksDrawn.contains(objkey)) continue; // do not draw again
								...
								if (rcDest == null) continue;
								// check to see if rcDest is acceptable
								boolean found = false;
								for (Enumeration en = nodes.elements(); en.hasMoreElements(); ) {
									rcNode nnn = (rcNode)en.nextElement();
									if (nnn.equals(rcDest)) { found = true; break; }
								}
								if (!found) continue; // don't draw
								
								if (rcDest.wconn.containsKey(objkey)) {
									OSLink revLink = (OSLink)rcDest.wconn.get(objkey);
									if ((revLink.nState & OSLink.STATE_CONNECTED) != OSLink.STATE_CONNECTED || (revLink.nState & OSLink.STATE_FREE) != OSLink.STATE_FREE) {
										link = revLink;
									}
								}
				...
				if ( LTFNcontains(0,n)==null ) {
					...
					if (n.getOpticalSwitchInfo() != null){
						if (!n.getOpticalSwitchInfo().isAlive)  
							...
							paintNode(g, n, fmOS);
							if (n!=null && n.client != null && n.client.opticalSwitchControl != null) {
								if (!haveControl) {
									gmapPan.currentControl = n.client.opticalSwitchControl;
								}
								haveControl = true;
							}
						}
						...
						GenericEntity ge = n.getOpticalSwitch();
						if (ge != null) {
							if (ge instanceof OpticalSwitch) {
								paintOSNode(g, n, fmOS);
							} else if (ge instanceof ComputerHost) {
								paintHostNode(g, n, fmOS);
							} else if (ge instanceof CienaHost) {
								paintCienaNode(g, n, fmOS);
							} else if (ge instanceof Force10Host) {
								paintForce10Node(g, n, fmOS);
							} else
								paintNode(g, n, fmOS);
						} else
							paintNode(g, n, fmOS);
		...
		//TODO - it's always disabled
//      gmapPan.createPathButton.setEnabled(haveControl);
		gmapPan.createPathButton.setEnabled(false);
...
//		System.out.println(System.currentTimeMillis() + ": " + haveControl);
					...
					try {
						...
						if ( objLink instanceof OSLink ) {
							...
							if (link.szSourcePort != null) { // type one sw
								if (link.rcSource != null && link.rcSource.equals(n) && link.szSourcePort.name.equals(port)) {
								...
								if (link.rcSource != null && link.rcSource.equals(n) && link.szSourceNewPort.name.equals(port)) {
									return (link.rcDest != null && LTFNcontains(0,link.rcDest)!=null);
								}
								if (link.rcDest != null && link.rcDest.equals(n) && link.szDestPort.equals(port)) {
									return (link.rcSource != null && LTFNcontains(0,link.rcSource)!=null);
	...
	public void setRemoveLinkButtons(boolean enabled) {
		//TODO it's always disabled for the moment
...
//		System.out.println("mousePressed called");
		...
		if (but == MouseEvent.BUTTON3) {
			...
			if(currentLayout.equals("Elastic") || 
					gmapPan.cbLayout.getSelectedItem().equals("Layered") || 
...
//					&& (pick.getOpticalSwitchInfo() == null || pick.getOpticalSwitchInfo().isAlive)) {
	...
	protected void addToPath(rcNode pick) {
		...
		if (singleEndPointsMode) {
			while (selectedNodes.size() > 1) selectedNodes.remove(1);
	...
	protected void removeFromPath(rcNode pick) {
		...
		if (allPathMode) {
			...
			while (poz < selectedNodes.size()) {
				if (pick.equals(selectedNodes.get(poz))) break;
				poz++;
			}
			...
			if (v == null) {
				logger.warning("Can not remove "+pick.UnitName+" since there is no path between "+p1.UnitName+" and "+p2.UnitName);
				return;
			}
			if (fdxPath) {
				path.clear();
				...
				if (v == null) {
					logger.warning("Can not remove "+pick.UnitName+" since there is no path between "+p2.UnitName+" and "+p1.UnitName);
					return;
				}
			}
			selectedNodes.remove(poz);
	...
	public void redoSelectedPath() {
		if (selectedNodes.size() != 0) {
			...
			for (int i=0; i<selectedNodes.size(); i++) {
				if (i != 0)
					if (multipleEndPointsMode && i != 1) buf.append(" + ");
					else
						buf.append(" -> ");
		...
		} else gmapPan.currentPath.setText("");
	}
...
//		System.out.println("mouseDragged "+e);
	...
	protected boolean checkPath(rcNode n) {
		...
		if (obj != null) {
			...
			if (n1.client != null && n1.client.osControl == null) return false;
			if (selectedNodes.size() == 0) { // first node
				if (obj[2] == null) return false; // must have connection fake -> node
			}
		} else
			if (n.client != null && n.client.osControl == null) return false;
		...
		if (singleEndPointsMode) {
			if (LTFNcontains(0, n) == null) return false;
			if (selectedNodes.size() > 0) {
	...
	public OpticalSwitchMonitorControl getAddFirstMonitor() {
		...
		for (int i=0; i<selectedNodes.size(); i++) {
			...
			if ((obj = LTFNcontains(0, n)) != null) {
				rcNode n1 = (rcNode)obj[3];
				if (n1 != null && n1.client.osControl != null) return n1.client.opticalSwitchControl;
				continue;
			}
			if (n.client.opticalSwitchControl != null) {
				return n.client.opticalSwitchControl;
			}
		}
		return null;
	}
	
	public OpticalSwitchMonitorControl getDelFirstMonitor() {
		
		if (participatingRCNodes == null || participatingRCNodes.size() == 0) return null;
		for (int i=0; i<participatingRCNodes.size(); i++) {
			...
			if (n.client.osControl != null) return n.client.opticalSwitchControl;
		}
		return null;
 	}
	...
	protected Vector pathBetweenEndPoints(rcNode n1, rcNode n2, Vector path) {
		...
		if ((objs = LTFNcontains(0, n1)) != null) {
			...
			if (path.contains(dest)) return null;
			path.add(dest);
			...
			if (n1.client == null || n1.client.osControl == null) return null;
		}
		...
		for (Enumeration en = n1.wconn.keys(); en.hasMoreElements(); ) {
			...
			if (obj instanceof OSLink) {
				OSLink link = (OSLink)obj;
				if (link.szSourceOSPort != null && (link.szSourceOSPort.type() == PortType.INPUT_PORT || link.linkID != null)) continue;
				rcNode dest = link.rcDest;
				if (dest == null || dest.equals(n1)) dest = link.rcSource;
	...
	public void mouseMoved(MouseEvent e) {
		...
		if (selectionMode) {
			...
			for (Enumeration en = nodes.elements(); en.hasMoreElements();) {
				...
				if ( LTFNcontains(0,n)!=null) {
					if (n.limits.contains(x, y)) {
						...
						else setCursor(selectableCursor);
						return;
					}
				}
			...
			if ( nodeOver!=null && (nodeOver.getOpticalSwitchInfo() == null || !nodeOver.getOpticalSwitchInfo().isAlive)) {
				setCursor(unselectableCursor);
				return;
...
//			repaint();
	...
	public String getToolTipText(MouseEvent event) {
		...
		if (linkSelectionMode) return linkToolTip;
		...
		if ( nodeOver!=null) {
			...
			if (nodeOver.getOSwConfig() != null && !nodeOver.getOSwConfig().isAlive)
				return linkToolTip;
			GenericEntity ge = nodeOver.getOpticalSwitch();
			if (ge != null) {
				if (!(ge instanceof OpticalSwitch))
					return null;
			}
...
//			range.setBounds(0, 0, getWidth(), getHeight());
			...
			for(int i=0; i<nSteps && !currentTransformCancelled; i++){
				...
				repaint();
				try{ Thread.sleep(STEP_DELAY); }catch(InterruptedException ex) { }
			}
...
//		range.setBounds(0, 0, getWidth(), getHeight());
		...
		for(Iterator it=vGraph.gnodes.iterator(); it.hasNext();){
			...
			if(gn.rcnode.x < range.x) gn.rcnode.x = range.x;
			if(gn.rcnode.y < range.y) gn.rcnode.y = range.y;
			if(gn.rcnode.x > range.getMaxX()) gn.rcnode.x = (int) range.getMaxX();
			if(gn.rcnode.y > range.getMaxY()) gn.rcnode.y = (int) range.getMaxY();
	...
	void setLayoutType(String type){
		...
		if(type.equals("Random") || type.equals("Grid") || type.equals("Map") || type.equals("None")){
			synchronized(syncGraphObj){
				unfixNodes();
				vGraph = GraphTopology.constructGraphFromInetAndMaxFlowAndOS(nodes, maxFlow, 
						bUseInet, bUseOS, this);
	...
	public String[] getLink(String linkID) {
		...
		for (int i=0; i<oslinks.size(); i++) {
			...
			if (l.linkID == null || !l.linkID.equals(linkID)) continue;
			v.add(l);
		}
		...
		for (int i=0; i<v.size(); i++) {
			...
			if (link.szSourceOSPort != null) {
				if (link.szSourceOSPort.type() == PortType.INPUT_PORT) {
					String key = link.szDestName+":"+link.szDestPort;
					p.put(key, new Object[] { link.szSourceName, link.szSourcePort.name, link.rcDest, link.rcSource, link.szDestName, link.szDestPort });
				} else {
					String key = link.szSourceName+":"+link.szSourcePort.name;
					p.put(key, new Object[] { link.szDestName, link.szDestPort, link.rcSource, link.rcDest, link.szSourceName, link.szSourcePort.name }); 
				}
			} 
		}
		...
		while (p.size() != 0) {
			...
			while (obj != null) {
				...
				if (LTFNcontains(0, next) != null) {
					...
					if (next.getOpticalSwitch() != null) {
						GenericEntity ge = next.getOpticalSwitch();
						if (ge instanceof OpticalSwitch) {
							lia.net.topology.Link l[] = ((OpticalSwitch)ge).getCrossConnects();
							if (l != null) {
								for (int i=0; i<l.length; i++)
									if (l[i].sourcePort().name().equals(destPort)) {
										nextPort = l[i].destinationPort().name();
										break;
									}
							} else {
								break;
			...
			while (true) {
				...
				if (LTFNcontains(0, sourceNode) != null) {
					...
					if (sourceNode.getOpticalSwitch() != null) {
						GenericEntity ge = sourceNode.getOpticalSwitch();
						if (ge instanceof OpticalSwitch) {
							lia.net.topology.Link l[] = ((OpticalSwitch)ge).getCrossConnects();
							if (l != null) {
								for (int i=0; i<l.length; i++)
									if (l[i].destinationPort().name().equals(sourcePort)) {
										previousPort = l[i].sourcePort().name();
										break;
									}
							} else {
								break;
				...
				for (Enumeration en = p.keys(); en.hasMoreElements(); ) {
					...
					if (obj[0].equals(sourceName) && ((previousPort == null && obj[1] == null) || obj[1].equals(previousPort))) {
						found = true;
						p.remove(key);
			...
			for (int i=0; i<path.size(); i++) {
				String[] str = (String[])path.get(i);
				buf.append("<tr><td>");
				if (str[0] != null) buf.append(str[0]);
				buf.append("</td><td>");
				if (str[1] != null) buf.append(str[1]);
				buf.append("</td><td>");
				if (str[2] != null) buf.append(str[2]);
				buf.append("</td></tr>");
			}
	...
	private final String getRFID(Port p) {
		if (p instanceof AFOXOSPort) {
			AFOXOSPort a = (AFOXOSPort)p;
			if (a.getSerialNumber() != null) return a.getSerialNumber();
		}
		return "";
	}
	...
	public String formLinkToolTip(String linkID) {
		...
		for (int i=0; i<v.size(); i++) {
			...
			if (link.szSourceOSPort != null) {
				if (link.szSourceOSPort.type() == PortType.INPUT_PORT) {
					String key = link.szDestName+":"+link.szDestPort;
					p.put(key, new Object[] { link.szSourceName, link.szSourceOSPort.name(), link.rcDest, link.rcSource, link.szDestName, link.szDestPort, getRFID(link.szSourceOSPort), getRFID(link.szDestinationOSPort) });
					if (link.szDestinationOSPort instanceof AFOXOSPort) {
						hasRFID |= (((AFOXOSPort)link.szDestinationOSPort).getSerialNumber() != null);
					}
				} else {
					String key = link.szSourceName+":"+link.szSourceOSPort.name();
					p.put(key, new Object[] { link.szDestName, link.szDestPort, link.rcSource, link.rcDest, link.szSourceName, link.szSourceOSPort.name(), getRFID(link.szDestinationOSPort), getRFID(link.szSourceOSPort) });
					if (link.szSourceOSPort instanceof AFOXOSPort) {
						hasRFID |= (((AFOXOSPort)link.szSourceOSPort).getSerialNumber() != null);
					}
				}
			} 
		}
		...
		while (p.size() != 0) {
			...
			while (obj != null) {
				String destName = (String)obj[0];
				String destPort = (String)obj[1];
				path.add(new String[] { (String)obj[4], (String)obj[5], "Out", (String)obj[7] });
				path.add(new String[] { destName, destPort, "In", (String)obj[6]});
				rcNode next = (rcNode)obj[3];
				String nextPort = null;
				if (LTFNcontains(0, next) != null) {
					nextPort = null;
						...
						if (ge instanceof OpticalSwitch) {
							...
							if (l != null) {
								for (int i=0; i<l.length; i++)
									if (l[i].sourcePort().name().equals(destPort)) {
										nextPort = l[i].destinationPort().name();
										if (l[i].sourcePort() instanceof AFOXOSPort) {
											hasRFID |= (((AFOXOSPort)l[i].sourcePort()).getSerialNumber() != null);
										}
										break;
									}
							} else {
			...
			while (true) {
						...
						if (ge instanceof OpticalSwitch) {
							...
							if (l != null) {
								for (int i=0; i<l.length; i++)
									if (l[i].destinationPort().name().equals(sourcePort)) {
										previousPort = l[i].sourcePort().name();
										if (l[i].destinationPort() instanceof AFOXOSPort) {
											hasRFID |= (((AFOXOSPort)l[i].destinationPort()).getSerialNumber() != null);
										}
										break;
									}
							} else {
				...
				for (Enumeration en = p.keys(); en.hasMoreElements(); ) {
					...
					if (obj[0].equals(sourceName) && ((previousPort == null && obj[1] == null) || obj[1].equals(previousPort))) {
						...
						sourcePort = (String)obj[5];
						sourceNode = (rcNode)obj[2];
						path.add(0, new String[] { (String)obj[0], (String)obj[1], "In", (String)obj[6]});
						path.add(0, new String[] { sourceName, sourcePort, "Out",  (String)obj[7]});
						break;
					}
				}
				if (!found) break;
			...
			if (buf == null) {
				...
				buf.append("<html><body bgcolor=#ffffff><table bgcolor=#ffffff border=0 cellspacing=2 cellpadding=1>");
				if (!hasRFID)
					buf.append("<tr><td bgcolor=#c5d6e7>Device</td><td bgcolor=#c5d6e7>Port</td><td bgcolor=#c5d6e7>Port Type</td></tr>");
				else
					buf.append("<tr><td bgcolor=#c5d6e7>Device</td><td bgcolor=#c5d6e7>Port</td><td bgcolor=#c5d6e7>Port Type</td><td bgcolor=#c5d6e7>RFID</td></tr>");
			}
			
			for (int i=0; i<path.size(); i++) {
				String[] str = (String[])path.get(i);
				buf.append("<tr><td bgcolor=#c5d6e7>");
				if (str[0] != null) buf.append(str[0]);
				buf.append("</td><td bgcolor=#c5d6e7>");
				if (str[1] != null) buf.append(str[1]);
				buf.append("</td><td bgcolor=#c5d6e7>");
				if (str[2] != null) buf.append(str[2]);
				if (hasRFID) {
					buf.append("</td><td bgcolor=#c5d6e7>");
					if (str[3] != null) buf.append(str[3]);
				}
				buf.append("</td></tr>");
			}
			path.clear();
	...
	public Icon getSelectableIcon() {
		...
		if (selectableIcon != null) return selectableIcon;
	...
	public Icon getUnselectableIcon() {
		...
		if (unselectableIcon != null) return unselectableIcon;
	...
	private Cursor createSelectableCursor() {
		...
		return Toolkit.getDefaultToolkit().createCustomCursor(image, new Point(image.getWidth(null)/2, image.getHeight(null)/2), "Selectable");
	}
