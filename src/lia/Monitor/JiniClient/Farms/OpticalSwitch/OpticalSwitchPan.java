...
import lia.Monitor.JiniClient.Farms.OpticalSwitch.Config.RemoveLinksDialog;
import lia.Monitor.monitor.OSLinkLegend;
import net.jini.core.lookup.ServiceID;

public class OpticalSwitchPan extends JPanel implements graphical, ActionListener, ChangeListener, ComponentListener {

	/** Logger name */
	private static final transient String COMPONENT = "lia.Monitor.JiniClient.Farms.Gmap";
	/** Logger used by this class */
	private static final transient Logger logger = Logger.getLogger(COMPONENT);

	JPanel cmdPan;
	public OpticalSwitchGraphPan grapPan;
	...
	public OpticalSwitchMonitorControl currentControl = null;
	...
	private static OpticalSwitchPan _instance = null;
	
	private OpticalSwitchPan() {
		ginit(); 
	}
	
	public static synchronized OpticalSwitchPan getInstance() {
		if (_instance == null)
			_instance = new OpticalSwitchPan();
		return _instance;
	}
	...
	void ginit() {
		...
		grapPan = OpticalSwitchGraphPan.getInstance(this);
				...
				new Thread(new Runnable() {
					public void run() {
						...
						if (currentControl != null) {
							OpticalSwitchMonitorControl c = grapPan.getAddFirstMonitor();
							if (c != null) currentControl = c;
							currentControl.createPath(grapPan.allPathMode, grapPan.singleEndPointsMode, grapPan.multipleEndPointsMode, grapPan.selectedNodes, grapPan.fdxPath);
						}
						grapPan.selectedNodes.clear();
					}
				}).start();
				...
				new Thread(new Runnable() {
					public void run() {
						if (currentControl != null && grapPan.linkSelected != null) {
							OpticalSwitchMonitorControl c = grapPan.getDelFirstMonitor();
							if (c != null) currentControl  = c;
