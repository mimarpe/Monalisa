...
//	protected static Color unconnectedColor = new Color(255, 0, 0, 160);
	...
	private static boolean drawOSLedsForOSI(rcNode node, Graphics2D g2, int startX, int startY, int width, int height ,OpticalSwitch osi) {
		...
		synchronized (lock) {
			...
			if (l!=null) {
				for (int i=0; i<l.length; i++) {
					Link link = l[i];
					if (link == null || link.sourcePort() == null || link.destinationPort() == null) continue;
	...
	public static void drawOSLeds(rcNode node, Graphics2D g2, int startX, int startY, int width, int height) {
		...
		if (ge instanceof OpticalSwitch) {
			...
			if (osi != null) {
				if (drawOSLedsForOSI(node, g2, startX, startY, width, height, osi)) return;
			}
		}
