...
public class VoJobs extends JPanel implements graphical,  LocalDataFarmClient, ActionListener, ComponentListener {
	...
	protected void redraw() {
		...
		if (lastSelectedVO == null || currentSelectedVO == null || !lastSelectedVO.equals(currentSelectedVO)) {
			panel3d.clear();
			goodSeries.clear();
			if (currentSelectedVO == null) return;
	...
	protected void addVO(String voName) {
		...
		if (voMenu != null) voMenu.add(item);
	...
	protected void removeVO(String voName) {
		...
		if (vos.contains(voName)) {
			...
			if (voMenu != null) voMenu.remove(item);
			if (item.isSelected()) {
				if (vos.size() == 0) {
	...
	protected void getVOs() {
		...
		try {
			for (Iterator it = crtNodes.iterator(); it.hasNext(); ) {
				...
				Vector v = null;
				if (node == null || node.client == null || node.client.farm == null || node.client.farm.getClusters() == null) continue;
	...
	protected void setResult(MLSerClient client, Result r) {
		...
		if(r.ClusterName.endsWith("VO_JOBS")){
			...
			for (int i=0; i<r.param_name.length; i++) {
				if (r.param_name[i] == null || Double.isInfinite(r.param[i]) || Double.isNaN(r.param[i])) continue;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (src.equals(mPeriod)) {
			PlotIntervalSelector is = new PlotIntervalSelector(monitor.main, timeOfFirstResult, (continuous ? -1 : timeOfLastResult),
					null, null);
			is.setVisible(true);
			if (!is.closedOK()) {
				is.dispose();
			...
			try {
				for (int i=0; i<crtNodes.size(); i++) {
					rcNode n = (rcNode)crtNodes.get(i);
					if (n.client != null) n.client.deleteLocalClient(this);
				}
				lastSelectedVO = null;
				...
				for (int k=0; k<2; k++) {
					...
					for (int i=0; i<crtNodes.size(); i++) {
						rcNode n = (rcNode)crtNodes.get(i);
						if (n.client != null) n.client.addLocalClient(this, preds[k]);
					}
				}
			} catch (Exception ex) {
		...
		for (int i=0; i<voSelection.size(); i++) {
			...
			if (src.equals(item)) {
				...
				if (panel3d != null) panel3d.setTitle("TotalJobs for "+currentSelectedVO);
				redraw();
				return;
			}
		}
