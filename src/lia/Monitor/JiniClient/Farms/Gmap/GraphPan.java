...
public class GraphPan extends JPanel implements 
										MouseListener, 
										MouseMotionListener,
...
//	Graphics2D offgraphics;
		...
		if (n.errorCount > 0){ // || (n.haux.get("lostConn") != null && n.haux.get("lostConn").equals("1") )){
			...
			if ((px == null) || (px.len == 0)) {
				...
				for (int i = 0; i < px.len; i++) {
					...
					g.fillArc(n.limits.x, n.limits.y, 
	...
	public synchronized void paintComponent(Graphics g) {
		((Graphics2D)g).setRenderingHint(
				...
				gmapPan.optPan.kbMakeNice.isSelected() ? 
		...
		for (final rcNode n : nodes.values()) {
			...
			for (Enumeration e1 = n.conn.keys(); e1.hasMoreElements();) {
				...
				if (isHotLink(n, n1))
					...
					if(! gmapPan.optPan.kbShowPing.isSelected()){
						continue;
					}
...
//	}
	...
	void setLayoutType(String type){
	    ...
	    if(type.equals("Random") || type.equals("Grid") || type.equals("Map") || type.equals("None")){
	        synchronized(syncGraphObj){
	            unfixNodes();
	            vGraph = GraphTopology.constructGraphFromInetAndMaxFlow(nodes, maxFlow, 
	                    bUseInet);//, bUseOS);
