...
public class ReflRouterJiniService extends Thread 
								implements ServiceIDListener, ServiceDiscoveryListener, DiscoveryListener{
	
	/** Logger used by this class */
    private static final transient Logger logger = Logger.getLogger("lia.Monitor.JiniClient.ReflRouter.ReflRouterJiniService");

	private static final long WAIT_FOR_SID_TIMEOUT = AppConfig.getl("lia.Monitor.JiniClient.ReflRouter.WAIT_FOR_SID_TIMEOUT", 300) * 1000;
    ...
    private AtomicBoolean iAmMaster;
    private AtomicBoolean iAmAvailable;
    private boolean active = false;
    private Hashtable htKnownRR;	// key: ReflRouterJiniProxy, value: GenericMLEntry for that service
...
//    	    		_theInstance = createInstance();
   	    		logger.log(Level.INFO, "Created ReflRouterJiniService instance; secureLUS=" 
    ...
    private String [] getGroups(){
    	...
    	if(groups.size() == 0){
    		logger.log(Level.WARNING, "You should have at least a group defined in app.properties for key lia.Monitor.group");
    	}
    ...
    private void analyzeMasterMode(){
    	...
    	for(Iterator rrit = htKnownRR.entrySet().iterator(); rrit.hasNext(); ){
    		...
    		if((! crtExpired) &&
    				  ((bestRR == null) 
    				|| (crtSeesRR > bestSeesRR) 
    				|| ((crtSeesRR == bestSeesRR) && (rr.ip.compareTo(bestRR.ip) < 0)))){
    			bestRR = rr;
    	...
    	if(bestRR == null){
    		logger.log(Level.WARNING, "Could not select a 'best' ReflRouter. "
    				+ "I am "+myJiniProxy+" and I see "
    				+ ((Integer)gmle.hash.get("reflectors")).intValue()
    				+ " reflectors...");
    		iAmMaster.set(false);
    		return;
    	}
