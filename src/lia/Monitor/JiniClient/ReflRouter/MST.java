...
public class MST {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.Monitor.JiniClient.ReflRouter.MST");
...
//		double oldCost = tunnelsCost(oldTree);
	...
	public void computeST(){
		...
		trees.clear(); crtTree.clear(); nextTree.clear(); 
...
//			printTunnels("MST: Old tree's tunnels:", oldTree);
		...
		while(madeConnection){
			...
			bestTunnel[0] = null; bestTunnel[1] = null;
			startSubtree = null; bestSubtree = null;
			// for each subtree
			for(int i=0; i<trees.size(); i++){
				...
				for(int j=i+1; j<trees.size(); j++){
					...
					crtTunnel[0] = null; crtTunnel[1] = null;
					if(! getBestTunnelPair(subtree, subtree1, crtTunnel))
						continue;
					//System.out.println("crtTun "+crtTunnel[0]+" "+crtTunnel[1]);
					double crtCost  = crtTunnel[0].getInetQuality()
									+ crtTunnel[1].getInetQuality();
	...
	private boolean getBestTunnelPair(Vector st1, Vector st2, IPTunnel [] result){
		...
		for(int i=0; i<st1.size(); i++){
			...
			for(int j=0; j<st2.size(); j++){
				...
				if(t12.getNextStatus() == IPTunnel.MUST_DEACTIVATE || t21.getNextStatus() == IPTunnel.MUST_DEACTIVATE)
					continue;
