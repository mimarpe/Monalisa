...
public class PandaCmdSender {
	private static final transient Logger logger = Logger.getLogger("lia.Monitor.JiniClient.ReflRouter.PandaCmdSender");
		...
		while(tries < 2){
			...
			if(panda == null){
				try{
					...
					panda = (RemoteMonitorInterface) LocateRegistry.getRegistry(host, 2091, new TimeoutClientSocketFactory()).lookup("pandaPresence");
