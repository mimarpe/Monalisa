...
public class PandaProxyCmdSender {
	private static final transient Logger logger = Logger.getLogger("lia.Monitor.JiniClient.ReflRouter.PandaProxyCmdSender");
	...
	public boolean sendCommand(String host, String command, String peer){
		...
		int kangarooPort = Integer.valueOf(AppConfig.getProperty("PandaProxyCmdSender.KANGAROO_PORT", "3236")).intValue();
		int tries = 0;
		while(tries < 2){
			tries++;
			if(proxyRef == null){
				try{
					logger.log(Level.INFO, "Doing RMI get registry for kangaroo at "+kangarooHost+":"+kangarooPort+" ...");
					Registry registry = LocateRegistry.getRegistry(kangarooHost, kangarooPort, new DBSSLClientSocketFactory());
			...
			logger.log(Level.INFO, "Sending command: ["+host+"]->"+command+"("+peer+") via "+kangarooHost+":"+kangarooPort);
			if(host == null){
