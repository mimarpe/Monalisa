...
public class Main extends JiniClient implements LocalDataFarmClient {
	/** Logger name */
	private static final String COMPONENT = "lia.Monitor.JiniClient.ReflRouter.Main";
	...
	public void inializeMonitorUnit(ServiceItem si) {
		try {
			...
			if (ds == null) {
				logger.log(Level.WARNING, "Service could not be deserialized",
						new Object[] { si });
				return;
			}
			...
			if (sie != null) {
				...
				registryPort = (sie.REGISTRY_PORT == null) ? Registry.REGISTRY_PORT
						: sie.REGISTRY_PORT.intValue();
			} else {
				logger.log(Level.WARNING, "SiteInfoEntry == null for " + si
						+ " - discarding it");
				return;
			}
			if (un == null || ipad == null) {
				logger.log(Level.WARNING, "UnitName or IPaddress null for "
						+ si + " - discarding it");
				return;
			}
			if (gmle != null && gmle.hash != null) {
				...
				if (!ipad.equals(ip)) {
					logger.log(Level.WARNING,
							"IP address differs in gmle from service for " + un
									+ " serIP=" + ipad + " gmleIP=" + ip);
				}
			} else {
...
//              System.out.println("un="+un+" host="+host+" ip="+ipad);
			MLSerClient tcl = new tClient(un, ipad, host, mlPort, registryPort,
					MLCP, mle, sie, esie, proxytmClient, si.serviceID);
	...
	synchronized public void addNode(ServiceItem si, DataStore dataStore,
									MLSerClient client, String unitName, String ipad) {
