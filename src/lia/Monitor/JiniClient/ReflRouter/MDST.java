...
public class MDST extends MST {
	...
	private Vector activeNodes;		// nodes that are used to compute the tree
			...
			for(Enumeration e1 = nodes.elements(); e1.hasMoreElements(); ) {
				...
				if(t12.hasPeerQuality() || t21.hasPeerQuality()){
					...
					if(!(t12.hasInetQuality() && t21.hasInetQuality())){
                        ...
                        if ( logger.isLoggable(Level.FINER) ) {
                            logger.log(Level.FINER, "MST: tunnel "+t12+" must be ACTIVE! - has peerq, no Inet params");
                        }
						continue;
					}
				}
				...
				if(! t12.hasPeerQuality() && ! t21.hasPeerQuality()){
					...
					if(!(t12.hasInetQuality() && t21.hasInetQuality())){
                        ...
                        if ( logger.isLoggable(Level.FINER) ) {
                            logger.log(Level.FINER, "MST: tunnel "+t12+" must NOT be ACTIVE - no peerq, incomplete inet params !");
                        }
						continue;
					}
					...
					if(t12.hasHighPktLoss() || t21.hasHighPktLoss()){
                        ...
                        if ( logger.isLoggable(Level.FINER) ) {
                            logger.log(Level.FINER, "MST: tunnel "+t12+" must NOT be ACTIVE - no peerq, high packet loss !");
                        }
					}
				}
	 ...
	 */
	class Dijkstra {
		private Vector nodes;			// the list of nodes
		private Vector tunnels;
		private int [] parent;			// for each node, it's parent
		private double [] distance;		// the distance between source and each node
		...
		void compute(int s){
			...
			while(madeConnection){
				...
				for(i=0; i<n; i++){
					...
					for(j=0; j<n; j++){
						...
						if(cost < bestPairCost){
							bestPair = pair; bestPairCost = cost;
							bestNS = i; bestNN = j;
						}
					}
				}
				...
				if(bestPair != null){
					tunnels.add(bestPair[0]); tunnels.add(bestPair[1]);
	...
	Vector [] getCompCNX(Vector nodes){
		...
		for(i=0; i<k; i++){
			for(j=0; j<((Vector)rez[i]).size(); j++)
            if ( logger.isLoggable(Level.FINER) ) {
                logger.log(Level.FINER," "+((ReflNode)((Vector)rez[i]).get(j)).UnitName);
            }
		}
		return rez;
	}
	...
	public void computeST(){
		activeNodes.clear(); crtTree.clear(); nextTree.clear(); 
		toActivate.clear(); toDeactivate.clear();
		//if(simulation)
