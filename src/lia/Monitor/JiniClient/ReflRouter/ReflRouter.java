...
public class ReflRouter extends TimerTask {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.Monitor.JiniClient.ReflRouter.ReflRouter");
	...
	public final static long firstReRouteDelay = AppConfig.getl("lia.Monitor.JiniClient.ReflRouter.FirstReRouteDelay", 90) * 1000; // seconds
	...
	private Vector dlinks;			// vector with DLinks built from currently selected tunnels
	private Vector deadTunnels;		// list of tunnels that have to be removed at the next commandSend
				...
				if(! mst.simulation){
					if(available){
						if(ReflRouterJiniService.getInstance().isMasterMode()){
							waitDisconnectToFinish = AppConfig.getb("lia.Monitor.JiniClient.ReflRouter.waitDisconnectToFinish", false);
							sendDisconnectCmds();
							if((! waitDisconnectToFinish) ||
									(waitDisconnectToFinish && mst.toDeactivate.size() == 0)){
								sendConnectCmds();
							}else{
