...
public class VRVSGraphAnalyzer extends TimerTask {
	/** Logger name */
	private static final String COMPONENT = "lia.Monitor.JiniClient.ReflRouter";
	/** Logger used by this class */
	private static final Logger logger = Logger.getLogger(COMPONENT);

	private Hashtable snodes; // key=SID; value=ReflNode
	...
	private static final String [] alertLevel = new String [] { ALERT_NONE, ALERT_STATUS, 
		ALERT_INFO, ALERT_WARNING, ALERT_PANIC};
...
//	}
	...
	private Hashtable orphanPeers; // key="ReflNode.UnitName-r.NodeName"; value=Result
	private Vector orphans; // contains Strings with the keys form orphanPeers
	private Vector cycles; // contains vectors (cyles) with ReflNodes
	private Vector islands; // contains vectors (islands) with ReflNodes; connex components
	private Vector asymTunnels; // contains IPTunnels that have no corresponding IPTunnel
	private Vector poorQualTunnels; // contains IPTunnels
	private Hashtable onOffHistory; // key=UnitName; value=Vector containing times in last hour when connection with ML on that reflector was lost 
	private Hashtable unstableNodes; // key=UnitName; value=Long = number of state changes last hour
	private Hashtable stateHistory; // key = UnitName; value=Hash(key=State; value=last time)
	private Hashtable lastState; // key = UnitName; value=Hash(key=State; value=time) ; cleared each iteration
	private Hashtable alertCause; // key = UnitName; value=Hash(key=State; value=last time)
	
	private Object syncData; // used to synchronize getting and computing the above vectors and hashes
	...
	private void initDefaults(){
		alert2Email.put(ALERT_PANIC, splitCommaString(AppConfig.getProperty("PANIC_EMAIL", "catalin.cirstoiu@cern.ch")));
		alert2Email.put(ALERT_WARNING, splitCommaString(AppConfig.getProperty("WARNING_EMAIL", "catalin.cirstoiu@cern.ch")));
		alert2Email.put(ALERT_INFO, splitCommaString(AppConfig.getProperty("INFO_EMAIL", "catalin.cirstoiu@cern.ch")));
		alert2Email.put(ALERT_STATUS, splitCommaString(AppConfig.getProperty("STATUS_EMAIL", "catalin.cirstoiu@cern.ch")));
	...
	private boolean cyclesEqual(Vector ca, Vector cb) {
		...
		for(; i < ca.size(); i++)
			if(! ca.get(i).equals(cb.get((j+i) % cb.size())))
				break;
		if(i == ca.size()) // every node matched; cycles are equal
			return true;
