...
public class IPTunnel {
	private static final transient Logger logger = Logger.getLogger("lia.Monitor.JiniClient.ReflRouter.IPTunnel");
	...
	private HistoryParam peerQual;		// link quality, as reported by reflector
	private HistoryParam inetRTTime;	// link quality, as reported by ABPing
	private HistoryParam inetPktLoss;	// packet loss observed on this tunnel
	...
	public String toString(){
		...
		sb.append(" ");	ReflNode.appendDelay(sb, peerQual.getUpdateDelay());
		sb.append(" iRT=").append(hasInetQuality() ? ReflNode.numberFormat.format(getInetQuality()) : "?");
		sb.append("/").append(inetRTTime.size());
		sb.append(" ");	ReflNode.appendDelay(sb, inetRTTime.getUpdateDelay());
		sb.append(" loss=").append(hasPktLossInfo() ? ReflNode.numberFormat.format(inetPktLoss.getAvgValue(pktLossSamples) * 100) : "?").append("%");
		...
		sb.append(" "); ReflNode.appendDelay(sb, now - lastCommandTime);
	...
	private String stateStr(int state){
		switch(state){
			...
			case MUST_DEACTIVATE: return "MUST_DEactivate";
			case MUST_ACTIVATE: return "MUST_activate";
