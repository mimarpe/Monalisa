...
public class GraphPan extends JPanel implements MouseListener, MouseMotionListener, LayoutChangedListener, LocalDataFarmClient {
	
    /** Logger Name */
    private static final transient String COMPONENT = "lia.Monitor.JiniClient.VRVS3D.GPan";
    /** The Logger */ 
    ...
    private SearchUserLayer sl = new SearchUserLayer();
    ...
    private BasicStroke doubleStroke=new BasicStroke(2);
...
//    	userSearchMenuList.requestFocus();
    ...
    {
        gmapPan.optPan.meetingsUpdate.updateMeetingsMenu( GlobalNodes, gmapPan.optPan.communityMenuList, gmapPan.optPan.meetingsMenuList);
    }
...
//                            meetingID = userFullName.substring( nSecondSep+1);
                        ...
                        String userName = (bCutAfterPoundSign?clUList.get(i).toString():((MNode)clUList.get(i)).getName());
                        ...
                        if ( userID.length()>0 && (userInfo=LTFNcontains(1, userID))==null ) {
                            ...
                            monPredicate pred = new monPredicate(node.UnitName, "Users", userFullName, -1, -1, new String[] {"ProxyStatus", "UserStatus"}, null);
                            node.client.addLocalClient(this, pred);
                            monPredicate pred2 = new monPredicate(node.UnitName, "Users", userFullName, -30 * 60 * 1000 , -1, new String[] {"UserInfo"}, null);
...
//                            rcNode n = new rcNode();
                            ...
                            String userinfo5 = "<html><table border=0 cellspacing=0 cellpadding=4><tr><td><table border=0 cellspacing=0 cellpadding=1>"
                            	+"userinfo_start"
                            	+"<tr><td>Name:</td><td>"+niceUserName+"</td></tr>"
                            	+"userinfo_end"
                            	+"userstatus_startuserstatus_end"
                            	+"usermeeting_startusermeeting_end"
...
//              sidToCheck.add( node.sid);
				...
				if (!nodes.containsKey(node.sid)) contains = false;
...
//                    System.out.println("removing node "+fakeNode.UnitName);
    ...
    public void syncNodes()
    {
        ...
        for (rcNode n : GlobalNodes.values() ) {
			boolean contains = false;
			if (!nodes.containsKey(n.sid)) contains = false;
			else {
        ...
        try {
            ...
            for (Iterator it = nodes.keySet().iterator(); it.hasNext(); ) {
                ...
                if ( key instanceof ServiceID ) { //identify a real node
					boolean contains = false;
					if (!GlobalNodes.containsKey(node.sid)) contains = false;
					else {
...
//  }
    ...
    public void paintFakeNode( Graphics g, rcNode n, ArrayList<MeetingDetails.IDColor> lMeetingsSelected, boolean fullInfo)
    {
		...
		if ( bMeetings && fakeAttrs!=null ) {
		    ...
		    for ( final MeetingDetails.IDColor mElem : lMeetingsSelected) {
		        ...
		        if ( mElem.ID.equals(fakeAttrs[3]) && (userSearchHighlight == null || userSearchHighlight.length() == 0)) {//equal meeting ids
		        	selectedElem = mElem;
		        	break;
		        }
		    }
...
//            hunit = h2;
    ...
    public Color getLinkQualityColor(ILink lnk) {
        ...
        if(gmapPan.optPan.kbShowPing.isSelected() && lnk.inetQuality != null) crtCs = gmapPan.optPan.csPing;
        if(gmapPan.optPan.kbShowPeers.isSelected() && lnk.peersQuality != null) crtCs = gmapPan.optPan.csPeers;
        ...
        if (crtCs.maxValue > crtCs.minValue ) {
            R = (int) (val * (crtCs.maxColor.getRed() - crtCs.minColor.getRed()) / delta)
            + crtCs.minColor.getRed();
            ...
            R = (int) (val * (crtCs.minColor.getRed() - crtCs.maxColor.getRed()) / delta)
            + crtCs.maxColor.getRed();
    ...
    public void drawConn(Graphics gg, rcNode n , rcNode n1, ILink link) {
        ...
        int dx = ( n1.x - n.x ); int dy = n1.y - n.y;
        ...
        if(type.equals("Random") || type.equals("Grid")){
            synchronized(syncGraphObj){
                ...
                vGraph = GraphTopology.constructGraphFromPeersPingAndMSTAndUsers(nodesOK, mstData, 
                        gmapPan.optPan.kbShowPeers.isSelected(), 
                        gmapPan.optPan.kbShowPing.isSelected(), 
                        gmapPan.optPan.kbShowMST.isSelected(), 
                        this);
                    ...
                    if ( nFirstResSep!=-1 ) {
                        ...
                        if ( userInfo!=null && userInfo.length>3 && !((String)userInfo[3]).equals(meetingID) ) {
                            ...
                            if ( meetingName!=null ) {
                    			...
                    			userInfo[5] = ((String)userInfo[5]).replaceFirst("usermeeting_start.*usermeeting_end", "usermeeting_start"+userstatus_text+"usermeeting_end");
                            } else {
                    			userInfo[5] = ((String)userInfo[5]).replaceFirst("usermeeting_start.*usermeeting_end", "usermeeting_start"+"usermeeting_end");
                            }
                        }
                    };
            	}
            	if ( "UserInfo".equals(r.param_name[i]) && userInfo.length>5 ) {
            		...
            		if ( values.length>3 ) {
            			userinfo_text = //"<html><table border=0 cellspacing=1 cellpadding=0>"
            					"<tr><td>UserID:</td><td>"+values[0]+"</td></tr>"
            					+"<tr><td>Name:</td><td>"+userName+"</td></tr>"
            					+"<tr><td>Email:</td><td>"+values[2]+"</td></tr>"
            					+"<tr><td>Location:</td><td>"+values[3]+"</td></tr>";
            			userInfo[5] = ((String)userInfo[5]).replaceFirst("userinfo_start.*userinfo_end", "userinfo_start"+userinfo_text+"userinfo_end");
            		}
            	} 
            	if ( "UserStatus".equals(r.param_name[i]) && userInfo.length>5 ) {
            		...
            		if ( values.length>3 ) {
            			userstatus_text = //"<html><table border=0 cellspacing=1 cellpadding=0>"
            					...
            					+"<tr><td>Status:</td><td>"+values[0]+"</td></tr>"
            					+"<tr><td>Idle time:</td><td>"+values[3]+"</td></tr>";
            			userInfo[5] = ((String)userInfo[5]).replaceFirst("userstatus_start.*userstatus_end", "userstatus_start"+userstatus_text+"userstatus_end");
