...
public class VrvsSerMonitor extends SerMonitorBase {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.Monitor.JiniClient.CommonGUI");

	public final static boolean showMST = Boolean.valueOf(AppConfig.getProperty("lia.Monitor.showMST", "false")).booleanValue();
    ...
    {
		//      try to get location 
		if (node.mlentry != null && 
		        	!node.mlentry.LAT.trim().equals("") && !node.mlentry.LAT.trim().equals("N/A")
		    ...
		    node.LAT = "-21.22";
            node.LONG = "-111.15";
            logger.log(Level.INFO, "Setting default location for "+node.UnitName+" LAT="+node.LAT + " LONG=" + node.LONG);
		}
    }

	synchronized public void addNode(ServiceItem si, DataStore dataStore, tClient client,
										String unitName, String ipad) {
		rcNode n = new rcNode();
         ...
         */
		n.x = (int) ( init_max_width*GraphPan.fLayoutMargin + init_max_width*(1-2*GraphPan.fLayoutMargin)*Math.random() );
		n.y = (int) ( init_max_height*GraphPan.fLayoutMargin + init_max_height*(1-2*GraphPan.fLayoutMargin)*Math.random() );
		n.osgX = (int) ( init_max_width*GraphPan.fLayoutMargin + init_max_width*(1-2*GraphPan.fLayoutMargin)*Math.random() );
		n.osgY = (int) ( init_max_height*GraphPan.fLayoutMargin + init_max_height*(1-2*GraphPan.fLayoutMargin)*Math.random() );
        n.UnitName = unitName;
        n.setShortName();
	...
	private ReflNode getReflNodeByIP(String ipx) {
	    synchronized(reflNodes) {
	        for (Iterator nit = reflNodes.values().iterator(); nit.hasNext(); ){
	...
	public void processResult(MLSerClient cli, eResult er) {
		...
		if(er.ClusterName.equals("Reflector")){
			rcNode ns = (rcNode) snodes.get(cli.tClientID);
			...
			ReflNode srcNode = (ReflNode) reflNodes.get(cli.tClientID);
		...
		if(r.ClusterName.equals("Reflector")){
			...
			if(! srcNode.checkReflActive()){
				...
				for (int i = 0; i < r.param_name.length; i++){
					...
					else if (paramName.equals("Quality")) // not used yet
						DoubleContainer.setHashValue(ns.haux, "Quality", paramValue);
					else if (paramName.equals("LostPackages")) // not used yet
	...
	public void removeFarmNode(rcNode n){
		...
		ReflNode rn = (ReflNode) reflNodes.remove(n.sid);
		if(rn != null){
	...
	private void checkNodes(){
		...
		for(Enumeration ren = reflNodes.elements(); ren.hasMoreElements(); ){
			...
			if(! rn.checkReflActive()){
				rcNode ns = (rcNode) snodes.get(rn.sid);
	...
	private void checkIPTunnels(){
		...
		for(Enumeration rne = reflNodes.elements(); rne.hasMoreElements(); ) {
			...
			rcNode ns = (rcNode) snodes.get(from.sid);
			if(ns == null){
			...
			for(Iterator tit = from.tunnels.values().iterator(); tit.hasNext(); ) {
				...
				ReflNode to = tun.to;
				rcNode nw = (rcNode) snodes.get(to.sid);
				if(nw == null){
