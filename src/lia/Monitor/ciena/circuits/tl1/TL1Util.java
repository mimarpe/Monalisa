/*
 * $Id: TL1Util.java 7217 2011-12-10 16:04:37Z ramiro $
 * 
...
public class TL1Util {
    ...
    private static final transient Logger logger = Logger.getLogger(TL1Util.class.getName());
    ...
    public static final TL1Response[] execAndGet(final String tl1CMD, String cTag) throws Exception {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if(line.startsWith(TL1_END_CODE)) break;
                
                if(!line.startsWith(TL1Response.START_END_DELIMITER)) {
                    if (logger.isLoggable(Level.FINER)) {
                        logMsg.append("\n Ignoring line: [").append(line).append("] because it did not start with START_END_DELIMITER");
                    }
                    continue;
                }
                ...
                if(logger.isLoggable(Level.FINER)) {
                    logMsg.append("\n").append("OsrpTL1Response for line: ").append(line).append(" -> ").append(tl1Response);
                }
                osrpTL1Response.add(tl1Response);
            }
            
            if(logger.isLoggable(Level.FINE)) {
                logMsg.append("\n[ TL1Util ] [ execAndGet ] doCmd [ ").append(tl1CMD).append(" ] returning: ").append(osrpTL1Response.size()).append(" values");
            }
            
            return osrpTL1Response.toArray(new TL1Response[osrpTL1Response.size()]);
        } finally {
            if(logger.isLoggable(Level.FINE)) {
                logMsg.append("\n[ TL1Util ] [ execAndGet ] doCmd [ ").append(tl1CMD).append(" ]  DT wait CD/CI: ").append(TimeUnit.NANOSECONDS.toMillis(dtTl1)).append(" ms  Total DT: ").append(TimeUnit.NANOSECONDS.toMillis(Utils.nanoNow() - sTime)).append(" ms\n");
    ...
    public static final String getStringVal(final String key, final TL1Response osrpTL1Response) {
        if(osrpTL1Response == null || key == null) return null;
    ...
    public static final int getIntVal(final String key, final TL1Response osrpTL1Response) {
        ...
        if(key == null || osrpTL1Response == null || osrpTL1Response.paramsMap == null) return -1;
        ...
        try {
            ...
            if(logger.isLoggable(Level.FINE)) {
                logger.log(Level.FINE, "[ Tl1Util ] [ getIntVal ] Unable to determine the integer value for Key: " + key + " from OsrpTL1Response: " + osrpTL1Response, t);
            }
            retV = -1;  
        }
    ...
    public static final long getLongVal(final String key, final TL1Response osrpTL1Response) {
        ...
        try {
            ...
            if(logger.isLoggable(Level.FINE)) {
                logger.log(Level.FINE, "[ Tl1Util ] [ getLongVal ] Unable to determine the long value for Key: " + key + " from OsrpTL1Response: " + osrpTL1Response, t);
            }
            retV = -1;  
        }
