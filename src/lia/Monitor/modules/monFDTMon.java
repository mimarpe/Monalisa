...
public class monFDTMon extends SchJob implements LisaXDRModule, MonitoringModule {
	...
	public MonModuleInfo init(MNode node, String args) {
		...
		try{
			...
			logger.warning("Failed to parse the listen port: "+pModuleConfiguration.getProperty("port")+". Assuming default (11001)");
		}
		// start (if is not started already) an FDT monitoring server and register ourselves as listener for FDT commands
		logger.log(Level.FINE, "Initializing "+ModuleName+" to listen for fdt xdr connections on port "+lisaFdtPort);
	...
	public XDRMessage execCommand(String module, String command, List args) {
		...
		if (logger.isLoggable(Level.FINEST)) {
			logger.log(Level.FINEST, "[FDTMon] New XDR command received. Module: "+module+" Cmd: "+command + " Args:" + args);
		}
		try {
					...
					try {
						...
						for (int j = 1; j < args.size() - 1; j += 2) {
							try {
								...
								logger.log(Level.WARNING, "[FDTMon] [skipped] Invalid mon value. Cause: " + e.getMessage());
							}
						}
						...
						if(clusterName != null && nodeName != null) {
							Result rResult = new Result(Node.getFarmName(), clusterName, nodeName, ModuleName, aParamNames, aParamValues);
