...
public class monPN_PBS extends cmdExec implements MonitoringModule {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.Monitor.modules.monPN_PBS");
		...
		try {
			...
			if (!fd.exists()) {
	        	...
	        	try{
	        		...
	        		logger.log(Level.WARNING, "[monPN_PBS] The local command " + cmd + " not exist.", new Object[]{mlle});
	        	} catch(Exception ex) {}
            } else {
	...
	public Object doProcess() throws Exception {
			...
			try {
				...
				try {
		        	...
		        	try{
		        		...
		        		mlle.logParameters.put("Error Code", errorCode);
		        		mlle.logParameters.put(errorCode, VO_Utils.pnErrCodes.get(errorCode));
		        		logger.log(Level.INFO, "Second attempt to get nodes status failed, no results were sent", new Object[]{mlle, e2});
		        	} catch(Exception ex) {}
				}
			}
		...
		logger.log(Level.INFO, methodName + " - monPN_PBS [" + call_no + "] " +
        		"Sent " + results.size() + " results. " +
