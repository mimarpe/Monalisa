...
public class snmp_Disk_v2 extends snmpMon2 implements MonitoringModule {
    ...
    /** The Logger */
    private static final Logger logger = Logger.getLogger("lia.Monitor.modules.snmp_Disk_v2");
	...
	try {
	    ...
	    if(args!=null && args.length()>0) init_args(args);
	    // module init
	    init(node);
	} catch (SocketException e) {
	    // severe init error, cannot continue..
	    logger.log(Level.SEVERE, "[SNMP] CommInterface could not be initialized (Network problems or SNMP v2 not supported by SNMP Agent", e);
	    info.addErrorCount();
    ...
    private Map mDskPath_DiskIndex = new HashMap();
	...
	if (runNo++ == 0) {
		...
		if (logger.isLoggable(Level.WARNING))
		    logger.log(Level.WARNING, "[snmp_Disk_v2] Error while trying to read disks partition information. Using the old one:[" + mDskPath_DiskIndex + "]", t);
	    }
	}
	...
	for (Iterator iter = mDskPath_DiskIndex.entrySet().iterator(); iter.hasNext();) {
		...
		oids.add(aResTypesOids[i] + "." + iIndex);
	}
	...
	for (Iterator iter = mDskPath_DiskIndex.entrySet().iterator(); iter.hasNext();) {
		...
		if (mSNMPResults.containsKey(sPartKey)) {
		    ...
		    if (oValue == null || !(oValue instanceof SNMPInteger) || !(((SNMPInteger) oValue).getValue() instanceof BigInteger))
			continue;
	...
	try {
	    ...
	    throw new Exception("[snmp_Disk_v2] Error while trying to read disks partition information. Using the old one:[" + mDskPath_DiskIndex + "]", t);
	}
	...
	int iIndex;
		...
		} else // partition removed
		{
		    ...
		    eResult result = new eResult(Node.getFarmName(), Node.getClusterName(), Node.getName(), ModuleName, sRemovedParamsName);
	...
	for (int i = 0; i < tableVars.size(); i++) {
		...
		logger.log(Level.WARNING, "[snmp_Disk_v2]: Cannot parse Disk Index" + snmpOID.toString().substring(iIndexOf + 1));
		continue;
	    }
