...
public class snmp_IOpp_HC extends snmpMon implements MonitoringModule  {
    ...
    /** The Logger */ 
    private static final Logger logger = Logger.getLogger("lia.Monitor.modules.snmp_IOpp_HC");
    ...
    public Object   doProcess() {
        ...
        if(wasError) {
            logger.log(Level.WARNING, "Received error for " + Node.getName() + " Error description : " + errorDescription);
            resetOldCounters();
            return null;
        }
        ...
        if (snmpTime > 0 ) {
            if (last_measured >= snmpTime){//restarted ?
                logger.log(Level.WARNING, "last_measured [" + last_measured + " ] >= snmpTime [ " + snmpTime + " ] Was the device [ " + Node.getName() + " ] restarted ?");
        ...
        if ( dt == 0) {
            logger.log(Level.INFO, " diff == 0 for SNMPtime for " + Node.getName()+" ... probably the counters still not updated (... SNMP has high Load or SNMP queries are too often )");
            return null;
        }
        ...
        for ( int m=0; m < 2 ; m++ ) {
            for ( int ci =0; ci < NPORTS ; ci++ ) {
                try {
                    ...
                    if (logger.isLoggable(Level.FINEST)) {
                        logger.log(Level.FINEST, " [ " + Node.getName() + " ---> " + result.param_name[2*ci+m] + " ] New: " + counters[m][ci] + " Old: " + old_counters[m][ci] + " diff: " + diff); 
                    }
                    if ( diff  < 0 ) {
                        negativeDiff  = true;
                        logger.log(Level.WARNING, " Diff neg [ " + Node.getName() + " ---> " + result.param_name[2*ci+m] + " ] New: " + counters[m][ci] + " Old: " + old_counters[m][ci] + " diff: " + diff);
                        continue;
                    }
