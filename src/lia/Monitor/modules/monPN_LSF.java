...
public class monPN_LSF extends cmdExec implements MonitoringModule {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.Monitor.modules.monPN_LSF");
	...
	static String[] lsfMetric = { "ncpus", "mem", "maxmem", "swp", "maxswp",
			"r1m", "r15m"};
	/** Rename them into : */
    static String[] myResTypes = { "NoCPUs", "MEM_free", "MEM_total", 
    		"SWAP_free", "SWAP_total", "Load1", "Load15"};
    ...
    protected void setEnvironment() throws Exception {
        ...
        try {
			...
			if (!bfd.exists() || !lfd.exists()) { 
				...
				logger.log(Level.WARNING, "The LSF commands " + bhostsCmd + " or " + 
			    		lshostsCmd + " do not exist.", new Object[]{mlle});
			    throw new Exception("The LSF commands " + bhostsCmd + " or " + 
			    		lshostsCmd + " do not exist.");
			}
	...
	public Object doProcess() throws Exception {
		...
		logger.log(Level.INFO, ModuleName+": "+methodName+": "+nrCall+" => time to run: "+(tEnd-tStart)+"ms, sent "+results.size()+" results.");
		firsttime = false;
	...
	public void parseLshostsOutput(BufferedReader buffer) throws Exception {
		...
		try {
			while((line = buffer.readLine())!=null) {
				...
				if (line.startsWith("HOST_NAME") && line.indexOf("ncpus") > 0 &&
						line.indexOf("maxmem") > 0 && line.indexOf("maxswp") > 0) {
			...
			logger.log(Level.WARNING, (String)VO_Utils.pnErrCodes.get(iErrorCode), new Object[]{mlle, exc});
		    throw exc;
		}
	}
	...
	public void parseBhostsOutput(BufferedReader buffer) throws Exception {
		...
		try {
			while((line = buffer.readLine())!= null) {
				...
				if (line.startsWith("STATUS") && line.indexOf("MAX") > 0 &&
						line.indexOf("NJOBS") > 0 && haveNewRecord) {
				...
				if(line.indexOf("CURRENT LOAD USED FOR SCHEDULING") >=0 &&
						haveNewRecord) {
					line = buffer.readLine();
					if (line.indexOf("r1m") < 0 || line.indexOf("r15m") < 0 ||
							line.indexOf("swp") < 0 || line.indexOf("mem") < 0)
						throw new Exception ("Unrecognized line format in bhosts output: " +
								line + " ");
		...
		double dVal;
            	try {
            		...
            		if (sVal.indexOf("K") >= 0)
            			...
            			factor = 1024.0;
            		if (sVal.indexOf("K") >= 0 || sVal.indexOf("M") >= 0 ||
            				sVal.indexOf("G") >= 0)
			...
			haveParam = true;
            		...
            		logger.log(Level.WARNING, "Error parsing parameter, ignoring it: "
            				+ sVal, e);
            	}            	
            }
			if(firsttime)
	 ...
	 * @return
	 */
	protected String getSParameter(String paramName, String[] tokens, 
			Hashtable paramIndex) {
