/*
 * $Id: monOSPortsPower.java 6865 2010-10-10 10:03:16Z ramiro $
 */
...
public class monOSPortsPower extends cmdExec implements MonitoringModule, Observer {
    ...
    private static final transient Logger logger = Logger.getLogger(monOSPortsPower.class.getName());
    ...
    public static final String GLIMMER_TL1_CMD_PORT_POWER = "rtrv-port-power::all:" + OSTelnet.PPOWER_CTAG + OSTelnet.TL1_FINISH_CMD;
    ...
    public MonModuleInfo init(MNode Node, String arg) {
        this.Node = Node;
        logger.log(Level.INFO, "monOSPortsPower: farmName=" + Node.getFarmName() + " clusterName= "
    ...
    private Vector getPortsPowerCalient() {
		...
		for (Iterator it = newPortsList.iterator(); it.hasNext(); ) {
			...
			try {
                String[] lines = osConn.execCmdAndGet("rtrv-det-eqpt::"+portName+":" + OSTelnet.PPOWER_CTAG + OSTelnet.TL1_FINISH_CMD, OSTelnet.PPOWER_CTAG);
                String line;
                for(int linIt=0; linIt<lines.length; linIt++) {
					...
					try {
						...
						for (int i=0; i<arguments.length; i++) {
							if (arguments[i].indexOf(inPatternStr1) == 0 || arguments[i].indexOf(inPatternStr2) == 0) {
								String power = arguments[i].substring(inPatternStr1.length()+1);
								if (power == null) continue;
								power = power.trim();
								if (power.indexOf("\"") != -1) power = power.substring(0, power.length()-1);
								input.put(portName, power);
								continue;
							}
							if (arguments[i].indexOf(outPatternStr1) == 0 || arguments[i].indexOf(outPatternStr2) == 0) {
								String power = arguments[i].substring(outPatternStr1.length()+1);
								if (power == null) continue;
								power = power.trim();
								if (power.indexOf("\"") != -1) power = power.substring(0, power.length()-1);
            ...
            for (Iterator it = portsList.iterator(); it.hasNext(); ) {
                ...
                if(dip == Double.MAX_VALUE || dop == Double.MAX_VALUE) {
                    if(logger.isLoggable(Level.FINE)) {
                        logger.log(Level.FINE, " Wrong Power for port " + portName + " ---> InPower = " + sInP + " OutPower = " + sOutP);
                    }
                    continue;
                }
...
//        "GGN:PORTID=10006,PORTPOWER=-49.775"
    ...
    static public void main(String[] args) {
        ...
        if(isCalient) {
            ...
            aa.init(new MNode(host, ad, null, null), "SwitchType = Glimmerglass; PortMap = /home/ramiro/PortMap_Glimmer");
