/*
 * $Id: GenericUDPListener.java 7240 2012-04-18 08:14:20Z costing $
 */
...
public class GenericUDPListener extends Thread {
    ...
    /** The Logger */
    private static final transient Logger logger = Logger.getLogger(GenericUDPListener.class.getName());
     ...
     * @param conf
     */
    public GenericUDPListener(final InetAddress laddr, final int port, final GenericUDPNotifier notif, final UDPAccessConf conf) {
    ...
    private void bindSocket() throws IOException {
        ...
        if (socket == null) {
            ...
            throw new IOException(" Unable to bind the UDP socket on port " + port + " ]. GenericUDP socket == null !! ");
        }
                ...
                if (bSockNotBound) {
                    ...
                    while (ioErrC < 10 && bSockNotBound) {
                        try {
                            ...
                            logger.log(Level.WARNING, " [ GenericUDPListener ]  Unable to bind socket IOErrorCount = " + ioErrC, ioe);
                            try {
                                Thread.sleep(500);
                            } catch (Throwable ignore) {
                            	// ignore
