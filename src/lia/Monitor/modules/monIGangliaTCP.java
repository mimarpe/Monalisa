...
public class monIGangliaTCP extends cmdExec implements MonitoringModule {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.Monitor.modules.monIGangliaTCP");
    ...
    static {
        try {
            shouldOverwriteTime = Boolean.valueOf(AppConfig.getProperty("lia.Monitor.modules.monIGangliaTCP.shouldOverwriteTime", "false")).booleanValue();
        }catch(Throwable t){
            ...
            if(arg1 != null) {
                ...
                if(args != null && args.length >0) {
                    for(int i=0; i<args.length; i++) {
                        if(args[i].indexOf("gPort") != -1) {
                            try {
                                port = Integer.parseInt(args[i].split("(\\s)*=(\\s)*")[1]);
                                portSetted = true;
                            }catch(Throwable t){
                                port = 8649;
                            }
                        } else if(args[i].indexOf("shouldOverwriteTime") != -1) {
                            try {
                                shouldOverwriteTime = Boolean.valueOf(args[i].split("(\\s)*=(\\s)*")[1]).booleanValue();
                            }catch(Throwable t){
                                shouldOverwriteTime = false;
                            }
                        } else if (args[i].indexOf("socketTimeout") != -1) {
                            try {
                                socketTimeout = Integer.valueOf(args[i].split("(\\s)*=(\\s)*")[1]).intValue();
                            } catch (Throwable t) {
                                socketTimeout = 20 * 1000;
                            }
                        }  else if (args[i].indexOf("CanSuspend") != -1) {
                            try {
                                canSuspend = Boolean.valueOf(args[i].split("(\\s)*=(\\s)*")[1]).booleanValue();
                            } catch (Throwable t) {
                                canSuspend = false;
                            }
                        } else if (args[i].indexOf("shouldLogWrongHostsTime") != -1) {
                            try {
                                shouldLogWrongHostsTime = Boolean.valueOf(args[i].split("(\\s)*=(\\s)*")[1]).booleanValue();
                            }catch(Throwable t){
                                shouldLogWrongHostsTime = true;
                            }
                        } else if(args[i].indexOf("TIME_FRAME") != -1) {
                            try {
                                TIME_FRAME = Long.valueOf(args[i].split("(\\s)*=(\\s)*")[1]).longValue()*1000;
                            }catch(Throwable t) {
                                TIME_FRAME = 5 * 60 * 1000;///5 min
                            }
                        } else if(args[i].indexOf("LOG_DELAY") != -1) {
                            try {
                                LOG_DELAY = Long.valueOf(args[i].split("(\\s)*=(\\s)*")[1]).longValue()*1000;
                            }catch(Throwable t) {
                                LOG_DELAY =  60 * 60 * 1000;//every hour
                            }
                        } else if(args[i].indexOf("WrongHostsTimeLogFile") != -1) {
                            try {
                                wrongHostsTimeLogFile = args[i].split("(\\s)*=(\\s)*")[1];
                            }catch(Throwable t){
        ...
        if(shouldLogWrongHostsTime) {
            ...
            if(wrongHostsTimeLogFile == null) {
                ...
                if(wrongHostsTimeLogFile == null){
                    ...
                    logger.log(Level.WARNING, "monIGangliaTCP ... TIME_FRAME = " + TIME_FRAME +" LOG_DELAY = " + LOG_DELAY);
