/*
 * $Id: monCienaVCG.java 7303 2012-08-24 17:55:03Z ramiro $
 */
...
public class monCienaVCG extends cmdExec implements MonitoringModule, Observer {
	...
	private static final transient Logger logger = Logger.getLogger(monCienaVCG.class.getName());
	...
	static {
		AppConfig.addNotifier(new AppConfigChangeListener() {
            ...
            public void notifyAppConfigChanged() {
				shouldMonitorPerformance.set(AppConfig.getb("lia.Monitor.modules.monCienaVCG.shouldMonitorPerformance", false));
			}
		});
	}
    ...
    public MonModuleInfo init(MNode Node, String args) {
		...
		if (args == null) {
			...
			throw new IllegalArgumentException("[ monCienaVCG ] Null params in init. The module is unable to monitor the CD/CI");
		}

		if (args.length() == 0) {
			logger.log(Level.SEVERE, "[ monCienaVCG ] The args list is empty. The module is unable to monitor the CD/CI");
			throw new IllegalArgumentException("[ monCienaVCG ] The args list is empty. The module is unable to monitor the CD/CI");
		}
		...
		if (argsTokens != null && argsTokens.length > 0) {
				...
				try {
					...
					if (argT.startsWith("vcgConfFile")) {
						...
						if (fileTks == null || fileTks.length != 2) {
							...
							throw new IllegalArgumentException("[ monCienaVCG ] cannot parse vcgConfFile param [ " + argT + " ]  ... ");
						}

						vcgConfFile = new File(fileTks[1]);

						if (!vcgConfFile.exists()) {
							logger.log(Level.SEVERE, "[ monCienaVCG ] The vcgConfFile [ " + vcgConfFile + " ] does not exist");
							throw new IllegalArgumentException("[ monCienaVCG ] The vcgConfFile [ " + vcgConfFile + " ] does not exist");
						}

						if (!vcgConfFile.canRead()) {
							logger.log(Level.SEVERE, "[ monCienaVCG ] The vcgConfFile [ " + vcgConfFile + " ] does not have read access");
							throw new IllegalArgumentException("[ monCienaVCG ] The vcgConfFile [ " + vcgConfFile + " ] does not have read access");
						}
						...
						if (mTks == null || mTks.length != 2) {
							...
							throw new IllegalArgumentException("[ monCienaVCG ] cannot parse OC1_BWMULTIPLIER param [ " + argT + " ]  ... ");
						}
						...
						try {
							...
							logger.log(Level.SEVERE, "[ monCienaVCG ] cannot parse OC1_BWMULTIPLIER param [ " + argT + " ]  ... Cause:", t);
						}
			...
			logger.log(Level.SEVERE, "[ monCienaVCG ] Unable to determine the arguments tokens. The module is unable to monitor the CD/CI");
			throw new IllegalArgumentException("[ monCienaVCG ] Unable to determine the arguments tokens. The module is unable to monitor the CD/CI");
		}

		if (vcgConfFile == null) {
			logger.log(Level.SEVERE, "[ monCienaVCG ] Unable to determine vcgConfFile. The module is unable to monitor the CD/CI");
			throw new IllegalArgumentException("[ monCienaVCG ] Unable to determine vcgConfFile. The module is unable to monitor the CD/CI");
		}
		...
		try {
			...
			throw new IllegalArgumentException("[ monCienaVCG ] Got exception parsing the conf files: " + t.getMessage());
		}
    ...
    public Object doProcess() throws Exception {
		...
		try {
			...
			if (logger.isLoggable(Level.FINER)) {
				logger.log(Level.FINER, "[ monCienaVCG ] doProcess took: " + TimeUnit.NANOSECONDS.toMillis((Utils.nanoNow() - sPerfTime)) + " ms.");
			}
