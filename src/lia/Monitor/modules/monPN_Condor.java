...
public class monPN_Condor extends cmdExec implements MonitoringModule {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.Monitor.modules.monPN_Condor");
	...
	static protected String[] condorMetrics = {
		"Cpus",
		"TotalLoadAvg",
		"VirtualMemory",
		"TotalVirtualMemory",
    ...
    static protected String[] myResTypes = {
		"NoCPUs",
		"Load1",
		"VIRT_MEM_free",
		"VIRT_MEM_total",
	...
	public MonModuleInfo init(MNode Node , String args) {
		...
		if ( args != null ) {
			...
			for ( int j=0; j < argList.length; j++ ) {
				if (argList[j].toLowerCase().indexOf("disablestatistics") != -1) {
				...
				if (argList[j].toLowerCase().indexOf("slotsfactor") != -1) {
					try {
						String sFactor = argList[j].split("(\\s)*=(\\s)*")[1].trim();
		...
		if (serverNames.size() == 0) {
		    ...
		    for (int ind = 0; ind < serverNames.size(); ind++) { 
		        ...
		        condorCmd.append(condorLocation + "/bin/condor_status -l -pool " +
		                serverNames.get(ind) + constraints);					
		        condorCmd.append(" && echo ML_PN_OK");
		        nCommands++;
		    }
		}
	...
	public Object doProcess() throws Exception {
		...
		if(haveCommand){
			...
			try {
				...
				try {
					...
					logger.log(Level.INFO, "Second attempt to get nodes status failed, no results were sent - exception:", e);
					if (errorCode.intValue() != 0) {
		...
		logger.log(Level.INFO, "Sent " + ret.size() + " results. " +
        		"Execution time: " + totalProcessingTime + " ms. " +
        		"Status = " + errorCodeParam + " ("+VO_Utils.pnErrCodes.get(Integer.valueOf(errorCodeParam))+")");
		logger.log(Level.INFO, "[monPN_Condor] execution time for doProcess(): "
	...
	public void parseCondorOutput(BufferedReader buff) throws Exception {
		...
		try {
			...
			if (this.nSuccessfulCommands < this.nCommands) {
	        	throw new Exception("The condor_status command returned error! - " +
	        			nSuccessfulCommands + " commands of " + nCommands +
	...
	Hashtable parseStatusRecord(StringBuilder rec) {
		...
		while (st.hasMoreTokens()) {
			...
			if (paramName.equals("Cpus") || paramName.equals("TotalLoadAvg")) {
				...
				nodeInfo.put(paramName, Double.valueOf(dval)); continue;
			}						
			
			if (paramName.equals("VirtualMemory") || 
					paramName.equals("TotalVirtualMemory") || paramName.equals("Memory")) {
				double dval = Double.parseDouble(paramValue);
				nodeInfo.put(paramName, Double.valueOf(dval)); continue;
			}			
		}
	...
	void addVMInfo(Hashtable nodeData, Hashtable oldNodeData) {
		...
		while (hkeys.hasMoreElements()) { 
			...
			if (paramName.equals("Cpus") || paramName.equals("VirtualMemory") ||
					paramName.equals("Memory") || paramName.equals("TotalVirtualMemory")) {
