...
/** Logger used by this class */
  private static final transient Logger logger = Logger.getLogger("lia.Monitor.modules.monVOgsiftpIO");
    
//JGW  static protected String MyModuleName = "monVOgsiftpIOold";
  protected static String MyModuleName = "monVOgsiftpIO";
  protected static String[] MyResTypes = {"ftpInput",
                                          "ftpOutput"};
  ...
  protected void  parseFtpStat (BufferedReader buff) throws Exception  {
    ...
    try {
      while ( (record = buff.readLine()) != null) {
        linecnt++;
        if (record.length() == 0 )                 {continue;} // empty line
        if (record.startsWith(COMMENT_LINE_START)) {continue;} // comment
