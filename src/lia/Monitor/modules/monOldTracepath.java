...
public class monOldTracepath extends cmdExec implements MonitoringModule {
	/** Logger name */
	private static final transient String COMPONENT = "lia.Monitor.modules.monOldTracepath";
	...
	private void initDefaults() {
		...
		try {
			...
			logger.log(Level.INFO, "monOldTracepath: Starting... I will search conf file (when available from Geo&IPID service) for "
					...
					synchronized(peers){
						for(int i=0; i<peers.size(); i++){
							...
							if(! crtPeers.contains(oldPeer)){
								peers.remove(i); i--;
								eResult eresult = new eResult(Node.getFarmName(),
										Node.getClusterName(), oldPeer, ModuleName, new String[0]);
			...
			while(true){
				...
				for(int i=0; i<crtPeers.size(); i++){
					...
					try{
						...
						// building eResult...
						eResult eresult = new eResult(Node.getFarmName(),
							Node.getClusterName(), peer, ModuleName, ResTypes);
						eresult.time = NTPDate.currentTimeMillis();
		...
		void check4deadTraces(){
			...
			for(Enumeration en = traceCache.keys(); en.hasMoreElements(); ){
				...
				if(now - res.time > TRACE_EXPIRE_DELAY){
					...
					res.time = now;
					logger.log(Level.INFO, "TraceFilter: trace info to "+res.NodeName+" has expired. Removing it. "+res.param_name.length);
	...
	Vector getTracerouteCmdOut(String peer) throws Exception {
		...
		try {
			...
			for (;;) {
				...
				if (lin == null)
					break;
				if(crtLine == 1)	// skip the first line
