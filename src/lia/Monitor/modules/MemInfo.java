...
public class MemInfo extends AbstractSchJobMonitoring {
	 ...
	 * Message logger
	 */
	static final transient Logger logger = Logger.getLogger(MemInfo.class.getName());
	...
	public Object doProcess() throws Exception {
		...
		if (isLinuxOS()){
			...
			mem_actualfree = (mem_free > 0 ? mem_free : 0) + 
			                 (mem_buffers > 0 ? mem_buffers : 0) +
		...
		if (isSolarisOS()){
			try{
				...
				if (idx>=0){
					...
					switch (prtconf.charAt(idx2+1)){
						case 'T' : total_mem *= 1024;
						           break;
						           
						case 'G' : total_mem *= 1024 * 1024;
								   break;
						
						case 'K' : total_mem /= 1024;
						           break;
					}
			...
			try{
				final String swap = ExternalProcesses.getCmdOutput(Arrays.asList("swap", "-l"), true, 30L, TimeUnit.SECONDS);
