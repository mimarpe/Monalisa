...
public class monOsgVO_IO extends monVoModules implements MonitoringModule {
    ...
    public monOsgVO_IO() {
        ...
        logger.log(Level.INFO, "Info content: name: " + info.name + " / id: " + info.id + 
        		" / type: " + info.type + " / state: " + info.state + " / err: " + info.error_count + ".");
    }
    ...
    public MonModuleInfo init(MNode inNode, String args) {
        ...
        logger.log(Level.INFO, methodName + "(): Instantiating instance for Cluster (node in cf) " + clusterModuleName + " at " + currDate + 
        	"\narguments: " + ((args == null) ? "NO ARGUMENTS" : args) + 
        	"\nNode Info: name " + (Node.name == null ? "null" : Node.name) + " short_name " + (Node.name_short == null ? "null" : Node.name_short) + " cl.name " + (Node.cluster == null ? "null" : Node.cluster.name) + " fa.name " + (Node.farm == null ? "null" : Node.farm.name) + ".");
        ...
        try {
            ...
            if (!probe.isFile()){
            	...
            	try{
            		...
            		logger.log(Level.WARNING, "Gridftp log (" + ftplog + ") not found. No GridFTP transfer information will be provide.", new Object[]{mlle});
            	} catch(Exception e) {}
            	...
            	try{
            		...
            		mlle.logParameters.put("Error Code", errorCode);
            		mlle.logParameters.put(errorCode, VO_Utils.voIOErrCodes.get(errorCode));
            		logger.log(Level.WARNING, "Gridftp log (" + ftplog + ") is not readable. No GridFTP transfer information will be provide.", new Object[]{mlle});
            	} catch(Exception e) {}
            } else {
            	logger.log(Level.INFO, "Gridftp log (" + ftplog + ")\n  last modified " + (new Date(probe.lastModified())).toString() + " - size(" + probe.length() + " Bytes)");
            	haveLogFile = true;
            }
    ...
    public Object doProcess() throws Exception {
        ...
        try {
				...
				if(shouldProcess == true){
	                ...
	                if (logBuffer == null){
		            	...
		            	try{
		            		...
		            		logger.log(Level.WARNING, "BufferedReader is null for [ " + ftplog + " ] at " + call_no +"th call.", new Object[]{mlle});
		            	} catch(Exception e) {}
	                } else {
		                ...
		                try {
		                   ...
		                   /** change the status */
			            	...
			            	try{
			            		...
			            		mlle.logParameters.put("Error Code", errorCode);
			            		mlle.logParameters.put(errorCode, VO_Utils.voIOErrCodes.get(errorCode));
			            		logger.log(Level.WARNING, "Throwable exception when parse gridftp.log file.", new Object[]{mlle, t});
			            	} catch(Exception e) {}
		                }
            ...
            /** message to ML log file */
            logger.log(Level.INFO, methodName + " - monOsgVO_IO [" + call_no + "] " +
            		"Sent " + moduleResults.size() + " results. " +
            		"Execution time: " + (stop - start) + " ms. " +
            		"Status=" + errorCodeParam + ": "+VO_Utils.voIOErrCodes.get(Integer.valueOf(errorCodeParam)));
		...
		try {
            ...
            if(stop >= start){
	            ...
	            for(int i = 0; i < 4 ; i++){
	            	...
	            	switch(i){
	            		case 0: res[i].time = start-1; break;
	            		case 1: res[i].time = start; break;
	            		case 2: res[i].time = stop; break;
	            		case 3: res[i].time = stop+1; break;
	            	}
	            	if(i==0 || i==3)
	            		res[i].addSet(paramName, 0D);
	            	if(i==1 || i==2)
	            		res[i].addSet(paramName, 1000.0 * (Double.valueOf(((String) info.get("NBYTES")))).doubleValue() / (stop - start) / factor );
            	...
            	try{
            		...
            		msg = msg + " DATA before START (DATA="+(String)info.get("DATE")+" START="+(String)info.get("START")+").";
			...
			if(ts != TRANSFER_CODE){
				...
				String msg = "Trasfer CODE = "+ts+". "+"VO = "+user+". "+"DEST = "+dest+". "+"HOST = "+src +". ";
				MLLogEvent mlle = new MLLogEvent();
            	mlle.logParameters.put("Error Code", errorCode);
            	mlle.logParameters.put(errorCode, msg);
    ...
    public class UserFTPData {
        ...
        UserFTPData(String clusterName, String nodeName) {
            ...
            node_name = nodeName;
            ftpInput = 0D;  ftpOutput = 0D;
            ftpRateIn = 0D; ftpRateOut = 0D;
            shouldSend = false;
        }
