...
public class snmp_CatSwitch extends snmpMon2 implements MonitoringModule {
	...
	static final Logger logger = Logger.getLogger("lia.Monitor.modules.snmp_CatSwitch");
	...
	public MonModuleInfo init(MNode node, String args) {
		try {
			...
			if (logger.isLoggable(Level.INFO))
				logger.log(Level.INFO, "SNMP module configuration for [ " + Node.cluster + "/" + Node.name + " ]: " + this.toString());
	 ...
	 */
	private void initPortMap(final String[] saInputPortsList) throws SNMPGetException, IOException, SNMPBadValueException {
				...
				for (int ii = 0; ii < vConfiguration.length; ii++) {
					try {
						...
						if (logger.isLoggable(Level.WARNING))
							logger.log(Level.WARNING, "[snmp_CatSwitch] Could not understand parameter" + vConfiguration[ii]);
					}
				}// for
			} catch (Throwable t) {
				if (logger.isLoggable(Level.WARNING))
					logger.log(Level.WARNING, "[snmp_CatSwitch] Could not parse configuration parameter" + sConfiguration, t.getMessage());
			}
	...
	private Object doProcess0() throws Exception {
		...
		if (info.getState() != 0) {
			throw new Exception("There was some exception during init ..." + info.getErrorDesc() + " [" + info.getErrorCount() + "]");
		}
		...
		if (updateStatsPeriods == 0) {
			...
			if (bShowStats) {
				eResult eResult = new eResult(Node.getFarmName(), Node.getClusterName() + "_Stats", Node.getName(), ModuleName, null);
				...
				for (Iterator iter = mIfDescr_Data.entrySet().iterator(); iter.hasNext();) {
					try {
						Map.Entry entry = (Map.Entry) iter.next();
						String ifDescr = (String) entry.getKey();
						Map attrs = (Map) entry.getValue();
						rResult.addSet(ifDescr.replace('/', '_') + "_SPEED", ((Double) attrs.get(IF_SPEED)).doubleValue());
				...
				try {
					Map chasis = super.snmpBulkGet(new String[] { oid_sysName, oid_sysDescr, oid_sysLocation, oid_entPhysicalDescr });
			...
			if (lStatsOIDsQuery.size() > 0)
				try {
					...
					if (t != null && (t instanceof SNMPTimeTicks)) {
						...
						if (value != null) // uptime is reported in hundreths of second
		...
		for (Iterator iter = mIfDescr_Data.entrySet().iterator(); iter.hasNext();) {
			try {
				...
				oids.add(((mode == MODE32) ? oid_ifOutOctets : oid_ifHCOutOctets) + "." + attrs.get(IF_INDEX).toString());
				...
				if (++chunk % 4 == 0) { // get chunk (this performs like the
					...
					try {
						...
						if (mode == MODE64 /* && sge.errorStatus */) {
							if (logger.isLoggable(Level.INFO))
								logger.log(Level.INFO, "HighCounters are not supported, fallback to 32bit counters. Cause:", sge);
							mode = MODE32;
						}
						if (logger.isLoggable(Level.WARNING))
							logger.log(Level.WARNING, "[SNMP] An error was occured ..Invalid OIDs." + Node.getName() + " ] :" + sge.getMessage());
	...
	private void updateResult(Result result, String ifDescr, String sCounter) {
		...
		Map attrs = (Map) mIfDescr_Data.get(ifDescr);
		...
		if (counter.getCurrentTimestamp().longValue() > 0) {
			if (counter.getCurrentTimestamp().longValue() < counter.getPreviousTimestamp().longValue()) { // restarted
				logger.log(Level.WARNING, "last_measured [" + counter.getPreviousTimestamp().longValue() + " ] >= snmpTime [ " + counter.getCurrentTimestamp().longValue()
		...
		if (dt == 0) {
			logger.log(Level.INFO, " diff == 0 for " + Node.getName() + "/" + ifDescr
		...
		if (diff < 0) {
			logger.log(Level.WARNING, " Diff neg [ " + Node.getName() + " ---> " + ifDescr + " ] New: " + counter.getCurrentCounter() + " Old: " + counter.getPreviousCounter()
					+ " diff: " + diff);
			// reset counters and skip this port
			counter.reset();
			return;
		...
		// else, diff >0
		if (logger.isLoggable(Level.FINE)) {
			logger.log(Level.FINE, " [  " + Node.getName() + " ---> " + ifDescr + " ] Counter[: " + sCounter + counter + " diff: " + diff);
		}
		// return rate in Mbps
	...
	private void updateMappings() throws SNMPGetException, IOException, SNMPBadValueException {
		...
		for (Iterator iter = m.entrySet().iterator(); iter.hasNext();) {
			try {
				...
				Map attrs = (Map) mIfDescr_Data.get(key);
				if (attrs == null) {
					attrs = new HashMap();
	...
	private void updatePortsStatus(Map mOID_Value, List lIfDescrRequested) {
		...
		try {
			...
			logger.log(Level.WARNING, "Failed to update" + lIfDescrRequested + ": Got exception while trying to read SNMP timestamp ", t);
			return;
		}
					...
					if (mOID_Value.containsKey(_oid)) {
						...
						if (mode == MODE64) {
							if (!(value instanceof SNMPCounter64)) {
								// SNMPUnknown?
								logger.log(Level.WARNING, "[SNMP: " + this.Node.getName() + "]  Invalid SNMPCounter64 value " + value.getClass() + " for OID: " + _oid
							...
							if (!(value instanceof SNMPCounter32)) {
								logger.log(Level.WARNING, "[SNMP: " + this.Node.getName() + "]  Invalid SNMPCounter32 value " + value.getClass() + " for OID: " + _oid
										+ " ifIndex: " + ifDescr + " ...Skipping it");
								counter_data.reset();
								iUnknownCounters++;
								continue;
	...
	private void resetCounters(List lIfDescr) {
		...
		for (Iterator iter = lIfDescr.iterator(); iter.hasNext();) {
			try {
				ifDescr = (String) iter.next();
				data = (Map) mIfDescr_Data.get(ifDescr);
				if (data == null)
					continue;
	...
	private void updateHighSpeeds() throws SNMPGetException, IOException, SNMPBadValueException {
		...
		for (Iterator<Map.Entry<String, Map<String, Number>>> iter = mIfDescr_Data.entrySet().iterator(); iter.hasNext();) {
			try {
				Map.Entry<String, Map<String, Number>> entry = iter.next();
				String ifDescr = entry.getKey();
				Map<String, Number> attrs = entry.getValue();
				if (attrs.get(IF_INDEX) == null)
					continue;

				String ifSpeedOID = new StringBuilder(oid_IfSpeed).append(".").append(attrs.get(IF_INDEX).toString()).toString();
		...
		for (Iterator iter = lUnresolvedIfs.iterator(); iter.hasNext();) {
			try {
				String ifDescr = (String) iter.next();
				Map attrs = (Map) mIfDescr_Data.get(ifDescr);
		...
		for (Iterator iter = lUnresolvedIfs.iterator(); iter.hasNext();) {
			try {
				...
				String ifSpeedOID = new StringBuilder(oid_IfHighSpeed).append('.').append(attrs.get(IF_INDEX).toString()).toString();
		...
		try {
			...
			for (Iterator iter = mIfDescr_Data.entrySet().iterator(); iter.hasNext();) {
				try {
					...
					String ifMtuOID = new StringBuilder(oid_IfMtu).append(".").append(attrs.get(IF_INDEX).toString()).toString();
					String ifTypeOID = new StringBuilder(oid_IfType).append(".").append(attrs.get(IF_INDEX).toString()).toString();
					String ifAliasOID = new StringBuilder(oid_IfAlias).append(".").append(attrs.get(IF_INDEX).toString()).toString();
	...
	private static final class CounterData {
		...
		public String toString() {
			return "{CurrentCounter:" + currentCounter + " CurrentTimestamp:" + currentTimestamp + "}, {PreviousCounter:" + previousCounter + " PreviousTimestamp:"
					+ previousTimestamp + "}, RateTransport:" + transport;
		}
