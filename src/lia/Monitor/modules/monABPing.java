...
public class monABPing extends SchJob implements MonitoringModule {
	...
	private static final transient Logger logger = Logger.getLogger("lia.Monitor.modules.monABPing");
    ...
    static {
        ...
        try {
            errorTimeDelta = Long.valueOf(AppConfig.getProperty("lia.Monitor.modules.monABPing.errorTimeDelta", null)).longValue()*1000;
        } catch(Throwable t){
	...
	private static final boolean useContactEmail = Boolean.valueOf(AppConfig.getProperty("include.MonaLisa.ContactEmail", "false")).booleanValue();
	...
	class ConfigLoader implements Runnable {
		...
		private ABPingFastReply pinger;		
		private StringBuilder sbLog;
		private int sbLogIndex;
		...
		protected ConfigLoader(ABPingFastReply pinger) {
			...
			this.pinger = pinger;
			sbLog.append("monABPing at: farmName=" + Node.getFarmName() + " clusterName= " + Node.getClusterName() + " nodeName=" + Node.getName() + "\n");
		...
		public void run() {
            ...
            for (int iURL = 0;(iURL < configURLs.size()) && (result != CFG_OK); iURL++) {
				...
				} while ((result == CFG_NETWORK_ERROR) &&
						(retries < MAX_RETRIES));				
            }
             ...
             * log my error.
             */
            if ((result == CFG_HOSTNAME_NOT_FOUND) &&
            		(NTPDate.currentTimeMillis() - lastErrorTime > errorTimeDelta)) {
             ...
             * and update config
             */
            if ( (currentConfigURL != previousConfigURL) ||
            	(currentHostName != previousHostName)) {
		...
		private String getABPingParams(){
            ...
            try{
            	...
            	result.append("&FarmIP="); result.append(URLEncoder.encode(myIPaddress, "UTF-8"));
            	result.append("&HostName="); result.append(URLEncoder.encode(myFullHostname, "UTF-8"));
                ...
                for(Enumeration en = peers.elements(); en.hasMoreElements();) {
        			...
        			peerStat.append(URLEncoder.encode(peerName
        					+"|"+pinger.getPeerPacketLoss(peerName)
		...
		private int loadConfig(Vector hostnames, String url) {
			...
			if ((hostnames == null) || (hostnames.isEmpty())) {
				sbLog.append("monABPing: hostname " + hostnames + 
						" not found in config URL " + url + ".\n");
			...
			try {
				...
				pinger.setConfig(OVERALL_COEF, RTT_COEF, PKT_LOSS_COEF, JITTER_COEF, RTT_SAMPLES, PKT_LOSS_MEM, PACKET_SIZE, PING_INTERVAL);
	...
	public MonModuleInfo init(MNode Node, String arg) {
		this.Node = Node;
		logger.log(Level.INFO, "monABPing: farmName=" + Node.getFarmName() + " clusterName= " + Node.getClusterName() + " nodeName=" + Node.getName());
		pinger = new ABPingFastReply(Node.getName());
		
		initDefaults();
		
		MonALISAExecutors.getMLHelperExecutor().scheduleWithFixedDelay(new ConfigLoader(pinger), 0, cfgLoadDelta, TimeUnit.MILLISECONDS);
		...
		try {
			for (int k = 0; k < 10000; k++) {
				...
				for (int q = 0; q < bb.size(); q++)
					System.out.println((Result) bb.get(q));
