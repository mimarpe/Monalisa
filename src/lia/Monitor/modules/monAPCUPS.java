...
public class monAPCUPS extends cmdExec implements MonitoringModule {
    ...
    private static final transient Logger logger 	= Logger.getLogger("lia.Monitor.modules.monAPCUPS");
	...
	public MonModuleInfo init(final MNode node, final String args) {
		...
		for (int i=0; i<vsPossiblePaths.length; i++){
			final File f = new File(vsPossiblePaths[i]);
			
			if (f.exists() && f.isFile()){
				sCommand = vsPossiblePaths[i];
				iFoundCount ++;
			}
		}
	...
	public Object doProcess() throws Exception {
		...
		if (br == null) {
			...
			try {
				...
				while ((line = br.readLine()) != null) {
					...
					if (sKey.equals("HOSTNAME") || sKey.equals("RELEASE") || sKey.equals("CABLE") || sKey.equals("DATE") || sKey.equals("VERSION") || sKey.equals("UPSNAME"))
		...
		if ((r.param==null || r.param.length==0) && (er.param==null || er.param.length==0))
			throw new IOException("No APC UPS detected. If apcupsd is correctly installed, please give the exact command to execute (path included) as an argument to the module");
