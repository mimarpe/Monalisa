...
public class monProcStat extends monProcReader {
    ...
    private static final transient Logger logger = Logger.getLogger(monProcStat.class.getName());
     ...
     * counter diffs
     */
    private long[] diff = new long[ResTypes.length];
            ...
            try {
                ...
                try {
                    ...
                    if(logger.isLoggable(Level.FINE)) {
                        logger.log(Level.FINE, " [ monProcStat ] [ HANDLED ] Checking for /proc/stat yield a caught exception", pft);
                    } else {
                        logger.log(Level.INFO, " [ monProcStat ] [ HANDLED ] Cannot use /proc/stat for local monitoring");
                    }
                } finally {
                ...
                try {
                    ...
                    if(logger.isLoggable(Level.FINE)) {
                        logger.log(Level.FINE, " [ monProcStat ] [ HANDLED ] Checking for /proc/vmstat yield a caught exception", pft);
                    } else {
                        logger.log(Level.INFO, " [ monProcStat ] [ HANDLED ] Cannot use /proc/vmstat for local monitoring");
                    }
                } finally {
                    Utils.closeIgnoringException(fr);
                    Utils.closeIgnoringException(br);
                ...
                logger.log(Level.WARNING, "[ monProcStat ] [ HANDLED ] Got exception in init. The module will not be used for local monitoring", t);
            }
        }//synchronized
    }//static
                    ...
                    if(diff[4] < 0) {
                        ...
                        if(hasCPUIntStats) {
                            if(diff[5] < 0) {
                                ...
                                if(diff[6] < 0) {
                                    bCPU = false;
                                    logger.log(Level.WARNING, " Decreasing CPU_softint old: " + old[6] + " new: " + cur[6]);
                                } else {
                                    sum += diff[5] + diff[6];
                                    if(hasCPUStealStats) {
                                        if(diff[7] < 0) {
                                            bCPU = false;
                                            logger.log(Level.WARNING, " Decreasing CPU_steal old: " + old[7] + " new: " + cur[7]);
                                        } else {
                                            sum += diff[7];
