/*
 * $Id: monProcReader.java 6940 2010-11-03 10:21:16Z costing $
 */
...
public abstract class monProcReader extends cmdExec implements MonitoringModule {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.Monitor.modules.monProcReader");
    ...
    private void createReaders() throws Exception {
        cleanup();
        if(PROC_FILE_NAMES == null) throw new Exception(" PROC_FILE_NAMES is null");
        ...
        for(int i=0; i<PROC_FILE_NAMES.length; i++) {
            try {
                ...
                if(logger.isLoggable(Level.FINE)) {
                    logger.log(Level.FINE, " [ monProcReader ] [ HANDLED ] Got exc creating Readers [ " + i + " ] :- " + PROC_FILE_NAMES[i], t);
                }
            }
        }
    }
        ...
        if(bufferedReaders != null) {
            for (int i=0; i<bufferedReaders.length; i++) {
                try {
                    ...
                    if(logger.isLoggable(Level.FINER)) {
                        logger.log(Level.FINER, " [ monProcReader ] [ HANDLED ]Got exception closing buffered reader [ " + i + " ]", t);
                    }
                } finally {
        ...
        if(fileReaders != null) {
            for (int i=0; i<fileReaders.length; i++) {
                try {
                    ...
                    if(logger.isLoggable(Level.FINER)) {
                        logger.log(Level.FINER, " [ monProcReader ] [ HANDLED ]Got exception closing file reader [ " + i + " ]", t);
                    }
                } finally {
