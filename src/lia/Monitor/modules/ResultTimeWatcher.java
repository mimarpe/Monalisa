...
public final class ResultTimeWatcher extends Thread {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.Monitor.modules.ResultTimeWatcher");

    private String fileName;
    private long logDTTime;
    private Hashtable ht;
    private long acceptedDTError;
    ...
    static {
        try {
            //in miutes
            CLEANUP_DELAY = Long.valueOf(AppConfig.getProperty("lia.Monitor.modules.ResultTimeWatcher.CLEANUP_DELAY", "60")).longValue()*60*1000; 
        }catch(Throwable t) {
    ...
    private void add(String ClusterName, String NodeName, long rTime, long cTime) {
        if(NodeName == null || ClusterName == null) return;
        ...
        if(diff > acceptedDTError) {
            synchronized(ht) {
                if(!ht.containsKey(key)) {
                    ...
                    if(logger.isLoggable(Level.FINER)) {
                        logger.log(Level.FINER, "ResultTimeWatcher adding key "+key+" cTime="+new Date(cTime)+" rTime="+new Date(rTime));
                    }
                }
            ...
            synchronized(ht) {
                if(ht.containsKey(key)) {
                    ...
                    shouldWrite = true;
                    if(logger.isLoggable(Level.FINER)) {
                        logger.log(Level.FINER, "ResultTimeWatcher REMOVING key "+key+" cTime="+new Date(cTime)+" rTime="+new Date(rTime));
                    }
                }
            }
        }
        ...
        synchronized(ht) {
            for(Enumeration en = ht.keys(); en.hasMoreElements();) {
                ...
                if(ri.uTime + CLEANUP_DELAY < cTime) {
                    ht.remove(key);
                    if(logger.isLoggable(Level.FINER)) {
                        logger.log(Level.FINER, "ResultTimeWatcher removing key " +key+" cTime="+new Date(ri.uTime)+" rTime="+new Date(ri.rTime));
                    }
                }
            }
        }
