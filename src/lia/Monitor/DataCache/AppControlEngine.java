/*
 * $Id: AppControlEngine.java 6865 2010-10-10 10:03:16Z ramiro $
 */
...
public class AppControlEngine {
	...
	private static final transient Logger logger = Logger.getLogger(AppControlEngine.class.getName());
	// internal buffer used to communicate with AppControl dispatcher
	final static int MAX_QUEUE_SIZE = AppConfig.geti("lia.Monitor.DataCache.AppControlEngine.CLIENT_QUEUE_MAX_SIZE", 1 << 10);
	// how often the remote client context is checked for validity (in minutes)
	final static int CONTEXT_CHECK_INTERVAL = AppConfig.geti("lia.Monitor.DataCache.AppControlEngine.CONTEXT_CHECK_INTERVAL", 30);
	...
	public AppControlEngine() throws GCredentialException, GSSException {
		...
		this.gssData = new GSSInitData(store, passwd, alias, store, passwd, lia.util.net.Util.getCCB(), GSSCredential.INITIATE_AND_ACCEPT);
	...
	private class AppControlRemoteClient implements Runnable {
		...
		private tcpClientWorker srcProxy;
		...
		private PrintWriter pw = new PrintWriter(new OutputStreamWriter(baos));
				...
				try {
					...
					if (oMessage == null) {
						if (!this.srcProxy.isConnected()) {
							if (logger.isLoggable(Level.INFO))
								logger.log(Level.INFO, "[AppControlRemoteClient " + getName() + "] Proxy connection closed.");
							break main_loop;
						}
					}
					if (oMessage != null && oMessage instanceof monMessageProxy) {
						...
						if (!(proxyMessage.message.result instanceof byte[])) {
							if (logger.isLoggable(Level.SEVERE))
								logger.log(Level.SEVERE, "[AppControlRemoteClient " + getName() + "] Invalid monMessageProxy .Got "
						...
						if (logger.isLoggable(Level.FINE))
							logger.log(Level.FINE, "[AppControlRemoteClient " + getName() + "] subscribe request received:"
						...
						if (msgType.equalsIgnoreCase(AppControlMessage.APP_CONTROL_MSG_AUTH_START)
							...
							if (outToken != null) {
								if (this.acceptSecCtx.context.isEstablished()) {
									...
									monMessage = new monMessage(AppControlMessage.APP_CONTROL_MSG_AUTH_FINISHED, this.sessioID, outToken);
									srcProxy.WriteObject(monMessage, tcpClientWorker.ML_AGENT_MESSAGE);
									if (logger.isLoggable(Level.FINE))
										logger.log(Level.FINE, "[AppControlRemoteClient " + getName() + "] [AUTH] Context established. Sending AUTH_FINISHED"
														+ getSecurityCtxInfo(this.acceptSecCtx.context));
								} else {
									monMessage = new monMessage(AppControlMessage.APP_CONTROL_MSG_AUTH, this.sessioID, outToken);
									// AppControlEngine.this.proxiesComm.rezToAllProxies(monMessage);
									// send to the source, withe ML_AGENT_MESSAGE priority
									srcProxy.WriteObject(monMessage, tcpClientWorker.ML_AGENT_MESSAGE);
									logger.fine("[AppControlRemoteClient " + getName() + "] [AUTH] Send gss message to peer " + getName()
													+ ":" + outToken.length);
								}
							} else {
									logger.log(Level.WARNING,"[AppControlRemoteClient " + getName() + "]  [AUTH] Received null GSS token. Discarding");
							}
							...
							break main_loop;
								...
								logger.log(Level.FINE, "[AppControlRemoteClient " + getName()
												+ "] Got APP_CONTROL_MSG_PROXY_ERR from proxy. Quitting session: " + new String(payload));
							break main_loop;
							...
							AppControlMessage response = new AppControlMessage(cmd.cmdID, new String(this.baos.toByteArray()));
							byte[] clearResponse = Utils.writeObject(response);
							// send the response
							monMessage = new monMessage(AppControlMessage.APP_CONTROL_MSG_CMD, this.sessioID, this.acceptSecCtx.wrap(
											clearResponse, 0, clearResponse.length));
							srcProxy.WriteObject(monMessage, tcpClientWorker.ML_AGENT_MESSAGE);
							if (logger.isLoggable(Level.FINEST))
								logger.log(Level.FINEST, "[AppControlRemoteClient " + getName() + "] Command response sent:" + response);
						} else {
							if (logger.isLoggable(Level.WARNING))
								logger.log(Level.WARNING, "[AppControlRemoteClient " + getName() + "] Discarding unknown message type:"
												+ msgType + " payload:" + new String(payload));
						}
						/**
						 * </monMessage dispatch>
						 */
					    ...
					    if(logger.isLoggable(Level.FINEST))
					        logger.log(Level.FINEST, "[AppControlRemoteClient " + getName() + "] Queue polling period elapsed, no valid message received");
					...
					errMessage = "[AppControlRemoteClient " + getName() + "] Protocol exception occurred. Terminating sesion.,, " + t;
		...
		private String getSecurityCtxInfo(ExtendedGSSContext context) {
			...
			return info + "\nLifeTime:" + context.getLifetime() + "\nA/I/C :" + context.getAnonymityState() + "/" + context.getIntegState()
		...
		private void sendErrMsg(String sErrMsg) {
			try {
				...
				if (logger.isLoggable(Level.SEVERE))
					logger.log(Level.SEVERE, "Got exception while trying to send err message to remote client, Message not sent", e);
			}
		}
		...
		private void sendRetryMsg(String sErrMsg) {
			try {
				monMessage monMessage = new monMessage(AppControlMessage.APP_CONTROL_MSG_AUTH_RETRY, this.sessioID, sErrMsg);
				srcProxy.WriteObject(monMessage, tcpClientWorker.ML_AGENT_MESSAGE);
			} catch (Exception e) {
				if (logger.isLoggable(Level.SEVERE))
					logger.log(Level.SEVERE, "Got exception while trying to send err message to remote client, Message not sent", e);
