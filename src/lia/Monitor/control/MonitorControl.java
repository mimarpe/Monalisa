...
public class MonitorControl extends JFrame  implements ControlI {
    /** The Logger */ 
    private static final transient Logger logger = Logger.getLogger("lia.Monitor.control.MonitorControl");
...
//    private static boolean debug = Boolean.valueOf(AppConfig.getProperty("lia.Monitor.debug", "false")).booleanValue();
	...
	private JMenuItem addModule ,addElement , removeElement ,expand ,showLog, stopServiceMenuItem, restartServiceMenuItem;
	private String addModuleString =" Add Module ";
	...
	private String showLogString =" Show Log ";
...
//        setSize(640,480);
    ...
    public void sendCMDToReflector(String cmd) {
        ...
        if ( monitor == null ) {
            messagePanel.addMessage("monitor == null Please send an Email with this error at: Ramiro.Voicu@cern.ch" , true);
            return;
        }
	...
	public void changeTime(int time){
		...
		if(selectedObject instanceof MNode){
			...
			try{
				...
				messagePanel.addMessage("Changing time for Module "+moduleName+" for Node "+node.toString() , false);
			...
			try{
				messagePanel.addMessage("Changing time for Module "+moduleName+" for Cluster "+selectedObject.toString(),false);
	...
	public void removeElement(Object object){
		...
		if(object instanceof MNode){
			...
			try{
				messagePanel.addMessage(" Removing Node "+node.toString() +"  From Cluster "+node.getCluster().toString(), false);
...
//         }
	...
	public void showSpecialPopup(){
		...
		addModule.addActionListener(new ActionListener(){
					public void actionPerformed(ActionEvent e){
						String host =JOptionPane.showInputDialog(control," Please input host name for Farm Monitor" ,"Subscribing to a Farm Monitor ",JOptionPane.QUESTION_MESSAGE );
						if(host != null){
							subscribe(host);
						}
					}
	...
	public void subscribe(String host){
		...
		try{
			...
			messagePanel.addMessage(" >>>> Successful in subscribing to host "+host , false);
		 	...
		 	JOptionPane.showConfirmDialog(control," Unable to get Farm Monitor at  "+host ," Connect Failure " ,JOptionPane.OK_CANCEL_OPTION,JOptionPane.INFORMATION_MESSAGE);
	...
	class TreeListener extends MouseAdapter {
		public void mouseClicked(MouseEvent me){
            int onmask = InputEvent.SHIFT_DOWN_MASK | InputEvent.ALT_DOWN_MASK | InputEvent.CTRL_DOWN_MASK | InputEvent.BUTTON2_MASK;
            String modifText = InputEvent.getModifiersExText(me.getModifiersEx());
            
            //TODO - NOT TEXT ... BUT IT DOES NOT WORKED !!!
            if ( modifText != null
                    && modifText.indexOf("Ctrl") != -1
                    && modifText.indexOf("Alt") != -1
	...
	class PopupListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent we){
			...
			addDialog.show();
				...
				selectedObject =treePanel.getSelectedComponent();
				if(selectedObject instanceof MFarm){
					inputDialog = new InputDialog(control ," Adding Cluster to "+selectedObject.toString() ," Enter the name of Cluster ");
					inputDialog.show();
					
				}
				else if(selectedObject instanceof MCluster){
					inputDialog = new InputDialog(control ," Adding Node to "+selectedObject.toString() ," Enter the name of Node ");
					inputDialog.show();
				...
				selectedObject =treePanel.getSelectedComponent();
				int answer =JOptionPane.showConfirmDialog(control," Are you sure to delete  "+selectedObject.toString() ," Delete Confirmation " ,JOptionPane.OK_CANCEL_OPTION,JOptionPane.QUESTION_MESSAGE);
