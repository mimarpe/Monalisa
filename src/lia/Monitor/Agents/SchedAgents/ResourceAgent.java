...
public class ResourceAgent extends AbstractAgent {
	...
	/** Logger Name */
	private static final String COMPONENT = "lia.Monitor.Farm.myMon";
	...
	public void doWork() {
		...
		AgentMessage respMessage = createMsg (msgID++,
				SchedAgentMessage.REGISTER, 1, 5, agentInfo.agentAddr, 
		...
		switch ((agMessage.messageType).intValue()) {
			case SchedAgentMessage.REQUEST:
				...
				System.out.println ("### Received request with ID: "+
						((AgentMessage)msg).messageID+" in time:  "+
						(System.currentTimeMillis()-((AgentMessage)msg).timeStamp.longValue())+
						" from " + agentS + " data: " + req);
				...
				AgentMessage respMessage = createMsg (msgID++,
						SchedAgentMessage.RESPONSE, 1, 5, agentS, null, 
						myOffer);
				agentComm.sendMsg(respMessage);
				break;
			...
			case SchedAgentMessage.SUBMIT:
				...
				System.out.println("### Resource agent: " + agentInfo.agentName +
						" received submit request from " + agentS);
