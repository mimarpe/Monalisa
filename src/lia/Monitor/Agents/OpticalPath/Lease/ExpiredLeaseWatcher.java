...
public class ExpiredLeaseWatcher extends Thread {
    ...
    private static final transient Logger logger = Logger.getLogger(ExpiredLeaseWatcher.class.getName());
    ...
    private PriorityQueue<Lease> leases;
    private Object syncLeases;
    ...
    public void renew(Lease lease, long period) {
        ...
        synchronized(syncLeases) {//it does not get sort ... if not removed and added again!!!
            if(leases.remove(lease)) {
                ...
                logger.log(Level.INFO, " [ ExpiredLeaseWatcher ] [ HANDLED ] NoSuch lease " + lease + " in my list ... Maybe it already expired" );
            }
            syncLeases.notify();
        }
    }
    ...
    private ArrayList<Lease> getExpiredLeases(long now) {
        ...
        while(leases.size() > 0) { 
            ...
            if(waitTime <= 0) {
                if(logger.isLoggable(Level.FINEST)) {
                    logger.log(Level.FINEST, "Adding EXPIRED lease " + head + " leases size " + leases.size() + " now: " + new Date(now) + " now " + now + " lease expiretime " + head.getExpireTime());
                }
        ...
        if(logger.isLoggable(Level.FINEST)) {
            logger.log(Level.FINEST, " [ ExpiredLeaseWatcher ] Expired Leases Size " + expiredLeases.size() + " lease size = " + leases.size() );
        }
        ...
        while (hasToRun) {
            ...
            try {
                synchronized(syncLeases) {
                    ...
                    if(logger.isLoggable(Level.FINEST)) {
                        logger.log(Level.FINEST, "\n\n [ ExpiredLeaseWatcher ] ExpiredLeaseWatcher getting expired leases now: [ " + now + " ] ms ... I have " + leases.size() +" leases" );
                    }
                    ...
                    if (expiredLeases.size() == 0) {//nothing expired ...
                        ...
                        long waitTime = head.getExpireTime() - now;
                        if(logger.isLoggable(Level.FINEST)) {
                            logger.log(Level.FINEST, "\n\n [ ExpiredLeaseWatcher ] ExpiredLeaseWatcher will sleep for [ " + waitTime + " ] ms ... I have " + leases.size() +" leases" );
                        }
                        ...
                        if(logger.isLoggable(Level.FINEST)) {
                            ...
                            logger.log(Level.FINEST, "\n\n [ ExpiredLeaseWatcher ] ExpiredLeaseWatcher desired sleep [ " + waitTime + " ] ms ... I have slept for [ " + dtSleep + " ] ms. Leses Size = " + leases.size() +" leases" );
                        }
                    }
                }//sync
                
                if(logger.isLoggable(Level.FINER)) {
                    logger.log(Level.FINER, "\n\n [ ExpiredLeaseWatcher ] Start Removing expired leases ... [ " + expiredLeases.size()); 
                }
