...
public class OSwFSM {
    ...
    private static final transient Logger logger = Logger.getLogger(OSwFSM.class.getName());
        ...
        synchronized(lock) {
            ...
            try {
                if(CROSS_CONNS_TRANSITIONS[oscc.state][newState]) {
                    ...
                    throw new InvalidStateTransitionException("Invalid transition from current port power state: " + oscc.getDecodedState() + " to new state: " +OSwCrossConn.decodeState(newState));
                }
            }finally{
                if(logger.isLoggable(Level.FINE)) {
        ...
        synchronized(lock) {
            ...
            try {
                if(PORT_POWER_TRANSITIONS[port.powerState][newState]) {
                    ...
                    throw new InvalidStateTransitionException("Invalid transition from current port power state: " + port.getDecodedPortPowerState() + " to new state: " + OSwPort.decodePortPowerState(newState));
                }
            } finally {
                if(logger.isLoggable(Level.FINE)) {
                    if(logger.isLoggable(Level.FINER)) {
                        sb.append("\n [ OSwFSM ] Port extended state after transaction [ ").append(port.getExtendedState()).append(" ]");
                    }
                    logger.log(Level.FINE, sb.toString());
                }
            }
