...
//        String pDconn;
    ...
    private class AgentMsgSessionTask implements Runnable {
         ...
         * MCONN - REQ
         */
        private boolean handleMCONN(RemoteAgentMsgWrapper raMsg, long sTime) throws Exception {
...
//                            theAgent.cfg.setNewConfiguration(SyncOpticalSwitchInfo.fromOpticalSwitchInfo(Util.getOpticalSwitchInfo(new StringReader(raMsgs[i].conf))), -1);
    ...
    public MLOpticalPathSession(String sessionID, OpticalPathAgent_v2 theAgent, XDRGenericComm xdrComm, long startTime) {
    ...
    public MLOpticalPathSession(OpticalPathRequest opr, OpticalPathAgent_v2 theAgent, long startTime) {
    ...
    void notifyXDRMessage(XDRMessage xdrMsg, long startTime) {
        synchronized (xdrMsgSessionTask.ll) {
            ...
            if (!xdrMsgSessionTask.running.get()) {
                ...
                OpticalPathAgent_v2.executor.execute(xdrMsgSessionTask);
            }
        }
    }
    ...
    void notifyAgentMessage(AgentMessage am, RemoteAgentMsgWrapper raMsg, long notifTime) throws Exception {
        OpticalPathAgent_v2.executor.execute(new AgentMsgSessionTask(am, raMsg, notifTime));
    }

    void notifyAgentMessage(AgentMessage am, RemoteAgentMsgWrapper[] raMsgs, long notifTime) throws Exception {
        OpticalPathAgent_v2.executor.execute(new AgentMsgSessionTask(am, raMsgs, notifTime));
    }
        ...
        try {
            ...
            for (Enumeration en = opr.links.keys(); en.hasMoreElements();) {
                ...
                if (!agentAddr.equals(theAgent.agentInfo.agentAddr)) {
                    ...
                    lrm.add(lrmLease);
                    Lease lease = new Lease(sessionID, agentAddr, 30 * 1000, MLOpticalPathSession.this, null);
                    logger.log(Level.WARNING, "\n\n [ MCONN ] Added to \nLRM: " + lrmLease + "\nELW: " + lease + " \n");
