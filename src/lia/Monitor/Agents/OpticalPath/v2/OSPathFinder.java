...
class OSPathFinder {
    ...
    private static final transient Logger logger = Logger.getLogger(OSPathFinder.class.getName());
    
    private OpticalPathAgent_v2 theAgent;
    private PriorityQueue remainingVertices;
    private ArrayList nodesInPath;
     ...
     */
    synchronized DNode getShortestPath(final OSwConfig srcOS, final OSwConfig dstOS, final boolean isFdx, final String allowedMLIDCrossConn) throws ProtocolException {
        
        if(logger.isLoggable(Level.FINER)) {
            logger.log(Level.FINER, " [ OSPathFinder ] getShortestPath ( src =" + srcOS.name + ", dst = " + dstOS.name + ", isFDX = " + isFdx + ", addMLID = " + allowedMLIDCrossConn + ")");
        }
        
        if (srcOS.equals(dstOS)) return new DNode(srcOS);
        ...
        do {
            ...
            if( u.oswConfig.osPorts == null ) {
                throw new ProtocolException("DNode has an oswConfig for: " + u.oswConfig.name + " with no OS Ports associated");
            }
            ...
            for (int i=0; i<osPortsLen; i++) {
                ...
                if(oswPort.fiberState == OSwPort.FIBER && oswPort.type == OSwPort.OUTPUT_PORT && oswPort.oswLink != null && oswPort.oswLink.type == OSwLink.TYPE_SWITCH) {

                    //check if the other end is already in the shortest path
                    if(nodesInPath.contains(oswPort.oswLink.destination)) continue;
                    ...
                    if(!OpticalPathAgent_v2.checkPort(oswPort, allowedMLIDCrossConn, u.oswConfig)) continue;
                    ...
                    for(int rpi=0; rpi<remotePortsNo; rpi++) {
                        ...
                        if(tmpRemotePort.name != null && tmpRemotePort.name.equals(oswPort.oswLink.destinationPortName) && tmpRemotePort.type == OSwPort.INPUT_PORT) {
                    ...
                    if(!OpticalPathAgent_v2.checkPort(remotePort, allowedMLIDCrossConn, remoteOSwConfig)) continue;
                    ...
                    if (isFdx) {
                        ...
                        for (int il=0; il<osPortsLen; il++) {
                            ...
                            //check only for outgoing ports which are connected (have fiber) and have other switches at the other end
                            if(tmpLocalPearPort.fiberState == OSwPort.FIBER && tmpLocalPearPort.type == OSwPort.INPUT_PORT && tmpLocalPearPort.name.equals(oswPort.name)) {
                                localPearPort = tmpLocalPearPort;
                                break;
                            }
                        }
                        
                        if(localPearPort == null) continue;
                        if(!OpticalPathAgent_v2.checkPort(localPearPort, allowedMLIDCrossConn, u.oswConfig)) continue;
                    ...
                    double possibleCost = u.distance + ((isFdx)?((oswPort.oswLink.quality + fdxCost)/2.0):oswPort.oswLink.quality);
