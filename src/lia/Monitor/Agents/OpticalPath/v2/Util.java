...
//        return sb.toString();
    ...
    private static final void addOSPortToConfig(OSwPort port, OSwConfig oswConfig, final int lineNo) throws ConfigurationParsingException {
        ...
        for(int i=0; i < oswConfig.osPorts.length; i++) {
            if(port.equals(oswConfig.osPorts[i])) {
                throw new ConfigurationParsingException("The OSPort [ " + port + " ] defined at line " + lineNo + " is already defined");
            }
        }
    ...
    private static final void parseConnMap(String line, OSwConfig oswConfig, final int lineNo) throws ConfigurationParsingException {
        ...
        if(fields.length == 2) {
            ...
            if(fields[1].equalsIgnoreCase("DISCONNECTED_IN")) {
                ...
                throw new ConfigurationParsingException("[ Building Connection Map ] Parsing exception at 2-field line " + lineNo);
            }
            return;
        }
        
        if(fields.length < 4) {
            throw new ConfigurationParsingException("[ Building Connection Map ] Expecting at least 4 fields at line " + lineNo);
        }
        
        String portNameS = fields[0];
        if(portNameS == null || portNameS.length() == 0) {
            throw new ConfigurationParsingException("[ Building Connection Map ] The Source Port Name is null or not defined at line " + lineNo);
        }
        
        String typeS = fields[1];
        if(typeS == null || typeS.length() == 0) {
            throw new ConfigurationParsingException("[ Building Connection Map ] The type for the port " + portNameS + " is null or not defined at line " + lineNo);
        }
        
        String endPointName = fields[2];
        if(endPointName == null || endPointName.length() == 0) {
            throw new ConfigurationParsingException("[ Building Connection Map ] The End Point name is null or not defined for port " + portNameS + " at line " + lineNo);
        }
        ...
        try {
            ...
            logger.log(Level.WARNING, "[ Building Connection Map ] got exception parsing quality at line " + lineNo + ".", t);
            throw new ConfigurationParsingException(t);
        }
        
        if( quality < OpticalLink.MIN_QUAL || quality > OpticalLink.MAX_QUAL) {
            throw new ConfigurationParsingException("[ Building Connection Map ] Wrong quality parameter at line " + lineNo);
        }
        ...
        if(typeS.equalsIgnoreCase("host")) {
            ...
            throw new ConfigurationParsingException("[ Building Connection Map ] The type parameter cannot be understand at line " + lineNo);
        }
            ...
            for(String line = br.readLine(); line != null; line = br.readLine()) {
                ...
                if (line.startsWith("SwitchName")) {
                    ...
                    if(tokens == null || tokens.length != 2) {
                        throw new ConfigurationParsingException("SwitchName parameter is not ( correctly ) defined at line " + lineNo + ".");
                    }
                ...
                if (line.startsWith("SwitchType")) {
                    String tokens[] = line.split("((\\s)*=(\\s)*)|((\\s)+)");
                    if(tokens == null || tokens.length != 2) {
                        throw new ConfigurationParsingException("SwitchType parameter is not ( correctly ) defined at line " + lineNo + ".");
                    }
                ...
                if(line.startsWith("#")) {//comment line
                    if(line.startsWith("#Connection Map")) {//next line will define the connection map
                        if(state == GENERAL_INFO) {
                            if(oswConfig.type == OSwConfig.GLIMMERGLASS || oswConfig.type == OSwConfig.CALIENT) {
                                if(oswConfig.name != null && oswConfig.name.length() != 0) {
                                    ...
                                    throw new ConfigurationParsingException("State Parsing Exception: The \"#Connection Map\" section started but the switch name is not defined!");
                                }
                            } else {
                                throw new ConfigurationParsingException("State Parsing Exception: The \"#Connection Map\" section started but the switch type is not defined or is not a known switch type!");
                            }
                        } else {
                            throw new ConfigurationParsingException("State Parsing Exception: The \"#Connection Map\" section must be defined after the switch name and switch type");
                        }
                        ...
                        if(state == CONNMAP_PARSE) {
                            ...
                            throw new ConfigurationParsingException("State Parsing Exception: The \"#Cross-Connect Links\" section must be defined after the \"#Connection Map\" section");
                        }
                    }
                    continue;
                }//if - comment
