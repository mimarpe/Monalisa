...
public class XDRMessagesHandler implements XDRMessageNotifier {
    ...
    private static final transient Logger logger = Logger.getLogger(XDRMessagesHandler.class.getName());
            ...
            if (!OpticalPathAgent_v2.ping_pong_mode) {
                ...
                if(xdrMsg.olID != null) {
                    if(xdrMsg.olID.equals("NOSENSE")) {// a new session must be created
                        final String sessionID = agent.getAddress() + "//::" + OpticalPathAgent_v2.opticalContor.incrementAndGet();
