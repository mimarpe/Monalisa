...
public class OSAdminImpl extends RangePortUnicastRemoteObject implements OSAdminInterface {
    ...
    public String connectPorts(String sPort, String dPort, String connParams, boolean fullDuplex) throws RemoteException {
        if(logger.isLoggable(Level.FINER)) {
            logger.log(Level.FINER, " changePortState called with sPort ["+sPort+"] :- dPort ["+dPort+"] :- connParams ["+connParams+"] fullDuplex ["+fullDuplex+"]");
        }
        if(parent != null) {
            try {
	...
	public String changeRSVP(String msgRetryInvl, String ntfRetryInvl, String grInvl, String grcvInvl) throws RemoteException {
		if (logger.isLoggable(Level.FINER)) {
			logger.log(Level.FINER, "changeRSVP called with msgRetryInvl ["+msgRetryInvl+"], ntfRetryInvl ["+ntfRetryInvl+"], grInvl ["+grInvl+"], grcvInvl ["+grcvInvl+"]");
		}
		if (parent != null) {
			try {
	...
	public String addCtrlCh(String name, String remoteIP, String remoteRid, String port, String adj, String helloInvl, String helloInvlMin, String helloInvlMax, 
			String deadInvl, String deadInvlMin, String deadInvlMax) throws RemoteException {
		if (logger.isLoggable(Level.FINER)) {
			logger.log(Level.FINER, "addCtrlCh called with name[ "+name+"], remoteIP ["+remoteIP+"], remoteRid ["+remoteRid+"], port ["+port+"], adj ["+
					adj+"], helloInvl["+helloInvl+"], helloInvlMin["+helloInvlMin+"], helloInvlMax["+helloInvlMax+"], deadInvl ["+deadInvl+"], deadInvlMin ["+
					deadInvlMin+"], deadInvlMax ["+deadInvlMax+"]");
		}
		if (parent != null) {
			try {
				return parent.addCtrlCh(name, remoteIP, remoteRid, port, adj, helloInvl, helloInvlMin, helloInvlMax, deadInvl, deadInvlMin, deadInvlMax);
			} catch (Throwable t) {
				return t.getLocalizedMessage();
			}
		}
	...
	public String changeCtrlCh(String name, String remoteIP, String remoteRid, String port, String adj, String helloInvl, 
		...
		if (logger.isLoggable(Level.FINER)) {
			logger.log(Level.FINER, "changeCtrlCh called with name[ "+name+"], remoteIP ["+remoteIP+"], remoteRid ["+remoteRid+"], port ["+port+"], adj ["+
					adj+"], helloInvl["+helloInvl+"], helloInvlMin["+helloInvlMin+"], helloInvlMax["+helloInvlMax+"], deadInvl ["+deadInvl+"], deadInvlMin ["+
					deadInvlMin+"], deadInvlMax ["+deadInvlMax+"]");
		}
		if (parent != null) {
			try {
				return parent.changeCtrlCh(name, remoteIP, remoteRid, port, adj, helloInvl, helloInvlMin, helloInvlMax, deadInvl, deadInvlMin, deadInvlMax);
			} catch (Throwable t) {
				return t.getLocalizedMessage();
			}
		}
	...
	public String addAdj(String name, String ctrlCh, String remoteRid, String ospfArea, String metric, String ospfAdj, String adjType, String rsvpRRFlag, 
		...
		if (logger.isLoggable(Level.FINER)) {
			logger.log(Level.FINER, "addAdj called with name["+name+"] ctrlCh ["+ctrlCh+"] remoteRid ["+remoteRid+"] ospfArea ["+ospfArea+
					"] metric ["+metric+"] ospfAdj ["+ospfAdj+"] adjType ["+adjType+"] rsvpRRFlag ["+rsvpRRFlag+"] rsvpGRFlag ["+rsvpGRFlag+
					"] ntfProc ["+ntfProc+"]");
		}
		if (parent != null) {
			try {
	...
	public String changeAdj(String name, String ctrlCh, String remoteRid, String ospfArea, String metric, String ospfAdj, String adjType, String rsvpRRFlag, 
		...
		if (logger.isLoggable(Level.FINER)) {
			logger.log(Level.FINER, "changeAdj called with name["+name+"] ctrlCh ["+ctrlCh+"] remoteRid ["+remoteRid+"] ospfArea ["+ospfArea+
					"] metric ["+metric+"] ospfAdj ["+ospfAdj+"] adjType ["+adjType+"] rsvpRRFlag ["+rsvpRRFlag+"] rsvpGRFlag ["+rsvpGRFlag+
					"] ntfProc ["+ntfProc+"]");
		}
		if (parent != null) {
	...
	public String changeLink(String name, String localIP, String remoteIP, String linkType, String adj, String wdmAdj, 
		...
		if (logger.isLoggable(Level.FINER)) {
			logger.log(Level.FINER, "changeLink called with name["+name+"] localIP ["+localIP+"] remoteIP ["+remoteIP+"] linkType ["+linkType+"] adj ["+adj+"] wdmAdj ["+wdmAdj+"] remoteIf ["+remoteIf+"] wdmRemoteIf ["+
					wdmRemoteIf+"] lmpVerify ["+lmpVerify+"] fltDetect ["+fltDetect+"] metric ["+metric+"] port ["+port+"]");
		}
		if (parent != null) {
			try {
