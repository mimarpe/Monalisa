...
public class RCMonPanel extends JPanel  implements DataPlotterParent {
    /** The Logger */ 
    private static final transient Logger logger = Logger.getLogger(RCMonPanel.class.getName());


    public String name;
    private LocalDataFarmProvider dataprovider;
    ...
    private tRCFrame parentFrame; 
    private DataPlotterParent thisReference;
    //	private ConfigListener listener;
    Color c;
    private Vector winList = new Vector();
    ...
    public RCMonPanel(LocalDataFarmProvider dataprovider, String name, Hashtable config, tRCFrame parentFrame) {
        ...
        setBorder(BorderFactory.createBevelBorder(BevelBorder.LOWERED,
                                                  Color.white, Color.white, new Color(0, 98, 137), new Color(0,
                                                                                                             59, 95)));
        this.name = name;
        this.config = config;
        thisReference = this;
    ...
    private void reunionStringVectors(Vector base, Vector probe) {
        for (Iterator bit = probe.iterator(); bit.hasNext(); ) {
            String el = (String)bit.next();
            if (!base.contains(el)) base.add(el);
        }
    }
    ...
    void makeBasicTree(MFarm farm) {
        ...
        jscrollpane.setBorder(BorderFactory.createBevelBorder(
...
//            String watchedFarm = "devel_lnx_64";
            ...
            for (Enumeration ec = farm.getClusters().elements(); ec.hasMoreElements();) {
            ...
            for(int tci=0; tci < root.getChildCount(); tci++){
                ...
                if(!set.containsKey(tc.name)){ // this cluster is no longer in config; delete it
                    ...
                    for(Enumeration en = cc.getNodes().elements(); en.hasMoreElements(); ){
                        MNode cn = (MNode) en.nextElement();
                        nset.add(cn.name); 
                    }
                    for(int tni=0; tni<tcn.getChildCount(); tni++){
                        DefaultMutableTreeNode tnn = (DefaultMutableTreeNode)tcn.getChildAt(tni);
    ...
    private class PlotListener implements ActionListener {
        ...
        private void helpSelection(){
            JOptionPane.showMessageDialog(parentFrame, 
                                          "Please select at least a Cluster or at least a Node \n" +
                                          "and one or more parameters to plot!",
                                          "Information",
        ...
        private void buildPredicates(Object source){
            ...
            if(predicates.size() > 0){
                ...
                for(Iterator pit = predicates.iterator(); pit.hasNext(); ){
                    ...
                    if(newParams != null) pred.parameters = newParams;
                }
            }
        }
        ...
        private long getHistoryTime(Object source){
            ...
            for(Iterator cit = clusters.iterator(); cit.hasNext(); ){
                ...
                if (cluster.indexOf("IEPM-BW") != -1 || cluster.indexOf("_WS") != -1 || cluster.indexOf("Abing") != -1) {
                    switch(type){
                        case TYPE_HISTORY:
                            if(rezTime < DEFAULT_HIST_IPERF) rezTime = DEFAULT_HIST_IPERF;
                            break;
                        case TYPE_REALTIME:
                        case TYPE_SUMMARY:
                            if(rezTime < DEFAULT_RT_IPERF) rezTime = DEFAULT_RT_IPERF;
                            break;
                    }
                } else if (cluster.indexOf("VO_") != -1) {
                    switch(type){
                        case TYPE_HISTORY:
                            if(rezTime < DEFAULT_HIST_VOMOD) rezTime = DEFAULT_HIST_VOMOD;
                            break;
                        case TYPE_REALTIME:
                        case TYPE_SUMMARY:
                            if(rezTime < DEFAULT_RT_IPERF) rezTime = DEFAULT_RT_IPERF;
            ...
            if(source == valPanel.show){
                ...
                if(clusterSelected){
                    ...
                    if(valPanel.lastSummaryBtn == valPanel.clusterSummary)
                        crtPlot = new BarClusterSummaryPlot(thisReference, dataprovider, predicates, clusters, summaryType, pieOrBar, currentUnits); 
                    else if(valPanel.lastSummaryBtn == valPanel.nodeSummary)
                        crtPlot = new BarNodeSummaryPlot(thisReference,	dataprovider, predicates, clusters, summaryType, pieOrBar, currentUnits); 
                    else{
