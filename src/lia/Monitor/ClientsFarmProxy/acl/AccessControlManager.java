 ...
 * 
 * $Id: AccessControlManager.java 6865 2010-10-10 10:03:16Z ramiro $
 */
...
public final class AccessControlManager {
    ...
    private static final transient Logger logger = Logger.getLogger(AccessControlManager.class.getName());
    ...
    static {
        ...
        inetHMaps.put("lia.Monitor.ClientsFarmProxy.acl.AccessControlManager.allowedServices", allowedServicesAddresses);
        ...
        stringHMaps.put("lia.Monitor.ClientsFarmProxy.acl.AccessControlManager.allowedServicesNames", allowedServicesNames);
        stringHMaps.put("lia.Monitor.ClientsFarmProxy.acl.AccessControlManager.bannedServicesNames", bannedServicesNames);
    ...
    private static final ArrayList<InetAddress> getInetAddresses(final String[] list) {
        ...
        for(final String sAddress: list) {
            try {
                ...
                logger.log(Level.WARNING, " [ AccessControlManager ] Got exception processing filter addresses list for address: " + sAddress, t);
            }
        }
    ...
    //configuration helper function
    private static final void reloadInetAccessList(final String configProperty, final Set<InetAddress> set) throws Exception {
    ...
    //configuration helper function
    private static final void reloadStringAccessList(final String configProperty, final Set<String> set) throws Exception {
        final String[] confList = AppConfig.getVectorProperty(configProperty);
        if(confList != null && confList.length > 0) {
    ...
    private static final void reloadConfig() {
        ...
        for(final Map.Entry<String, Set<InetAddress>> entry: inetHMaps.entrySet()) {
            ...
            try {
                ...
                logger.log(Level.WARNING, " [ AccessControlManager ] reloadProps got exception processing Key: " + property + "\n The new set is: " + set, t);
            }
        }//end for
                ...
                public void run() {
                    ...
                    for(final AccessControlListener listener: listenersList) {
                        try {
                            ...
                            logger.log(Level.WARNING, " [ AccessControlManager ] [ HANDLED ] Internal Notifier Thread got exception notifying " + listener, t);
                        }
                    }
                }
            }.start();
        ...
        if(allowedSet.size() == 0) {
            if(bannedSet.size() != 0) {
                if(bannedSet.contains(itemToCheck)) {
                    logger.log(Level.WARNING, "[ AccessControlManager ] [ checkAccess ] The item: " + itemToCheck + " is in the banned access list.");
                    return false;
                }
            }
        } else {
            if(!allowedSet.contains(itemToCheck)) {
                logger.log(Level.WARNING, "[ AccessControlManager ] [ checkAccess ] The item: " + itemToCheck + " is not in allowed access list.");
                return false;
            }
        }
        return true;
     ...
     */
    public static final boolean checkServiceAccess(final InetAddress serviceAddress, final String serviceID, final String mlServiceName) {
    ...
    public static final void addListener(final AccessControlListener listener) {
        if(listener == null) throw new NullPointerException("AccessControlManager cannot add null AccessControlListener");
    ...
    public static final void removeListener(final AccessControlListener listener) {
        if(listener == null) throw new NullPointerException("AccessControlManager cannot remove null AccessControlListener");
