 ...
 * 
 * $Id: CompConfigV2.java 6865 2010-10-10 10:03:16Z ramiro $
 * 
 */
...
public class CompConfigV2 {
    ...
    private static final transient Logger logger = Logger.getLogger(CompConfigV2.class.getName());
    ...
    private static MNode[] compareParams(final MNode newNode, final MNode oldNode, final MFarmClientConfigInfo lastConfMon) {
        ...
        if(!newNode.name.equals(oldNode.name)) {
            ...
            logger.log(Level.WARNING, " [ CompConfigV2 ] { BUGGG??? ?! } compareParams: newNode.name = " + newNode.name + " oldNode.name: " + oldNode.name);
            throw new IllegalArgumentException("compareParams: newNode.name = " + newNode.name + " oldNode.name: " + oldNode.name);
        }
    ...
    private static MCluster[] compareNodes(final MCluster newCluster, final MCluster oldCluster, final MFarmClientConfigInfo lastConfMon) {
        ...
        if(!newCluster.name.equals(oldCluster.name)) {
            //  assert ... ? or throw exception
            logger.log(Level.WARNING, " [ CompConfigV2 ] { BUGGG??? ?! } compareNodes: newCluster.name = " + newCluster.name + " oldCluster.name: " + oldCluster.name);
            throw new IllegalArgumentException("compareNodes: newCluster.name = " + newCluster.name + " oldCluster.name: " + oldCluster.name);
        }
...
//        lastConfMon.setParamsCount(cParamsCount);
     ...
     * @return
     */
    public static MFarm[] compareClusters(final MFarm newConf, final MFarm oldConf, final MFarmClientConfigInfo lastConfMon) {
        ...
        if(!newConf.name.equals(oldConf.name)) {
            //  assert ... ? or throw exception
            logger.log(Level.WARNING, " [ CompConfigV2 ] { BUGGG??? ?! } compareClusters: newConf.name = " + newConf.name + " oldConf.name: " + oldConf.name);
            throw new IllegalArgumentException("compareClusters: newConf.name = " + newConf.name + " oldConf.name: " + oldConf.name);
        }
