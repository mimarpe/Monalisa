/*
 * $Id: ExportStatistics.java 6865 2010-10-10 10:03:16Z ramiro $
 */
...
public class ExportStatistics extends Thread {
    ...
    private static final transient Logger logger = Logger.getLogger(ExportStatistics.class.getName());
    ...
    private static final class ApMonParamEntry {
        ...
        public String toString() {
            ...
            sb.append(" ApMonParamEntry - param=").append(paramName).append(", value=").append(paramValue).append(", type=").append(paramType);
            return sb.toString();
        }
    }
    ...
    private ExportStatistics() throws Exception {
        ...
        ApMon tmpApMon = null;
        final String sConfig = AppConfig.getProperty("lia.Monitor.ClientsFarmProxy.apMon", "http://mickyt.rogrid.pub.ro/ProxyMonitorUrl").trim();
    ...
    public void run() {
        ...
        for (;;) {
            try {
                ...
                apm.sendParameter(clusterName, nodeName, apmonParamEntry.paramName, apmonParamEntry.paramType, apmonParamEntry.paramValue);
                if(logger.isLoggable(Level.FINEST)) {
                ...
                try {
                    ...
                    if (logger.isLoggable(Level.FINE)) {
                        logger.log(Level.FINE, " [ ExportStatistics ] got exception sendig param: " + apmonParamEntry, t);
                    }
                }
    ...
    public void addParam(String param, Integer type, Object value) {
        if(_thisInstance == null) {
            if(logger.isLoggable(Level.FINER)) {
                logger.log(Level.FINER, " [ ExportStatistics ] ignoring param " + param + " because the thread was not started");
            }
            return;
        }
        
        if (param == null || type == null || value == null) return;
        final ApMonParamEntry entry = new ApMonParamEntry(param, value, type);
        if(!sendQueue.offer(entry)) {
            if(logger.isLoggable(Level.FINE)) {
                logger.log(Level.FINE, " [ ExportStatistics ] ignoring param " + param + " because the queue is full....");
            }
        } else {
            if(logger.isLoggable(Level.FINER)) {
