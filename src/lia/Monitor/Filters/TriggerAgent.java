...
public class TriggerAgent extends GenericMLFilter {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.Monitor.Filters.TriggerAgent");
	...
	public static final long dt1= Long.valueOf(AppConfig.getProperty("TriggerAgent.NotifyTime", "60").trim()).longValue()*1000;
	...
	private int qualExpiringDelta = 1 * 60 * 1000; 	// after this time, the quality with a peer node expires
    private static long qualSendDelta = 30 * 1000;  // send Qul results every ... 30s
    
    static {
        try {
            //in seconds
            qualSendDelta = Long.valueOf(AppConfig.getProperty("lia.Monitor.Filters.TriggerAgent.qualSendDelta", "30")).longValue()*1000;
        }catch(Throwable t){
