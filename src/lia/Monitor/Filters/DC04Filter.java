...
public class DC04Filter extends GenericMLFilter {
    ...
    /** Logger name */
    private static final transient String COMPONENT = "lia.Monitor.Filters";

    /** Logger used by this class */
    private static final transient Logger logger = Logger.getLogger(COMPONENT);

    //How long should wait for results
    private static final long dtResultTimeout = Long.valueOf(AppConfig.getProperty("lia.Monitor.Filters.DC04Filter.ResultDelay", "20")).longValue() * 1000 * 60;

    //Resend Alert after
    private static final long dtResendTimeout = Long.valueOf(AppConfig.getProperty("lia.Monitor.Filters.DC04Filter.ResendDelay", "360")).longValue() * 1000 * 60;

    private static String Name = "DC04Filter";

    private static final String mailaddress = AppConfig.getProperty("lia.Monitor.Filters.DC04RCPT", "ramiro@cs.pub.ro,ramiro@roedu.net");
    ...
    private void sendMail(String host, String msg) {
        try {
            MailFactory.getMailSender().sendMessage(FarmMonitor.realFromAddress, "support@monalisa.cern.ch", RCPT, "[ DC04 ] Host DOWN @ " + host, msg);
        }catch(Throwable t) {
    ...
    private void verifyNodes() {
        ...
        if ( deadHosts.size() == 0 && rht.size() == 0 ) return;
        ...
        for ( int i = 0; i < deadHosts.size(); i++){
            ...
            sendMail(host, " [ " + new Date() + " ] Host " + host + " did not respond for the last " + dtResultTimeout/(1000*60) +" minutes!" );
        } 
        ...
        for ( Enumeration en = rht.keys(); en.hasMoreElements(); ) {
            ...
            if ( time < now ) {
                ...
                sendMail(host, " [ " + new Date() + " ] Host " + host + " STILL not responding for the last " + dtResendTimeout/(1000*60) +" minutes!" );
                rht.put(host, Long.valueOf(now + dtResendTimeout));
            }
        }
    }//end verifyNodes()
