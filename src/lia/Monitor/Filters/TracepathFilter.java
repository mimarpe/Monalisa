...
public class TracepathFilter implements MonitorFilter, Runnable {
	...
	/** Logger name */
    private static final transient String COMPONENT = "lia.Monitor.Filters.TracepathFilter";
    /** Logger used by this class */
    private static final transient Logger logger = Logger.getLogger(COMPONENT);
    static String Name = "TracepathFilter";
	static String whoisQuery = AppConfig.getProperty("lia.Monitor.Tracepath.WhoisScript" ,"http://monalisa2.cern.ch/cgi-bin/findip");
	static String geoQuery = AppConfig.getProperty("lia.Monitor.Tracepath.GeoScript" ,"http://monalisa2.cern.ch/cgi-bin/findas");
	static String ipidQuery = AppConfig.getProperty("lia.Monitor.Tracepath.IPidScript" ,"http://monalisa2.cern.ch/cgi-bin/topo/addtrace.py");
...
//					logger.log(Level.INFO, "TracepathFilter: received a result from monOldTracepath module!");
	...
	public void finishIt() {
		  ...
		  clients.clear(); clients= null;
		  buff.clear(); buff = null;
		  buff1.clear(); buff1 = null;
		  ans.clear(); ans = null;
		  old_ans.clear(); old_ans = null;

		  ipCache.clear(); ipCache = null;
		  geoCache.clear(); geoCache = null;
		  traceCache.clear(); traceCache = null;
	...
	void update(eResult r){
		if(r.param_name.length == 0){
			eResult rez = new eResult(r.FarmName, r.ClusterName, 
					r.NodeName, Name, r.param_name);
	...
	void check4deadTraces(){
		...
		for(Enumeration en = traceCache.keys(); en.hasMoreElements(); ){
			...
			if(now - res.time > TRACE_EXPIRE_DELAY){
				...
				res.time = now;
				logger.log(Level.INFO, "TracepathFilter: trace info to "+res.NodeName+" has expired. Removing it. "+res.param_name.length);
