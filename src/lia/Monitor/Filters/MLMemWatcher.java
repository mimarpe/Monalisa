...
public class MLMemWatcher extends GenericMLFilter {
    ...
    /** Logger name */
    private static final transient String COMPONENT = "lia.Monitor.Filters.MLMemWatcher";
            ...
            if (a1Count > 10) {
                if (lastA1SentMail + dtResendAlertTime < NTPDate.currentTimeMillis()) {
                    try {
                        ...
                        msg.append("[ ").append(new Date()).append(" / ").append(new Date(NTPDate.currentTimeMillis())).append(" ] ").append(fullName).append(" Mem Status: \n");
            ...
            if (a1Count > 10) {
                if (lastA1SentMail + dtResendAlertTime < NTPDate.currentTimeMillis()) {
                    ...
                    if(RCPT != null && RCPT.length >= 1) {
                        ...
                        logger.log(Level.INFO, "MLMemWatcher Sending Email failed !! No RCPT defined in ml.properties. \n Please add lia.Monitor.Filters.MLMemWatcher.RCPT in ml.properties");
                    }
                    lastA1SentMail = NTPDate.currentTimeMillis();
                }
            }
