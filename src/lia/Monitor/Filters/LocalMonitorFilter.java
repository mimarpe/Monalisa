/*
 * $Id: LocalMonitorFilter.java 6865 2010-10-10 10:03:16Z ramiro $
 */
...
public class LocalMonitorFilter extends GenericMLFilter {
    ...
    private static final transient Logger logger = Logger.getLogger(LocalMonitorFilter.class.getName());
    ...
    public static final String EXTENDED_CMD_STATUS = 
        Utils.getPromptLikeBinShCmd("hostname") +
        Utils.getPromptLikeBinShCmd("hostname -f") +
        Utils.getPromptLikeBinShCmd("hostname -i") +
        Utils.getPromptLikeBinShCmd("free -t -m") +
        Utils.getPromptLikeBinShCmd("uname -a") +
        Utils.getPromptLikeBinShCmd("uptime") +
        Utils.getPromptLikeBinShCmd("netstat -tanp") +
        Utils.getPromptLikeBinShCmd("vmstat 1 10") +
        Utils.getPromptLikeBinShCmd("pstree -u -p") +
    ...
    private void sendMail(String msg) {
        try {
            ...
            PMSender.getInstance().sendMessage("mlstatus@monalisa.cern.ch", RCPT, "[ LocalMonitorFilter ] Status @ " + FarmMonitor.getStandardEmailSubject(), msg, true);
        }catch(Throwable t) {
            if(logger.isLoggable(Level.FINEST)) {
     ...
     */
    public monPredicate[] getFilterPred() {     return null;   }
        ...
        if ( r.ClusterName != null && r.ClusterName.equals("MonaLisa") && r.NodeName != null ) {
            for ( int j = 0; r.param_name != null && j < r.param_name.length; j++ ) {
                if ( r.param_name[j].indexOf("Load5") != -1 ) {
                    ...
                    if(logger.isLoggable(Level.FINEST)) {
                        logger.log(Level.FINEST," notifyResult loadExceeds = " + loadExceeds.get() + " notified " + r.param[j] + " TSh = " + getLoadThreshold());
                    }
                }
            }
        }
    ...
    public static final String getStatus() {
        ...
        Utils.appendExternalProcessStatus(new String[] {"/bin/sh", "-c", EXTENDED_CMD_STATUS}, new String[] { "COLUMNS=200", "PATH=/bin:/usr/bin:/usr/local/bin:/usr/sbin:/sbin:/usr/local/sbin"}, sb);
    ...
    private static void reloadConfig() {
        ...
        synchronized(RCPTsLock) {
            ...
            try {
                includeContactMail = Boolean.valueOf(AppConfig.getProperty("lia.Monitor.Filters.LocalMonitorFilter.includeContactMail", "false")).booleanValue();
            } catch (Throwable t) {
                includeContactMail = false;
                if(logger.isLoggable(Level.FINER)) {
                    logger.log(Level.FINER, " [ LocalMonitorFilter ] Got exc parsing lia.Monitor.Filters.LocalMonitorFilter.includeContactMail ", t);
                }
            }
            ...
            if(includeContactMail) {
                ...
                try {
                    ...
                    if(logger.isLoggable(Level.FINER)) {
                        logger.log(Level.FINER, " [ LocalMonitorFilter ] Got exception parsing MonaLisa.ContactEmail", t);
                    }
                    contactMails = null;
                }
        ...
        try {
            loadThreshold = Double.parseDouble(AppConfig.getProperty("lia.Monitor.Filters.LocalMonitorFilter.Load5_Threshold", "15"));
        } catch (Throwable t) {
            loadThreshold = 15;
        }
