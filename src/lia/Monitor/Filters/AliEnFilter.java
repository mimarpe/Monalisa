...
public class AliEnFilter extends GenericMLFilter implements AppConfigChangeListener {
    ...
    static final transient Logger logger = Logger.getLogger("lia.Monitor.Filters.AliEnFilter");
    ...
    /** 
     * Consider a job from the central services expired after this amount of time (given in ml.prop in seconds) 
    ...
    /** 
     * Consider a FTD transfer from the central services expired after this amount of time (given in ml.prop in seconds)
     * Default value: 1 week 
     */ 
    ...
    /** 
     * Remove a job in zombie state after ZOMBIE_EXPIRE time (given in ml.properties in seconds)
    ...
    /** 
     * get the Sites and SEs from LDAP each LDAP_QUERY_INTERVAL (given in ml.properties seconds)
     * Default value: 2 hours  
     */
    ...
    /** 
     * tells if the JOB_SYNC_SCRIPT and TRASNFER_SYNC_SCRIPT should be run when AliEn filter starts
     * Default value: false 
     */
    static final boolean RUN_JOB_SYNC_SCRIPT = AppConfig.getb("lia.Monitor.Filters.AliEnFilter.RUN_JOB_SYNC_SCRIPT", false);
    
    /** 
     * if RUN_JOB_SYNC_SCRIPT is true, how often we should run the sync scripts
     * Default value: 2 hours  
     */
	...
	final static int [] timeWatchedCSJobStatuses = {
			JobUtil.JS_INSERTING, JobUtil.JS_ASSIGNED, JobUtil.JS_STARTED, 
			JobUtil.JS_RUNNING, JobUtil.JS_SAVING, JobUtil.JS_SAVED,
			JobUtil.JS_DONE, JobUtil.JS_ANY_ERROR};
	
	/** Produce summaries for time deltas from the first to the second CS Job status in this list.
	 * Whatever status appears here MUST be also put in the list above (timeWatchedCSJobStatuses). */
	final static int [][] timeWatchedCSJobTransitions = { 
			{JobUtil.JS_INSERTING,	JobUtil.JS_ASSIGNED},
			{JobUtil.JS_ASSIGNED,	JobUtil.JS_RUNNING},
			{JobUtil.JS_RUNNING,	JobUtil.JS_SAVING},
			{JobUtil.JS_SAVING,		JobUtil.JS_DONE},
			{JobUtil.JS_ASSIGNED,	JobUtil.JS_DONE},
			{JobUtil.JS_INSERTING,	JobUtil.JS_ANY_ERROR},
			{JobUtil.JS_ASSIGNED,	JobUtil.JS_ANY_ERROR},
			{JobUtil.JS_RUNNING,	JobUtil.JS_ANY_ERROR},
			{JobUtil.JS_ASSIGNED,	JobUtil.JS_STARTED},
			{JobUtil.JS_STARTED,	JobUtil.JS_RUNNING},
			{JobUtil.JS_SAVING,		JobUtil.JS_SAVED},
    ...
    private static final class JobUtil {
		...
		/** convert the job status from int to string, as defined in AliEn::Util.pm
		 *     {'INSERTING' => 1,
	     *      'SPLITTING' => 2, 'SPLIT' => 3,
	     *      'QUEUED' => 4,
	     *      'WAITING' => 5, 'ASSIGNED' => 6,
	     *       'STARTED' => 7,
	     *       'INTERACTIV' => 8, 'IDLE' => 9,
	     *       'RUNNING' => 10, 'SAVING' => 11, 'SAVED' => 12,
	     *       'MERGING' => 13, 'FORCEMERGE' => 14,
	     *       'DONE' => 15, 'DONE_WARNING' => 16,
	     ...
	     *       'FAILED' => -13, 'KILLED' => -14, 'ZOMBIE' => -15, 'ERROR_VT' => -16, 'ERROR_RE' => -17};
		 * @param status 
		 * @return String
	     */
		...
		static boolean isCummulativeStatus(final int status){
			return  status == JS_STARTED || status == JS_SAVED || status == JS_SAVED_WARNING ||
					status == JS_ASSIGNED || isFinalStatus(status);
		}
		
		/**
		 * returns true if the given status is final, i.e.
		 * the job can be removed if expired in this state
		 * @param status 
		 * @return boolean
		 */
		static boolean isFinalStatus(final int status){
			return  status == JS_DONE   || status == JS_DONE_WARNING || 
					status == JS_KILLED || status == JS_FAILED;
		}
		...
		static boolean isActiveJob(final int status){
			return status == JS_STARTED || status == JS_RUNNING ||
					status == JS_SAVING || status == JS_SAVED || status == JS_SAVED_WARNING;
		}
		
		
		
		/**
		 * get the position in the status time array for the given status,
		 * or -1 if the given's status time should not be stored.
		 * 
		...
		/**
		 * compares the two given statuses and returns true if they are the same or equivalent.
		 * For example, JS_ANY_ERROR is equivalent to any error status, except for zombie.  
		 * @param status1 
		 * @param status2 
		 * @return boolean
		 */
		...
		private static final int [] csSid = new int[] { JS_INSERTING, JS_SPLITTING, JS_SPLIT, JS_WAITING, // NO QUEUED anymore 
			  JS_TO_STAGE, JS_A_STAGED, JS_STAGING,
			  JS_ASSIGNED, JS_MERGING, JS_FORCEMERGE, JS_DONE, JS_DONE_WARNING, JS_ERROR_A, // DONE for splitted jobs
		...
		private static final int [] siteSid = new int[] { JS_ASSIGNED, JS_STARTED, JS_RUNNING, JS_SAVING, JS_SAVED_WARNING, JS_SAVED, JS_DONE, JS_DONE_WARNING, // DONE for simple jobs
			  JS_ERROR_E, JS_ERROR_IB, JS_ERROR_R, // NO JS_ERROR_S, // NO INTERACTIV and IDLE
			  JS_ERROR_SV, JS_ERROR_V, JS_ERROR_VN, JS_EXPIRED, 
			  JS_FAILED, JS_KILLED, JS_ZOMBIE, JS_ERROR_VT, JS_ERROR_RE};

		
		/** 
		 * Fills the given arrays with the known jobs statuses both with
		 * CS and Site -related statuses. 
		 * @param daCS 
		 * @param daSite 
		 * @param daUser 
		 */
    ...
    /** 
     * Helper class to identify AliEn transfers status. The status
     * information is taken from AliEn Central services. */
    private static class TransferUtil {
    	...
    	/**
    	 * convert the transfer status from int to string, as defined in AliEn::Util.pm
    	 *  'INSERTING' => 1,
         *  'WAITING' => 2,
         *  'ASSIGNED' => 3,
         *  'LOCAL COPY' => 4,
		 ...
		 * @return boolean*/
		static boolean isCummulativeStatus(final int status){
			return (status == TS_DONE 
				 || status == TS_KILLED 
				 || status == TS_FAILED 
				 || status == TS_EXPIRED);
		}

		private static final int [] csSid = new int[] { TS_INSERTING, TS_WAITING, TS_ASSIGNED, TS_LOCAL_COPY,
			  TS_TRANSFERRING, TS_CLEANING, TS_DONE, TS_FAILED, TS_KILLED};
    ...
    private static final class JobAgentUtil {
    	...
    	private static final int [] jaSid = new int[] { AS_REQUESTING_JOB, AS_INSTALLING_PKGS, 
			AS_JOB_STARTED,	AS_RUNNING_JOB, AS_DONE, AS_ERROR_HC, AS_ERROR_IP,
			AS_ERROR_GET_JDL, AS_ERROR_JDL, AS_ERROR_DIRS, AS_ERROR_START}; 
    ...
    private final class JobInfo {
    	...
    	/** 
    	 * return true if the current result refers to the same transfer 
    	 * it's the same transfer if total_size=prev, percent>prev, src_IP=prev, dst_IP=prev
    	 */
    		...
    		if(o instanceof Result){
    			...
    			if(r.ClusterName.equals("Job_XRD_Transfers")){
	    			...
	    			for(int i=r.param_name.length-1; i>=0; i--){
	    				...
	    				}else if(name.equals("cpu_time") || name.equals("cpu_ksi2k") || name.equals("cpu_hep06")
	    						|| name.equals("run_time") || name.equals("run_ksi2k") || name.equals("run_hep06"))
    	...
    	/**
    	 * Handle data about a root job - we are interested in the number of bytes read and 
    	 * their source. We account only bytes read from remote sources.  
    	 */
    	...
    	public boolean summarize(final DataArray sum, final DataArray min, final DataArray max, 
    			final DataArray jobsInState, final Hashtable<String, DataArray> htUserSums){
    		if(logger.isLoggable(Level.FINEST)){
...
//    	long errTime;		// when it went into an error state (but NOT ZOMBIE)
    	...
    	/**
    	 * if job was created after running the sync with alien script, don't
    	 * report cumulative changed statuses the first time summarize runs
    	 */
    	...
    	/**
    	 * Allows initialization of all fields. In this case of JobStatusCS construction, 
    	 * cumulative parameters aren't summarized at the first summarize()'s run to avoid
    	 ...
    	 */
    	public JobStatusCS(final String sJobID, final String sOrgName, 
    			final int iStatus, final String sSubmitHost, final String sExecHost, 
    	...
    	public void setStatusTime(final int iStatus, final long time){
    		...
    		if(idx >= 0 && this.statusTime[idx] == 0)
    			this.statusTime[idx] = time;
    	}
    	
    	/**
    	 * Get the time when entering the given status, or 0 if that time is not stored,
    	 * or job has not reached that status yet.
    	 * @param iStatus 
    	 * @return long
    	 */
		...
		/** 
		 * The received execHost is of the form user@host where user is the account 
		 * where ClusterMonitor is running on that site, and the host is the machine there.
		 * This also tries to determine the exec site.
		 * 
		...
		public void setExecHost(final String sExecHost, final Object oRes){
    		...
    		if(sExecHost.equals("NO_SITE")){
    			...
    			if(nExecSite == null){
    				logger.log(Level.WARNING, "Received a bad execHost="+sExecHost
    						+". Keeping previous execSite="+this.execSite
		...
		/**
		 * Sets the submit host and alien user for this job. It also tries to determine
		 * the submit site.  
		 * @param sSubmitHost 
		 */
    	...
    	/** 
    	 * Add summary data about this job. 
    	 * Returns false if this job should be removed from the hashes. 
    	 * @param htJobsInOrg 
    	 * @param jobsInCS 
    	 * @param htUsersJobs 
    	...
    	public boolean summarize(final Hashtable<String, DataArray> htJobsInOrg, final DataArray jobsInCS, final Hashtable<String, DataArray> htUsersJobs, 
    		...
    		if(logger.isLoggable(Level.FINEST)){
    			logger.log(Level.FINEST, "Summarizing jobStatusCS "
    					+this.jobID+"@"+this.orgName+" submitSite="+this.submitSite
    					+" execSite="+this.execSite);
    		}
    		...
    		if(now - this.lastUpdateTime > PARAM_EXPIRE){
    			...
    			if((this.lastUpdateTime != 0 && now - this.lastUpdateTime > JOB_STATUS_CS_EXPIRE)
    					|| (runTime != 0 && now - runTime > JOB_STATUS_CS_EXPIRE)
    					|| this.notInAliEn ){
					...
					logger.log(Level.WARNING, "LOST JOB: jobID="+this.jobID+"@"+this.orgName
							+" status="+JobUtil.jobStatusToText(this.status)
							+" submitHost="+this.submitHost+" execHost="+this.execHost
							+" lastUpdateTime="+new Date(this.lastUpdateTime)
							+" submitTime="+new Date(getStatusTime(JobUtil.JS_INSERTING))
							+" startTime="+new Date(runTime)
							+" finishTime="+new Date(getStatusTime(JobUtil.JS_DONE))
							+" notInAliEn="+this.notInAliEn);
					// and delete this job info from the hashes
					return false;
        		}
	    				...
	    				if(! this.reportedTransitionsTime[i]){ 
	    					...
	    					if (firstTime > 0 && secondTime > 0){
	    						...
	    						if(secondTime >= firstTime){
	    							...
	    							logger.warning("Invalid times for job "+this.jobID+" status transitions: "+
	    									firstName+"="+new Date(firstTime)+" "+
	    									secondName+"="+new Date(secondTime));
	    						}
    ...
    /**
     * Class used to store FTD transfer status. It is used only in the AliEn Central Services ML.
     * It aggregates data from AliEn TransferManager. 
     */ 
    private final static class TransferStatusCS {
		 ...
		 * @param lFinishTime */
		public TransferStatusCS(final String sTransferID, final String sOrgName, 
				final int iStatus, final long lSize, final String sUser, final String sSrcSE, final String sDstSE, 
				final long lReceiveTime, final long lStartTime, final long lFinishTime){
		...
		/**
		 * merge the parameters of the given 'nTS' TransferStatusCS with the ones
		 * from the current TransferStatusCS. This is called when the ML synchronizes
		 * its view about the current active transfers by querying the AliEn Central
		 * Services. The returned list contains all active transfers. If ML receives
		 * data about a transfer before this synchronization, the status will be updated
		 * only if it's older than the one from AliEn.
		 * @param nTS 
		 */
		...
		/** 
		 * add my data to the summaries regarding: 
		 * - the number of transfers in each state
		 * - rates for completed transfers since last report
		 * it returns false if this transfer has reached a final point and 
		 * should be deleted form the hashes
		 * @param transfStats 
		 * @param transfSEs 
		 * @return boolean
		 */
...
//				tsKey = srcSE+"-"+dstSE;
			...
			if(logger.isLoggable(Level.FINEST)){
		        logger.log(Level.FINEST, "Summarizing transferStatusCS "
		                +this.transferID+"@"+this.orgName+" srcSE="+this.srcSE+" dstSE="+this.dstSE
		                +" state="+this.status+" ("+TransferUtil.transferStatusToText(this.status)+")");
			}
			...
			if(now - this.lastUpdateTime > PARAM_EXPIRE){
				...
				if((this.lastUpdateTime != 0 && now - this.lastUpdateTime > TRANSFER_STATUS_CS_EXPIRE)
						|| (this.startTime != 0 && now - this.startTime > TRANSFER_STATUS_CS_EXPIRE)
						|| this.notInAliEn){
					...
					logger.log(Level.WARNING, "LOST TRANSFER: transferID="+this.transferID+"@"+this.orgName
							+" status="+TransferUtil.transferStatusToText(this.status)
							+" srcSE="+this.srcSE+" dstSE="+this.dstSE+" size="+this.size+" user="+this.user
							+" lastUpdateTime="+new Date(this.lastUpdateTime)
							+" receiveTime="+new Date(this.receiveTime)
							+" startTime="+new Date(this.startTime)
							+" finishTime="+new Date(this.finishTime)
							+" notInAliEn="+this.notInAliEn);
					return false;
				}
			}
    ...
    /** 
     * Class used to store data about ROOT jobs.
     * Currently it suppports the transfers between jobs. 
     */
...
//    				}
    	...
    	/** 
    	 * add summary data about this ROOT job; 
    	 * returns false if this job should be removed from the hashes 
    	 * @param hostStats 
    	 * @param destStats 
    	...
    	/** 
    	 * based on the new data in transf compared to prevTransf, 
    	 * add the moved bytes to all statistics: host, dest, dest2host 
    	 */ 
    	private void summarizeTransfers(final Hashtable<String, DataArray> hostStats, final Hashtable<String, DataArray> destStats, final Hashtable<String, DataArray> destToHostsStats, final double timeInterval){
    	...
    	/** 
    	 * update a DataArray in hash->{key} by adding to the given param the given value.
    	 * If timeInterval is != 0, it also adds a rate for that, for that time interval
    	 */
    	...
    	/**
    	 * based on the new params and stats that were accumulated from previous report
    	 * update the statistics for the host where this job runs
    	 * (events, cputime, runtime, job states) 
    	 */
    	private void summarizeJobParams(final Hashtable<String, DataArray> hostStats, final double timeInterval){
			...
			while (itMissing.hasNext()){
    			...
    			if(s.equals("DONE")) // or other cummulative params
    				sda.addToParam(s+"_jobs_R", 1.0 / timeInterval);
    		}
			
			this.prevStats.setAsDataArray(this.stats);
    		// finally, add the current status, if not cummulative
    		if(this.status != null && (! this.status.equals("DONE"))) // or other cummulative params
    ...
    private final class NodeInfo {
    	...
    	/**
    	 * a few of the job stats - the cummulative ones (saved and all errors)
    	 * note that jobStats is updated by the summarize in JobInfo and it's reset in the NodeInfo's addJobRez
    	 */
...
//    	}
    	...
    	/** 
    	 * Add to the given vector information about the cummulative stats of the jobs currently
    	 * running on this node. This method sends a 0 status before and after any activity to
    	 * ease the integration of these values. 
    	 * @param vrez 
    	 */
    ...
    /**
     * Class used to store information about the RFCP transfers from
     * a single castorgrid server to castor2 using 'rfcp'. The purpose 
     * is to do an aggregation of the raw data (each transfer) and provide
    ...
    private static final class RfcpInfo {
    	...
    	/** 
    	 * Add another traffic data about this castorgrid host.
    	 * The information can be of 4 types: "staging", "migrating", "gc" and "fsfill".
    	 * @param da 
    	 * @param type 
    	 */
    	...
    	/** 
    	 * Summarize data for this node (for last 2 minutes):
    	 * - adds the total sum and rates for migrating and staging transfers
    	 * - adds the average speed of the transfers
    	 * - adds the total number of errors
    	 * - adds the values from the garbage collector
    	 * - sets the maximum value for fsFill (between my and existing (if any)) value
    	 * - sets the max value for gc cache usage
    	 * returns false if this node should be removed from hashes 
    	 * @param sumSummary 
    	 * @param maxSummary 
    	 * @return boolean
    	 */
    	public boolean summarize(final DataArray sumSummary, final DataArray maxSummary){
    		...
    		while (it.hasNext()){
    			...
    			if(paramName.endsWith("_speed") && paramValue > 0)
...
//    	}
    ...
    /** 
     * Class used to summarize the parameters of API services on site. 
     * It generates a sum for each parameter for all the services.
     * Computed data is published in node _TOTALS_, same cluster */
    ...
    private static final class JobAgentInfo {
    	...
    	public void setData(final Result r){
    		...
    		for(int i=r.param_name.length-1; i>=0; i--){
    			...
    			// for now we don't care about the bg monitoring part 
    		}
    		if(sjaID != null)
    			this.jaID = sjaID;
    	...
    	/** Add data about this JobAgent to the given summary.
    	 * Returns false if this JA should be removed from hashes. 
    	 * @param sum 
    	 * @param ttlSum 
    	 * @return boolean */
    ...
    /** 
     * Helper class to hold information about CEs.
     * Currently this is used to aggregate the number of slots/running/queued jobAgents,
     * as the AliEn CE knows.
     */
    private static final class CEInfo {
    	...
    	/** 
    	 * Add data about this CE to the given summary.
    	 * Returns false if this CE should be removed from hashes. 
    	 * @param sumJA 
    	 * @return boolean */
    ...
    /** 
     * Helper class to hold information about DAQs.
     * Currently this is used to aggregate the transferred amount (size and files).
     */
    private static final class DAQInfo {
    	...
    	/** 
    	 * Summarize data about this DAQ
    	 * Returns false if this DAQ should be removed from hashes. 
    	 * @param daqSummary 
    	 * @return boolean
    	 */
    ...
    private static final class FTSTransferInfo {
    	...
    	/** 
    	 * Summarize the data about this transfer by adding all useful data
    	 * to the given hash of (key=channel name; value = DataArray).
    	...
    	public boolean summarize(final Hashtable<String, DataArray> htChannelsSummary){
    		...
    		for(int i=1; i<this.vStatus.size(); i++){
    			daCh.addToParam(this.vStatus.get(i)+"_avg_time", 
    				(this.vTimes.get(i).longValue() - this.vTimes.get(i-1).longValue())/1000.0);
    		}
    		return true;
    	}
    }
    ...
    /**
     * Helper class to store the information and generate summaries about the test FDT transfers.
     * These transfers are always from the current ML service to a given destination.
     * They can be summarized for the "CRT_ML-DEST_ML" or given "linkName" link.
     * The DEST_ML is extracted from the IP address of the destination. 
     */
    ...
    /** 
     * Helper class to hold information about AliEn SE transfers.
     * Currently this is used to aggregate the number of transferred files
     * and the amount of traffic performed by alien (both prompt and jobs).
     * 
    	...
    	public void addData(final String operation, final Result r){
    		...
    		for(int i = r.param.length-1; i >= 0 ; i--){
    			...
    			if(paramName.equals("size")){
    				...
    				boolean success = paramValue > 0.5;
    ...
    /** 
     * Helper class that allows re-reading the configuration by running the
     * LDAP_QUERY_SCRIPT at some time interval. 
     */
    final class ConfigLoader extends TimerTask {
    	...
    	/** 
    	 * Synchronize ML's image with AliEn's image about the currently active jobs.
    	 * This is executed until it succeeds, but no more than 3 attempts are made in a row.
    	 * Then, it is repeated with a period of JOB_SYNC_RUN_INTERVAL. 
    	...
    	private boolean getAliEnJobs(){
    		...
    		if(buff == null){
   			...
   			try{
   				...
   				final ArrayList<JobStatusCS> crtAliEnJobs = new ArrayList<JobStatusCS>(1024);
   				...
   				while((line = buff.readLine()) != null){
    				...
    				if(jobID != null && status != 0 && execHost != null && submitHost != null){
    					crtAliEnJobs.add(
    							new JobStatusCS(jobID, orgName, status, submitHost, execHost, 
    	...
    	/** 
    	 * Synchronize ML's image with AliEn's image about the currently active transfers.
    	 * This is executed until it succeeds, but no more than 3 attempts are made in a row.
    	 * Then, it is repeated with a period of JOB_SYNC_RUN_INTERVAL. 
    	 * @return true if it was successful.   
    	 */
    	private boolean getAliEnTransfers(){
    		...
    		String cmd = ALIEN + " -x " + TRANSFER_SYNC_SCRIPT;
    		BufferedReader buff = procOutput(cmd, JOB_SYNC_SCRIPT_TIMEOUT);
    		if(buff == null){
    			logger.log(Level.WARNING, "Got null OutputStream from "+cmd);
    			return false;
    		}
   			try{
   				...
   				while((line = buff.readLine()) != null){
    				...
    				if(transferID != null && status != 0){
    					crtAliEnTransfers.add(
    							new TransferStatusCS(transferID, orgName, status, size, user, 
    									srcSE, dstSE, receiveTime, startTime, finishTime));
    				}
    				}
    				catch (Exception e){
    					continue;
    	...
    	/**
    	 * Extract useful info from ALiEn LDAP: 
    	 * Sites domains, SE's hosts, Max nr. of jobs in each CE, Sites CEs' hosts. 
    	 * @return true if it was successful.
    	 */
    	private boolean getAliEnConf(){
    		String cmd = ALIEN_PERL+" "+LDAP_QUERY_SCRIPT;
    		logger.log(Level.INFO, "Getting AliEn config with "+cmd);
    		BufferedReader buff = procOutput(cmd, 15 * 60 * 1000); // it can take a long time to resolve the DNS names
    		if(buff != null){
    			String line;
	    			...
	    			while((line = buff.readLine()) != null){
	    				...
	    				if(crt!=null && (crt == sites_domains || crt == ses_hosts || crt == sites_ce_hosts)){
		    				...
		    				if(value.startsWith(">")){
			    				...
			    				for(StringTokenizer stk = new StringTokenizer(value, " "); stk.hasMoreTokens(); ){
		    				...
		    				if(nets.size() > 0){
		    					if(redirect != null){
		    						NetMatcher nm = crt.get(redirect); // this HAS to be there already
		    						if(nm != null)
...
//	    				ses.put("Alice::CERN::MonaLisa", new NetMatcher(new String[] {"monalisa.cern.ch"}));
    	...
    	private void checkVoBoxMon(){
    		...
    		String cmd = ALIEN_PERL+" "+VOBOX_MON_SCRIPT
    				+" >>"+FarmHOME+"/vobox_mon.log"
    				+" 2>>"+FarmHOME+"/vobox_mon.log &";
    		BufferedReader buff = procOutput(cmd, 5 * 1000); // a minute should be enough to read ldap
			if(buff != null){
				try{
					buff.close();
				}catch(Throwable t) {
		...
		public void run(){
    		...
    		if((now - this.jobsLastAttemptTime >= this.jobAttemptInterval) && (RUN_JOB_SYNC_SCRIPT)){
    			...
    			boolean jobsWasSuccessful = getAliEnJobs();
    		...
    		if((now - this.transfersLastAttemptTime >= this.transferAttemptInterval) && (RUN_JOB_SYNC_SCRIPT)){  // I will use the same trigger
    			...
    			boolean transfersWasSuccessful = getAliEnTransfers();
...
//        htSitesJobsStats = new Hashtable();
        if(ALIEN_ROOT != null && ALIEN_ROOT.length() > 0){
        	...
        	Timer tmConf = new Timer(true);
        	tmConf.schedule(new ConfigLoader(), 0, 2 * 60 * 1000);
        }else{
...
/// SITE JOB DATA ///////////////////////////////////////////////////////////////////////////////////////	
	...
	/** 
	 * Search for a job info object, given the jobID (and ceName for faster access).
	 * If create is true and ceName is given, the JobInfo object will be created if not found.
	 */
	private JobInfo findJobInfo(final String ceNameParam, final String jobID, final boolean create){
		...
		if(ceName == null){
			...
			for(Iterator<Map.Entry<String, Hashtable<String, JobInfo>>> iteCE = this.htComputingElements.entrySet().iterator(); iteCE.hasNext(); ){
				final Map.Entry<String, Hashtable<String, JobInfo>> meCE = iteCE.next();
...
/// JOB STATUS CS DATA ///////////////////////////////////////////////////////////////////////////////////////	
	...
	/** 
	 * Search for a job status from CS object, given the jobID and the Organisation Name.
	 * If create is true, the JobStatusCS object will be created if not found.
	 */
	...
	/**
	 * This is called when this filter starts, and then on a regular basis,
	 * with a vector of JobStatusCS objects that describe the jobs currently
	 * running in AliEn. This is done to synchronize the filter's hashes with
	 * the image of the jobs as AliEn sees it. This is needed because dome UDPs
	 * might get lost and therefore ML might report different data from AliEn.
	 * @param crtAliEnJobs 
	 */
	synchronized void addJobStatusCSBulk(final List<JobStatusCS> crtAliEnJobs){
		...
		while (orgIterator.hasNext()){
			...
			for(final Iterator<JobStatusCS> ojit = htJobStats.values().iterator(); ojit.hasNext(); ){
				final JobStatusCS oldJS = ojit.next();
				boolean found = false;
		...
		logger.log(Level.INFO, "Of the loaded jobs "
				+ignoredJobs+" were ignored, "
				+addedJobs+" were added, "
				+updatedJobs+" were updated and "
				+markedForDeletion+" were marked for deletion.");
	}
...
/// TRANSFER STATUS CS DATA (FTD transfers) /////////////////////////////////////////////////////////	
	...
	/** 
	 * Search for a transfer status from CS object, given the transferID and the Organisation Name.
	 * If create is true, the TransferStatusCS object will be created if not found.
	 */
	...
	/**
	 * This is called when this filter starts, or as soon as possible, only once,
	 * with a vector of TransferStatusCS objects that describe the transfers currently
	 * running in AliEn. It will add them (or unknown details) to filter's hashes.
	...
	synchronized void addTransferStatusCSBulk(final List<TransferStatusCS> crtAliEnTransfers){
		...
		for(Iterator<TransferStatusCS> tit = crtAliEnTransfers.iterator(); tit.hasNext(); ){
			...
			if(oTS == null){
				...
				Hashtable<String, TransferStatusCS> htTransferStats = this.htOrgCSTransferStats.get(nTS.orgName);
		...
		logger.log(Level.INFO, "Of the loaded transfers "
				+ignoredTransfers+" were ignored, "
				+addedTransfers+" were added, "
				+updatedTransfers+" were updated and "
				+markedForDeletion+" were marked for deletion.");
	}
...
/// NODE DATA ///////////////////////////////////////////////////////////////////////////////////////	
	...
	/**
	 * Search for a NodeInfo object for the given host (and site, for faster access).
	 * If create is true, and site given, it will also create it.
	 * @param siteParam 
	 * @param host 
	 * @param create 
	 * @return node info
	...
	NodeInfo findNodeInfo(final String siteParam, final String host, final boolean create){
		...
		if(site == null){
			...
			for(Iterator<Map.Entry<String, Hashtable<String, NodeInfo>>> iteS = this.htSites.entrySet().iterator(); iteS.hasNext(); ){
				final Map.Entry<String, Hashtable<String, NodeInfo>> meS = iteS.next();
...
/// NETWORK MATCHING functions to identify Sites, SEs ... //////////////////////////////////////	
	/** 
	 * convert an IP from a double value to a InetAddress. I don't know for sure if
	 * the byte order is good; it might have problems on other machines. 
	 */
	...
	private static String getNetMatch(final Hashtable<String, NetMatcher> matchers, final ReentrantReadWriteLock rwLock, final InetAddress ip){
		...
		if (rwLock!=null){
			...
			try{
				return getNetMatchUnsync(matchers, ip);
			}
			finally{
	...
	static String getNetMatch(final Hashtable<String, NetMatcher> matchers, final ReentrantReadWriteLock rwLock, final String hostName){
		...
		if (rwLock!=null){
			...
			try{
				return getNetMatchUnsync(matchers, hostName);
			}
			finally{
				rwLock.readLock().unlock();
			}
...
//	}
	...
	/**
	 * Add external xrd data regarding a job. By external data we mean job traffic performed
	 * while preparing job's files and while saving the produced files.
	 * @param srcIP 
	 * @param dstIP 
	 * @param transfMBytes 
	 * @param transfFiles 
	...
	/** 
	 * Add traffic data from a job. This produces the following clusters/nodes/params:  
	 ...
	 */
	private void addJobXrdData(final String srcFarmParam, final String dstFarmParam, final String srcSE, final String dstSE, 
								final double transf_mbytes, final double transf_files){
		...
		if(srcFarm.equals(UNKNOWN)){
			...
			else if(! dstFarm.equals(siteName)) // UNK -> other farm. UNK must be this farm
				srcFarm = siteName;
		...
		if(logger.isLoggable(Level.FINER))
			logger.log(Level.FINER, "Got mbytes="+transf_mbytes+" files="+transf_files
					+" from srcFarm="+srcFarm+" dstFarm="+dstFarm
					+" srcSE="+srcSE+" dstSE="+dstSE);
		else if(unknownTransfer || (srcSE == null && dstSE == null))
			logger.log(Level.INFO, "Got unknown transfer mbytes="+transf_mbytes
					+" files="+transf_files+" from srcFarm="+srcFarm+" dstFarm="+dstFarm
					+" srcSE="+srcSE+" dstSE="+dstSE);
	}
...
/// CASTORGRID statistics ///////////////////////////////////////////////////////////////
	...
	/**
	 * Add data about a castorgrid Xrootd 'rfcp' transfer. 
	 * The results should have these 3 kinds of parameters:<br>
...
/// ROOT job data /////////////////////////////////////////////////////////////////////////////
	...
	/** 
	 * Search for a ROOT job info object, given the farmName, jobID and subJobID.
	 * If create is true, the RootJobInfo object will be created if not found.
	 */
...
// API Service data /////////////////////////////////////////////////////////////////////////////
	...
	/** 
	 * Search for a APIService info object, given the farmName, apiService.
	 * If create is true, the APIServiceInfo object will be created if not found.
	 */
...
//	 JobAgent data /////////////////////////////////////////////////////////////////////////////
	...
	/** 
	 * Search for a JobAgent info object, given the farmName, jobAgent.
	 * If create is true, the JobAgentInfo object will be created if not found.
	 */
...
//	 CE data /////////////////////////////////////////////////////////////////
	...
	/** 
	 * Search for a CE info object, given the farmName, ceName.
	 * If create is true, the CEInfo object will be created if not found.
	 */
...
// DAQ data /////////////////////////////////////////////////////////////////
	...
	/** 
	 * Search for a DAQ info object, given the farmName, daqHost.
	 * If create is true, the DAQInfo object will be created if not found.
	 */
...
/// SUMMARIZE functions ///////////////////////////////////////////////////////////////////////
	...
	private void summarizeJobs(final List<Serializable> rez){
		...
		for(Iterator<Map.Entry<String, Hashtable<String, JobInfo>>> cit = this.htComputingElements.entrySet().iterator(); cit.hasNext(); ){
			final Map.Entry<String, Hashtable<String, JobInfo>> cme = cit.next();
			...
			for(Iterator<Map.Entry<String, JobInfo>> jit = htJobsInCE.entrySet().iterator(); jit.hasNext(); ){
			...
			this.knownJobStates.setAsDataArray(jobs); this.knownJobStates.setToZero();
			this.knownJobParams.setAsDataArray(sum); this.knownJobParams.setToZero();
		...
		for(Iterator<Entry<String, DataArray>> uit = htUserSum.entrySet().iterator(); uit.hasNext(); ){
			final Entry<String, DataArray> usme = uit.next();
	...
	/** 
	 * summarize data from all jobs in each ORG and produce a vector with Results.
	 * $ORG_CS_Jobs_Summary
	 *    + jobs -> states of jobs waiting in central services
	 * $ORG_Sites_Jobs_Summary
	 *    + Site1..N -> for each site, states of jobs running .. saving in that site
	 * $ORG_Users_Jobs_Summary
	 *    + User1..N -> for each user, states of submitted jobs (how many jobs has the user in each state)  
	 */
	private void summarizeJobStatusCS(final List<Serializable> rez){
		...
		for(Iterator<Entry<String, Hashtable<String, JobStatusCS>>> oit = this.htOrgCSJobStats.entrySet().iterator(); oit.hasNext(); ){
			final Entry<String, Hashtable<String, JobStatusCS>> ome = oit.next();
			...
			// summarize jobs
			for(Iterator<Entry<String, JobStatusCS>> jit = htJobsInOrg.entrySet().iterator(); jit.hasNext(); ){
				final Entry<String, JobStatusCS> jme = jit.next();
				final JobStatusCS jscs = jme.getValue();
				
				if(! jscs.summarize(htOrgJobsSummary, daOrgCSJobs, htUsersJobsSummary, 
						htOrgSitesStatusTimeDeltas, htUsersStatusTimeDeltas)){
			...
			for(Iterator<Entry<String, DataArray>> sit = htOrgJobsSummary.entrySet().iterator(); sit.hasNext(); ){
				final Entry<String, DataArray> ssme = sit.next();
			...
			for(Iterator<Entry<String, DataArray>> uit = htUsersJobsSummary.entrySet().iterator(); uit.hasNext(); ){
				final Entry<String, DataArray> ume = uit.next();
			...
			for(Iterator<Entry<String, DataArray>> sit = htOrgSitesStatusTimeDeltas.entrySet().iterator(); sit.hasNext(); ){
				final Entry<String, DataArray> ssme = sit.next();
				final String site = ssme.getKey();
				...
				for(int i=0; i<vsParams.length-1; i+=2){
					...
					if(dCNT > 0){
						...
						logger.warning("Found timeDelta with zero cnt for site="+site+" with "
								+sAVG+" = "+dAVG+" and "
								+sCNT+" = "+dCNT);
			...
			for(Iterator<Entry<String, DataArray>> uit = htUsersStatusTimeDeltas.entrySet().iterator(); uit.hasNext(); ){
				final Entry<String, DataArray> usme = uit.next();
				final String user = usme.getKey();
				...
				for(int i=0; i<vsParams.length-1; i+=2){
					...
					if(dCNT > 0){
						...
						logger.warning("Found timeDelta with zero cnt for user="+user+" with "
								+sAVG+" = "+dAVG+" and "
								+sCNT+" = "+dCNT);
						// this should not happen!
						daTimeDeltas.setParam(sAVG, 0);
			...
			// update the list of known job states
			this.knownJobStatusCS.setAsDataArray(daOrgCSJobs); this.knownJobStatusCS.setToZero();
			this.knownJobStatusSite.setAsDataArray(daOrgSitesJobs); this.knownJobStatusSite.setToZero();
		}
	}

	/** 
	 * summarize data from all transfers in each ORG and produce a vector with Results.
	 * $ORG_CS_Transfers_Summary
	 *    + transfers -> states of transfers as in central services
	 * $ORG_SEs_Transfers_Summary
	...
	private void summarizeTransferStatusCS(final List<Serializable> rez){
		...
		for(Iterator<Entry<String, Hashtable<String, TransferStatusCS>>> oit = this.htOrgCSTransferStats.entrySet().iterator(); oit.hasNext(); ){
			final Entry<String, Hashtable<String, TransferStatusCS>> ome = oit.next();
			final String orgName = ome.getKey();
...
//			htOrgTransfersSESummary.clear();
			...
			// summarize transfers
			for(Iterator<Entry<String, TransferStatusCS>> tit = htTransfersInOrg.entrySet().iterator(); tit.hasNext(); ){
			...
			// build results
			for(Iterator<Entry<String, DataArray>> sit = this.htOrgTransfersStatusSummary.entrySet().iterator(); sit.hasNext(); ){
				final Entry<String, DataArray> ssme = sit.next();
...
//					sit.remove();	// we already set it once to zero; if still not updated, we can remove it
			...
			for(Iterator<Entry<String, DataArray>> sit = this.htOrgTransfersSESummary.entrySet().iterator(); sit.hasNext(); ){
				final Entry<String, DataArray> ssme = sit.next();
				final String SE1_SE2 = ssme.getKey();
				final DataArray transfStats = ssme.getValue();
...
//					sit.remove(); // we already set it once to zero; if still not updated, we can remove it
	...
	private void summarizeNodes(final List<Serializable> rez){
		...
		for(Iterator<Entry<String, Hashtable<String, NodeInfo>>> sit = this.htSites.entrySet().iterator(); sit.hasNext(); ){
			final Entry<String, Hashtable<String, NodeInfo>> sme = sit.next();
			...
			for(Iterator<Entry<String, NodeInfo>> nit = htNodesInSite.entrySet().iterator(); nit.hasNext(); ){
	...
	private void summarizeSitesTraffic(final List<Serializable> rez){
		...
		for(Iterator<Entry<String, XrdInfo>> sit = this.htSitesTraffic.entrySet().iterator(); sit.hasNext(); ){
	...
	private void summarizeSEsTraffic(final List<Serializable> rez){
		...
		for(Iterator<Entry<String, XrdInfo>> seit = this.htSEsTraffic.entrySet().iterator(); seit.hasNext(); ){
	...
	private void summarizeCastorgridTraffic(final List<Serializable> rez){
		...
		for(Iterator<Entry<String, RfcpInfo>> csit = this.htCastorgridServers.entrySet().iterator(); csit.hasNext(); ){
	...
	/** 
	 * Summarizes the Root Jobs, providing summaries per host, per read-destination
	 * and per dest-host read details. 
	 */
	private void summarizeRootJobs(final List<Serializable> rez){
		...
		for(Iterator<Entry<String, Hashtable<String, RootJobInfo>>> sit = this.htSitesRootJobs.entrySet().iterator(); sit.hasNext(); ){
			...
			daHostTotals.setToZero(); daDestTotals.setToZero();
			hostStats.clear(); destStats.clear(); destToHostsStats.clear();
			
			// compute the summary for this farm
			int aliveJobs = 0;
			for(Iterator<RootJobInfo> rjit = htRootJobsInFarm.values().iterator(); rjit.hasNext(); ){
			...
			int aliveHosts = 0;
			for(Iterator<Entry<String, DataArray>> hsit = hostStats.entrySet().iterator(); hsit.hasNext(); ){
				final Entry<String, DataArray> hsme = hsit.next();
			...
			for(Iterator<Entry<String, DataArray>> dsit = destStats.entrySet().iterator(); dsit.hasNext(); ){
				final Entry<String, DataArray> dsme = dsit.next();
			...
			for(Iterator<Entry<String, DataArray>> dhsit = destToHostsStats.entrySet().iterator(); dhsit.hasNext(); ){
				final Entry<String, DataArray> dhsme = dhsit.next();
	...
	/** 
	 * Summarize the api services, by adding for each farm a _TOTALS_ node with the
	 * sum for its parameters */
	private void summarizeAPIServices(final List<Serializable> rez){
		final DataArray sum = new DataArray();
		
		for(Iterator<Entry<String, Hashtable<String, APIServiceInfo>>> sit = this.htSitesAPIServices.entrySet().iterator(); sit.hasNext(); ){
			...
			// compute the summary for this farm
			for(Iterator<APIServiceInfo> asit = htAPIServicesInFarm.values().iterator(); asit.hasNext(); ){
	...
	private void summarizeJobAgents(final List<Serializable> rez){
		...
		for(Iterator<Entry<String, Hashtable<String, JobAgentInfo>>> sit = this.htSitesJobAgents.entrySet().iterator(); sit.hasNext(); ){
			final Entry<String, Hashtable<String, JobAgentInfo>> sme = sit.next();
			final String fName = sme.getKey();
			...
			ttlSum.clear();
			// compute the summary for this farm
			for(Iterator<JobAgentInfo> jait = htJobAgentsInFarm.values().iterator(); jait.hasNext(); ){
	...
	private void summarizeCEs(final List<Serializable> rez){
		...
		for(Iterator<Entry<String, Hashtable<String, CEInfo>>> sit = this.htSitesCEs.entrySet().iterator(); sit.hasNext(); ){
			final Entry<String, Hashtable<String, CEInfo>> sme = sit.next();
			final String fName = sme.getKey();
			final Hashtable<String, CEInfo> htCEsInFarm = sme.getValue();
			sumJA.clear();
			// compute the summary for this farm
			for(Iterator<CEInfo> ceit = htCEsInFarm.values().iterator(); ceit.hasNext(); ){
	...
	private void summarizeDAQ(final List<Serializable> rez){
		...
		for(Iterator<Entry<String, Hashtable<String, DAQInfo>>> sit = this.htSitesDAQ.entrySet().iterator(); sit.hasNext(); ){
			...
			for(Iterator<DAQInfo> daqit = htDAQInFarm.values().iterator(); daqit.hasNext(); ){
	...
	private void summarizeFTSTransfers(final List<Serializable> rez){
		...
		for(Iterator<Entry<String, Hashtable<String, FTSTransferInfo>>> ftit = this.htFTSTransfers.entrySet().iterator(); ftit.hasNext(); ){
			...
			for(Iterator<FTSTransferInfo> ftsTrIt = htFTD.values().iterator(); ftsTrIt.hasNext(); ){
		...
		for(Iterator<Entry<String, DataArray>> chit = channelsSummary.entrySet().iterator(); chit.hasNext(); ){
			final Entry<String, DataArray> cme = chit.next();
	...
	private void summarizeFDTTestTransfers(final List<Serializable> rez){
		...
		for(Iterator<FDTTestTransferInfo> ftit = this.htFDTTestTransfers.values().iterator(); ftit.hasNext(); ){
			final FDTTestTransferInfo fdtTr = ftit.next();
			if(! fdtTr.summarize(htFDTLinksSummary))
				ftit.remove();
		}
		for(Iterator<Entry<String, DataArray>> lit = htFDTLinksSummary.entrySet().iterator(); lit.hasNext(); ){
			final Entry<String, DataArray> lme = lit.next();
	...
	private void summarizeQuotaInfos(final List<Serializable> rez){
		...
		for(Iterator<QuotaInfo> qit = this.htQuotaInfos.values().iterator(); qit.hasNext(); ){
		...
		for(Iterator<Entry<String, DataArray>> sit = htQuotaSummary.entrySet().iterator(); sit.hasNext(); ){
			final Entry<String, DataArray> lme = sit.next();
	...
	private void summarizeRootCafUsageInfos(final List<Serializable> rez){
		...
		for(Iterator<Entry<String, Hashtable<String, RootCafUsageInfo>>> cit = this.htRootCafUsageInfos.entrySet().iterator(); cit.hasNext(); ){
			...
			for(Iterator<RootCafUsageInfo> iit = htInfos.values().iterator(); iit.hasNext() ; ){
			...
			for(Iterator<Entry<String, DataArray>> sit=htUserSummaries.entrySet().iterator(); sit.hasNext(); ){
				final Entry<String, DataArray> sme = sit.next();
			...
			for(Iterator<Entry<String, DataArray>> sit=htGroupSummaries.entrySet().iterator(); sit.hasNext(); ){
				final Entry<String, DataArray> sme = sit.next();
	...
	private void summarizeXrdServerInfos(final List<Serializable> rez){
		...
		for(Iterator<XrdServerInfo> sit = this.htXrdServers.values().iterator(); sit.hasNext(); ){
		...
		for(Iterator<Entry<String, DataArray>> sit = htSummaries.entrySet().iterator(); sit.hasNext(); ){
			final Entry<String, DataArray> sme = sit.next();
	...
	private void summarizeSEsAliEnTraffic(final List<Serializable> rez){
		...
		for(Iterator<Entry<String, SETransferInfo>> seit = this.htAliEnSEs.entrySet().iterator(); seit.hasNext(); ){
	...
	synchronized public Object expressResults() {
		...
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeJobs()", t); }
		try{
			summarizeJobStatusCS(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeJobStatusCS()", t); }
		try{
			summarizeTransferStatusCS(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeTransferStatusCS()", t); }
		try{
			summarizeNodes(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeNodes()", t); }
		try{
			summarizeSitesTraffic(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeSitesTraffic()", t); }
		try{
			summarizeSEsTraffic(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeSEsTraffic()", t); }
		try{
			summarizeCastorgridTraffic(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeCastorgridTraffic()", t); }
		try{
			summarizeRootJobs(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeRootJobs()", t); }
		try{
			summarizeAPIServices(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeAPIServices()", t); }
		try{
			summarizeJobAgents(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeJobAgents()", t); }
		try{
			summarizeCEs(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeCEs()", t); }
		try{
			summarizeDAQ(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeDAQ()", t); }
		try{
			summarizeFTSTransfers(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeFTSTransfers()", t); }
		try{
			summarizeFDTTestTransfers(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeFDTTestTransfers()", t); }
		try{
			summarizeQuotaInfos(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeQuotaInfos()", t); }
		try{
			summarizeRootCafUsageInfos(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeRootCafUsageInfos()", t); }
		try{
			summarizeXrdServerInfos(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeXrdServerInfos()", t); }
		try{
			summarizeSEsAliEnTraffic(rez);
		}catch(Throwable t){ logger.log(Level.WARNING, "Error doing summarizeSEsAliEnTraffic()", t); }
		...
		if(logger.isLoggable(Level.FINEST)){
			final StringBuilder sb = new StringBuilder();
			for(Iterator<Serializable> rit = rez.iterator(); rit.hasNext(); ){
				final Result r = (Result) rit.next();
		...
		if (logger.isLoggable(Level.FINE)){
			...
			for (final Iterator<AtomicInteger> it = m.values().iterator(); it.hasNext(); ){
				final AtomicInteger ai = it.next();
			...
			for (final Iterator<Entry<Integer, AtomicInteger>> it = m.entrySet().iterator(); it.hasNext(); ){
				final Entry<Integer, AtomicInteger> me = it.next();
				
				int value = me.getValue().get();
				
				lSum += value;
				
				sb.append(me.getKey()).append("\t->\t").append(value).
					append("\t(").append((lSum*100)/lTotalCount).append("%)\n");
			}
	...
	private static final GenericLastValuesCache<Integer, String> hostCache = new GenericLastValuesCache<Integer, String>() {
		...
		protected String resolve(final Integer key) {
			...
			if (db.moveNext()){
				if (logger.isLoggable(Level.FINE))
					logger.log(Level.FINE, "Resolved "+key+" to "+db.gets(1));
