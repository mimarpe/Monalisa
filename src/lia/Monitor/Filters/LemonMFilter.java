/*
 * $Id: LemonMFilter.java 6865 2010-10-10 10:03:16Z ramiro $
 */
...
public class LemonMFilter extends GenericMLFilter implements Observer {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.Monitor.Filters.LemonMFilter");
    ...
    static {
        ...
        try {
            MF2_SLEEP_TIME = Long.valueOf(AppConfig.getProperty("lia.Monitor.Filters.LemonMFilter.SLEEP_TIME", "60000").trim()).longValue();
        }catch(Throwable t1){
            MF2_SLEEP_TIME = 30 * 1000;
        }
                        ...
                        for (Enumeration e2 = hnodes.keys(); e2.hasMoreElements();) {
                            ...
                            if (endLease < System.currentTimeMillis()) {
                                if(logger.isLoggable(Level.FINE)) {
                                    logger.log(Level.FINE, Name + " Removing node [ " + key + " ] for [ " + clus + ", " + para + " ] ");
                                }
                                hnodes.remove(key);
                            }
                        }
    ...
    void update(Result r) {
        ...
        for (int l = 0;  l < r.param_name.length; l++) {
            for (int kk = 0; kk < interestedParams.length; kk++) {
                String para = interestedParams[kk];
    ...
    public Object expressResults() {
        ...
        if ( v!= null && v.size() > 0) {
            if(dumpResults) {
                ...
                for(int i = 0; i< v.size(); i++) {
                    sb.append("\n").append("GR [ ").append(i).append("] = ").append(GResult2String((Gresult)v.elementAt(i)));
                }
            ...
            if (filterStime > 0 && filterStime < System.currentTimeMillis() ) {
                if(!simulate) {
                    if(logger.isLoggable(Level.FINER)) {
                        logger.log(Level.FINER, " Returning [ " + ((v == null)?"v == null":"v.size() == " + v.size()) + " ]");
                    }
                    return v;
                } 
        ...
        logger.log(Level.INFO, "\n\n Not sending YET the results .... Waiting to fill the hs to grow [ " + ((v == null)?"v == null":"v.size() == " + v.size()) + " ]");
        return null;
    }
    ...
    private void reloadFilterConf() {
	    ...
	    fis.close();
            ...
            try {
                startDelay = Long.valueOf(p.getProperty("lia.Monitor.Filters.LemonMFilter.DelayStart", "120")).longValue()*1000;
            }catch (Throwable t1)  {
                startDelay = 120 * 1000;
            }
            try {
                simulate = Boolean.valueOf(p.getProperty("lia.Monitor.Filters.LemonMFilter.simulate", "false")).booleanValue();
            }catch (Throwable t1)  {
                simulate = false;
            }
            
            try {
                dumpResults = Boolean.valueOf(p.getProperty("lia.Monitor.Filters.LemonMFilter.dumpResults", "false")).booleanValue();
            }catch (Throwable t1)  {
                dumpResults = false;
            }
            ...
            if (newIParam == null) {
                interestedParams = defaultInterestedParams;
                logger.log(Level.WARNING, Name + " reloadFilterConf newIParam == null Setting defaultInterestedParams!!!!!!!!");
                return;
            }
            ...
            if (newIParam.length() == 0) {
                interestedParams = defaultInterestedParams;
                logger.log(Level.WARNING, Name + " reloadFilterConf newIParam.size() == 0 Setting defaultInterestedParams!!!!!!!!");
                return;
            }
            ...
            if (tokens == null || tokens.length == 0) {
                interestedParams = defaultInterestedParams;
                logger.log(Level.WARNING, Name + " reloadFilterConf tokens.size() == 0 Setting defaultInterestedParams!!!!!!!!");
                return;
            }
            ...
            if (np.size() == 0) {
                interestedParams = defaultInterestedParams;
                logger.log(Level.WARNING, Name + " reloadFilterConf newTokens.size() == 0 Setting defaultInterestedParams!!!!!!!!");
                return;
            }
            ...
            try {
                ...
                for(;line != null;line = br.readLine()) {
                    try {
                        ...
                        if(line.startsWith("#") && !line.startsWith("#RemapParam")) continue;
                        ...
                        if(!remapStarted) continue;
                        
                        String ltokens[] = line.split("((\\s)*(=)(\\s)*|(\\s)+)");
                        
                        if (ltokens == null || ltokens.length < 2) continue;
                        
                        if(ltokens[0] == null || ltokens[1] == null) continue;
