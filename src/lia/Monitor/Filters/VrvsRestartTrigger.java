...
public class VrvsRestartTrigger extends GenericMLFilter {
    ...
    /** Logger name */
    private static final transient String COMPONENT = "lia.Monitor.Filters.VrvsRestartTrigger";
    ...
    private static final boolean useRestartCmd = Boolean.valueOf(AppConfig.getProperty("lia.Monitor.useVrvsRestartScript", "false").trim()).booleanValue();

    //should I restart the reflector using 'restartH323Cmd'?
    private boolean useH323RestartCmd = Boolean.valueOf(AppConfig.getProperty("lia.Monitor.useH323RestartScript", "false").trim()).booleanValue();;

    //After how many hours should I resend the mail? (in minutes!!!)
    private static final long dtResendAlertTime = Long.valueOf(AppConfig.getProperty("lia.Monitor.Filters.VrvsRestartTrigger.resendAlertTime", "360").trim()).longValue() * 1000 * 60;

    //but no often than
    private static final long dtResendAlertTime2 = Long.valueOf(AppConfig.getProperty("lia.Monitor.Filters.VrvsRestartTrigger.resendAlertTime", "360").trim()).longValue() * 1000 * 60;
    
    private static final long restartH323Delay = Long.valueOf(AppConfig.getProperty("lia.Monitor.Filters.VrvsRestartTrigger.restartH323Delay", "10").trim()).longValue() * 1000;
    ...
    private static long dt1 = Long.valueOf(AppConfig.getProperty("lia.Monitor.Filters.VrvsRestartTrigger.Alarm1Time", "70").trim()).longValue() * 1000;

    private static long atime1;

    // after 'dt2' trying to restart the reflector!
    private static long dt2 = Long.valueOf(AppConfig.getProperty("lia.Monitor.Filters.VrvsRestartTrigger.Alarm2Time", "200").trim()).longValue() * 1000;

    private static long atime2;

    // after 'dt1' sec sending email
    private static long H323dt1 = Long.valueOf(AppConfig.getProperty("lia.Monitor.Filters.VrvsRestartTrigger.H323Alarm1Time", "90").trim()).longValue() * 1000;

    private static long H323atime1;

    // after 'dt2' trying to restart the reflector!
    private static long H323dt2 = Long.valueOf(AppConfig.getProperty("lia.Monitor.Filters.VrvsRestartTrigger.H323Alarm2Time", "220").trim()).longValue() * 1000;

    private static long H323atime2;

    private static final String rcptMailAddresses = AppConfig.getProperty("lia.Monitor.vrvs.VrvsRestartTrigerRCPT", "mlvrvs@monalisa-chi.uslhcnet.org").trim();
    ...
    private static final long DELAY_START = Long.valueOf(AppConfig.getProperty("lia.Monitor.Filters.VrvsRestartTrigger.startDelay", "120").trim()).longValue()*1000;

    //  Email Stuff
    private static final String contactEmail = AppConfig.getProperty("MonaLisa.ContactEmail", null);

    private static final boolean useContactEmail = Boolean.valueOf(AppConfig.getProperty("include.MonaLisa.ContactEmail", "false").trim()).booleanValue();
    
    private long lastA1SentMail = 0;
    private long lastA2SentMail = 0;

    private long lastA1H323SentMail = 0;
    private long lastA2H323SentMail = 0;
    ...
    private static final String H323VerifyString = AppConfig.getProperty("lia.Monitor.Filters.VrvsRestartTrigger.H323VerifyString", "H323 host(s) connected:");
    private static final int port = Integer.valueOf(AppConfig.getProperty("lia.Monitor.VRVS_port", "46011").trim()).intValue();
        ...
        public void run() {
            try {
                ...
                if (restarted) {
                    ...
                    MailFactory.getMailSender().sendMessage(FarmMonitor.realFromAddress, fromAddr, RCPT, "H323 Restart Status @ "+fullName, sb.toString());
                    if (logger.isLoggable(Level.FINER)) {
    ...
    class H323Verifier extends Thread {
        ...
        public H323Verifier() {
            verifyTime = Long.valueOf(AppConfig.getProperty("lia.Monitor.Filters.VrvsRestartTrigger.H323VerifyTime", "30").trim()).longValue()*1000;
        }
        
        public void run() {
            logger.log(Level.INFO, "H323Verifier started [ " + hostName +":" + port + " @ " + verifyTime/1000 + " s ] ");
...
//        RCPT = new String[] {"ramiro@roedu.net","ramiro@monalisa-chi.uslhcnet.org"};
    ...
    private void alarm1() {
        ...
        try {
            if ( lastA1SentMail + dtResendAlertTime2 < NTPDate.currentTimeMillis() ) {
                ...
                msg.append("[ " + new Date() + " / ").append(new Date(NTPDate.currentTimeMillis())).append(" ] vrvs_mgr seems to be down @ " + fullName);
                appendSystemStatus(msg);
                MailFactory.getMailSender().sendMessage("mlvrvs@monalisa-chi.uslhcnet.org", RCPT, "vrvs_mgr Down @ " + fullName, msg.toString());
    ...
    private void alarm1H323() {
        ...
        try {
            if ( lastA1H323SentMail + dtResendAlertTime2 < NTPDate.currentTimeMillis() ) {
                ...
                StringBuilder msg = new StringBuilder();
                msg.append("[ " + new Date() + " / ").append(new Date(NTPDate.currentTimeMillis())).append(" ] H323 seems to be down @ " + fullName);
                appendSystemStatus(msg);
                MailFactory.getMailSender().sendMessage("mlvrvs@monalisa-chi.uslhcnet.org", RCPT, "vrvs_h323 Down @ " + fullName, msg.toString());
        ...
        if (!useRestartCmd) {
            try {
                ...
                if ( lastA2SentMail + dtResendAlertTime2 < NTPDate.currentTimeMillis() ) {
                    StringBuilder msg = new StringBuilder();
                    msg.append("[ " + new Date() + " / ").append(new Date(NTPDate.currentTimeMillis())).append(" ] vrvs_mgr @ " + fullName + " is down, \nbut it has not been restarted because this option is disabled");
                    appendSystemStatus(msg);
                    MailFactory.getMailSender().sendMessage(FarmMonitor.realFromAddress, fromAddr, RCPT, "vrvs_mgr Down @ " + fullName + "! Restart DISABLED", msg.toString());
        ...
        try {
            ...
            try {
                if (restartCmd != null) {
                    Object reflLock = VrvsUtil.getReflLock(); 
                    synchronized(reflLock) {
                        sb.append("\n" + new Date() + " / " + new Date(NTPDate.currentTimeMillis()) + " Trying to restart using cmd = " + restartCmd);
                        ...
                        procVrvs = null;
                        sb.append("\n\n-----------------------------------------------------\n");
                        sb.append("\n" + new Date() + " / " + new Date(NTPDate.currentTimeMillis()) + " Finish restarting the reflector!");
                        try {
                            Thread.sleep(2000);
                        }catch(Exception e1){}
                        if (logger.isLoggable(Level.FINER)) {
            ...
            appendSystemStatus(sb);
            MailFactory.getMailSender().sendMessage(FarmMonitor.realFromAddress, fromAddr, RCPT, "Restart Status @ "+fullName, sb.toString());
            if (logger.isLoggable(Level.FINER)) {
                logger.log(Level.FINER, "Mail sending terminated");
            }
        } catch (Throwable t) {
    ...
    private void alarm2H323() {
            ...
            if (!useH323RestartCmd) {
                try {
                    logger.log(Level.WARNING, "vrvs_h323 DOWN! Should restart but this option is disalbed! Sending Email...");
                    if ( lastA2H323SentMail + dtResendAlertTime2 < NTPDate.currentTimeMillis() ) {
                        StringBuilder msg = new StringBuilder();
                        msg.append("[ " + new Date() + " / " + new Date(NTPDate.currentTimeMillis()) + " ] vrvs_h323 @ " + fullName + " is down, \nbut it has not been restarted because this option is disabled");
                        appendSystemStatus(msg);
                        MailFactory.getMailSender().sendMessage(FarmMonitor.realFromAddress, fromAddr, RCPT, "vrvs_h323 Down @ " + fullName + "! Restart DISABLED", msg.toString());
                        lastA2SentMail = NTPDate.currentTimeMillis();
                    }
                } catch (Throwable t) {
                    logger.log(Level.WARNING, " Got Exception ", t);
    ...
    public Object expressResults() {
        try {
            if (firstTime) {
                ...
                logger.log(Level.INFO, " alarm1 " + dt1 / 1000 + "s alarm2 " + dt2 / 1000 + "s alarm3 " + "s DELAY_START " + DELAY_START/1000 + "s");
                atime1 = NTPDate.currentTimeMillis() + dt1;
                atime2 = NTPDate.currentTimeMillis() + dt2;
                return null;
            }
