...
public class CienaAlarmTrigger extends GenericMLFilter implements AppConfigChangeListener {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.Monitor.Filters.CienaAlarmTrigger");
    ...
    public Object expressResults() {
        ...
        if (isFinest) {
            logger.log(Level.FINEST, "[ CienaAlarmTrigger ] minAlertTime = " + TimeUnit.NANOSECONDS.toMinutes(minAlertTimeNano) + " minutes, " +
            		"lastAlarmSentNano = " + lastAlarmSentNano + " lastAlarmSent = " + new Date(lastAlarmSent) +
            				"RCPT = " + Arrays.toString(RCPT) + " AISNCS = " + Arrays.toString(AISNCS));
        }
        ...
        if(isFinest) {
            logger.log(Level.FINER, "[ CienaAlarmTrigger ] lastAlarmSentNano=" + lastAlarmSentNano + ", nowNano=" + nanoNow
                       + " dtNanos=" + dtNanos + ", dtNanosInSeconds=" + TimeUnit.NANOSECONDS.toSeconds(dtNanos) 
                       + ", minAlertTimeNano=" +minAlertTimeNano + ", minAlertTimeNanoSeconds=" + TimeUnit.NANOSECONDS.toSeconds(minAlertTimeNano));
        }
        
        if (lastAlarmSentNano > 0 && dtNanos < minAlertTimeNano) {
            if(isFiner) {
                logger.log(Level.FINER, "[ CienaAlarmTrigger ] no new results to analyze minAlertTimeNano not yet reached");
            }
            return null;
        }
        ...
        if (lastResultsTmp.size() > 0) {
            logger.log(Level.INFO, "[ CienaAlarmTrigger ] Notifying " + lastResultsTmp.size() + " alarms: " + lastResultsTmp);

            StringBuilder subj = new StringBuilder(8192);
            StringBuilder sb = new StringBuilder(16384);

            subj.append("Ciena Alarms @ ").append(farmName).append(" - ").append(lastResultsTmp.size()).append(" total");

            boolean bShouldSendMail = false;

            sb.append("\n\nThere are ").append(lastResultsTmp.size()).append(" alarm(s) at ").append(farmName).append(": \n\n");
            ...
            for (int i = 0; i < AISNCS.length; i++) {
                ...
                for (Iterator<eResult> it = lastResultsTmp.iterator(); it.hasNext();) {
                    ...
                    if (er.param.length > 0) {
                        try {
                            ...
                            if (TL1Response.aisnc.equals(AISNCS[i])) {
                ...
                if (almsCount > 0) {
                    sb.append("\n ").append(almsCount).append(" ").append(AISNCS[i]).append(" alarm(s):\n\n");
                    subj.append(" / ").append(almsCount).append(" ").append(AISNCS[i]);
                    sb.append(tsb);
                    sb.append("\n");
                } else {
                    subj.append(" / ").append(0).append(" ").append(AISNCS[i]);
                    sb.append("\n 0 ").append(AISNCS[i]).append(" alarm(s).\n");
                }
            }
            ...
            try {
                MailFactory.getMailSender().sendMessage("mlstatus@monalisa.cern.ch", RCPT, subj.toString(), sb.toString());
                logger.log(Level.INFO, " [ CienaAlarmTrigger ] Notified alarms: lastAlarmSent = " + new Date(lastAlarmSent) +  ", lastAlarmSentNano = " + lastAlarmSentNano + " \n"
                           + "Rcpt: " + Arrays.toString(RCPT) + "\n"
                           + "Subj: " + subj.toString() + "\n"
                           + "Msg: " + sb.toString() + "\n");
            } catch (Throwable t) {
    ...
    private void reloadConf() {
        try {
            minAlertTimeNano = TimeUnit.MINUTES.toNanos(AppConfig.getl("lia.Monitor.Filters.CienaAlarmTrigger.minAlertTime", 15));
        } catch (Throwable t) {
        ...
        try {
            mailaddress = AppConfig.getProperty("lia.Monitor.Filters.CienaAlarmTrigger.RCPT", "ramiro@roedu.net,Ramiro.Voicu@cern.ch").trim();
        } catch (Throwable t) {
        ...
        logger.log(Level.INFO, " minAlertTime = " + TimeUnit.NANOSECONDS.toMinutes(minAlertTimeNano) + " minutes, RCPT = " + Arrays.toString(RCPT) + " AISNCS = " + Arrays.toString(AISNCS));
    }
