...
public class RepositoryWatcher implements MonitorFilter, Runnable {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.Monitor.Filters.RepositoryWatcher");
	...
	triggered = true;
        ...
        "\nNTPDate Time is : "+(new Date(NTPDate.currentTimeMillis())).toString()+"\n"
	    );
	}
	catch (Throwable t) {
	...
	triggered2 = true;
        ...
        "\nLocal Time is : "+(new Date()).toString()+
        "\nNTPDate is : "+(new Date(NTPDate.currentTimeMillis())).toString()+"\n"
	    );
	}
	catch (Throwable t) {
	    logger.log(Level.WARNING, "RepositoryWatcher : got exception sending mail", t);
