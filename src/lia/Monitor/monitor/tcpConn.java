/*
 * Created since very beginning :) $Id: tcpConn.java 7283 2012-07-03 23:55:32Z ramiro $
 */
...
public class tcpConn implements Runnable {
	...
	public static tcpConn newConnection(tcpConnNotifier connNotifier, InetAddress address, int ser_port) throws Exception {
	...
	private tcpConn(tcpConnNotifier connNotifier, Socket socket) throws Exception {
		...
		runningThread.setName("( ML ) tcpConn for " + socket.getInetAddress() + ":" + socket.getPort() + ":-" + socket.getLocalPort());
	...
	public void close_connection() {
		...
		if (notified.compareAndSet(false, true)) {
			...
			try {
				...
				logger.log(Level.WARNING, runningThread.getName() + " got exception notifying the connNotifier that connection was closed", t);
			}
		}
	}
			...
			if (isSending && now > deadLine) {
				logger.log(Level.WARNING,
				        runningThread.getName()
				                ...
				                + TimeUnit.NANOSECONDS.toSeconds(SO_TIMEOUT_NANOS) + " seconds. The connection will be closed!");
				try {
					close_connection();
				} catch (Throwable t) {
		...
		synchronized (oosSyncSend) {
			try {
				if (oos != null) {// Just to avoid NPE
					...
					baos.reset();
					if (COUNT_CONFIG_SENT && (o instanceof MonMessageClientsProxy) && (((MonMessageClientsProxy) o).tag != null)
			...
			if (connected.get()) {
				...
				while (connected.get()) {
					try {
						...
						logger.log(Level.WARNING, localThreadName + " class not found exception. will ignore it. Cause: ", es);
					} catch (IOException ioe) {
						logger.log(Level.WARNING, localThreadName + " IO Exception. will stop! Cause: ", ioe);
						connected.set(false);
						break;
					} catch (Throwable t) {
						logger.log(Level.WARNING, localThreadName + " Got general exception while reading ... will stop! Cause: ", t);
						connected.set(false);
						break;
					}
				}// while
	...
	public String toString() {
		...
		sb.append("tcpConn for: Local :- [ ").append(socket.getLocalAddress()).append(":").append(socket.getLocalPort());
