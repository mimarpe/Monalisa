...
public abstract class snmpMon2 extends SchJob {
    ...
    /** The Logger */
	private static final transient Logger logger = Logger.getLogger(snmpMon2.class.getName());
	...
	protected void init(MNode node) throws SocketException {
		...
		snmpComInterface = new SNMPv1CommunicationInterface(iSNMPVersion, iaRemoteAddress, iRemotePort, sCommunity, iaLocalAddress);

		if (iReadTimeOut == NULL_INT) {
			try {
				iReadTimeOut = Integer.valueOf(AppConfig.getProperty("lia.Monitor.SNMP_timeout", "-1").trim()).intValue();
			} catch (Throwable t) {
				iReadTimeOut = -1;
			}
		}
		...
		if(logger.isLoggable(Level.FINER)) {
		    logger.log(Level.FINER, " [SNMP] snmpComInterface for " + iaRemoteAddress.getHostAddress() + " receiveBufferSize: " + snmpComInterface.getReceiveBufferSize() + " bytes");
		}
	}
	 ...
	 */
	protected Map<String, SNMPObject> snmpBulkGet(final String[] itemIDs) throws SNMPGetException, SNMPBadValueException, IOException {
	 ...
	 */
	protected Map<String, SNMPObject> getMIBTable(final String baseID) throws SNMPGetException, IOException, SNMPBadValueException {
