...
public class ConfigurationTracker {
	 ...
	 */
	public static final long PATHLOAD_CONFIG_TIMER_RUN    = Long.parseLong(
			AppConfig.getProperty("lia.monitor.Modules.monPathload.configTimer", "60")) * 1000;
	...
	private static final transient Logger logger = Logger.getLogger(
			"lia.Monitor.modules.monPathload.ConfigurationTracker");
					...
					if (vec.size() > 1) {
						try {
							...
							logger.log(Level.SEVERE, "[ConfigurationTracker] Found more than one PathloadUrl for " + 
									ConfigurationTracker.PATHLOAD_PEER_GROUP + 
									". Could not get a comparator.");
						}
					} else {
						PathloadUrlDiscoveryJini pud = (PathloadUrlDiscoveryJini) vec.get(0);
						setUrl(pud.getMyUrl());
    ...
    public void setUrl(String value) {
    	...
    	if ((pathloadConfigUrl != null) 
    			&& (pathloadConfigUrl.equals(value))) {
