...
public class PeerCache {
	 ...
	 * My Logger Component
	 */
	private static final transient Logger logger = Logger.getLogger("lia.Monitor.modules.monPathload.peerCache");
	 ...
	 */
	public static final long PATHLOAD_CACHE_TIMER_CONFIG = Long.parseLong(
			AppConfig.getProperty("lia.monitor.Modules.monPathload.configTimer", "30")) * 1000;
	 ...
	 */
	public static final long PATHLOAD_CACHE_TIMER_RUN    = Long.parseLong(
			AppConfig.getProperty("lia.monitor.Modules.monPathload.runTimer", "30")) * 1000;
	 ...
	 */
	public static final double PATHLOAD_CACHE_ERROR_COEF = Double.parseDouble(
			AppConfig.getProperty("lia.monitor.Modules.monPathload.errorCoef", "30"));
	
	/**
	 * 
	 */
	public static final double PATHLOAD_MIN_FLEETS = Integer.parseInt(
			AppConfig.getProperty("lia.monitor.Modules.monPathload.minFleets", "2"));
	 ...
	 * 
	 */
	public static final String PATHLOAD_PEER_GROUP = 
		AppConfig.getProperty("lia.monitor.Modules.monPathload.peerGroup");
	
	/**
	 * 
	 */
	public static final String PATHLOAD_PEER_URL =
		AppConfig.getProperty("lia.util.Pathload.client.PathloadConnector");
	
	/**
	 * 
	 */
	public static final String PATHLOAD_REPORT_ONLY =
		AppConfig.getProperty("lia.monitor.Modules.monPathload.reportOnly");
	
	private static final String ModuleName = "monPathload";
	private static final String ClusterName = "Pathload";
	private static final String[] ResTypes = { "AwBandwidth_Low", "AwBandwidth_High", "MeasurementDuration" , "MeasurementStatus"};
	...
	public boolean start() {
		...
		if (status == PeerCache.PATHLOAD_CACHE_STATUS_RUNNING) {
			logger.log(Level.WARNING, 
					"[monPathload] PeerCache Trying to start, but i'm already running.");			
			return false;
		}
			...
			try {
				...
				if (myFarmName.equals("??")) {
					throw new IllegalArgumentException(
							"[monPathload] I must know my farmName beoynd this point."); 
				}
				...
				if (appPathload == null) {
					...
					if (!appPathload.check()) {
						throw new IllegalArgumentException(
								"[monPathload] AppPathload checks failed. Please verify " + 
				...
				configGet = new ConfigGet(this, 
						myFQDNHostname, 
						myIpAddress,
						myFarmName,
						myGroup);				
				configGetTimer = new Timer(configGet,30 * 1000, 
						PeerCache.PATHLOAD_CACHE_TIMER_CONFIG);
				
				runClient = new RunClient(this);
				runClientTimer = new Timer(runClient, 60 * 1000, 
						PeerCache.PATHLOAD_CACHE_TIMER_RUN);				
								
			    if (PATHLOAD_PEER_URL != null) {
			    	logger.log(Level.INFO, "monPathload will be using Jini services for PathloadConnector Url discovery.");
			    	changeUrl(null, PATHLOAD_PEER_URL);
			    } else if (PATHLOAD_PEER_GROUP != null) {
			    	logger.log(Level.INFO, "monPathload will be using the static definition of the PathloadConnector Url.");
			    	configurationTracker.start();
			    } else {
					throw new IllegalArgumentException(
							"[monPathload] Neither Pathload Url or Pathload PeerGroup is " +
	...
	public boolean stop() {
		if (status == PeerCache.PATHLOAD_CACHE_STATUS_STOPPED) {
			logger.log(Level.WARNING, 
					"[monPathload] PeerCache Trying to stop but i'm not running.");			
			return false;
		}
	...
	public boolean putResult(PathloadResult result) {
		...
		Result r = new Result(
				this.myFarmName,
				PeerCache.ClusterName,
				result.getNodeName(),
				PeerCache.ModuleName,
				PeerCache.ResTypes);
	
		result = correctData(result);
		if ((PATHLOAD_REPORT_ONLY != null) &&
				(result.getExitStat_value() >= 0) &&
		...
		synchronized(lock) {
			if ((result.getExitStat_value() 
				== PathloadResult.MEASUREMENT_STATUS_FAILED) &&
				(isOutOfSync == true)) {
	...
	public boolean setServletResponse(ServletResponse servletResponse) {
		if (servletResponse == null) return false;
			...
			synchronized(lock) {
				if (key.equals("if")) {
					...
					try {
						...
						logger.log(Level.WARNING, "NullPointerException while trying to parse speed ", 
								ex.getMessage());
					}
					if (myMaxSpeed < 0) {
						myMaxSpeed = NOT_SET;
	...
	private PathloadResult correctData(PathloadResult result) {
		if ((myMaxSpeed == NOT_SET) || (result == null)) return result;
		
		if (PATHLOAD_CORRECT_EVIDENT_ERRORS) {
			if ((result.isAwbwHigh()) && (result.isAwbwLow()) &&
					(result.getAwbwHigh_value() == 0.0) &&
					(result.getAwbwLow_value() > 0) &&
					(result.getExitStat_value() == 1)) {
				result.setExitStat_value(5);
				result.setAwbwHigh(false);
			}			
		}
		
		if ((result.isAwbwLow() && result.isAwbwHigh()) &&
				(result.getAwbwLow_value() > result.getAwbwHigh_value())) {
		...
		if (PATHLOAD_CORRECT_EVIDENT_ERRORS) {
			...
			if (result.isAwbwHigh() && result.isAwbwLow()) {
				if ((myMaxSpeed > 600) && ((result.getAwbwHigh_value() - result.getAwbwLow_value()) > 
					((PATHLOAD_CACHE_ERROR_COEF / 100) * myMaxSpeed))) {
			...
			if ((result.isAwbwLow()) && (result.getAwbwLow_value() == 0.0) &&
					(!result.isAwbwHigh())) {
	...
	public void changeUrl(String oldUrl, String newUrl) {
		...
		synchronized(lock) {
			if (oldUrl != null) {
				configGet.shutdown("Changing URL from " + 
						oldUrl + " to " + newUrl);
			}
			...
			logger.log(Level.INFO, "[PeerCache] PathloadConfigURL changed from " +
					oldUrl + " to " + newUrl + ".");
		}
	}
}
