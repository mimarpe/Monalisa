...
public class ConfigGet implements Runnable {
	...
	private static final transient Logger logger = Logger.getLogger("lia.Monitor.modules.monPathload.configGet");
	
	private int status;
	private ServletResponse servletResponse;
	private PathloadStub stub;
	 ...
	 * @param farmGroups
	 */
	public ConfigGet(PeerCache peerCache, String hostname, String ipAddress, 
		...
		this.stub = new PathloadStub(hostname, ipAddress, farmName, 
				farmGroups);
	...
	public void run() {		
		logger.log(Level.FINEST, 
				"[monPathload] ConfigGet Running config thread.");
		...
		switch(status) {
			case ConfigGet.STATUS_DO_NOTHING: break;
	...
	private void requestToken() {
		try {
			logger.log(Level.FINEST, 
					"[monPathload] ConfigGet Requesting Token.");
			...
			if (servletResponse != null) {
				...
				if (servletResponse.hasToken()) {
					...
					logger.log(Level.FINE, 
							"[monPathload] ConfigGet Got token: " + 
							servletResponse.toString());
					peerCache.gotToken();
				}
				if (servletResponse.hasMyGroup() &&
				...
				logger.log(Level.FINEST, 
						"[monPathload] ConfigGet Token request denied.");
			}
			...
			logger.log(Level.SEVERE, "[monPathload] ConfigGet - runState " +
					e.getMessage(), new Object[] { logEv });
		}
	}
		...
		if (!peerCache.mustReleaseToken()) {			
			...
			if ((servletResponse == null) ||
				...
				logger.log(Level.SEVERE, 
						"[monPathload] ConfigGet Acquired token, yet token is now null.");						
			} else {
				try {
