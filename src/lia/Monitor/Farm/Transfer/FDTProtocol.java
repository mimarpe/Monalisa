...
public class FDTProtocol extends TransferProtocol implements ShutdownReceiver {
	...
	public final static Logger logger = Logger.getLogger(FDTProtocol.class.getName());
	...
	private AtomicInteger serverUsage;
	
	/** Handler for the log cleaner task */
	private ScheduledFuture sfLogCleaner;
	
	/** Handler for the LISA XDR module commands */
	private XDRCommandHandler xdrCommandHandler;
	...
	public FDTProtocol() {
		...
		sfLogCleaner = MonALISAExecutors.getMLHelperExecutor().scheduleWithFixedDelay(new LogCleaner(), 1, 1, TimeUnit.MINUTES);
	}
	...
	public void updateConfig() {
		...
		if((fdtServer != (serverInstance != null)) 
				|| (! serverOptions.equals(fdtOptions)) || (! fdtBasedir.equals(basedir))
				|| (serverPort != fdtPort) || (lisaFdtPort != fdtLisaPort)
				) {
