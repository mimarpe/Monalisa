...
class LinkRequestInstance {
	...
	public synchronized boolean checkStatus(StringBuilder activeList) {
		...
		if(lastUpdateTime + REQUEST_TIMEOUT < now){
			logger.warning("Request "+requestID+" not updated for more than "+REQUEST_TIMEOUT/1000+" sec. Stopping it.");
			stop();	// if it's active for more than the allowed period
		}
		if(logger.isLoggable(Level.FINEST))
			logger.finest("Status for request "+requestID+" is active="+active
					+" running for: "+(now-startTime)/1000
					+" sec; not updated for: "+(now-lastUpdateTime)/1000+" sec;"
					+" time to live: "+(lastUpdateTime+REQUEST_TIMEOUT-now)/1000+" sec.");
