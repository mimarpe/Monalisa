...
public class FDTInstance implements ProtocolInstance {
	...
	private ProcessRunner prBackgroundProcess = new ProcessRunner();
	 ...
	 */
	public FDTInstance(String transferID, String destIP, String fdtOptions, long maxSpeed, String files, long periodSeconds) {
	...
	public boolean start() {
		try {
			...
			logger.log(Level.WARNING, "Error starting FDT "+(transferID == null ? "server." : "transfer "+transferID), ioe);
			return false;
		}finally{
			started = true;
		}
	...
	public boolean stop() {
		try {
			...
			logger.log(Level.WARNING, "Error stopping FDT "+(transferID == null ? "server." : "transfer "+transferID), ioe);
			return false; 
		}
		return true;
	}
	...
	public boolean checkStatus(List lResults) {
		...
		if(mem2mem){
			...
			Result r = new Result(TransferUtils.farmName, "FDT_Transfers", transferID, TransferUtils.resultsModuleName,
					new String [] { "period", "timeleft"}, 
					new double [] { period / 1000, timeLeft / 1000 });
			r.time = now;
			lResults.add(r);
