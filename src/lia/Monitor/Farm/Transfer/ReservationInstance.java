...
class ReservationInstance implements ProtocolInstance {
	...
	private String rsvID;		// reservation ID
	private long bw;			// bandwidth allocated to this reservation, in bits/s
	private long period;		// period for which this reservation is made, in millis
	...
	public boolean checkStatus(List lResults) {
		...
		Result r = new Result(TransferUtils.farmName, "RSV_Transfers", rsvID, TransferUtils.resultsModuleName,
				new String [] { "bandwidth_Mb", "period", "timeleft"}, 
				new double [] { active ? bw / 1000.0 / 1000.0 : 0.0, period / 1000, timeLeft / 1000 });
		r.time = now;
		lResults.add(r);
		if(! active)
