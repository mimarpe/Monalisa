...
public abstract class TransferProtocol implements DropEvent {
	...
	private ScheduledFuture sfStatusChecker;
	...
	public TransferProtocol(String protocolName) {
		...
		sfStatusChecker = MonALISAExecutors.getMLHelperExecutor().scheduleWithFixedDelay(new StatusChecker(), delay, lStatusCheckSeconds, TimeUnit.SECONDS);
	}
	...
	public void setConfig(Properties allProperties) {
		...
		for(Iterator pit = allProperties.entrySet().iterator(); pit.hasNext(); ) {
