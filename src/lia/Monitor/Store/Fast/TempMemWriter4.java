/*
 * $Id: TempMemWriter4.java 7128 2011-03-14 09:45:52Z costing $
 */
...
public final class TempMemWriter4 extends Writer implements TempMemWriterInterface {
	...
	/** The Logger */
	static final transient Logger	logger			= Logger.getLogger("lia.Monitor.Store.Fast.TempMemWriter4");
	...
	public TempMemWriter4() {
        ...
        MonALISAExecutors.getMLStoreExecutor().scheduleWithFixedDelay(new TempMemWriterTask(), 500, 1000, TimeUnit.MILLISECONDS);
	 ...
	 */
	private void addData(final long rectime, final Result r, final String sParamName, final double mval, final double mmin, final double mmax) {
		...
		public void run() {
			synchronized (sync) {
				...
				if (((free < t) && (iLimit > 16)) || (iLimit > iHardLimit)) {
					...
					while ((free > t * 2) && (dataCount > ((2 * iLimit) / 3) || iLimit - dataCount < 1000) && (iLimit < iHardLimit))
						iLimit = (iLimit * 3) / 2;
				}

				if (logger.isLoggable(Level.FINEST)) {
					logger.log(Level.FINEST, "Readjusted limit to " + iLimit + " (" + iOldLimit + ")/" + iHardLimit + " because : " + free + "/" + total + " : " + dataCount);
				}

				if (iOldLimit != iLimit && logger.isLoggable(Level.FINE)) {
					logger.log(Level.FINE, "Readjusted limit to " + iLimit + " (" + iOldLimit + ")/" + iHardLimit + " because : " + free + "/" + total + " : " + dataCount);
				}
				...
				if (dataCount > (11 * iLimit) / 10 || free < 5 * 1024 * 1024) {
					...
					if (logger.isLoggable(Level.FINEST)) {
						logger.log(Level.FINEST, "TempMemWriter4 cleanup took : " + (NTPDate.currentTimeMillis() - lStart) + " ms for " + lNumberOfValuesToRemove + " values in " + hmData.size() + " series");
					}
	 ...
	 * list heads.
	 */
	private TreeSet<ExtendedResultPointer>	llStartPointers	= new TreeSet<ExtendedResultPointer>();
	...
	final boolean removeHead(final int numberOfValuesToRemove) {
		if (logger.isLoggable(Level.FINEST)) {
			logger.log(Level.FINEST, " TempMemWriter4 entering removeHead [ " + numberOfValuesToRemove + " ] ... llStartPointers.size() == " + llStartPointers.size() + " dataCount == " + dataCount);
		}
		...
		if (logger.isLoggable(Level.FINEST)) {
			logger.log(Level.FINEST, " TempMemWriter4 exiting removeHead [ " + iNumberOfValuesToRemove + " ] ... llStartPointers.size() == " + llStartPointers.size() + " dataCount == " + dataCount);
		}
	...
	private final class MemDataSplitter extends DataSplitter {
		...
		@Override
		public Vector<Object> getAndFilter(monPredicate pred) {
