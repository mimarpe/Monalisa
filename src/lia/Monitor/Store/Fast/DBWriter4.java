/*
 * $Id: DBWriter4.java 6865 2010-10-10 10:03:16Z ramiro $
 */
...
public final class DBWriter4 extends Writer {
	...
	/** Logger */
	static final Logger	logger = Logger.getLogger("lia.Monitor.Store.Fast.DBWriter4");
	 ...
	 * @param _iWriteMode
	 */
	public DBWriter4(final long _lGraphTotalTime, final long _lGraphSamples, final String _sTableName, final int _iWriteMode) {
		...
		if (iWriteMode==DB4_MODE_COMPACT && !bCompact){
			logger.log(Level.WARNING, "For "+sTableName+" you selected the compact mode but didn't specify a number of samples. Using raw mode instead.");
		}
	...
	public boolean cleanup(final boolean bCleanHash) {
		...
		if (bDelete) iOperationsCount++;
		if (bAnalyze) iOperationsCount++;
		if (bCluster) iOperationsCount++;
		if (bReindex) iOperationsCount++;
		...
		new Thread() {
			...
			public void run() {
				...
				logger.log(Level.INFO, "DB4 CLEANUP START ('" + sTableName + "', del=" + bDelete + 
						", cluster="+bCluster+
						", analyze="+bAnalyze+
						", reindex="+bReindex+")");
				...
				try {
					...
					final DB db = new DB("SELECT tablename FROM pg_tables WHERE tablename LIKE '" + sTableName + "_%';");

					final ArrayList<String> alTables = new ArrayList<String>(db.count());
					...
					for (int i = alTables.size() - 1; i >= 0; i--) {
						...
						if (bDelete) {
							if (execMaintenance("DELETE FROM " + sName + " WHERE rectime<" + lDelThreshold + ";", true, db)) {
								...
								if (iUpdateCount==0){
									...
									if (db.query("SELECT count(1) FROM "+sName+";") && db.moveNext() && db.geti(1, -1) == 0){
						...
						if (bLocalCluster || bCluster) {
							if (!execMaintenance("CLUSTER " + sName + "_time_idx ON " + sName + ";", true, db)) {
								if (!execMaintenance("CREATE INDEX " + sName + "_time_idx ON " + sName + "(rectime);", true, db))
									continue;
					...
					logger.log(Level.INFO, "DB4 CLEANUP ('" + sTableName + "', del=" + bDelete + " (" + lTotalDelete + ", " + iDeletedTables + " tables touched), cluster=" + 
							bCluster + " (" + iClusteredTables + " operations), analyze=" + 
							bAnalyze + " (" + iAnalyzedTables + " operations), reindex="+
							bReindex+" ("+iReindexedTables+" operations), empty tables: "+
							iTestedTables+" tests, "+iDropedTables+" droped, "+
							"took: " + (NTPDate.currentTimeMillis() - lNow) / 60000d + " minutes (" + (lTotalActualTime / 60000d) + " work minutes)");
