...
public final class CacheElement implements Serializable {
	...
	private static final String	COMPONENT			= "lia.Monitor.Store.Fast.CacheElement";
	 ...
	 */
	public CacheElement(final long _lInterval, final Result result, final int _iParam, final long lTime, final boolean bWrite, final WriterInterface w) {
		r = result;
		iParam = _iParam;
	 ...
	 * @return true if the value was taken into account, false if it was ignored
	 */
	public synchronized boolean update(final double dNewValue, final double dNewMin, final double dNewMax, final long lUpdateTime, final boolean bWrite) {
		...
		if (lUpdateTime - lLastWritten >= lInterval) { // we should write a new value to the database
			final double dIntersect = dPrevValue + (dNewValue - dPrevValue) * (lLastWritten + lInterval - lLastUpdate) / (lUpdateTime - lLastUpdate);

			final double avg = (dValue * (lLastUpdate - lLastWritten) + ((dPrevValue + dIntersect) / 2) * (lLastWritten + lInterval - lLastUpdate)) / lInterval;
			...
			if (lUpdateTime > lLastWritten) {
				avg = (dValue * (lLastUpdate - lLastWritten) + ((dNewValue + dPrevValue) / 2) * (lUpdateTime - lLastUpdate)) / (lUpdateTime - lLastWritten);
			} else {
