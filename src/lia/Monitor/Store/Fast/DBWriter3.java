/*
 * $Id: DBWriter3.java 6865 2010-10-10 10:03:16Z ramiro $
 */
...
public final class DBWriter3 extends Writer {
	/** Logger Name */
	private static final String			COMPONENT				= "lia.Monitor.Store.Fast";
	 ...
	 */
	static boolean						shouldCompactTables		= Boolean.valueOf(AppConfig.getProperty("lia.Monitor.shouldCompactTables", "true")).booleanValue();

	/** Total table time */
	long								lGraphTotalTime;			// in milliseconds

	private long						lGraphSamples;			// integer, >0
	...
	static {
		try {
			...
			if (sPrefferDouble != null && sPrefferDouble.length() > 0 && (sPrefferDouble.startsWith("1") || sPrefferDouble.toLowerCase().startsWith("t")))
	 ...
	 * @throws IOException
	 */
	public DBWriter3(long _lGraphTotalTime, long _lGraphSamples, String _sTableName, int _iWriteMode) throws IOException {
		...
		if (cleanupInterval < 1000 * 60 * 60 * 24) // cleanups with min 1 day between them
	...
	private final void readPreviousValues() {
		...
		synchronized (mLock) {
			...
			if (m == null) {
				logger.log(Level.INFO, "DBWriter3 (" + sTableName + ") : falling back to reading last values from the table");
				m = new HashMap<Object, CacheElement>();
				...
				for (int i = 0; i < vIDs.size(); i++) {
					...
					db.query("SELECT rectime,mval FROM " + sTableName + "_" + id + " ORDER BY rectime DESC LIMIT 1;", true);
	...
	public final boolean cleanup(final boolean bCleanHash) {
		...
		final boolean bShouldCluster = (now - lastCompactTime > 2 * cleanupInterval) && (now - lastCompactTime > 1000 * 60 * 60 * 72);
		...
		if (lastCleanupTime + cleanupInterval < now && !bCleanupStarted) {
			new Thread("(ML) DB3 cleaner for "+sTableName) {
				...
				public void run() {
					...
					if (bDBMaintenance){
						if (!execMaintenance("REINDEX SYSTEM "+AppConfig.getProperty("lia.Monitor.DatabaseName", "mon_data")+";", true, db))
							execMaintenance("REINDEX DATABASE " + AppConfig.getProperty("lia.Monitor.DatabaseName", "mon_data") + ";", false, db);
					}

					logger.log(Level.INFO, "DB3 CLEANUP FOR " + sTableName + "  TOOK : " + ((NTPDate.currentTimeMillis() - now) / 1000) + "sec for " + vIDs.size() + " tables");
