/*
 * $Id: TempMemWriter2.java 7128 2011-03-14 09:45:52Z costing $
 */
...
public final class TempMemWriter2 extends Writer implements TempMemWriterInterface {
	...
	/** The Logger */
	static final transient Logger	logger			= Logger.getLogger("lia.Monitor.Store.Fast.TempMemWriter2");
	...
	final class TempMemWriterTask implements Runnable {
		...
		public void run() {
			synchronized (hmData) {
				...
				if (iLimit != iOldLimit) {
					logger.log(Level.INFO, "Readjusted limit to " + iLimit + " (" + iOldLimit + ")/" + iHardLimit + " because : " + free + "/" + total + " : " + dataCount);
				}
				...
				if (dataCount > (11 * iLimit) / 10) {
					...
					if (logger.isLoggable(Level.FINEST)) {
						logger.log(Level.FINEST, "TempMemWriter2 cleanup took : " + (NTPDate.currentTimeMillis() - lStart) + " ms for " + lNumberOfValuesToRemove + " values in " + hmData.size() + " series");
					}
	 ...
	 */
	private LinkedList<Pointer>	llStartPointers	= new LinkedList<Pointer>();
