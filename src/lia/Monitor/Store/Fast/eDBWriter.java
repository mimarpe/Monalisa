...
public class eDBWriter extends Writer implements AppConfigChangeListener {
	...
	/** The Logger */
	private static final Logger	logger				= Logger.getLogger("lia.Monitor.Store.Fast.eDBWriter");

	private String				driverString		= "org.postgresql.Driver";

	private boolean				shouldAnalyzeTables	= false;

	private long				lGraphTotalTime;															// in milliseconds
	...
	public boolean setOnline(final long now) {
		...
		db.query("INSERT INTO monitor_tables_onlinetime (mt_tablename, online_since) VALUES ('" + sTableName + "'," + now + ")");
		
		analyzeTable();

		if (logger.isLoggable(Level.FINE)) {
			logger.log(Level.FINE, " [ eDBW ] " + sTableName + " BECOME ONLINE :) ! [ " + (NTPDate.currentTimeMillis() - now) + " ] ");
		}
	...
	private final void initDBStructure() {
		...
		if (driverString.indexOf("mckoi") == -1) {
			if (driverString.indexOf("postgres") >= 0) {
				bCreate = db.query("CREATE TABLE " + sTableName + " (" + "rectime bigint," + "mfarm text," + "mcluster text," + "mnode text," + "mfunction text," + "mval text" + ") WITHOUT OIDS;", true);
				...
				bCreate = db.query("CREATE TABLE " + sTableName + " (" + "rectime BIGINT," + "mfarm VARCHAR(100)," + "mcluster VARCHAR(100)," + "mnode VARCHAR (100)," + "mfunction VARCHAR(100)," + "mval LONGBLOB" + ") TYPE=InnoDB;", true);
			}
		}
		else {
			bCreate = db.query("CREATE TABLE " + sTableName + " (" + "rectime BIGINT," + "mfarm VARCHAR(100) INDEX_NONE," + "mcluster VARCHAR(100) INDEX_NONE," + "mnode VARCHAR (100) INDEX_NONE," + "mfunction VARCHAR(100) INDEX_NONE," + "mval BLOB INDEX_NONE" + ");", true);
		}

		if (bCreate) {
			db.query("CREATE INDEX " + sTableName + "_rectime_cnf_idx ON " + sTableName + " (rectime, mcluster, mnode, mfunction);", true);
		}
		...
		if (logger.isLoggable(Level.FINE)) {
			logger.log(Level.FINE, " [ eDBW ] " + sTableName + " online_since = " + onlineSince + " [ " + new Date(onlineSince.get()) + " ] ");
		}
	...
	private final void storeData(final eResult r) {
		...
		try {
			for (int i = 0; i < r.param_name.length; i++) {
				db.query("INSERT INTO " + sTableName + " (rectime, mfarm, mcluster, mnode, mfunction, mval) VALUES (" + r.time + "," + (r.FarmName == null ? "NULL" : "'" + r.FarmName + "'") + "," + (r.ClusterName == null ? "NULL" : "'" + r.ClusterName + "'") + "," + (r.NodeName == null ? "NULL" : "'" + r.NodeName + "'") + "," + "'"+Formatare.mySQLEscape(r.param_name[i])+"', '"+Formatare.mySQLEscape(serializeToString(r.param[i]))+"')");
			}
		}
		catch (Throwable t) {
			// ignore
	...
	private long				cleanupInterval		= 60 * 1000;		//10 min
