/*
 * $Id: DBWriter2.java 6865 2010-10-10 10:03:16Z ramiro $
 */
...
public final class DBWriter2 extends Writer {
	...
	private static boolean					shouldCompactTables		= Boolean.valueOf(AppConfig.getProperty("lia.Monitor.shouldCompactTables", "true")).booleanValue();

	private static boolean					shouldReadLastValues	= Boolean.valueOf(AppConfig.getProperty("lia.Monitor.Store.shouldReadLastValues", "true")).booleanValue();
	...
	private final void initDBStructure() {
		...
		if (driverString.indexOf("mckoi") == -1) {
			if (driverString.indexOf("postgres") >= 0) {
				bShouldImport = db.syncUpdateQuery("CREATE TABLE " + sTableName + " (" + "rectime bigint," + "id int," + "mval real," + "mmin real," + "mmax real" + ") WITHOUT OIDS;", true);
				...
				bShouldImport = db.syncUpdateQuery("CREATE TABLE " + sTableName + " (" + "rectime BIGINT," + "id int," + "mval FLOAT," + "mmin FLOAT," + "mmax FLOAT" + ") TYPE=InnoDB;", true);
			}

			if (bShouldImport) {
				db.syncUpdateQuery("CREATE INDEX " + sTableName + "_rectime_id_idx ON " + sTableName + " (id, rectime);", true);
			}
		} else {
			db.syncUpdateQuery("CREATE TABLE " + sTableName + " (" + "rectime BIGINT," + "id int," + "mval DOUBLE INDEX_NONE," + "mmin DOUBLE INDEX_NONE," + "mmax DOUBLE INDEX_NONE" + ");", true);
	...
	private final void readPreviousValues() {
		...
		synchronized (mLock) {
			...
			if (m == null) {
				logger.log(Level.INFO, "DBWriter2 (" + sTableName + ") : falling back to reading last values from the table");
				m = new HashMap<Object, CacheElement>();
	...
	public final boolean cleanup(final boolean bCleanHash) {
		...
		if (lastCleanupTime + cleanupInterval < now) {
			if (execMaintenance("DELETE FROM " + sTableName + " WHERE rectime<" + (now - lGraphTotalTime) + ";", true, db)) {
