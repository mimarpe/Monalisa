...
public final class BatchProcessor extends Thread {
	...
	/** The Logger */
	private static final  Logger			logger						= Logger.getLogger("lia.Monitor.Store.Sql.BatchProcessor");
		...
		if (vData.size() > BUFFER_SIZE) {
			...
			synchronized (fileFlushLock){				
				try{
					...
					logger.log(Level.WARNING, "Buffer overrun and I cannot write to " + sCannonicalOutputFileName + " : " + tableName + " (" + rectime + "," + (id>0?id+",":"") + mval + "," + mmin + "," + mmax + ")");
	...
	private static int saveIDObjects() {
		...
		for (int i = 0; i < vSize; i++) {
			...
			q = "INSERT INTO "+o.sTable+" VALUES ("+o.rectime+","+o.id+","+o.mval+","+o.mmin+","+o.mmax+");";
			
			if (!db.query(q, true)){
				if (hasDB()){
					if (logger.isLoggable(Level.INFO))
						logger.log(Level.INFO, "Creating table: "+o.sTable);
					
					if (logger.isLoggable(Level.FINE))
						logger.log(Level.FINE, "INSERT failed");
					
					if (db.syncUpdateQuery("CREATE TABLE "+o.sTable+" (rectime int, id int, mval real, mmin real, mmax real);", false)){
						db.syncUpdateQuery("CREATE INDEX "+o.sTable+"_idx ON "+o.sTable+" (id, rectime);", false);
						db.syncUpdateQuery("CREATE INDEX "+o.sTable+"_time_idx ON "+o.sTable+" (rectime);", false);
						db.maintenance("CLUSTER "+o.sTable+"_time_idx ON "+o.sTable+";");
						db.maintenance("ALTER TABLE "+o.sTable+" ALTER rectime SET statistics 1000");
						db.maintenance("ALTER TABLE "+o.sTable+" ALTER id SET statistics 1000");
	...
	private static final void createV3Table(final String sTableName) {
		...
		if (db.syncUpdateQuery("CREATE TABLE " + sTableName + " (rectime integer, mval " + DBWriter3.sFloatingPointDef + ", mmin " + DBWriter3.sFloatingPointDef + ", mmax " + DBWriter3.sFloatingPointDef + ") WITHOUT OIDS;")){
