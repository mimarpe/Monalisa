/*
 * $Id: TransparentStoreFast.java 7127 2011-03-14 09:42:53Z costing $
 */
...
public final class TransparentStoreFast implements TransparentStoreInt, Runnable, AppConfigChangeListener {
	...
	private static final long	LAST_CONF_MAXTIME 		= AppConfig.getl("lia.Monitor.Store.TransparentStoreFast.LAST_CONF_MAXTIME", 60 * 24 * 30) * 1000L * 60;
	...
	private static double memoryQueryThreshold = AppConfig.getd("lia.Monitor.Store.TransparentStoreFast.memoryQueryThreshold", 10);
	...
	static {
        ...
        logger.log(Level.INFO, " [ TSF ] SHOULD_STORE_CONF = " + SHOULD_STORE_CONF+", SHOULD_KEEP_MEM_CONF = "+SHOULD_KEEP_MEM_CONF);
	}

	private static long					lLastConfCleanupTime	= 0;

	private static final String			driverString			= AppConfig.getProperty("lia.Monitor.jdbcDriverString", "org.postgresql.Driver").trim();
	...
	static final class MonitorIDsUpdater {
		 ...
		 * @param o
		 */
		public void addSample(final Object o){
	 ...
	 * @see lia.Monitor.Store.TransparentStoreInt#setStoreFilters(Vector, Vector)
	 */
	public void setStoreFilters(final Vector<monPredicate> vAccept, final Vector<monPredicate> vReject, final Vector<monPredicate> vExtraMemory) {
	...
	private void setDontStore() {
		...
		if (vsDontStore != null) {
			for (int i = 0; i < vsDontStore.length; i++) {
				monPredicate pred = lia.web.utils.Formatare.toPred(vsDontStore[i]);
		...
		if (vsDontStore != null) {
			for (int i = 0; i < vsDontStore.length; i++) {
				final monPredicate pred = lia.web.utils.Formatare.toPred(vsDontStore[i]);
		...
		if (vsStoreOnly != null){
			for (int i = 0; i < vsStoreOnly.length; i++){
				final monPredicate pred = lia.web.utils.Formatare.toPred(vsStoreOnly[i]);
		...
		if (vsStoreMemory != null){
			for (int i = 0; i < vsStoreMemory.length; i++){
				final monPredicate pred = lia.web.utils.Formatare.toPred(vsStoreMemory[i]);
	...
	private void load(final boolean bIsReloading) {
		...
		try {
			...
			tmw.setConstaints(AppConfig.getProperty("lia.Monitor.Store.membuffer.constraints"), AppConfig.getProperty("lia.Monitor.Store.membuffer.reject"));
			...
			try {
				...
				if (nr == -1) {
					...
					System.setProperty("lia.Monitor.Store.TransparentStoreFast.writer_1.table_name", "monitor_s_e_3hour");
				...
				if ((nr == 0 || AppConfig.getb("lia.Monitor.JStore.logResults", false)) && !AppConfig.getb("lia.Monitor.JStore.disable_internal_results_logger", false)) {
				...
				if (driverString.indexOf("postgres") >= 0) {
					db.query("CREATE TABLE monitor_n_conf (conftime BIGINT, mfarm VARCHAR(100), conf text) WITHOUT OIDS;", true);
					db.query("CREATE TABLE saved_bprevdata (sp_name text primary key, sp_value bytea) WITHOUT OIDS;", true);
				} else {
					db.query("CREATE TABLE monitor_n_conf (conftime BIGINT, mfarm VARCHAR(100), conf BLOB)", true);
					db.query("CREATE TABLE saved_bprevdata (sp_name varchar(50) primary key, sp_value blob) TYPE=InnoDB;", true);
					db.query("ALTER TABLE saved_bprevdata MODIFY sp_value LONGBLOB", true);
				}

				db.query("CREATE TABLE monitor_tables_onlinetime (mt_tablename varchar(50) primary key, online_since bigint default 0)", true);
				db.query("CREATE TABLE monitor_tables (mt_tablename varchar(50) primary key, mt_status int default 0)", true);
				db.query("UPDATE monitor_tables SET mt_status=mt_status-1 WHERE mt_status>=0");
				...
				for (int i = 0; i < nr; i++) {
					long lTotalTime = Long.parseLong(AppConfig.getProperty("lia.Monitor.Store.TransparentStoreFast.writer_" + i + ".total_time", "0").trim()) * 1000;
					long lSamples = Long.parseLong(AppConfig.getProperty("lia.Monitor.Store.TransparentStoreFast.writer_" + i + ".samples", "1").trim());
					String sTableName = AppConfig.getProperty("lia.Monitor.Store.TransparentStoreFast.writer_" + i + ".table_name", "writer_" + i).trim();

					int iWriteMode = Integer.parseInt(AppConfig.getProperty("lia.Monitor.Store.TransparentStoreFast.writer_" + i + ".writemode", "0").trim());

					final String sAcceptConstraints = AppConfig.getProperty("lia.Monitor.Store.TransparentStoreFast.writer_" + i + ".constraints");
					final String sRejectConstraints = AppConfig.getProperty("lia.Monitor.Store.TransparentStoreFast.writer_" + i + ".reject");
					...
					if (lTotalTime > 0 && sTableName.length() > 0 && (iWriteMode == 0 || iWriteMode == 1)) {
						try {
							...
							int iImport = Integer.parseInt(AppConfig.getProperty("lia.Monitor.Store.TransparentStoreFast.writer_" + i + ".import", "1").trim());
							...
							if (db.moveNext()) {
								...
								db.query("INSERT INTO monitor_tables (mt_tablename, mt_status) VALUES ('" + sTableName + "', 1);");
								if (iImport > 0)
							...
							if (db.moveNext()) {
								db.query("UPDATE monitor_tables SET mt_status=1 WHERE mt_tablename='" + sBackupTableName + "';");
								lAllTables.add(sBackupTableName);
							} else {
								logger.log(Level.INFO, "New table detected : " + sBackupTableName);
								db.query("INSERT INTO monitor_tables (mt_tablename, mt_status) VALUES ('" + sBackupTableName + "', 1);");
								if (iImport > 0)
							...
							logger.log(Level.SEVERE, "Exception in initializing the store element '" + sTableName + "' : ", e);
						}
					...
					} else if (lTotalTime > 0 && sTableName.length() > 0 && (iWriteMode == 3 || iWriteMode == 4)) {
						try {
							int iImport = Integer.parseInt(AppConfig.getProperty("lia.Monitor.Store.TransparentStoreFast.writer_" + i + ".import", "0").trim());
							long lLimit = Long.parseLong(AppConfig.getProperty("lia.Monitor.Store.TransparentStoreFast.writer_" + i + ".countLimit", "-1").trim());
					...
					} else if (lTotalTime > 0 && sTableName.length() > 0 && (iWriteMode == 5 || iWriteMode == 6)) {
						try {
							int iImport = Integer.parseInt(AppConfig.getProperty("lia.Monitor.Store.TransparentStoreFast.writer_" + i + ".import", "1").trim());
					...
					} else if (lTotalTime > 0 && sTableName.length() > 0 && (iWriteMode == 7 || iWriteMode == 8)) {
						int iImport = 0;
					...
					} else if (lTotalTime > 0 && sTableName.length() > 0 && (iWriteMode == 9 || iWriteMode == 10)) {
						try {
					...
					} else if (lTotalTime > 0 && sTableName.length() > 0 && (iWriteMode == 11 || iWriteMode == 12)) {
				...
				if (lNewTables.size() > 0 && (lAllTables.size() > 0 || lDeletedTables.size() > 0) && !bIsReloading) {
					String sQuery = "";
				...
				if (cleanupThread == null) {
					cleanupThread = MonALISAExecutors.getMLStoreExecutor().scheduleWithFixedDelay(this, 80, 20, TimeUnit.SECONDS);
				}
			} catch (Throwable t) {
	...
	public void addData(final Object _o) {
		...
		try {
			...
			for (int i = 0; i < vWriters.length; i++)
				if (vWriters[i] != null) {
					if (vWriters[i].isOnline()) {
						// System.err.println("TSF : send : "+o+" to writer "+i);
						vWriters[i].addSample(o);
					}
				}

			if (miUpdater != null)
				miUpdater.addSample(o);
	...
	public void updateConfig(final MFarm farm) {
		...
		try {
			...
			if (TransparentStoreFactory.isMemoryStoreOnly() || !SHOULD_STORE_CONF) {
				if (logger.isLoggable(Level.FINER))
					logger.log(Level.FINER, " [ TSF ] New conf but not making it persistent isMemoryStoreOnly = " + TransparentStoreFactory.isMemoryStoreOnly() + " SHOULD_STORE_CONF = " + SHOULD_STORE_CONF);
			...
			db.query("INSERT INTO monitor_n_conf (conftime, mfarm, conf) VALUES("+confTime+","+Formatare.mySQLEscape(farm.name)+",'"+Formatare.mySQLEscape(Writer.serializeToString(wsconf))+"');");
	...
	public Vector<String> getConfigurationFarms() {
		...
		final DB db = new DB("SELECT DISTINCT mfarm FROM monitor_n_conf GROUP BY mfarm");
	...
	public Vector<lia.ws.WSConf> getConfig(final long fromTime, final long toTime) {
		...
		return getConfigsByQuery("SELECT conf FROM monitor_n_conf WHERE conftime > "+fromTime+" AND conftime < "+toTime+";");			
	}

	private Vector<lia.ws.WSConf> getConfigsByQuery(final String query){
		final DB db = new DB();
	...
	private Object getConfigByQuery(final String query){
	...
	public Object getConfig(final String FarmName) {
		...
		if (db.query("SELECT MAX(conftime) AS mconftime FROM monitor_n_conf WHERE mfarm = '" + FarmName + "';") && db.moveNext())
			cfTime = db.getl(1, 0);
		
		if (cfTime > 0) {
			config=getConfigByQuery("SELECT conf FROM monitor_n_conf WHERE mfarm = "+Formatare.mySQLEscape(FarmName)+" AND conftime = "+cfTime);
		}
	...
	public Vector<lia.ws.WSConf> getConfig(final String FarmName, final long fromTime, final long toTime) {
		...
		return getConfigsByQuery("SELECT conf FROM monitor_n_conf WHERE mfarm = "+Formatare.mySQLEscape(FarmName)+" AND conftime > "+fromTime+" AND conftime < "+toTime+";");
	}
			...
			for (int i = 0; i < definedWritersLen; i++) {
				...
				if (writer != null && peerWriter != null) {
					...
					if (onlineWriter.getOnlineTime() + onlineWriter.getTotalTime() < now) {//we should switch
						...
						if (logger.isLoggable(Level.FINER)) {
							logger.log(Level.FINER, " [ TSF ] [ checkForRotate ] Offline now:" + onlineWriter.getTableName() + " Online now: " + offlineWriter.getTableName());
						}
					}
				}
			}
		...
		try {
			...
			try {
				...
				try{
					for (int i = 0; i < vWriters.length; i++)
						if (vWriters[i] != null) {
							int iSave = vWriters[i].save();
							vWriters[i].cleanup(bShouldCleanHash);
	 ...
	 */
	@Override
	public Vector<Object> select(final monPredicate p) {
		try {
            if(logger.isLoggable(Level.FINE)) {
                logger.log(Level.FINE, "TSF: select("+p+")");
            }
			
			final Vector<Object> vResults = getResults(new monPredicate[] { p }, false);
	 ...
	 */
	private final Vector<Object> getResults(final monPredicate p[], final boolean bNormalized) {
		if (p==null || p.length==0)
			return new Vector<Object>();
		
		final DataSplitter ds = getDataSplitter(p, -1, bNormalized);
	
		final Vector<Object> vResults = ds.toVector();
	...
	private static final class DataSplitter3 extends DataSplitter {
		 ...
		 * @param v
		 */
		public void putData(final int id, final Vector<Object> v) {
	 ...
	 */
	private DataSplitter getDataSplitter3(final monPredicate p[], final int indent, final int interval, final boolean bNormalized) {
		...
		while (it.hasNext()){
			...
			if (interval > 1)
				q = "SELECT rectime,mval,mmin,mmax FROM select_data_" + vWriters[indent].getTableName() + "(" + id + ", " + tmin + ", " + tmax + ", " + interval + ");";
			else
				q = "SELECT rectime,mval,mmin,mmax FROM " + vWriters[indent].getTableName() + "_" + id + " WHERE rectime>=" + tmin + " AND rectime<=" + tmax + " ORDER BY rectime ASC;";
	...
	private Vector<Object> getResults2(final monPredicate p[], final int indent, final boolean bNormalized) {
		...
		final String q = "SELECT rectime,id,mval,mmin,mmax FROM " + vWriters[indent].getTableName() + " WHERE rectime>=" + tmin + " AND rectime<=" + tmax + " AND id IN (" + sbIDs.toString() + ") ORDER BY rectime ASC;";
	...
	private List<TableTime> getWritersList(){
		...
		for (int indent = 0; indent < definedWritersLen; indent++) {
			if (vWriters[indent] != null) {
				...
				logger.log(Level.WARNING, " [ TSF ] [ HANDLED ] Writer element " + indent + "/" + vWriters.length + " is null. Please fix the configuration.");
			}
		}
		Collections.sort(l);
	...
	public DataSplitter getUniformDataSplitter(final monPredicate p[], final long lCompactInterval) {
		...
		try {
			...
			if (ttBest == null) {
				...
				logger.log(Level.FINER, "Choosing the last of the eligible tables, "+ttBest.indent+", because no other matched our query");
			}
	...
	private List<TableTime> getEligibleTables(final long lCompactInterval, final boolean bPrepareOrder){
		...
		if (lCompactInterval > 0) {
			while (it.hasNext()) {
				...
				if (vTypes[t.indent] == 0 || vTypes[t.indent] == 3 || vTypes[t.indent] == 5 || vTypes[t.indent] == 7 || vTypes[t.indent] == 11) {
	...
	private DataSplitter getDataSplitter(final monPredicate p[], final long lCompactInterval, final boolean bNormalized) {
		...
		try {
			...
			final DataSplitter dsRez = new DataSplitter(new Vector<Object>(1), now + p[0].tmin, now + p[0].tmax);
			...
			if (tmax > lTempMin && (TransparentStoreFactory.isMemoryStoreOnly() || Math.abs(tmin-tmax)/memoryQueryThreshold<tmwTotalTime)) {
				...
				if (tmw instanceof TempMemWriter) {
					...
					Vector<Object> vData = ((TempMemWriter) tmw).getDataAsVector();
				...
				logger.log(Level.FINER, "Memory absmin = " + lTemp + ", lTempMin = " + (NTPDate.currentTimeMillis() + lTempMin) + ", tmax=" + tmax);
			}
			...
			if (tmax <= tmin || (lCompactInterval > 1 && tmax <= tmin + lCompactInterval) || (tmax <= tmin + 2 * 60 * 1000 && lTempMin < tmin)) {
				if (logger.isLoggable(Level.FINER))
					logger.log(Level.FINER, "exiting quickly because the memory buffer contained all possible values (tmax=" + tmax + ", tmin=" + tmin + ", lCompactInterval=" + lCompactInterval + ", lTempMin=" + lTempMin + ")");
			...
			while (it.hasNext()) {
				...
				if (vTypes[t.indent] < 0 || vTypes[t.indent] == 2 || vTypes[t.indent] == 9 || vTypes[t.indent] == 10)
					continue;
	...
	private void addQueryResults(final DataSplitter dsRez, final monPredicate[] p, final TableTime t, final long lCompactInterval){
		if (logger.isLoggable(Level.FINER)) {
			logger.log(Level.FINER, "Extracting " + p[0] + " from " + vWriters[t.indent] + " (" + t.indent + ") of type " + vTypes[t.indent]);
		}
	...
	private Vector<Object> getOldResults(final monPredicate p[], final int indent, final boolean bNormalized){
		...
		for (int i = 0; i < p.length; i++) {
			...
			String query = DataSelect.rquery(p[i], vWriters[indent].getTableName());

			if (logger.isLoggable(Level.FINE)) {
				logger.log(Level.FINE, " Query 1 is '" + query + "' for " + p[i]);
			}

			if (query.indexOf("WHERE 0=1") < 0) // don't even try to execute such bogus queries
				vRez.addAll(getResults(p[i], indent, query));

			// the alternative table now
			query = DataSelect.rquery(p[i], vWriters[indent + definedWritersLen].getTableName());

			if (logger.isLoggable(Level.FINE)) {
				logger.log(Level.FINE, " Query 2 is '" + query + "' for " + p[i]);
			}
	 ...
	 * one bigger table.
	 */
	private DataSplitter getDataSplitter4(final monPredicate[] p, final int table, final long lCompactInterval, final boolean bNormalized){
	 ...
	 */
	public static DataSplitter getDataSplitter4(final monPredicate[] p, final DBWriter4 writer, final long lCompactInterval){
		return getDataSplitter4(p, writer, lCompactInterval, false);
	}
	
	private static DataSplitter getDataSplitter4(final monPredicate[] p, final DBWriter4 writer, final long lCompactInterval, final boolean bNormalized){
		if (p==null || p.length==0)
			return null;
		...
		for (int i=0; i<p.length; i++){
			final Vector<Integer> vIDs = IDGenerator.getIDs(p[i]);
		...
		if (hmIDs.size()==1){
			...
			if (tsTableIDs.size() > 10 && ReplicationManager.getInstance().getOnlineBackendsCount()>1){	
				...
				final int chunks = Math.min(2*ReplicationManager.getInstance().getOnlineBackendsCount(), workers.size());

				final int elements = 1+(tsTableIDs.size() / chunks); 

				if (logger.isLoggable(Level.FINE))
					logger.log(Level.FINE, "Splitting "+tsTableIDs.size()+" IDs in "+chunks+" chunks, ~"+elements+" elements each");
				...
				while (it.hasNext()){
					if (tsChunk.size() > elements){
						queueWork(new DB4QueryExecutor(writer, lCompactInterval, tmin, tmax, ds, sTableExt, tsChunk), results);
						count ++;
			...
			if (logger.isLoggable(Level.FINE))
				logger.log(Level.FINE, "Executing in single shot mode, everything ("+tsTableIDs.size()+" ids) are in a single table: "+sTableExt);
		...
		while (it.hasNext()){
			...
			final DB4QueryExecutor qe = new DB4QueryExecutor(writer, lCompactInterval, tmin, tmax, ds,
					sTableExt, tsTableIDs);
	...
	static {
		...
		final int workersCount = AppConfig.geti("lia.Monitor.Store.parallel_queries", 2 * ReplicationManager.getInstance().getBackendsCount());
	...
	private static final class DB4QueryExecutor implements Runnable {
		...
		public DB4QueryExecutor(final DBWriter4 writer,
			final long lCompactInterval, final long tmin, final long tmax,
	 ...
	 * @param tableIDs
	 */
	static void getTableFromWriter4(final DBWriter4 writer,
			final long lCompactInterval, final long tmin, final long tmax,
			final DataSplitter ds, final String sTableExt, final TreeSet<Integer> tableIDs) {
		...
		Vector<Object> vData = new Vector<Object>(0);
		...
		if (logger.isLoggable(Level.FINE)){
			logger.log(Level.FINE, "Extracted "+count+" results from "+writer+"("+sTableExt+", "+tsTableIDs.size()+" IDs) using "+db.getBackend());
		}
	}
	...
	public synchronized final long getStartTime() {
		if (logger.isLoggable(Level.FINEST))
			logger.log(Level.FINEST, "getStartTime called with prev lDataStartTime = "+lDataStartTime+", lMaxHistoryTime = "+lMaxHistoryTime);
		...
		public void run() {	
			...
			for (int i=0; i<vWriters.length; i++){
				...
				if (w.getTotalTime() == lMaxHistoryTime){
					...
					if (vTypes[i] == 0 || vTypes[i] == 3 || vTypes[i] == 5 || vTypes[i] == 7 || vTypes[i] == 11)
						iSamples = (int) vSamples[i];
					
					if (iSelected<0 || iSelectedSamples==-1 || (iSamples>0 && iSelectedSamples > iSamples)){
						iSelected = i;
			...
			if (vTypes[iSelected] == 0 || vTypes[iSelected] == 1 || (vTypes[iSelected] >= 3 && vTypes[iSelected] <= 6)) {
				final DB db = new DB("SELECT rectime FROM " + vWriters[iSelected].getTableName() + " ORDER BY rectime ASC LIMIT 1;");
			...
			if (vTypes[iSelected] == 7 || vTypes[iSelected] == 8) {
				...
				for (int i = 0; i < vIDs.size() && (i < count || lDataStartTime <= 0); i++) {
					...
					if (!db.query("SELECT rectime FROM " + vWriters[iSelected].getTableName() + "_" + id + " ORDER BY rectime ASC LIMIT 1;", true)) {
						continue;
					}
			...
			if (vTypes[iSelected] == 11 || vTypes[iSelected] == 12) {
				...
				while (db.moveNext()){
					if (db2.query("SELECT rectime FROM " + vWriters[iSelected].getTableName() + "_" + Writer.nameTransform(db.gets(1)) + " ORDER BY rectime ASC LIMIT 1;", true) && db2.moveNext()) {
	...
	public void notifyAppConfigChanged() {
		...
		memoryQueryThreshold = AppConfig.getd("lia.Monitor.Store.TransparentStoreFast.memoryQueryThreshold", memoryQueryThreshold);
