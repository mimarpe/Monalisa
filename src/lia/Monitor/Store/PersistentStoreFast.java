...
public class PersistentStoreFast extends AbstractPersistentStore {
	/** Logger Name */
	private static final String	COMPONENT					= "lia.Monitor.Store";
	...
	public PersistentStoreFast(String[] constraintParams){
		...
		try {
			...
			for (int i = 0; i < nr; i++) {
				long lTotalTime = Long.parseLong(AppConfig.getProperty("lia.Monitor.Store.TransparentStoreFast.writer_" + i + ".total_time", "0")) * 1000;

				if (lMinTime == 0 || lMinTime > lTotalTime) {
					lMinTime = lTotalTime;
					sTableNames[0] = AppConfig.getProperty("lia.Monitor.Store.TransparentStoreFast.writer_" + i + ".table_name", "writer_" + i);
				}
			}
		} catch (Throwable t) {
			logger.log(Level.WARNING, "PersistentStoreFast : could not determine the table names. Check your App.properties file!");
		}
	}
	...
	public java.util.Vector getConfig(long fromTime, long toTime) throws StoreException {
		return getConfigByQuery("SELECT conf FROM monitor_conf WHERE conftime > "+fromTime+" AND conftime < "+toTime+";");
	}

	@Override
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public java.util.Vector getConfig(String FarmName, long fromTime, long toTime) {
		return getConfigByQuery("SELECT conf FROM monitor_conf WHERE mfarm = "+Formatare.mySQLEscape(FarmName)+" AND conftime > "+fromTime+" AND conftime < "+toTime+";");
	}

	@Override
	public java.lang.Object getConfig(final String FarmName) {
		return getConfigByQuery("SELECT conf FROM monitor_conf  WHERE  mfarm="+Formatare.mySQLEscape(FarmName)+" ORDER BY conftime LIMIT 1");
	}
