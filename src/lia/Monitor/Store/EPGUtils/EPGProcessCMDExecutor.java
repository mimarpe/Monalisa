/*
 * $Id: EPGProcessCMDExecutor.java 6946 2010-11-12 16:54:44Z ramiro $
...
public class EPGProcessCMDExecutor {
    ...
    static final Logger logger = Logger.getLogger(EPGProcessCMDExecutor.class.getName());
    ...
    EPGProcessCMDExecutor(final File pgDirectory) {
        ...
        try {
            ...
            final ExitStatus exitStatus = execWithOutput(Arrays.asList(new String[] {"/usr/bin/file", fullPGExecPath}));
            if(exitStatus.getExecutorFinishStatus() == ExecutorFinishStatus.NORMAL && exitStatus.getExtProcExitStatus() == 0) {
                ...
                if(tosArch == null) {
                    throw new IllegalArgumentException("Unable to determine file type for: " + fullPGExecPath + " file exit status: " + exitStatus);
                }
            } else {
                throw new IllegalArgumentException("Unable to determine file type for: " + fullPGExecPath + " file exit status: " + exitStatus);
            }
        ...
        try {
            ...
            if(exitStatus.getExecutorFinishStatus() == ExecutorFinishStatus.NORMAL && exitStatus.getExtProcExitStatus() == 0) {
                ...
                if(split == null || split.length == 0) {
                    throw new IllegalArgumentException("Unable to determine PG version for: " + fullPGExecPath + "; exit status: " + exitStatus);
                }
    ...
    private void initProcEnv() {
        ...
        final String newPath = pgDirAbsPath + ":" + pgDirAbsPath + "/bin:/bin:/usr/bin:/sbin:/usr/sbin:" + ((existingPath == null) ? "" : existingPath);
    ...
    public ExitStatus execWithOutput(List<String> cmd, ProcessNotifier notifier) throws IOException, InterruptedException {
        return procBuilder.command(cmd).returnOutputOnExit(true).start().waitFor();
    }
