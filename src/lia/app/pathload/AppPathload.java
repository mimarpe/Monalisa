...
public class AppPathload implements AppInt {
	 ...
	 * an output log file.
	 */
	public static String MONALISA_RECEIVER_VERBOSE = AppConfig.getProperty( 
		"lia.util.Pathload.client.receiverVerbose");
	 ...
	 * @since ML v.1.5.16
	 */
	public static String MONALISA_RECEIVER_DEBUG = AppConfig.getProperty( 
		"lia.util.Pathload.client.receiverDebug");
	 ...
	 * @since ML v.1.5.16
	 */
	public static String MONALISA_RECEIVER_ADRFAIL = AppConfig.getProperty( 
		"lia.util.Pathload.client.receiverFailOnADRFail"); 
	...
	static {
		...
		String temp = AppConfig.getProperty(
				"lia.util.Pathload.client.binDir", "Control/bin");
		MONALISA_BIN_DIR = getAbsolutePath(temp);
		
		temp = AppConfig.getProperty( 
				"lia.util.Pathload.client.libDir", "Control/lib");
		...
		temp = AppConfig.getProperty( 
				"lia.util.Pathload.client.pathoadBinDir", "Control/bin");
	 ...
	 */
	private static final transient Logger logger = Logger.getLogger(MODULE_NAME);
	...
	public boolean stop() {
		...
		if ((sResult != null )
			...
			bResult = false;
			logger.log(Level.WARNING, MODULE_NAME + " : pathload_snd failed to stop because it "
					+ "is not running.");
		}
	...
	public boolean restart() {
		...
		bResult = stop();
		if (!bResult) {
			logger.log(Level.WARNING, MODULE_NAME + " : Restarting pathload_snd. Pathload instance "
					+ "not running");
		}
		bResult = start();		
		return bResult;
	...
	public int status() {
		...
		if ((sResult != null )
				&& (sResult.length()-2 >= 0)
				&& (sResult.charAt(sResult.length()-2) == '1')) {
		...
		try {
			for (int i = 0; i < arg0.length; i++) {
				...
				if (!sCommand.equals(UPDATE_COMMAND)) {
					logger.log(Level.WARNING, MODULE_NAME + " : Invalid update command "
							+ sCommand
							+ " Required: " 
							+ UPDATE_COMMAND);
					bErrorFlag = true;
			...
			logger.log(Level.WARNING, MODULE_NAME + " : Catch-all Exception triggered. Please revise code.\n"
					+ e.getMessage());
			bErrorFlag = true;					
		}
	...
	public boolean init(String arg0) {		
		...
		if ((prop.getProperty("parameters") != null) && 
			(prop.getProperty("parameters").length()>0)) {
	...
	private boolean extractFiles() {
		...
		try {
			...
			String jarFilename = MONALISA_LIB_DIR + "/" + 
				PATHLOAD_JAR + getArch() + ".jar"; 
			...
			if (jarVersion.indexOf(getArch()) != -1) {
				for (int i=0 ; i < files.length ; i++) {
					File destfile = new File(PATHLOAD_BIN_DIR + "/" + files[i]);
					if (destfile.exists()) {
						destfile.delete();
					}
					JarEntry je = new JarEntry(files[i]);
					bis = new BufferedInputStream(jf.getInputStream(je));
					fos = new FileOutputStream(PATHLOAD_BIN_DIR + "/" + files[i]);
					int bNO;
					...
					bis.close();
					fos.close();
					AppUtils.getOutput(new String[] {
							"/bin/chmod",
							"a+x",
							PATHLOAD_BIN_DIR + "/" + files[i]						
					});
				}
			} else {
				String message = "Pathload version in jar file is: " +  jarVersion + 
					" while my requirements are: " + getArch();
			...
			String message = MODULE_NAME + " : SecurityException. I am not permitted " +
					"to write to " + PATHLOAD_BIN_DIR;
			...
			String message = MODULE_NAME + " : IOException, while extracting from " + 
				PATHLOAD_JAR + getArch() + "\n" + e.getMessage();
			...
			logger.log(Level.WARNING, "Pathload Jar " + PATHLOAD_JAR + getArch() + 
					" seems to be invalid.");
		} finally {
			try {
				if (bis != null) {
				...
				logger.log(Level.WARNING, MODULE_NAME + " : IOException, while " +
						"extracting from " + PATHLOAD_JAR + 
	...
	public boolean check() {
		...
		if (checkCmd == null) {
			...
			if ((MONALISA_SENDER_LOG_FILE != null) && 
					(MONALISA_RECEIVER_LOG_FILE != null)) {
			...
			if (MONALISA_SENDER_LOG_FILE == null) {
				...
				checkCmd.append("OUTPUT=`touch " +   
						MONALISA_SENDER_LOG_FILE +" >/dev/null 2>&1 ; ` ; ");
			...
			if (MONALISA_RECEIVER_LOG_FILE == null) {
				...
				checkCmd.append("OUTPUT=`touch " +   
						MONALISA_RECEIVER_LOG_FILE +" >/dev/null 2>&1 ; ` ; ");
				checkCmd.append("if [[ $? -eq 0 ]] ;  then ");
	...
	public String getPathloadClientExecCmd(String myIpAddress) {
		if (pathloadRcvCmd == null) {
			...
			pathloadRcvCmd.append(PATHLOAD_BIN_DIR + 
					"/pathload_rcv -q -N stdout ");
			
			MONALISA_RECEIVER_VERBOSE = AppConfig.getProperty( 
				"lia.util.Pathload.client.receiverVerbose");
			MONALISA_RECEIVER_DISABLE_LOG = Boolean.valueOf(
					AppConfig.getProperty(
							"lia.util.Pathload.client.disableRecvLog", "false")).booleanValue();
			
			if (!MONALISA_RECEIVER_DISABLE_LOG) {
				if (MONALISA_RECEIVER_LOG_FILE != null) {
					pathloadRcvCmd.append("-o " +  
							MONALISA_RECEIVER_LOG_FILE + " ");
	...
	private String procOutput(String[] command) {
		...
		try {
			...
			logger.log(Level.SEVERE, "[appPathload] RunClient caught interruptedException while waiting for " + 
					  command +
					  "to finish.");
		}
