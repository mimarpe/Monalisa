...
//            boolean bIgnoreZero = pgetb(prop, "ignorezero", false);
            ...
            try{
                for (int i=0; i<vNodes.size(); i++){
                    ...
                    if (sHREF.length()>0){
                        ...
                        if (node.data==null) sTooltipType = "router";
                        else sTooltipType = ""+node.data.size();
			...
			hmKeys.put(":IDREF:", "node_"+node.realname);
                        ...
                        sbMap.append("<area id=\"node_"+node.realname+"\" shape=\"circle\" href=\""+sHREF+"\" target=\"_parent\" coords=\""+node.x+","+node.y+","+node.r+"\" "+getTooltip(prop, null, sTooltip, hmKeys)+"/>");
                    }
                }
...
//                        Iterator it = link.data.keySet().iterator();
			...
			hmKeys.put(":IDREF:", "link_"+link.src.realname+"_"+link.dest.realname);
                        ...
                        sbMap.append("<area id=\"link_"+link.src.realname+"_"+link.dest.realname+"\" shape=\"poly\" "+getTooltip(prop, null, sTooltip, hmKeys)+" href=\""+sHREF+"\" target=_parent coords=\"");
        ...
        Utils.logRequest("farmmap", (int)(System.currentTimeMillis()-lStart), request, false);
