...
public class display extends CacheServlet {
	 ...
	 * Repository version
	 */
	public static final String		sRepositoryVersion	= "1.4.2";

	/**
	 * Date of last significant change
	 */
	public static final String		sRepositoryDate		= "2012.08.03";
	...
	/** Logger Name */
	//private static final transient String	COMPONENT	= "web.display";
			...
			try {
				...
				if (kind.equals("hist") || kind.equals("combined_hist") || kind.equals("image") || kind.equals("farm_info")) {
					...
					if (pgetb(prop, "areachart", false)) {
						long lTemp = pgeti(prop, "compact.displaypoints.areachart", 300);
					...
					if (logTiming())
						logTiming("display.getCacheTimeout() : Returning " + diff + " (" + lOldDiff + ") because " + lHistIntervalMax + "-" + lHistIntervalMin + ", " + pgeti(prop, "compact.displaypoints", 90));
	...
	public String getCacheKeyModifier() {
		...
		if (sess != null) {
			for (String vsSessionAttrib : vsSessionAttribs)
		...
		for (Map.Entry<String, String> me : hmCookies.entrySet()){
	...
	private static final class DisplayStructure {
		 ...
		 * 
		 * @param _sImage file name
		 * @param _sStatistics statistics table
		 * @param _sSeries series names
		 * @param lCacheTimeout for how long (in seconds) will this content be cached
		 */
		public DisplayStructure(String _sImage, String _sStatistics, String _sSeries, long lCacheTimeout) {
	 ...
	 * 
	 * @param sFileName path to the file to be deleted
	 * @param lSecondsToLive delete this file after this much time, in seconds
	 */
	 ...
	 * 
	 * @param _request the http request containing the cookies
	 * @param prop Properties object to override the values is (can be null)
	 ...
	 * 
	 * @param pMaster instance of a master page (global template)
	 * @param prop the configuration for the currently displayed chart
	 * @param sResDir path to the folder that holds the html templates
	 */
	public static final void initMasterPage(Page pMaster, Properties prop, String sResDir) {
		Vector<String> vAlternate = toVector(prop, "alternate.pages", null);
		Vector<String> vAlternateDescr = toVector(prop, "alternate.descr", null);
		Vector<String> vAlternateExplain = toVector(prop, "alternate.explain", null);

		boolean bFirst = true;
		
		Page pAlternate = new Page(sResDir + "masterpage/alternate.res");
		for (int i = 0; i < vAlternate.size(); i++) {
			String sAlternate = vAlternate.get(i);
			String sDescr     = vAlternateDescr!=null && vAlternateDescr.size() > i ? (String) vAlternateDescr.get(i) : sAlternate;
			String sExplain   = vAlternateExplain!=null && vAlternateExplain.size() > i ? vAlternateExplain.get(i).trim() : ""; 
		...
		int iRefreshTime = pgeti(prop, "refresh.time", 0);
	...
	public final void execGet() {
		...
		if (gets("get_statistics").length() > 0 || gets("get_series").length() > 0) {
			...
			if (ds != null) {
				try {
					...
					byte[] content = (bStatistics ? ds.sStatistics : ds.sSeries).getBytes(); 
		...
		for (int i=0; i<vSetCookies.size(); i++){
			String s = vSetCookies.get(i);
			String val = pgets(prop, s);
			
			boolean bOk = setCookie("prop_"+s, val, 60*60*24*365); // 1 year memory :)
		...
		String sX = sImgSize.substring(0, sImgSize.indexOf("x"));
		String sY = sImgSize.substring(sImgSize.indexOf("x")+1);
		...
		if (pgets(prop, "resolution").length() > 0 || pgets(prop, "pagetitle").length() > 0 || pgets(prop, "page", "").equals("stats")) {
			...
			while (eParams.hasMoreElements()) {
				...
				String[] vsValues = request.getParameterValues(sParameter);

				for (int i = 0; vsValues != null && i < vsValues.length; i++)
					sb.append(encode(sParameter) + "=" + encode(vsValues[i]) + "&");
			}

			String sServlet = pgets(prop, "resolution").length() > 0 && !pgets(prop, "page", "").equals("stats") ? "genimage" : "stats";
		...
		String[] vsSeries = request.getParameterValues("plot_series");

		if (vsSeries != null && vsSeries.length > 0) {
			for (String vsSerie : vsSeries) {
		...
		String[] vsColors = Utils.getValues(prop, "colors");
		...
		try {
			if (kind.startsWith("hist")) {
				...
				String sFileName = buildImage(prop, hmSeries, null);
		...
		if (!download || tsDownloadSeries.size()==0 || tmDownloadData.size()==0){
			...
			pwOut.print(bCSV ? "Time" : "<table>\n<tr><th>Time</th>");			
				
			Iterator<String> it = tsDownloadSeries.iterator();
			...
			while (it.hasNext()){
				...
				pwOut.print(bSingle ? s.substring(s.indexOf("@")+1) : s);
				
				if (!bCSV) pwOut.print("</th>");
			}
				
			pwOut.println(bCSV ? "" : "</tr>");
				
			Iterator<Map.Entry<Long, HashMap<String, Double>>> it2 = tmDownloadData.entrySet().iterator();
				
			while (it2.hasNext()){
				final Map.Entry<Long, HashMap<String, Double>> me = it2.next();
					
				if (!bCSV) pwOut.print("<tr><td>");
				
				pwOut.print(me.getKey().toString());
				
				if (!bCSV) pwOut.print("</td>");
		...
		Utils.logRequest("display", (int)(System.currentTimeMillis() - lStart), request, false);
	 ...
	 * 
	 * @param prop properties used to define the file
	 * @return a Page object
	 */
	public Page buildFarmInfo(Properties prop) {
		...
		Page p = new Page(sResDir + "display/" + sPage);

		String sFarm = gets("farm");
		...
		Vector<String> v = toVector(prop, "Farms", null);
...
//		}
	...
	private void setMinMax(final Properties prop, final Properties propExtra) {
		...
		long lStart = pgetl(prop, "interval.start", 0);
		long lEnd = pgetl(prop, "interval.end", 0);
		...
		if (propExtra!=null){
			long lTemp = Utils.getCompactInterval(propExtra, lIntervalMin, lIntervalMax);
	...
	private final Plot getCategoryPlot(Properties prop, HashMap<String, String> hmSeries, HashMap<String, String> hmLegends) {
		return getCategoryPlot(prop, hmSeries, hmLegends, false);
	}

	private final Plot getCategoryPlot(Properties prop, HashMap<String, String> hmSeries, HashMap<String, String> hmLegends, boolean bRange) {
	 ...
	 * 
	 * @param vPreds Vector of mon_predicate objects
	 * @return a DataSplitter with the values taken from monitor_ids table
	 */
	private final static DataSplitter getDBLast(Vector<monPredicate> vPreds){		
		...
		final DB db = new DB(sbQuery.toString());
	...
	private final Plot getCategoryPlot(Properties prop, HashMap<String, String> hmSeries, HashMap<String, String> hmLegends, boolean bRange, boolean bVertical) {
		String vsDescr[] = null;
		monPredicate[] preds;
		String[] series = null;
		...
		boolean bSize = pgetb(prop, "size", false);
		boolean bSpiderLogScale = pgetb(prop, "spider_logscale", false);
		...
		int descrLen = vsMultiSeries.length;

		if (vsDescr == null || vsDescr.length != descrLen) {
			String[] vsDescrOrig = vsDescr;
		...
		Vector<String> vLocalActualSeries = new Vector<String>();
		...
		for (i = 0; i < series.length; i++) {
			...
			String sLabel = getDescr(prop, series[i]);
			...
			for (int j = 0; j < vsMultiSeries.length; j++) {
				...
				if (!bHistoryChart) {
					...
					if (vTemp!=null && vTemp.size() > 0) {
						...
						Result r = Utils.copyResult(vTemp.get(0));
						vTemp.clear();
						r.param[0] = dVal;
						vTemp.add(r);
					}
				...
				if (vTemp == null || vTemp.size() == 0) {
					...
					if (o != null && (o instanceof Result)) {
						...
						if (vTemp.size() > 1) {
							...
							boolean bSumMultipleValues = pgetb(prop, "multiple_values.sum", true);
							boolean bAvgMultipleValues = pgetb(prop, "multiple_values.avg", false);
			...
			if (bEnabled && cdSecondary != null && pSecondary!=null) {
				...
				Object oTemp = Cache.getLastValue(pSecondary);
		...
		if (bIgnoreZero) {
			...
			for (i = 0; i < categoryDataset.getRowCount(); i++) {
				String sRow = (String) categoryDataset.getRowKey(i);
		...
		if (bSpiderWebPlot) {
			...
			if (dMaxValue > 0 && rad > 0) {
				...
				for (i = 1; i <= iCircles; i++) {
					...
					String sValue = DoubleFormat.point(dValue);
			...
			NumberFormat nf = new MyNumberFormat(bSize, sSizeIn, sSuffix);
			...
			CategoryPlot categoryPlot = new MyCategoryPlot(categoryDataset, catAxis, numberAxis, cir, hmLegends);

			if (pgetb(prop, "urls.enabled", false)) {
				StandardCategoryURLGenerator scurlg = new StandardCategoryURLGenerator(pgets(prop, "urls.prefix", "display?page=" + gets("page")), pgets(prop, "urls.series_parameter_name", "plot_series"), pgets(prop, "urls.category_parameter_name", "category"));
			...
			if (cdSecondary != null) {
				...
				nf = new MyNumberFormat(bSize, sSizeIn, sSuffix);
	...
	public static Font getScalledFont(final Properties prop, final Font font){
		double dScaleFont = pgetd(prop, "font.scale", -1);
	 ...
	 */
	public static Font getTickLabelFont(Properties prop, String axis) {
		Font font = Axis.DEFAULT_TICK_LABEL_FONT;
		
		double defaultFontSize = pgetd(prop, "default.font.size", -1);

		double defaultLabelFontSize = pgetd(prop, "default.axis.font.size", defaultFontSize);

		double defaultTickFontSize = pgetd(prop, "default.tick.font.size", defaultLabelFontSize);		
		
		double axisFontSize = pgetd(prop, axis+".tick.font.size", defaultTickFontSize);
	...
	public static final Font getLabelFont(final Properties prop, final String axis) {
		...
		double axisFontSize = pgetd(prop, axis+".axis.font.size", defaultLabelFontSize);
	...
	private final JFreeChart buildCategoryChart(Properties prop, HashMap<String, String> hmSeries) {
	...
	private final Page buildRealTimePage(Properties prop, HashMap<String, String> hmSeries) {
		...
		Page pSeries = new Page(sResDir + "display/hist_series.res");

		displayCategories(vTotalSeries, prop, p, pSeries, hmSeries, hmActualSeries);
	...
	private final Page buildCombinedBar(final Properties prop, final HashMap<String, String> hmSeries, final String[] vsModules) {
		...
		String[] vsCharts = Utils.getValues(prop, "charts");
		String[] vsChartDescr = Utils.getValues(prop, "charts.descr");
		
	
		CategoryPlot parent;
	
		boolean bRange = pgetb(prop, "samerange", false);
	
		boolean b3D = pgetb(prop, "3d", true);
	
		ValueAxis valueAxis = b3D ? new NumberAxis3D(pgets(prop, "ylabel")) : new NumberAxis(pgets(prop, "ylabel"));
		...
		List<CategoryPlot> lCategs = new LinkedList<CategoryPlot>();
	
		HashMap<String, String> hmModules = new HashMap<String, String>();
	
		if (vsModules != null && vsModules.length > 0)
			for (String vsModule : vsModules)
				for (String vsChart : vsCharts)
		...
		if (hmModules.size() <= 0)
			for (String vsChart2 : vsCharts)
				hmModules.put(vsChart2, "");
	
		String[] vsUnselectModules = request.getParameterValues("unselect_module");
		for (int i = 0; vsUnselectModules != null && i < vsUnselectModules.length; i++)
			hmModules.remove(vsUnselectModules[i]);
	
		String sOrientation = pgets(prop, "orientation");
		boolean bVertical = true;
		...
		boolean bColapsedLegend = pgetb(prop, "colapsedlegend", true);
	
		HashMap<String, String> hmLegends = new HashMap<String, String>();
		
		HashMap<String, String> hmSeriesOrig = new HashMap<String, String>(hmSeries);
		
		hmSeries.clear();
		
		for (int i = 0; vsCharts != null && i < vsCharts.length; i++) {
			try {
				Properties p2 = Utils.getProperties(sConfDir, vsCharts[i], prop, false);
					
				HashMap<String, String> hmTemp = new HashMap<String, String>(hmSeriesOrig);
	
				CategoryPlot cp = (CategoryPlot) getCategoryPlot(p2, hmTemp, bColapsedLegend ? hmLegends : new HashMap<String, String>(), bRange, bVertical);
	
				hmSeries.putAll(hmTemp);
				
				int rows = ((DefaultCategoryDataset) cp.getDataset()).getRowCount();
				int cols = ((DefaultCategoryDataset) cp.getDataset()).getColumnCount();
		...
		Iterator<CategoryPlot> itCategs = lCategs.iterator();
	
		while (itCategs.hasNext()) {
			DefaultCategoryDataset cd = (DefaultCategoryDataset) (itCategs.next()).getDataset();
	
			Iterator<String> it = vTotalSeries.iterator();
			while (it.hasNext()) {
				String s = it.next();
				if (hmActualSeries.get(s) == null){
					try{
		...
		if (!bRange) {
			CategoryAxis axis = parent.getDomainAxis();
			//axis.setLabelAngle(bVertical ? VERTICAL_ANGLE : HORIZONTAL_ANGLE);
			axis.setCategoryLabelPositions(bVertical ? CategoryLabelPositions.UP_45 : CategoryLabelPositions.STANDARD);
		...
		Page pModules = new Page(sResDir + "display/modules.res");
		Page pModule = new Page(sResDir + "display/module.res");
		for (int i = 0; vsCharts != null && i < vsCharts.length; i++) {
			String sName = vsCharts[i];
	 ...
	 * @param lWeights list of maximum values, should have the same number of values as how many subcharts are contained in the plot
	 * @param lPlots list of subplots 
	 * @param prop .properties' file contents, will be checked for:<br>
	...
	private static void setPlotWeights(final Plot plot, final List<? extends Plot> lPlots, final Properties prop){
		...
		if (!bAutoDone){
			String[] vsPercents = Utils.getValues(prop, "percents");
		...
		for (int i=0; i<iCount; i++){
			if (plot instanceof CombinedDomainCategoryPlot || plot instanceof CombinedRangeCategoryPlot){
				CategoryPlot subplot = (CategoryPlot) lPlots.get(i);
			...
			if (plot instanceof CombinedDomainXYPlot){
				...
				XYPlot subplot = (XYPlot) lPlots.get(i);
	...
	private static final double log(double d){
		return d>1 ? Math.log(d) : 1;
	}
	
	private final void displayCategories(Vector<String> _vTotalSeries, Properties prop, Page p, Page pSeries, HashMap<String, String> hmSeries, HashMap<String, String> _hmActualSeries) {
		boolean bDisplayAll = pgetb(prop, "displayall", true);
		boolean bSortBySuffix = pgetb(prop, "sort.bysuffix", false);

		String sSuffixDelimiter = pgets(prop, "sort.bysuffix.delimiter", " .-_");
		String sSortBySuffixSeparator = pgets(prop, "sort.bysuffix.separator", "<hr size=1 noshade>");

		Vector<String> vSeparate = toVector(prop, "separate", null);
		...
		Iterator<String> it = _vTotalSeries.iterator();
		while (it.hasNext()) {
			...
			String sLabel = getDescr(prop, sName); 
			...
			if (_hmActualSeries.get(sName) != null || bDisplayAll) {
				...
				if (vSeparate.contains(sName)) {
					if (bSortBySuffix) {
						String sNewSuffix = getStringSuffix(sName, sSuffixDelimiter);
	...
	private DataSplitter getDataSplitter(final Vector<monPredicate> vPreds, final Properties prop, final long lCompactInterval) {
		...
		if (vPreds != null && vPreds.size() > 0) {
			...
			monPredicate p = new monPredicate("*", "*", "*", -1, -1, new String[] { "*" }, null);
	...
	private final double getSmoothStart(monPredicate pred, Properties prop) {
		monPredicate pt = new monPredicate(pred.Farm, pred.Cluster, pred.Node, pred.tmin, pred.tmax, pred.parameters, pred.constraints);
	
		pt.tmax = pt.tmin;
		pt.tmin = store.getStartTime() - NTPDate.currentTimeMillis();
	
		DataSplitter ds = getDataSplitter(new monPredicate[] { pt }, prop, 6000001);
	
		Vector<Object> v = ds.get(pt);
	...
	private final JFreeChart buildHistoryChart(Properties prop, HashMap<String, String> hmSeries){
		String sPageTitle = pgets(prop, "title", "History");
		XYPlot chartPlot = buildHistoryPlot(prop, hmSeries, null, null);

		JFreeChart chart = new JFreeChart(sPageTitle, JFreeChart.DEFAULT_TITLE_FONT, chartPlot, true);
	...
	private final Page buildHistoryPage(Properties prop, HashMap<String, String> hmSeries) {
		if (logTiming())
		...
		if (bHistogram) {
			...
			XYPlot chartPlot = buildHistoryPlot(prop, hmSeries, p, null);
	 ...
	 * 
	 * @param lTime moment in time
	 * @param lCompactInterval desired distance between two points
	 * @param tz time zone
	 * @return bin time
	 */
	public static final RegularTimePeriod getTimePeriod(final long lTime, final long lCompactInterval, final TimeZone tz){
		...
		switch ((int)(lCompactInterval/Utils.TIME_MINUTE)){
			case 1: return new Minute(d, tz);
			case 60: return new Hour(d, tz);
			case 60*24: return new Day(d, tz);
			case 60*24*7: return new Week(d, tz, Locale.getDefault());
			case 60*24*7*30: return new Month(d, tz, Locale.getDefault());
			default: return new Minute(d, tz);
		}
	}
	 ...
	 * 
	 * @param prop page configuration
	 * @param hmSeries object to save the discovered series into
	 * @param p wrapper page for the chart
	...
	private XYPlot buildHistogramPlot(final Properties prop, final HashMap<String, String> hmSeries, final Page p){ 
		...
		for (int k = 0; k < hdw.series.length; k++) {
			...
			for (int j=0; j<rezSize; j++){
				...
				if (tmDownloadData != null){
					if (logTiming()){
						logTiming(sDownloadSeries+" : "+er.time+" ("+er.param[0]+") : "+rtp.getFirstMillisecond()/1000+", "+rtp.getMiddleMillisecond()/1000+", "+rtp.getLastMillisecond()/1000);
					}
		...
		NumberFormat nf = new MyNumberFormat(bSize, sSizeIn, sSuffix);
		NumberTickUnit ntu = new NumberTickUnit(1.0, nf);
		...
		if (pgetb(prop, "tooltips.enabled", true)){
			ttg = new MyXYToolTipGenerator(
					bIgnoreZero, 
					bSize, 
					sSizeIn, 
					bInBits,
					pgets(prop, "tooltip.prefix", ""), 
					sSuffix, 
			...
			ttg2 = new MyXYToolTipGenerator(
					bIgnoreZero, 
					bSize, 
					sSizeIn,
					bInBits,
					pgets(prop, "tooltip.prefix.second_axis", pgets(prop, "tooltip.prefix", "")), 
					pgets(prop, "tooltip.suffix.second_axis", sSuffix), 
					pgets(prop, "labels.date.format", (lIntervalMin - lIntervalMax) < 1000 * 60 * 60 * 24 * 20 ? "MMM d, HH:mm" : "yyyy, MMM d")
			);
		...
		if (bSecondAxis){
			...
			for (int i=0; i<alPaintSeries.size(); i++){
				Paint paint = alPaintSeries.get(i);
	...
	private final String buildImage(Properties prop, HashMap<String, String> hmSeries, StringBuilder sb) throws IOException {

		String sKind = pgets(prop, "kind", "hist").toLowerCase();
		...
		int width = pgeti(prop, "width", 400);
	...
	private final XYPlot buildHistoryPlot(final Properties prop, final HashMap<String, String> hmSeries, final Page p, final Properties propExtra) {
		...
		if (bSum){
			Vector<String> vUnselect = toVector(pgets(prop, "displaysum_unselect", "SUM,AVG,_TOTALS_"));
			
			if (vUnselect.size()>0){
				String[] newUnselect = new String[vUnselect.size() + (vsUnselect!=null ? vsUnselect.length : 0)];
		...
		DataSplitter ds = getDataSplitter(hdw.selectedPreds, prop, lCompactInterval);
		...
		boolean bSize = pgetb(prop, "size", false);
		boolean bTotal = pgetb(prop, "showtotal", false);

		Page pStats = new Page(sResDir + "display/hist_statheader.res");
		Page pStat = new Page(sResDir + "display/hist_stat.res");
		
		Page pAnn = new Page(sResDir+"display/hist_annotations.res");
		...
		HashMap<Long, Double> hmSum = bSum ? new HashMap<Long, Double>() : null;
		HashMap<Long, HashMap<String, Double>> hmSumSeen = bSum ? new HashMap<Long, HashMap<String, Double>>() : null;

		long skipNull = pgeti(prop, "skipnull", 0);
		...
		List<Vector<Result>> vsData = new ArrayList<Vector<Result>>(hdw.series.length);
		...
		for (int k = 0; k < hdw.series.length; k++) {
			...
			if (hmSeries.size()==0 || hmSeries.containsKey(hdw.series[k])) {
				...
				if (rezSize > 0) {
					...
					ExtendedResult er = (ExtendedResult) vs.lastElement();
				...
				if (bInBits){
					...
					if (sSizeIn.equals("K")) total /= 1.024d;
					if (sSizeIn.equals("M")) total /= 1.024d*1.024d;
					if (sSizeIn.equals("G")) total /= 1.024d*1.024d*1.024d;
					if (sSizeIn.equals("T")) total /= 1.024d*1.024d*1.024d*1.024d;
					if (sSizeIn.equals("P")) total /= 1.024d*1.024d*1.024d*1.024d*1.024d;
				}
				...
				if (lAnnotations!=null && lAnnotations.size()>0){
					...
					while (it.hasNext()){
						...
						pAnn.append("("+sDateStart+", "+sDateEnd+")<br>"+
									    "<font color=#"+Utils.toHex(a.textColor)+"><b>"+a.text+"</b></font><br><br>"
						);
					}
		...
		boolean bTotalRow = pgetb(prop, "totalrow", false);
		...
		if (bTotalRow) {
			...
			Page pFooter = new Page(sResDir + "display/hist_statfooter.res");
		...
		boolean bError = iErr == 1 && bInt == false && bIntegrateData == false && pgetb(prop, "disableerr", false) == false && !bArea;
		...
		double dSkipFactor = pgetd(prop, "skipfactor", 1.5);

		if (skipNull == 0) {
			if (((float) lMinInterval / (float) lCompactInterval) > 1.0 || (Math.abs(dSkipFactor-1.5)<1E-10)) // assume points are scattered and we have to unite more of them
		...
		boolean bGapIfNoData = bArea ? pgetb(prop, "gap_if_no_data.area", !bIntegrateData) : pgetb(prop, "gap_if_no_data", true);
		...
		for (i = 0; i < hdw.series.length; i++) {
			...
			if ((hmSeries.get(hdw.series[i]) == null && hmSeries.size() > 0)) {
				...
				if (bShowSeriesAnnotations && (lSeriesAnnotations=ac.getSeriesAnnotations(hdw.series[i]))!=null && lSeriesAnnotations.size()>0){
					...
					Iterator<Annotation> it = lSeriesAnnotations.iterator();
				...
				for (int k = 0; vs!=null && k < size; k++) {
					...
					if (bError) {
						long lmilli = second.getFirstMillisecond(calendar);
					...
					if (lastMatchTime == 0){
						if (er.time >= lMinTime) {
							...
							if (pgetb(prop, "first.null", false) && bArea && lastMatchTime >= lDataMinTime + lCompactInterval) {
								long l = lastMatchTime - lCompactInterval;

								RegularTimePeriod rtp = getTableTime(l, tz, lCompactInterval);
							...
							if (pgetb(prop, "single.first.point", true) && bArea && lastMatchTime >= lDataMinTime + lCompactInterval && lastMatchTime<lDataMinTime+2*lCompactInterval){
								long l = lastMatchTime - lCompactInterval;
					...
					if (!bArea) {
						...
						RegularTimePeriod rtp = getTableTime(er.time, tz, lCompactInterval);
						ttxyd.add(rtp, Double.valueOf(dVal), sLabel, false);
					}
					
					if (aNextStart!=null && er.time>=aNextStart.from){
						if (er.time<=aNextStart.to){
					        XYPointerAnnotation xypointerannotation = new XYPointerAnnotation(aNextStart.text, er.time, dVal, 5*Math.PI/4);
					...
					for (int k = size-1; vs!=null && k >= 0; k--) {
						...
						if (er.time<=aNextEnd.to){
							if (er.time>=aNextEnd.from){
								XYPointerAnnotation xypointerannotation = new XYPointerAnnotation(aNextEnd.text, er.time, er.param[0], 7*Math.PI/4);
								xypointerannotation.setBaseRadius(35D);
								xypointerannotation.setTipRadius(10D);
								xypointerannotation.setFont(new Font("SansSerif", 0, 9));
				...
				if (bArea && lastMatchTime > 0 && (bIntegrateData || bAreaFinalPoint) && er!=null && er.param[0]>0) {
					while (lastMatchTime + lCompactInterval <= now - lIntervalMax) {
						...
						RegularTimePeriod rtp = getTableTime(lastMatchTime, tz, lCompactInterval);
						ttxyd.add(rtp, Double.valueOf(er.param[0]), sLabel, false);
					}

					if (lastMatchTime != lMaxTime) {
						RegularTimePeriod rtp = getTableTime(lMaxTime, tz, lCompactInterval);
						ttxyd.add(rtp, Double.valueOf(er.param[0]), sLabel, false);
					}
				}
				else
				...
				if (pgetb(prop, "single.final.point", true) && bArea && lastMatchTime>0 && lastMatchTime<=lDataMaxTime-lCompactInterval && lastMatchTime>lDataMaxTime-2*lCompactInterval && er!=null){
					long l = lastMatchTime + lCompactInterval;
			...
			if (!bArea) {
				if (chartSeries.getItemCount() <= 0) {
					//chartSeries = new TimeSeries(null);
					//errorSeries = new XYSeries(null);
		...
		if (bSum && hmSum.size() > 0){
			try {
				Vector<Long> vTimes = new Vector<Long>();
				vTimes.addAll(hmSum.keySet());
				Collections.sort(vTimes);
				Collections.reverse(vTimes);
				
				TimeSeries chartSeries = new TimeSeries(getDescr(prop, sSumSeriesName), Second.class);
				...
				boolean bSumGaps = pgetb(prop, "sum.gaps", false);

				for (int j = iStart; j < iEnd; j++) {
					lTime = vTimes.get(j);

					Double dVal = hmSum.get(lTime);
					
					if (dVal != null){
						d = dVal.doubleValue();
						
						if (bAverage){
							HashMap<String, Double> hmSumFrom = hmSumSeen.get(lTime);
				...
				TimeSeriesCollection tsc = new TimeSeriesCollection();
				XYSeriesCollection xysc = new XYSeriesCollection();
				XYSeries errorSeries = new XYSeries(null);
		...
		XYDataset dataset = bArea ? (XYDataset) ttxyd : (XYDataset) cd;
		...
		double dUnit = pgetd(prop, "tick.unit", bSize ? 1.024 : 1.0);
		
		NumberFormat nf = new MyNumberFormat(bSize, sSizeIn, sSuffix);
		NumberTickUnit ntu = new NumberTickUnit(dUnit, nf);
		na.setTickUnit(ntu, false, false);
		na.setNumberFormatOverride(nf);
		...
		MyXYToolTipGenerator ttg = null;
		if (pgetb(prop, "tooltips.enabled", true)){
			ttg = new MyXYToolTipGenerator(bIgnoreZero, bSize, sSizeIn, bInBits, sPrefix, sSuffix, pgets(prop, "labels.date.format", (lIntervalMin - lIntervalMax) < 1000 * 60 * 60 * 24 * 20 ? "MMM d, HH:mm" : "yyyy, MMM d"));
			ttg.setAnnotations(ac);
		}
		...
		if (pgetb(prop, "urls.enabled", true)){
			...
			Vector<String> v = new Vector<String>(vLocalActualSeries.size()*2);
		...
		boolean bShowShapes = pgetb(prop, "show.shapes", true);
		boolean bShowLines = pgetb(prop, "show.lines", true);
		...
		if (pgetb(prop, "spline_renderer", false)){
			XYSplineRenderer spline = new XYSplineRenderer(pgeti(prop, "spline_precision", 5));
			...
			StandardXYItemRenderer xyir = new StandardXYItemRenderer(iOption, ttg, xyug);
		...
		Iterator<XYPointerAnnotation> it = lChartAnnotations.iterator();
		
		while (it.hasNext()){
			XYPointerAnnotation xypa = it.next();
		...
		if (bSumAnnotation && !bIntegrateData && iAvgCount>0){
			...
			Paint pSum = getPaint(prop, sSumSeriesName);
		...
		if (bArea) {
			if (pgetb(prop, "areachart.v2", true)) {
				XYAreaRenderer2 axyir = pgetb(prop, "areachart.stacked", true) ? new StackedXYAreaRenderer2() : new XYAreaRenderer2();
	...
	private final Page buildCombinedHistory(Properties prop, HashMap<String, String> hmSeries, String[] vsModules) {
		try {
			if (logTiming()){
				logTiming("buildCombinedHistoryPage: enter");
			}
	
			Page p = new Page(sResDir + "display/hist.res");
			...
			ArrayList<XYPlot> alCharts = new ArrayList<XYPlot>();
	...
	private void showIntervalSelectionForm(final Properties prop, final Page p, final long lNowParam) {
		...
		if (pgetb(prop, "intervalselection", false)) {
			...
			int iSelMethod = pgeti(prop, "intervalselection.method", 2);
			...
			if (!pgetb(prop, "intervalselection.calendar_based", false)) {
				for (; lTime >= 0; lTime -= lILength) {
					Date d = new Date(lNow - lTime);

					String sLabel = sdf.format(d);
	...
	private static void setOption(final String s, final Page p, final HttpSession sess, final Properties prop) {
		boolean b = getOption(s, sess, prop);
	...
	public static void setChartProperties(final JFreeChart chart, final Properties prop) {
		...
		float fFA = (float) pgetd(prop, "foreground.alpha", 0.8D);
		float fBA = (float) pgetd(prop, "background.alpha", 1D);
	...
	public static String saveImage(final JFreeChart chart, final Properties prop, final int defaultHeight, final int defaultWidth, final Page p, final long timeout) throws IOException {
		...
		if (legend!=null){
			...
			if (!includeLegendHeight){
				...
				final RectangleConstraint cst = new RectangleConstraint(constraintW, new Range(0.0, constraintW), LengthConstraintType.RANGE, 
	       														constraintH,new Range(0.0, constraintH), LengthConstraintType.RANGE);
	...
	private void setHistoryChartProperties(final JFreeChart chart, final Properties prop, final Page p) {
		...
		ValueAxis axis = chartPlot.getDomainAxis();
	...
	private final void showStatistics(String s) {
		response.setContentType("text/plain");

		pwOut.println("15MinValues\t" + Cache.size());

		Iterator<WriterConfig> it = ConfigBean.getConfig().iterator();
		...
		while (it.hasNext()) {
			...
			DB db = new DB("SELECT count(rectime) AS cnt FROM " + wc.sTableName + ";");
	...
	private double getPieData(final monPredicate pred, final DataSplitter ds, final String sFunction, final Properties prop, final long lCompactInterval, final String sSeriesName) {
		if (ds == null) {
			String sTable = pgets(prop, "history.smooth_from_table");

			if (pgetb(prop, "history.smooth_sets", false) == false || sTable.length() <= 0) {
				Object oTemp = Cache.getLastValue(pred);
				...
				if (vs.size() > 1) {
					if (pgetb(prop, "history.smooth_sets", false)) {
						...
						for (int i = 1; i < vs.size(); i++) {
							...
							double diff = d - dPrevValue;
							dPrevValue = d;
							if (diff > 0)
								dVal += diff;
						}
					} else {
						ExtendedResult r0 = (ExtendedResult) vs.get(0);
	...
	private JFreeChart buildPieChart(Properties prop) {
	...
	private JFreeChart buildPieChart(Properties prop, Page p, HashMap<String, String> hmSeries) {
		...
		Iterator<String> it = hmSeries.keySet().iterator();
		...
		if (bHistoryEnabled) {
			if (p != null) {
				...
				for (int i = 0; i < vsPieFunctions.length; i++)
					if (pgetb(prop, vsPieFunctions[i] + ".enabled", true)) {
						String sSel = sFunction.equals(vsPieFunctions[i]) ? " selected" : "";
			...
			if (bHistory) {
				...
				Vector<monPredicate> vPreds = new Vector<monPredicate>(hmPreds.values());
		...
		final DrawingSupplier ds = new DefaultDrawingSupplier(vPaints, DefaultDrawingSupplier.DEFAULT_OUTLINE_PAINT_SEQUENCE, DefaultDrawingSupplier.DEFAULT_STROKE_SEQUENCE, DefaultDrawingSupplier.DEFAULT_OUTLINE_STROKE_SEQUENCE, Utils.ML_DEFAULT_SHAPES);
	...
	private Page buildPiePage(Properties prop, HashMap<String, String> hmSeries) {
		...
		while (e.hasMoreElements()) {
			...
			if (!sKey.equals("submit_plot") && 
				!sKey.equals("plot_series") && 
				!sKey.equals("pTime") && 
				!sKey.equals("function") && 
				!sKey.startsWith("interval") && 
				!sKey.equals("quick_interval") && 
				!sKey.startsWith("unselect") && 
				!vOptionNames.contains(sKey)
			)
		...
		try {
			saveImage(chart, prop, 500, 800, p, getCacheTimeout());
	 ...
	 * 
	 * @param sShape description (should be actually one char)
	 * @return the shape
	 */
	 ...
	 * 
	 * @param c configuration character representing a shape
	 ...
	 * @param sName name of the series
	...
	private Shape[] getShapeSeries(Properties prop) {
		if (pgetb(prop, "areachart", false))
		...
		Vector<Shape> vShapeSeries = new Vector<Shape>();

		for (int i = 0; i < vIndividualSeries.size(); i++) {
			Vector<String> vLocalActualSeries = vIndividualSeries.get(i);

			for (int j = 0; j < vLocalActualSeries.size(); j++) {
				String sName = vLocalActualSeries.get(j);
		...
		if (!pgets(prop, "page").equals("rt") && !pgets(prop, "page").equals("combined_bar")) {
			...
			String sCustomShapeSeries = pgets(prop, "rt_custom_shapes");

			if (sCustomShapeSeries.length() > 0) {
				vShapeSeries.clear();

				for (int i = 0; i < sCustomShapeSeries.length(); i++) {
					char c = sCustomShapeSeries.charAt(i);
	...
	private Paint getPaint(final Properties prop, final String sName){
		...
		int colorIndex = sName.hashCode();
		
		if (colorIndex < 0)
			colorIndex *= -1;

		return getColor(prop, sName + ".color", (Color) default_paint_sequence[colorIndex % default_paint_sequence.length]);
	}
	...
	private Paint[] getPaintSeries(final Properties prop) {
		...
		final boolean bDoubleSeries = (sPageType.indexOf("hist")>=0 || (sPageType.equals("image") && sKind.indexOf("hist")>=0)) &&  
			pgetb(prop, "areachart", false)==false;

		for (int i = 0; i < vIndividualSeries.size(); i++) {
			Vector<String> vLocalActualSeries = vIndividualSeries.get(i);
	
			Iterator<String> it = vLocalActualSeries.iterator();
	
			while (it.hasNext()){
				String sSeriesName = it.next();
				
				Paint p = getPaint(prop, sSeriesName);
	 ...
	 * 
	 * @param sResDir path to the folder holding the html templates
	 * @param prop configuration of the current page
	 * @param p current wrapper page
	 * @param servlet current servlet that is executed (this method is static to be used from other servlets too).
	 */
	public static void showOptions(String sResDir, Properties prop, Page p, ServletExtension servlet) {
		...
		for (int i = 0; i < iOptionsCount; i++) {
			String sName = pgets(prop, "option_" + i + ".name");

			if (sName.equals("SEPARATOR")) {
				Page pSep = new Page(sResDir + "display/separator.res");
	...
	private static final void setExtraFields(Page p, Properties prop) {
		Vector<String> vFields = toVector(prop, "pass_fields", null);

		for (int i = 0; vFields != null && i < vFields.size(); i++) {
			String sKey = vFields.get(i);
	 ...
	 * 
	 * @param prop configation properties
