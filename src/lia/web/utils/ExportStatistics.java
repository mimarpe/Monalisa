...
public class ExportStatistics extends TimerTask {
    /** The Logger */ 
    private static final Logger logger = Logger.getLogger("lia.web.utils.ExportStatistics");

	private static Timer	timer		= new Timer();

	private Vector<MonitoringModule>			vModules	= new Vector<MonitoringModule>();
	...
	protected void init(String sCluster, int repositoryPort, String sExtraNodeString) {
		String sConfig = AppConfig.getProperty("lia.Repository.ApMonExport.ConfigFile", "http://monalisa.caltech.edu/repository.conf");
		...
		String sRepositoryAddr = sHost + (repositoryPort > 0 ? ":" + repositoryPort : "") + (sExtraNodeString != null ? sExtraNodeString : "");
	...
	public void addMonitoringModule(final MonitoringModule module) {
		if (module == null || node == null) // node may be null if sConfig isn't given
			return;
		...
		try{
			...
			try {
				...
				for (int i=0; i<paramNames.size(); i++){
					if (i%10==9){
						apm.sendParameters(node.cluster.name, node.name, paramNames1.size(), paramNames1, valueTypes1, values1);
						paramNames1.clear();
						valueTypes1.clear();
						values1.clear();
					}
