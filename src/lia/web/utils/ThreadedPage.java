...
public abstract class ThreadedPage extends ServletExtension implements Runnable {
	...
	/** Logger */
	static final Logger	logger = Logger.getLogger("lia.web.utils.ThreadedPage");
	...
	private static class CleanupThread extends Thread {
		 ...
		 * @param confDir
		 */
		public CleanupThread(String confDir) {
			sConfDir = confDir;
		}
		...
		if (AppConfig.getb("lia.web.robots_exclusion.enabled", true)){
			...
			if (!bRejected) {
				...
				if (vsIPClasses != null && vsIPClasses.length > 0) {
					...
					for (String vsIPClasse : vsIPClasses) {
			...
			if (bRejected){
				...
				try{
					...
					pwOut.println("<html><head><title>MonALISA Grid Monitoring tool</title></head><body><a href='http://monalisa.caltech.edu/' alt='MonALISA Grid Monitoring tool'>MonALISA home page</a></body></html>");
					pwOut.flush();
					pwOut.close();
				}
				catch (Exception e){
	...
	private final void execute(final HttpServletRequest req, final HttpServletResponse resp) {
		...
		if (sExtraCookies != null && sExtraCookies.length() > 0 && (sess = request.getSession(true)) != null) {
			StringTokenizer st = new StringTokenizer(sExtraCookies, "&");
			...
			while (st.hasMoreTokens()) {
				...
				if (idx > 0) {
					String sKey = s.substring(0, idx);
					String sVal = s.substring(idx + 1);
		...
		synchronized (oLock) {
			...
			if (lFirstRunDate < 0) {
				...
				try {
					...
					try {
						PrintWriter pw = new PrintWriter(new FileWriter(sConfDir + "REPOSITORY.start_date"));
			...
			if (exporter == null) {
				String sSite = AppConfig.getProperty("lia.web.is_site", null);
				...
				if (exporter == null) { // either explicit setting or if the repository classes cannot be created
					try {
		...
		if (!bFinishedOK) {
			...
			try {
				...
				pwOut.println("<!-- page timeout, getMaxRunTime()= " + getMaxRunTime() + 
						", lStart=" + lStartTime + 
						", now=" + System.currentTimeMillis() + 
						", bFinishedOk = "+bFinishedOK+
	 ...
	 * @param sMessage
	 */
	protected final void logTiming(String sMessage) {
		if (bTimingEnabled)
	...
	private static long lMinTime = -1;
	private static long lMaxTime = -1;
	...
	private static void updateStats(final long lTime){
		...
		synchronized (oStatsLock){
			if (iRequests == 0){
				iRequests = 1;
				lTotalTime = lMinTime = lMaxTime = lTime;
			}
			else{
				iRequests ++;
				lTotalTime += lTime;
				lMinTime = Math.min(lMinTime, lTime);
				lMaxTime = Math.max(lMaxTime, lTime);
			}
		}
	}
	...
	public static ExtendedResult getStats(){
		synchronized (oStatsLock){
			if (iRequests>0){
				...
				er.addSet("msPerRequest", lTotalTime/iRequests);
				er.min = lMinTime;
				er.max = lMaxTime;
	...
	private static String getCachedHostName(final String sIP){
		synchronized (ipCache){
			IPCacheEntry cache = ipCache.get(sIP);
	...
	private static void putCachedHostName(final String sIP, final String sName){
		IPCacheEntry cache = new IPCacheEntry(sName, sIP);
	 ...
	 * 
	 * @param _sIP
	 * @param bCacheOnly if <code>true</code> then the hostname will be returned only if found in cache 
	...
	protected String getBookmarkURL(){
		...
		//else{
			//System.err.println("getBookmarkURL : page is null, not reading any file");
		//}
		...
		for (int i=0; vCookies!=null && i<vCookies.length; i++){
			...
			String sValue = vCookies[i].getValue();
			
			if (sName.equals("JSESSIONID") || 
					sName.startsWith("__utm") || 
					sName.equals("vdnsessid") ||
					sName.equals("vdnsessid") ||
					sName.equals("LAZYJ_ID") ||
					sName.equals("void") ||
					sName.equals("AI_SESSION") ||
					sName.equals("cod") ||
					sName.equals("codMLMenu") ||
					sName.equals("csd") ||
					sName.equals("csdMLMenu") ||
					sName.startsWith("lastval_div_")
			)
				continue;
		...
		while (e.hasMoreElements()){
			...
			if (sName.startsWith("interval_date_") || 
					sName.startsWith("interval_hour_") || 
					sName.equals("quick_interval"))
				continue;
