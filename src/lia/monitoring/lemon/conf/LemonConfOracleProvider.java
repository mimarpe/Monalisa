...
public class LemonConfOracleProvider extends LemonConfProvider {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.monitoring.lemon.conf.LemonConfProvider");
    ...
    public LemonConfOracleProvider(String jdbcURL, String user, String passwd, long checkInterval, long errorCheckInterval) {
    ...
    public void setNewParams(String jdbcURL, String user, String passwd, long checkInterval, long errorCheckInterval) {
        logger.log(Level.INFO, "LemonConfOracleProvider using [ " + jdbcURL + ", " + user + ", " + passwd + " ] checkInt = " + checkInterval + " errCheck = " + errorCheckInterval);
    ...
    private Hashtable getHostCluster(Connection conn) {
                ...
                try { 
                    ...
                    if(hostname != null && hostname.length() > 0) {
                        if (clustername == null || clustername.length() == 0 ) {
                            logger.log(Level.WARNING, "LemonConfOracleProvider using [ " + jdbcURL + ", " + user + ", " + passwd + " ] checkInt = " + checkInterval + " errCheck = " + errorCheckInterval);
                            clustername = "N/A";
                        }
        ...
        if (hc != null && hc.size() == 0) return null;
    ...
    private Hashtable getMetricID_Class(Connection conn) {
                ...
                try { 
                    ...
                    if(metric_id != null) {
                        if (metric_class == null || metric_class.length() == 0 ) {
                            logger.log(Level.WARNING, "Got a null or 0 size() length metric_class for metric_id " + metric_id + " ... ignoring it");
                            continue;
                        }
                        if(logger.isLoggable(Level.FINER)) {
                            logger.log(Level.FINER, "metric_id: " + metric_id + " metric_class: " + metric_class + " ... adding to conf");
                        }
        ...
        if (midc != null && midc.size() == 0) return null;
        return midc;
    }
    ...
    private Hashtable getMetricFields(Connection conn) {
                ...
                try { 
                    ...
                    if(metric_class != null) {
                        if (field_name == null || field_name.length() == 0 ) {
                            logger.log(Level.WARNING,"Got a null or 0 size() length field_name for metric_class " + metric_class + " ... ignoring it" );
                            continue;
                        }
                        if (field_type == null || field_type.length() == 0 ) {
                            logger.log(Level.WARNING, "Got a null or 0 size() length field_type for metric_class " + metric_class + " field_name " + field_name + " ... ignoring it");
                            continue;
                        }
        ...
        if (mfh != null && mfh.size() == 0) return null;
        return mfh;
    }
                ...
                if(nextCheck < now) {
                    try {
                        ...
                        if (hch == null || mich == null || mfh == null) {
                            ...
                            logger.log(Level.WARNING, "One of the hashes is null [ hch (cdb_clustermap) " + ((hch == null)?"==":"!=") + " null, mich (metric_instances) "+ ((mich == null)?"==":"!=") + " null, mfh (metric_fields)] "+ ((mfh == null)?"==":"!=") + " null ]");
                        ...
                        if (clc == null ||
                                ...
                                (clc.hostsToClusterMap != null && hch != null && !clc.hostsToClusterMap.equals(hch)) ||
                                (clc.metricsIDToNameMap != null && mich != null && !clc.metricsIDToNameMap.equals(mich)) ||
                                (clc.metricsNameToFieldsMap != null && mfh != null && !clc.metricsNameToFieldsMap.equals(mfh))
                                ) {
