...
public class MLLemonConfProvider extends Observable implements Observer {
    ...
    private static final transient Logger logger = Logger.getLogger("lia.monitoring.lemon.conf.MLLemonConfProvider");
	    ...
	    fis.close();
            ...
            if (prop == null) {
                if (PersistentStoreFileName == null) {
                    ...
                    logger.log(Level.WARNING, "Got null for PersistentConfFile ... using older one ["+PersistentStoreFileName+"]");
                }
            } else {
            ...
            logger.log(Level.INFO, "\n\nMLLemonConfProvider: Using as PersistentStoreFileName " + PersistentStoreFileName+"\n\n");
            ...
            if (sRemapFile == null) {
                if (remapFile == null) {
                    ...
                    logger.log(Level.INFO, "\n\nMLLemonConfProvider: MLLemonReMappingFile ... using the older one [" + remapFile.getAbsolutePath()+ "]\n\n");
                }
            } else {
                ...
                logger.log(Level.INFO, "\n\nMLLemonConfProvider: Using MLLemonReMappingFile "+ sRemapFile +"["+remapDFW+"]\n\n");
            }
    ...
    public void loadML2Mappings() {
        ...
        try {
            ...
            for(String line = br.readLine(); line != null; line = br.readLine()) {
                try {
                    ...
                    if (line.length() == 0 || line.startsWith("#") && !line.startsWith("#Offset") && !line.startsWith("#ParamRemap")) continue; //ignore comment lines
                    ...
                    if (tokens != null && tokens.length >= 1 && tokens[0] != null) {
                        ...
                        if (offsetStarted && !remapStarted) {
                            ...
                            if(logger.isLoggable(Level.FINER)) {
                                logger.log(Level.FINER, "Added to Offset Map ( " + tokens[0] +", " + newOffsetMap.get(tokens[0]) +" ) ");
                            }
        ...
        if(logger.isLoggable(Level.FINER)) {
            logger.log(Level.FINER, "Finished (re)loading ML2LemonMappings ...  " + ((conf == null || conf.getIDPrefML() == null)?"null":""+conf.getIDPrefML().size()));
        }
    ...
    private void notifyChangedConf() {
        ...
        try {
            if( conf != null && conf.getIDPrefML() != null && conf.getLemonConf() != null ) {
                ...
                if ( lcf != null && 
                        lcf.hostsToClusterMap != null && 
                        lcf.metricsIDToNameMap != null &&
